#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e36.sh (The Apex Vanguard - Project Genesis V198e36)
# 快捷方式: xrv
# 【V198e36 热修复稳态版：1GB Swap 幂等检测 + Origin Guard 防 CDN 滥用 + Reality 指纹收敛】
#==============================================================================
...
readonly SCRIPT_VERSION="198e36"
...
install_self_entrypoint() {
    ...
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e36.sh" 2>/dev/null || true
        chmod +x "$SYMLINK" "$SCRIPT_DIR/ex198e36.sh" 2>/dev/null || true
    fi
...
hot_update_from_legacy() {
    local mode="${1:-manual}"
    title "V198e36 旧版环境热更新迁移器（兼容 ex198e32/ex198e33/ex198e34）"
    ...
    install_update_dat
    fix_xray_systemd_limits
    # V198e36: 先移除 V198e35 遗留的自动入口守卫，避免重启后链路被 iptables 误伤。
    ...
    if ensure_xray_is_alive; then
        # V198e36: 热更新不再自动启用 Origin Guard。V198e35 的自动守卫会在重启后误伤部分真实链路。
        remove_origin_guard auto >/dev/null 2>&1 || true
        echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
        info "热更新完成：旧配置、旧用户和旧端口已保留，V198e36 稳态热修复补丁已生效。Origin Guard 已默认关闭，可在防火墙菜单中手动开启。"
        if test "$mode" != "auto"; then
            do_summary
            local _p=""; read -rp "按 Enter 返回主菜单..." _p || true
        fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e36 - (The Apex Vanguard)${none}"
        ...
        echo -e "  ${cyan}11) 从 ex198e32/ex198e33/ex198e34/ex198e35 旧环境稳态热修复到 V198e36${none}"
...
rescue_after_v35_breakage() {
    title "V198e36 紧急修复：撤销 V198e35 重启后断流风险项"
    warn "将关闭 Origin Guard、清理 XRAY_ORIGIN_GUARD 链、写入稳态网络参数、关闭 ECN/TFO，并重启 Xray。"
...
#==============================================================================
# EOF - Apex Vanguard V198e36 System Advanced Control Ready.
#==============================================================================
