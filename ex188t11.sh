#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: exex188t11.sh (The Apex Vanguard - Pure Genesis V188t11)
# 快捷方式: xrv
#
# V188t11 终极原教旨修复日志:
#   1. 灵魂归位: 100% 还原 ex188.sh 中写死的 1GB 极简 Swap，拒绝花里胡哨的动态测算。
#   2. 参数归位: 100% 还原 ex188.sh 中强大的 do_perf_tuning (BBR/Sysctl 暴力写入集)。
#   3. 伪装库归位: 130+ 个顶级 SNI 域名探针全量保留，一字不删。
#   4. 强硬路由: 增量融入一键强锁 IPv4 优先并物理熔断 IPv6 的安全策略。
#   5. 终极防御: 全面修复 set -euo pipefail 下 read 变量未绑定引发的终端闪退问题。
# ==============================================================================

# 检查 Bash 运行环境
if test -z "$BASH_VERSION"; then
    echo "Error: Please run this script with bash: bash exex188t11.sh"
    exit 1
fi
...
# ── 全局常量与路径 ──────────────────────────────────────────────
readonly SCRIPT_VERSION="ex188t11-Ultimate"
...
check_disk_space() {
    ...
    if [ "$available_space_mb" -lt "$required_space_mb" ]; then
        warn "磁盘空间不足！当前可用: $((available_space_mb/1024))G | 最低需求: ${required_gb}G"
...
# ==============================================================================
# [ 区块 III: 环境预检、系统限制配置与 1GB Swap / IPv4 安全 ]
# ==============================================================================
...
enforce_ipv4_and_disable_ipv6() {
    title "强制网络路由策略 (IPv4 优先 / 禁用 IPv6)"
    
    echo -e "  ${magenta}[1/2]${none} 重写寻址权重矩阵 (gai.conf)..."
...
EOF
    ...
    echo -e "  ${magenta}[2/2]${none} 从内核系统总线上彻底焊死 IPv6 协议栈..."
...
EOF
    ...
    info "策略下发完毕：机器 IPv6 端口已封死，完全阻隔旁路探测！"
}
...
run_sni_scanner() {
    ...
    for sni in $sni_string; do
        if read -t 0.1 -n 1 -s _dummy 2>/dev/null; then
