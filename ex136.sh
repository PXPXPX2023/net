#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex136.sh (The Apex Vanguard - Project Genesis V136 [Perfect Horizon])
# 快捷方式: xrv
# 完美修正补丁: 
#   1. 绝杀打印 Bug：全面废弃错误的 echo -e 占位符，重构所有 print_node_block 与 SS 展示为标准的 printf "%-15s : %s\n"，完美实现军规级冒号对齐。
#   2. 补全幽灵函数：彻底修复菜单 10 -> 3 的报错，重写实装 do_xanmod_compile，提供真正的自动化内核源码拉取与 BBR3 编译逻辑。
#   3. 母本守护：绝对继承 ex118 的 BBR3/内核调优基线 (21MB缓冲/swappiness=1等)，并保留完整的多行 jq 管道流语法，绝不阉割行数。
#   4. 物理级调度：完整保留 RPS/XPS 万兆智能散列探针与 BQL 底层硬件队列强压限制。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex136.sh"
    exit 1
fi
...
# 彻底修复: 完全实装 Xanmod 源码编译逻辑
do_xanmod_compile() {
    title "源码编译安装 Xanmod 内核 + TCP BBRv3"
    warn "警告: 源码编译内核需要至少 2-4GB 空闲内存，且耗时 30分钟 ~ 2小时！"
    read -rp "确定要继续吗？(y/n): " confirm
    if [[ "$confirm" != "y" ]]; then return; fi
    print_magenta ">>> [1/4] 安装编译依赖..."
    export DEBIAN_FRONTEND=noninteractive
    apt-get update -y
    apt-get install -y build-essential libncurses-dev bison flex libssl-dev libelf-dev bc git curl dwarves
    print_magenta ">>> [2/4] 获取 Xanmod 源码 (含 BBR3)..."
    mkdir -p /usr/src
    cd /usr/src
    rm -rf linux-xanmod
    git clone --depth=1 https://gitlab.com/xanmod/linux.git linux-xanmod
    cd linux-xanmod
    print_magenta ">>> [3/4] 导入当前配置并编译..."
    make defconfig
    # 物理级开启 BBR3
    sed -i 's/# CONFIG_TCP_CONG_BBR is not set/CONFIG_TCP_CONG_BBR=y/' .config 2>/dev/null || true
    sed -i 's/CONFIG_DEFAULT_CUBIC=y/# CONFIG_DEFAULT_CUBIC is not set/' .config 2>/dev/null || true
    sed -i 's/# CONFIG_DEFAULT_BBR is not set/CONFIG_DEFAULT_BBR=y/' .config 2>/dev/null || true
    make -j$(nproc) bindeb-pkg
    print_magenta ">>> [4/4] 安装编译好的内核包..."
    cd /usr/src
...
do_perf_tuning() {
    ...
    # -- 绝对保留 ex118 的全部 17 项基线参数，并在末尾完整嵌入 V136 内存倾斜 --
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex136 The Apex Vanguard - Project Genesis V136${none}"
