#!/usr/bin/env bash
# ============================================================
# 脚本名称: g7g50x2.sh (The Omega Protocol V3 Edition)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 新增 [外部用户导入] 模块，支持平滑迁移其它机器的 备注/UUID/ShortId。
#   2. 自动重组匹配本机 pbk 与 IP，当场生成导入用户的新版分发链接。
#   3. 彻底修复底层 jq 写入函数残留的权限 Bug，绝杀 status 23。
#   4. 完美保留 GFW/CDN 探测、TLS1.3 质检、Top 20 矩阵扩容等尖端特性。
#   5. 独立节点备注、半高二维码、vnstat 计费与自毁卸载 100% 稳定运行。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash g7g50x2.sh"
    exit 1
fi
...
do_user_manager() {
    while true; do
        ...
        if test "$uopt" = "a"; then
            ...
            echo -e " 提示: 系统将把外部用户的凭证挂载到本机，生成由本机 IP 和 pbk 构建的新链接！"
            ...
            info "外部用户导入成功！该用户在当前机器的专属分发信息如下："
...
do_summary() {
    ...
    title "The Omega Protocol 节点详情中心"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray G7G50x2 The Omega Protocol V3 Edition${none}"
