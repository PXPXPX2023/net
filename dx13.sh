#!/usr/bin/env bash
# ============================================================
# 脚本名称: Dx13.sh (The Apex Vanguard V11 Edition)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 新增【初次安装、更新系统组件】模块，一键新机落地、环境清理、组件拉齐。
#   2. 深度融合 ChangeDNS 系统级域名解析重构引擎，锁定 resolvconf 防止飘移。
#   3. 吉隆坡时区 (Asia/Kuala_Lumpur) 硬件级时间强同步，杜绝 Reality 握手断流。
#   4. 彻底解决复制粘贴截断 Bug，商用流量计费中心汉化并支持 30 天精准回溯。
#   5. 彻底根治 status 23 权限拦截 Bug，采用 755/644 绝对防线穿透。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash Dx13.sh"
    exit 1
fi
...
do_sys_init() {
    while true; do
        title "初次安装、更新系统组件"
        echo "  1) 一键更新系统、安装常用组件并校准时间 (Asia/Kuala_Lumpur)"
        echo "  2) 修改系统 DNS 解析 (底层强锁防漂移)"
        echo "  0) 返回主菜单"
        hr
        read -rp "请选择: " sys_opt
        case "$sys_opt" in
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray G7G50x12 The Apex Vanguard V10 Edition${none}"
        ...
        case "$num" in
            ...
            3) 
               ...
               ;;
            4) print_magenta ">>> 正在同步最新规则库..."; bash -c "$(curl -fsSL https://raw.githubusercontent.com/XTLS/Xray-install/main/install-release.sh)" @ install-geodata >/dev/null 2>&1; systemctl restart xray >/dev/null 2>&1; info "Geo 更新成功"; read -rp "按 Enter 继续..." _ ;;
            5) do_update_core ;;
            6) 
               choose_sni
               if test $? -eq 0; then
