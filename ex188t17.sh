#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: exex188t17.sh (The Apex Vanguard - Ultimate Genesis V188t17)
# 快捷方式: xrv
#
# V188t17 终极原教旨修复日志:
#   1. 环境抢救: 恢复系统默认的 IFS 分隔符，彻底解决依赖安装失败与变量读取错位问题。
#   2. 抛弃 JQ: 在初次部署 do_install 时，弃用兼容性极差的 jq --slurpfile 注入，
#      改用纯 Bash 原生变量拼接写入 JSON，100% 免疫解析器崩溃和回车符炸弹。
#   3. 原装伪装: SNI 探针库完全替换为 ex188.sh 原版的 129 个顶级域名(垂直排版)。
...
if test -z "$BASH_VERSION"; then
    echo "Error: Please run this script with bash: bash exex188t17.sh"
    exit 1
fi
...
# ── 全局常量与路径 ──────────────────────────────────────────────
readonly SCRIPT_VERSION="ex188t17-Ultimate"
...
_turn_on_app() {
    ...
    sed -i 's/"shortIds": \["/&/' "$CONFIG" 2>/dev/null || true 
...
_turn_off_app() {
    ...
    sed -i '/"metadataOnly":/d' "$CONFIG" 2>/dev/null || true
    sed -i '/"routeOnly":/d' "$CONFIG" 2>/dev/null || true
    sed -i '/"sockopt":/d' "$CONFIG" 2>/dev/null || true
    sed -i '/"maxTimeDiff":/d' "$CONFIG" 2>/dev/null || true
...
do_app_level_tuning_menu() {
    while true; do
        ...
        case "$app_opt" in
            1|2|3|4|6|8|9|10)
                info "由于剥离了不稳定组件，请使用选项 26 进行一键全量开启应用层参数。"
                local _pause=""
...
# ==============================================================================
# [ 区块 XIII: 核心部署、管理与排版渲染矩阵 (原生 Bash 写入版) ]
# ==============================================================================
...
EOF
    ...
    # 抛弃 jq --slurpfile 带来的不稳定，使用纯 Bash 逻辑闭环写入
    local inbounds_json=""
...
do_summary() {
    ...
    local vless_inbound
    vless_inbound=$(grep -A 20 '"protocol": "vless"' "$CONFIG" 2>/dev/null || echo "")
    ...
    if [[ -n "$vless_inbound" ]]; then
        ...
        main_sni=$(echo "$vless_inbound" | grep '"serverNames":' | awk -F'"' '{print $4}' | head -n 1)
    ...
    local ss_inbound
    ss_inbound=$(grep -A 10 '"protocol": "shadowsocks"' "$CONFIG" 2>/dev/null || echo "")
...
do_user_manager() {
    clear
    error "出于稳定安全考虑，JQ 注入被彻底移除。若需多用户管理，请在 /usr/local/etc/xray/config.json 手动配置。"
    local _pause=""
    read -e -p "按 Enter 返回..." _pause || true
}
...
_global_block_rules() {
    clear
    error "出于稳定安全考虑，JQ 注入被彻底移除。若需修改路由规则，请直接编辑 /usr/local/etc/xray/config.json。"
    local _pause=""
    read -e -p "按 Enter 返回..." _pause || true
}
...
do_fallback_probe() {
    ...
    error "由于剥离了不稳定解析组件，请直接查看 /usr/local/etc/xray/config.json 以核验红区阻断参数。"
    echo ""
    local _pause=""
