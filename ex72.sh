#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex72.sh (The Apex Vanguard - Project Genesis V72)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 极客勘误：修复 XanMod 探针被 bash 误读导致报错的低级 Bug，重构为 awk -f 内存级解析，完美绕过 noexec。
#   2. 终极收网：融汇全量聊天溯源，达成 Linux 内核、网卡硬件、系统调度与应用层 100% 极速贯通。
#   3. 战神归位：OOM 免死与 I/O 抢占恢复【出厂默认强制注入】，依然受子菜单 11 持久化开关控制。
#   4. 智能继承引擎：反向探针精准判别“初次安装的默认注入”与“老用户主动关闭后的状态继承”。
...
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex72.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex68 The Apex Vanguard - Project Genesis V68${none}"
