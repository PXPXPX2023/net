#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex187.sh (Xray Advanced Management Script V187)
# 快捷方式: xrv
#
# V187 全量修复与终极融合日志:
#   1. 作用域修复: 修复 Bash 函数加载顺序导致的 command not found 错误 (优化菜单28项)。
#   2. 排版回溯: 完全恢复要求的节点信息排列格式 (协议框架, 外网IP, 端口, UUID, SNI 等)。
#   3. 菜单修复: 恢复 do_sys_init_menu 中遗失的官方预编译 XANMOD 安装入口。
#   4. 流量审计恢复: 恢复 do_status_menu 中缺失的 vnstat 每月结算日历修改与详单查询。
#   5. 模块补全: CAKE 挂载前置强制 modprobe sch_cake，彻底解决网卡队列未反馈的警告。
#   6. 底盘拒绝减配: 全量恢复 60+ 项系统网络栈极限调优及所有交互式确认逻辑。
# ==============================================================================

# 检查 Bash 运行环境
if test -z "$BASH_VERSION"; then
    echo "Error: Please run this script with bash: bash ex187.sh"
    exit 1
fi
...
# 安全 JSON 写入接口 (V187 修复: 强制追加 .json 后缀以适配核心检测)
_safe_jq_write() {
    backup_config
    local tmp_raw
...
verify_sni_strict() {
    ...
    # 修复逻辑反转: 0 代表 true(成功), 非 0 代表 false(失败)
    local pass=0
...
config_cake_advanced() {
    ...
    echo -e "  [1] 声明物理带宽限制 (如 900Mbit，填 0 取消限制): "
    read -rp "  请输入: " c_bw
    
    echo -e "  [2] 配置底层报文开销补偿 (如 Wireguard 填 32，VPN 填 48，填 0 禁用): "
    read -rp "  请输入: " c_oh
    
    echo -e "  [3] 最小数据单元截断 MPU (以太网标准通常为 64，填 0 禁用): "
    read -rp "  请输入: " c_mpu
    
    echo "  [4] RTT 延迟模型: "
    echo "    1) internet  (标准互联 85ms)"
    echo "    2) oceanic   (跨洋海缆 300ms - 推荐高延迟机器使用)"
    ...
    echo "    2) besteffort (盲推忽略特征，减少 CPU 开销 - 推荐)"
    ...
    # V187 致命修复: 确保 CAKE 模块在 tc qdisc 前挂载
    ...
    if tc qdisc show dev "$IFACE" 2>/dev/null | grep -q "cake"; then
        info "CAKE 队列已成功接管网卡接口。"
    else
        warn "网卡当前未运行 CAKE 调度器。请确认系统内核支持 sch_cake 模块。"
    fi
    ...
    while true; do
        ...
        case "$app_opt" in
            ...
            10)
                ...
                read -rp "配置已更新，按 Enter 继续..." _
                ;;
            11) toggle_buffer; systemctl restart xray >/dev/null 2>&1 || true; read -rp "配置已更新，按 Enter 继续..." _ ;;
            12) toggle_dnsmasq; read -rp "配置已应用，按 Enter 继续..." _ ;;
            13) toggle_thp; read -rp "配置已应用，按 Enter 继续..." _ ;;
            14) toggle_mtu; read -rp "配置已应用，按 Enter 继续..." _ ;;
            15) toggle_cpu; read -rp "配置已应用，按 Enter 继续..." _ ;;
            16) toggle_ring; read -rp "配置已应用，按 Enter 继续..." _ ;;
            17) toggle_zram; read -rp "配置已应用，按 Enter 继续..." _ ;;
            18) toggle_journal; read -rp "配置已应用，按 Enter 继续..." _ ;;
            19) toggle_process_priority; systemctl restart xray >/dev/null 2>&1 || true; read -rp "配置已应用，按 Enter 继续..." _ ;;
            20) toggle_cake; read -rp "配置已应用，按 Enter 继续..." _ ;;
            21) toggle_irq; read -rp "配置已应用，按 Enter 继续..." _ ;;
            22) 
                if [[ "$gso_off_state" == "unsupported" ]]; then
                    ...
                    toggle_gso_off
                    read -rp "配置已应用，按 Enter 继续..." _ 
                fi
                ;;
            23) toggle_ackfilter; read -rp "配置已应用，按 Enter 继续..." _ ;;
            24) toggle_ecn; read -rp "配置已应用，按 Enter 继续..." _ ;;
            25) toggle_wash; read -rp "配置已应用，按 Enter 继续..." _ ;;
            26)
...
do_install() {
    ...
    if [[ "$proto_choice" == "1" || "$proto_choice" == "3" ]]; then
        while true; do 
            read -rp "设置 VLESS 监听端口 (默认 443): " input_p
            input_p=${input_p:-443}
        ...
        read -rp "设置节点备注名称 (默认 xp-reality): " input_remark
    ...
    if [[ "$proto_choice" == "2" || "$proto_choice" == "3" ]]; then
        while true; do 
            read -rp "设置 SS 监听端口 (默认 8388): " input_s
    ...
    if ! bash -c "$(curl -fsSL --connect-timeout 10 https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install >/dev/null 2>&1; then
        warn "通过官方脚本拉取失败，您可以稍后通过菜单 5 尝试重试。"
    fi
...
do_user_manager() {
    while true; do
        ...
        while IFS='|' read -r num uid remark; do
            local utime
            utime=$(grep "^$uid|" "$USER_TIME_MAP" 2>/dev/null | cut -d'|' -f2 || echo "未知记录")
            echo -e "  $num) 用户: ${cyan}$remark${none} | 创建: ${gray}$utime${none} | ID: ${yellow}$uid${none}"
        done < "$tmp_users"
        hr
        
        echo "  a) 签发新用户凭证"
        echo "  m) 从外部导入历史用户数据"
        echo "  s) 绑定/修改用户专属网络伪装 (SNI)"
        echo "  d) 吊销用户访问权限"
        echo "  q) 退出"
        read -rp "请输入对应操作代码: " uopt
        ...
        case "$uopt" in
            a|A)
                ...
                sid=$(head -c 8 /dev/urandom | xxd -p | tr -d '\n' || echo "")
                ctime=$(date +"%Y-%m-%d %H:%M")
                read -rp "请指定用户名备注 (默认 User-$sid): " u_remark
            ...
            m|M)
                ...
                read -rp "请指定导入用户备注 (默认 Imported): " m_remark
                m_remark=${m_remark:-Imported}
                read -rp "请输入有效的 UUID: " m_uuid
                if [[ -z "$m_uuid" ]]; then continue; fi
                read -rp "请输入有效的 ShortId: " m_sid
                ...
                local m_sni
                read -rp "需绑定专属 SNI 吗？(直接回车则使用主 SNI): " m_sni
                if [[ -n "$m_sni" ]]; then
            ...
            s|S)
                ...
                if [[ -n "$t_uuid" ]]; then
                    ...
                    if [[ -n "$u_sni" ]]; then
                        ...
                        ensure_xray_is_alive
                        info "已成功将 SNI $u_sni 绑定至用户 $t_remark。"
            ...
            d|D)
                ...
                read -rp "请输入需要吊销的用户序列号: " dnum
                total=$(wc -l < "$tmp_users" 2>/dev/null || echo 0)
                
                if ((total <= 1)); then 
                    error "系统要求至少保留一个活跃用户，操作被中止。"
                else
...
_global_block_rules() {
    while true; do
        ...
        echo -e "  1) BT 协议解析与流量阻断        | 状态: ${yellow}${bt_en:-未知}${none}"
        echo -e "  2) 全局广告与恶意域名解析阻断   | 状态: ${yellow}${ad_en:-未知}${none}"
        echo "  0) 返回上级菜单"
        read -rp "请选择: " bc
        ...
        case "$bc" in
            1)
                ...
                ensure_xray_is_alive; info "BT 协议阻断状态已调整为: $nv" 
                ;;
            2)
                local nv="true"
                ...
                ensure_xray_is_alive; info "全局广告过滤状态已调整为: $nv" 
                ;;
            0) return ;;
        esac
    done
...
do_status_menu() {
    while true; do
        title "系统运行状态与流量统计监控"
        echo "  1) 查看底层 Xray 守护进程状态"
        echo "  2) 审查网络入口与路由解析配置"
        echo "  3) 调取网卡全域流量核算总账 (vnstat)"
        echo "  4) 追踪实时网络连接与独立访问者来源"
        echo "  5) 设置系统级内核抢占权重 (Nice 值调整)"
        echo "  6) 审查 Xray 服务运行信息日志"
        echo "  7) 审查 Xray 系统级错误异常日志"
        echo "  8) 自动化灾难备份与配置还原恢复"
        echo "  0) 返回主菜单"
        hr
        read -rp "请下达操作指令: " s
        ...
        case "$s" in
            ...
            3) 
                if ! command -v vnstat >/dev/null 2>&1; then 
                    warn "服务器未安装 vnstat，无法执行流量统计。"
                    read -rp "按 Enter 继续..." _
                    continue
                fi
                clear; title "网络流量审计系统 (vnstat)"
                
                # 恢复 Vnstat 详细管理功能
                local m_day
                m_day=$(grep -E "^[[:space:]]*MonthRotate" /etc/vnstat.conf 2>/dev/null | awk '{print $2}' | head -n 1 | tr -d '\r\n')
                m_day=${m_day:-"1 (默认)"}
                echo -e "  账单流量重置结算日: ${cyan}每月 $m_day 号${none}"
                hr
                ...
                hr
                echo "  1) 设定每月流量清零日"
                echo "  2) 查询指定年月的每日流量明细"
                echo "  0) 返回"
                read -rp "  请输入指令: " vn_opt
                case "$vn_opt" in
                    1) 
                        ...
                        ;;
                    2)
                        read -rp "输入查询年月 (格式如 $(date +%Y-%m)，回车默认近30天): " d_month
                        if [[ -z "$d_month" ]]; then 
                            ...
                            vnstat -d 2>/dev/null | grep -iE "($d_month| day |estimated|--)" | sed -e 's/estimated/估计流量/ig' -e 's/rx/接收/ig' -e 's/tx/发送/ig' -e 's/total/总计/ig' -e 's/daily/按天/ig' -e 's/monthly/按月/ig' || true
                        fi
                        read -rp "按 Enter 继续..." _ 
                        ;;
                    0|q|Q) 
            ...
            4)
                while true; do
                    ...
                    if [[ -n "$x_pids" ]]; then
                        ...
                        ss -ntupa 2>/dev/null | grep -E "pid=($x_pids)[,)]" | awk '{print $1"_"$2}' | sort | uniq -c | sort -nr | awk '{printf "    连接状态: %-15s : 并发总数 %s\n", $2, $1}' || echo "    暂无连接"
                        echo -e "\n  ${cyan}【外部访问来源追溯 (TOP 10)】${none}"
                        local ips
                        ...
                        if [[ -n "$ips" ]]; then
                            echo "$ips" | sort | uniq -c | sort -nr | head -n 10 | awk '{printf "    源 IP: %-18s (连接频次: %s)\n", $2, $1}'
                            local total_ips
                            total_ips=$(echo "$ips" | sort | uniq | wc -l)
                            echo -e "\n  当前唯一访客总量: ${yellow}${total_ips}${none}"
                        else 
                            echo -e "    ${gray}监听正常，当前暂无外部连接传入。${none}"
                        fi
                    else 
                        error "未捕获到 Xray 进程，服务可能已经停止。"
                    fi
            ...
            5)
                while true; do
                    ...
                    echo -e "  当前调度优先级参数 (Nice): ${cyan}${current_nice}${none} (有效控制区间: -20 到 -10)"
                    hr
                    read -rp "  请指定新抢占权重 (q 退出): " new_nice
            ...
            8)
                ...
                ls -t "$BACKUP_DIR"/config_*.json 2>/dev/null || echo "未找到任何可用快照"
                echo -e "\n  r) 立即回滚至最新有效快照\n  c) 为当前环境手动创建配置快照\n  0) 退出"
                read -rp "执行操作: " bopt
...
do_uninstall() {
    title "环境清理与系统还原"
    read -rp "警告: 该操作将不可逆地删除服务、配置及日志记录，是否确认注销？(y/n): " confirm
    if [[ "$confirm" != "y" ]]; then 
        return
    fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray 高级管理系统 (Industrial Base V187)${none}"
        
        local svc
        svc=$(systemctl is-active xray 2>/dev/null || echo "inactive")
        if [[ "$svc" == "active" ]]; then 
            svc="${green}活跃 (Active)${none}"
        else 
            svc="${red}离线 (Inactive)${none}"
        fi
        
        echo -e "  服务状态: $svc | 全局指令: ${cyan}xrv${none} | 连接端点: ${yellow}${SERVER_IP}${none}"
        echo -e "${blue}======================================================================${none}"
        echo "  1) 初始化核心并构建安全网络架构"
        echo "  2) 用户与连接凭证权限管理"
        echo "  3) 节点链接信息分发中心"
        echo "  4) 请求最新的全球 Geo 数据防线配置"
        echo "  5) 升级更新底层 Xray 通信组件"
        echo "  6) 重新规划隐蔽防封策略 (SNI 矩阵)"
        echo "  7) 防火墙安全规则干预 (BT阻断/广告过滤)"
        echo "  8) 审查外部主动探测回落拦截机制"
        echo "  9) 监控系统负载、流量与追踪外部并发源"
        echo "  10) 系统与内核参数深度调优控制台"
        echo "  0) 关闭并脱离面板管理"
        echo -e "  ${red}88) 注销并全面清理工作环境${none}"
        hr
        read -rp "请键入对应的操作命令: " num
        ...
        case "$num" in
            ...
            3) 
                do_summary
                while true; do 
                    read -rp "按 Enter 返回主菜单，或输入 b 重选伪装验证策略: " rb
                    if [[ "$rb" == "b" || "$rb" == "B" ]]; then 
                        if choose_sni; then 
                            _update_matrix
                            do_summary
            ...
            6) 
                if choose_sni; then 
                    _update_matrix
                    do_summary
                    read -rp "规则更新已送达内核，按 Enter 继续..." _
                fi 
                ;;
