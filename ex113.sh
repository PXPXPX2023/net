#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex113.sh (The Apex Vanguard - Project Genesis V113 [Absolute Unbroken])
# 快捷方式: xrv
# 终极无漏: 
#   1. 逻辑断链修复：紧急修复 CAKE 高阶参数在 update_hw_boot_script 中的注入丢失问题，确保带宽、跨洋 (Oceanic)、封包补偿等神级参数在重启与重载时绝对持久化。
#   2. 数组重构：重写屏蔽规则引擎 (_global_block_rules) 中的 jq 语法，采用 if-then-else 绝对映射，斩断旧版 jq 更新数组时的下标越界隐患。
#   3. 港口管治完全体：CAKE 高阶参数引擎扩容，完美应对 Virtio 锁死 GSO 时的队列失真。
#   4. 单兵极致：专为单人/高延迟/加密隧道环境打造，废除无用的 Diffserv 分类，强制全量盲走。
#   5. 陷阱免疫：全量继承无折叠多行赋值、virtio 锁死探针、双协议平权、SS 节点防蒸发提取。
#   6. 语法绝缘：完整保留 jq 的原子化容错赋值链 (.streamSettings = (.streamSettings // {}))，绝杀 JSON 结构残缺。
#   7. 大一统矩阵：全域 23 项极限微操 + CAKE 5 大高阶透视参数面板，构建单节点路由的终极物理形态。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex113.sh"
    exit 1
fi
...
config_cake_advanced() {
    ...
    if [ "$(check_cake_state)" = "true" ]; then
        ...
        tc qdisc replace dev "$IFACE" root cake $final_opts $ack_flag 2>/dev/null || true
        info "底层 qdisc 队列规则已实时重载并应用了你的高阶透视参数！"
    else
        warn "当前系统未启用 CAKE 队列，参数将在下次开启 CAKE 时自动生效。"
    fi
...
check_ackfilter_state() {
    local IFACE=$(ip route get 1.1.1.1 | awk '{print $5; exit}')
    if tc qdisc show dev "$IFACE" 2>/dev/null | grep -q 'ack-filter'; then echo "true"; else echo "false"; fi
}
check_gso_off_state() {
    local IFACE=$(ip route get 1.1.1.1 | awk '{print $5; exit}')
...
EOF
    ...
    if [ "$(check_cake_state)" = "true" ]; then
        if [ "$(check_ackfilter_state)" = "true" ]; then
            echo "tc qdisc replace dev \$IFACE root cake \$CAKE_OPTS ack-filter 2>/dev/null || true" >> /usr/local/bin/xray-hw-tweaks.sh
        else
            echo "tc qdisc replace dev \$IFACE root cake \$CAKE_OPTS 2>/dev/null || true" >> /usr/local/bin/xray-hw-tweaks.sh
        fi
    fi
...
toggle_cake() {
    ...
    if [ "$(check_cake_state)" = "true" ]; then
        ...
        local ack_flag=""
        if [ "$(check_ackfilter_state)" = "true" ]; then ack_flag="ack-filter"; fi
        tc qdisc replace dev "$IFACE" root cake $cake_opts $ack_flag 2>/dev/null || true
    fi
    update_hw_boot_script
}
...
toggle_ackfilter() {
    ...
    if [ "$(check_ackfilter_state)" = "true" ]; then
        tc qdisc replace dev "$IFACE" root cake $cake_opts 2>/dev/null || true
    else
        tc qdisc replace dev "$IFACE" root cake $cake_opts ack-filter 2>/dev/null || true
    fi
    update_hw_boot_script
}
    ...
    while true; do
        ...
        case "$app_opt" in
            1)
                if [ "$out_fastopen" = "true" ]; then
                    _safe_jq_write '
                      ...
                      del(.inbounds[]? | select(.protocol=="vless" or .protocol=="shadowsocks") | .streamSettings.sockopt.tcpNoDelay, .streamSettings.sockopt.tcpFastOpen)
                    '
                else
                    _safe_jq_write '
                      .outbounds = [
            ...
            2)
                if [ "$out_keepalive" = "30" ]; then
                    _safe_jq_write '
                      ...
                      del(.inbounds[]? | select(.protocol=="vless" or .protocol=="shadowsocks") | .streamSettings.sockopt.tcpKeepAliveIdle, .streamSettings.sockopt.tcpKeepAliveInterval)
                    '
                else
                    _safe_jq_write '
                      .outbounds = [
            ...
            26)
                ...
                echo ""
                print_red "=========================================================="
                print_yellow "警告：全域 23 项拓扑与内核状态已发生深层变革！"
...
do_sys_init_menu() {
    while true; do
        ...
        echo "  5) 系统内核网络栈极限调优"
        echo "  6) 全域 23 项极限微操 (CAKE/硬中断隔离/零拷贝/聚合反转)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex113 The Apex Vanguard - Project Genesis V113${none}"
