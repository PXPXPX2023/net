#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e5.sh (The Apex Vanguard - Project Genesis V196e5)
# 快捷方式: xrv
#
# 【V196e5 终极勘误与真理重塑】
#   1. 编译断层修复: 深度介入 Xanmod 源码编译过程，强锁 CONFIG_X86_64_VERSION 参数。
#   2. 缓存对齐修复: 显式注入 CONFIG_X86_L1_CACHE_SHIFT=6，彻底根除 7.x 内核 asm-offsets 暴毙。
#   3. 真·源码溯源: 彻底废弃停更的 GitHub 镜像，直连 GitLab 官方仓库拉取正宗 7.x Xanmod 源码。
#   4. APT 源净化: 修复预编译安装时 GPG 密钥 404 及 sources.list 重复冲突导致的静默瘫痪。
#   5. 完美融合: 全量继承 V193/V196 的 JQ 绝缘护盾 (select(. != null))、GitLab 源拉取引擎与系统微操。
# ==============================================================================

# 必须用 bash 运行
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex196e5.sh"
    exit 1
fi
...
# ------------------------------------------------------------------------------
# 【V196e5 终极勘误 1】：真·预编译官方 XANMOD 部署模块 (修复多重源与 404 死锁)
# ------------------------------------------------------------------------------
...
do_install_xanmod_main_official() {
    ...
    if ! apt-get update -y; then
        warn "APT 源刷新遇到轻微警告或错误，但环境已强行注册，继续执行降级探测..."
    fi
...
# ------------------------------------------------------------------------------
# 【V196e5 终极勘误 2】：真·Xanmod 源码极客编译引擎 (抛弃 Mainline 假货，直取 GitLab)
# ------------------------------------------------------------------------------
...
do_xanmod_compile() {
    ...
    if wget -qO "$psabi_script" https://dl.xanmod.org/check_x86-64_psabi.sh 2>/dev/null; then
        ...
        local detected
        # 正则剥离所有杂质，只提取1位纯数字，防止因为恶性换行符引发 gcc -march=x86-64-v 暴毙
    ...
    info "锁定宿主机 CPU 微架构级别为: x86-64-v${arch_level} (防御编译期段错误)"
    
    # [修复 L1_CACHE_SHIFT 丢失]: 强行注入 L1 缓存偏移量与 GENERIC 架构宏
    ...
    # 显式注入纯净架构数字指令，根绝 bad value for -march 报错
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e4 - (The Apex Vanguard)${none}"
...
main_menu
# ==============================================================================
# EOF - Apex Vanguard V196e4 System Advanced Control Ready.
# ==============================================================================

# --- ✂️ Part 4 结束，请合并完整的代码进行执行 ✂️ ---】
