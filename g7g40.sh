#!/usr/bin/env bash
# ============================================================
# 脚本名称: g7g40.sh (The Dimensional Shift Edition)
# 快捷方式: xrv
# 融合增量: 
#   1. 解决 Linux hash 缓存导致的“卸载后还是旧版”幽灵 Bug，引入 hash -r 强制清空。
#   2. 二维码边距精准砍半 (qrencode -m 2)，上下强制保留 2 条边距，紧凑不辣眼。
#   3. 十六进制 \x5B \x5D 防吞噬引擎完美保障 VLESS + SS 双协议合规写入。
#   4. 130+ 寡头矩阵 CF 物理探测与 vnstat 流量状态中心全量融合。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash g7g40.sh"
    exit 1
fi
...
preflight() {
    ...
    if test -f "$SCRIPT_PATH"; then
        ...
        hash -r 2>/dev/null  # 强制刷新终端缓存，防止读取幽灵旧版本
...
do_summary() {
    ...
    title "The Dimensional Shift 节点详情中心"
    ...
    if test "$v_count" -gt 0; then
        ...
        if test -n "$uuid" && test "$uuid" != "null"; then
            ...
            if command -v qrencode >/dev/null 2>&1; then
                echo -e "  ${cyan}手机扫码导入 (2格边距紧凑版):${none}"
                # 使用 -m 2 参数，精准将二维码上下边距砍半为 2 条！
                qrencode -m 2 -t ANSIUTF8 "$link"
            fi
        fi
    fi
    ...
    if test "$s_count" -gt 0; then
        ...
        if test -n "$s_port" && test "$s_port" != "null"; then
            ...
            if command -v qrencode >/dev/null 2>&1; then
                echo -e "  ${cyan}手机扫码导入 (2格边距紧凑版):${none}"
                qrencode -m 2 -t ANSIUTF8 "$link_ss"
            fi
        fi
    fi
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray G7G40 The Dimensional Shift Edition${none}"
