#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex37.sh (The Apex Vanguard - Project Genesis V9)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 深度植入 Burst 突发能力：动态 GOMEMLIMIT 防爆、Nice=-12 极限抢占。
#   2. Xray Socket 底层重构：强启 tcpNoDelay 与 tcpFastOpen，毫秒级响应。
#   3. 进程并发黄金线：精确锁定 65535，严防低配机 Go Runtime 内存溢出 (OOM)。
#   4. 融入上古遗毒清理、网卡发送队列 (TX Queue) 调优与 Xanmod 内核飞升。
#   5. 物理级 DNS 强锁、全能生态复原引擎，所有逻辑 100% 展开零缩水。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex37.sh"
    exit 1
fi
...
# 赋予 Xray 更高的 CPU 调度优先级 (-20 到 19，越小优先级越高，-12为极客高优调度)
Nice=-12
...
do_uninstall() {
    ...
    # 仅移除 update-dat.sh 相关的任务，绝不触碰 cc1.sh 
    ...
    print_green "卸载完成！Xray 及其相关配置已被彻底粉碎 (您注入的内核网络栈与并发限制调优已为您永久保留)。"
    exit 0
}
...
do_install() {
    ...
    # 注入符合低配机极限性能的安全并发突破设置 (131072) 与 极限抗突发参数
    fix_xray_systemd_limits
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex37 The Apex Vanguard - Project Genesis V9${none}"
