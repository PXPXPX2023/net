#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex53.sh (The Apex Vanguard - Project Genesis V19)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 质变升级：将“实时 IP 统计”进化为真正的【动态雷达】，支持 2秒自动心跳刷新与 r 键瞬间强刷。
#   2. 强化探针正则清洗引擎：精确匹配 PID 边界，跨越全系 Linux 发行版兼容抓取。
#   3. 维持系统初始化子菜单逻辑顺序：强制预编译打底 -> 源码编译 -> 网卡队列 -> 内核栈。
#   4. 维持多用户分发中心 (do_summary) 动态循环解析，全量展示专属节点明细。
#   5. 维持【极低延迟/小包秒开】架构：Xray 并发 4096，TX 队列 2000，Busy Poll 50。
#   6. 维持 NAPI 软中断配额：dev_weight=64，bias=1，杜绝算力独占引发微观抖动。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex53.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex53 The Apex Vanguard - Project Genesis V19${none}"
