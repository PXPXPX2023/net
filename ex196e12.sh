#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e12.sh (The Apex Vanguard - Project Genesis V196e12)
# 快捷方式: xrv
#
# 【V196e12 终极勘误与真理重塑】
#   1. 物理篡改 Makefile: 彻底绕开 Kconfig 变态的重置机制，物理级注入 -march 架构参数，根治暴毙！
#   2. 双通道内核锻造: 新增 Kernel.org 官方最新主线 / Xanmod GitLab 最新极客版双轨选择，完全接管宿主命运。
#   3. JQ 绝对绝缘: 全域覆盖 select(. != null) 护盾，杜绝任何数组/对象为空时的语法崩溃。
#   4. 极致内存防爆: 1GB 强制 Swap 切辟与 cc1.sh 系统自愈缓存清理守护进程。
# ==============================================================================

# 必须用 bash 运行
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex196e12.sh"
    exit 1
fi
...
readonly SCRIPT_VERSION="196e12"
...
# --- ✂️ 请将此行及其下方的 Part 2 紧接在 Part 1 结尾粘贴 ✂️ ---
...
_safe_jq_write() {
    ...
    set +e
    # 强制在 jq 层面进行全量检查，保证任何修改指令都不会因为遇到 null 或非法数组挂掉
...
# --- ✂️ 请将此行及其下方的 Part 3 紧接在 Part 2 结尾粘贴 ✂️ ---
...
# --- ✂️ 请将此行及其下方的 Part 4 紧接在 Part 3 结尾粘贴 ✂️ ---

# ------------------------------------------------------------------------------
# 【V196e12 核心修复 1】：预编译官方 XANMOD 部署模块 (修复多重源与 404)
# ------------------------------------------------------------------------------
...
do_install_xanmod_main_official() {
    ...
    if test -f "$cpu_level_script"; then
        ...
        local detected
        detected=$(awk -f "$cpu_level_script" 2>/dev/null | grep -oE 'x86-64-v[1-4]' | sed 's/.*v//' | sort -n | tail -n 1 || true)
        if [[ "$detected" =~ ^[1-4]$ ]]; then
            cpu_level="$detected"
        fi
    ...
    if ! wget -qO - https://dl.xanmod.org/archive.key | gpg --dearmor --yes -o /usr/share/keyrings/xanmod-archive-keyring.gpg 2>/dev/null; then
        error "从远端获取 GPG 密钥链发生错误，官方源已被防火墙或 DNS 污染阻断！"
        set -e; return 1
    fi
...
# --- ✂️ 请将此行及其下方的 Part 5 紧接在 Part 4 结尾粘贴 ✂️ ---

# ------------------------------------------------------------------------------
# 【V196e12 核心修复 2】：真·双通道极客内核源码编译中心
# ------------------------------------------------------------------------------
...
do_kernel_compile_menu() {
    while true; do
        ...
        echo "     - 独家采用【物理降维打击】修复 Makefile 架构锁死 Bug"
        echo ""
        echo "  0) 返回上级菜单"
        hr
        
        local k_opt=""; read -rp "请下达锻造路径指令 (0-2): " k_opt || true
        case "${k_opt:-}" in
            1) _compile_kernel_mainline; return ;;
...
_compile_kernel_mainline() {
    title "系统飞升：编译 Linux 官方主线最新内核 (稳如老狗版)"
    ...
    # 网络优化 (Mainline 默认无 BBR3)
    info "正在注入官方 BBR 拥塞控制组件..."
...
_compile_kernel_xanmod() {
    title "系统飞升：编译 真·Xanmod 极客内核 (物理降维版)"
    warn "警告: 此过程将极度消耗 CPU 与内存 (约 30-60 分钟)，低配机极易死机！"
    ...
    info "=== 动态连接 GitLab 溯源真正的 Xanmod 官方源码 ==="
    set +e
    local LATEST_TAG=$(curl -sL --connect-timeout 10 https://gitlab.com/xanmod/linux/-/tags?format=atom | grep "<title>" | grep -oE "[0-9]+\.[0-9]+\.[0-9]+-xanmod[0-9]+" | head -n 1 || echo "")
    if test -z "$LATEST_TAG"; then
        LATEST_TAG=$(curl -sL --connect-timeout 10 https://gitlab.com/api/v4/projects/xanmod%2Flinux/repository/tags | jq -r '.[0].name' 2>/dev/null | grep -v "null" || echo "")
    fi
    if test -z "$LATEST_TAG"; then 
        warn "动态获取 GitLab Xanmod Tag 失败，触发容灾固化高版本！"
    ...
    if ! tar -tzf "$KERNEL_FILE" >/dev/null 2>&1; then
        ...
        wget -q --show-progress "$KERNEL_URL" -O "$KERNEL_FILE"
        if ! tar -tzf "$KERNEL_FILE" >/dev/null 2>&1; then
            error "源码包彻底损坏，连接 GitLab 核心库阻断，编译中止。"
            set -e; return 1
        fi
    fi
    set -o pipefail
    ...
    set +e
    if wget -qO "$psabi_script" https://dl.xanmod.org/check_x86-64_psabi.sh 2>/dev/null; then
        chmod +x "$psabi_script" 2>/dev/null || true
        local detected
        detected=$(awk -f "$psabi_script" 2>/dev/null | grep -oE 'x86-64-v[1-4]' | sed 's/.*v//' | sort -n | tail -n 1 || true)
        if [[ "$detected" =~ ^[1-4]$ ]]; then arch_level="$detected"; fi
        rm -f "$psabi_script" 2>/dev/null || true
    fi
    set -e
    info "探针锁定宿主机 CPU 微架构最高支持级别为: x86-64-v${arch_level}"

    info "=== 生成并清洗内核配置 (真·Xanmod + BBR3 强开) ==="
    
    # 【敬畏内核】：完全让 Kconfig 根据其默认树状依赖推导，不再手工修改 CPU 宏，防止被 Kconfig 重置后引发连环断层
    info "正在利用源码原生配置生成最纯净的底层树..."
    make defconfig >/dev/null 2>&1 || true
    make scripts >/dev/null 2>&1 || true
    ...
    info "正在向 .config 中精准注入原生 BBR3 拥塞控制组件..."
    ./scripts/config --enable CONFIG_TCP_CONG_BBR 2>/dev/null || true
    ./scripts/config --enable CONFIG_DEFAULT_BBR 2>/dev/null || true
    ...
    # 【上帝之手·物理级篡改】：针对 Xanmod Kconfig 吃掉 CPU 架构导致 Makefile 传参失败的终极修复
    info "正在对 arch/x86/Makefile 进行物理降维锁定 (强锁 v${arch_level} 架构集)..."
    # 彻底删除原来依赖动态变量的危险行，硬写参数进去
    sed -i '/CONFIG_X86_64_VERSION/d' arch/x86/Makefile || true
    sed -i '/-march=x86-64-v/d' arch/x86/Makefile || true
    echo "KBUILD_CFLAGS += -march=x86-64-v${arch_level}" >> arch/x86/Makefile
...
_prepare_compile_env() {
    ...
    THREADS=1
    if test "$RAM" -ge 2000 2>/dev/null; then THREADS=$CPU; fi
...
_execute_compilation() {
    ...
    if test -n "$COMPILED_VER"; then
        info "极客锻造内核 ($COMPILED_VER) 已注入宿主机核心。"
    fi

    local CURRENT=$(uname -r 2>/dev/null || echo "")
    if test -n "$CURRENT"; then
        ...
        find /lib/modules -mindepth 1 -maxdepth 1 -type d 2>/dev/null | grep -v "$CURRENT" | xargs -r rm -rf >/dev/null 2>&1 || true
    fi
...
EOF_RPS_SRV
    ...
    info "极客内核编译与结构优化已全部就绪！系统将在 30 秒后强制重启..."
    sleep 30
    reboot
}

# --- ✂️ Part 5 结束，请复制并合并下方的 Part 6 ✂️ ---
# --- ✂️ 请将此行及其下方的 Part 6 紧接在 Part 5 结尾粘贴 ✂️ ---
...
# --- ✂️ 请将此行及其下方的 Part 7 紧接在 Part 6 结尾粘贴 ✂️ ---
...
check_maxtime_state() {
    local state=$(jq -r '.inbounds[]? | select(. != null) | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.realitySettings.maxTimeDiff // "false"' "$CONFIG" 2>/dev/null | head -n 1)
    if [ "$state" = "60000" ]; then echo "true"; else echo "false"; fi
}

check_routeonly_state() {
    local state=$(jq -r '.inbounds[]? | select(. != null) | select(.protocol=="vless" or .protocol=="shadowsocks") | .sniffing.routeOnly // "false"' "$CONFIG" 2>/dev/null | head -n 1)
    if [ "$state" = "true" ]; then echo "true"; else echo "false"; fi
}

check_sniff_state() {
    local state=$(jq -r '.inbounds[]? | select(. != null) | select(.protocol=="vless" or .protocol=="shadowsocks") | .sniffing.metadataOnly // "false"' "$CONFIG" 2>/dev/null | head -n 1)
    if [ "$state" = "true" ]; then echo "true"; else echo "false"; fi
}

check_affinity_state() {
    local limit_file="/etc/systemd/system/xray.service.d/limits.conf"
    if [ -f "$limit_file" ] && grep -q "^CPUAffinity=" "$limit_file" 2>/dev/null; then echo "true"; else echo "false"; fi
}

check_buffer_state() {
    local limit_file="/etc/systemd/system/xray.service.d/limits.conf"
    if [ -f "$limit_file" ] && grep -q "^Environment=\"XRAY_RAY_BUFFER_SIZE=64\"" "$limit_file" 2>/dev/null; then echo "true"; else echo "false"; fi
}
...
check_zram_state() {
    if ! modprobe -n zram >/dev/null 2>&1 && ! lsmod 2>/dev/null | grep -q zram; then echo "unsupported"; return; fi
...
check_gso_off_state() {
    ...
    local eth_info=$(ethtool -k "$IFACE" 2>/dev/null || echo "")
    if test -z "$eth_info"; then echo "unsupported"; return; fi
...
check_irq_state() {
    ...
    if test "$CORES" -lt 2 2>/dev/null; then echo "unsupported"; return; fi
    local IFACE=$(ip route get 1.1.1.1 2>/dev/null | awk '{print $5; exit}' || echo "")
    if test -z "$IFACE"; then echo "false"; return; fi
    local irq=$(grep "$IFACE" /proc/interrupts 2>/dev/null | head -n 1 | awk '{print $1}' | tr -d ':' || echo "")
    if test -n "$irq"; then
        local mask=$(cat "/proc/irq/$irq/smp_affinity" 2>/dev/null | tr -d '0' || echo "")
...
# --- ✂️ 请将此行及其下方的 Part 8 紧接在 Part 7 结尾粘贴 ✂️ ---
...
for cpu in /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor; do
    if test -f "$cpu"; then echo performance > "$cpu" 2>/dev/null || true; fi
done
EOF
    fi
    ...
    elif [ "$(check_gso_off_state)" = "false" ]; then
...
ACK_FLAG=""
if test -f "/usr/local/etc/xray/flags/ack_filter"; then ACK_FLAG="ack-filter"; fi
ECN_FLAG=""
if test -f "/usr/local/etc/xray/flags/ecn"; then ECN_FLAG="ecn"; fi
WASH_FLAG=""
if test -f "/usr/local/etc/xray/flags/wash"; then WASH_FLAG="wash"; fi
EOF
...
_apply_cake_live() {
    ...
    if test "$(check_cake_state)" = "true"; then
        local base_opts=""
        if test -f "$CAKE_OPTS_FILE"; then base_opts=$(cat "$CAKE_OPTS_FILE" 2>/dev/null || echo ""); fi
        
        local f_ack=""
        if test "$(check_ackfilter_state)" = "true"; then f_ack="ack-filter"; fi
        
        local f_ecn=""
        if test "$(check_ecn_state)" = "true"; then f_ecn="ecn"; fi
        
        local f_wash=""
        if test "$(check_wash_state)" = "true"; then f_wash="wash"; fi
...
# --- ✂️ 请将此行及其下方的 Part 9 紧接在 Part 8 结尾粘贴 ✂️ ---
...
toggle_dnsmasq() {
    if [ "$(check_dnsmasq_state)" = "true" ]; then
        ...
        _safe_jq_write 'select(.dns != null) | .dns = {"servers":["https://8.8.8.8/dns-query","https://1.1.1.1/dns-query","https://doh.opendns.com/dns-query"], "queryStrategy":"UseIP"}'
    else
        export DEBIAN_FRONTEND=noninteractive
...
EOF
        ...
        _safe_jq_write 'select(.dns != null) | .dns = {"servers":["127.0.0.1"], "queryStrategy":"UseIP"}'
    fi
}
...
toggle_ring() {
    local IFACE=$(ip route get 1.1.1.1 2>/dev/null | awk '{print $5; exit}' || echo "")
    if [ "$(check_ring_state)" = "unsupported" ] || [ -z "$IFACE" ]; then return; fi
    if [ "$(check_ring_state)" = "true" ]; then
...
toggle_journal() {
    ...
    if [ "$(check_journal_state)" = "true" ]; then
        ...
        if grep -q "^#Storage=" "$conf" 2>/dev/null; then sed -i 's/^#Storage=.*/Storage=volatile/' "$conf" 2>/dev/null || true
        elif grep -q "^Storage=" "$conf" 2>/dev/null; then sed -i 's/^Storage=.*/Storage=volatile/' "$conf" 2>/dev/null || true
...
toggle_buffer() {
    local limit_file="/etc/systemd/system/xray.service.d/limits.conf"
    if [ ! -f "$limit_file" ]; then return; fi
    if grep -q "XRAY_RAY_BUFFER_SIZE=64" "$limit_file" 2>/dev/null; then
...
toggle_routeonly() {
    if [ "$(check_routeonly_state)" = "true" ]; then
        _safe_jq_write '(.inbounds[]? | select(. != null) | select(.protocol=="vless" or .protocol=="shadowsocks") | .sniffing.routeOnly) = false'
    else
        _safe_jq_write '(.inbounds[]? | select(. != null) | select(.protocol=="vless" or .protocol=="shadowsocks") | .sniffing.routeOnly) = true'
    fi
}
...
toggle_cake_flag() {
    ...
    if [ -f "$FLAGS_DIR/$flag" ]; then 
        rm -f "$FLAGS_DIR/$flag" 2>/dev/null || true
    else 
        touch "$FLAGS_DIR/$flag" 2>/dev/null || true
    fi
    _apply_cake_live
}
...
_turn_on_app() {
    ...
    if [ -n "$has_reality" ] && [ "$has_reality" != "null" ]; then
        _safe_jq_write '(.inbounds[]? | select(. != null) | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.realitySettings.maxTimeDiff) = 60000'
    fi
...
# --- ✂️ 请将此行及其下方的 Part 10 紧接在 Part 9 结尾粘贴 ✂️ ---
        ...
        case "$app_opt" in
            1)
                if [ "$out_fastopen" = "true" ]; then
                    _safe_jq_write 'del(.outbounds[]? | select(. != null) | select(.protocol=="freedom") | .streamSettings?.sockopt?.tcpNoDelay, .streamSettings?.sockopt?.tcpFastOpen) | del(.inbounds[]? | select(. != null) | select(.protocol=="vless") | .streamSettings?.sockopt?.tcpNoDelay, .streamSettings?.sockopt?.tcpFastOpen)'
            ...
            2)
                if [ "$out_keepalive" = "30" ]; then
                    _safe_jq_write 'del(.outbounds[]? | select(. != null) | select(.protocol=="freedom") | .streamSettings?.sockopt?.tcpKeepAliveIdle, .streamSettings?.sockopt?.tcpKeepAliveInterval) | del(.inbounds[]? | select(. != null) | select(.protocol=="vless") | .streamSettings?.sockopt?.tcpKeepAliveIdle, .streamSettings?.sockopt?.tcpKeepAliveInterval)'
            ...
            3)
                if [ "$sniff_status" = "true" ]; then
                    _safe_jq_write '(.inbounds[]? | select(. != null) | select(.protocol=="vless") | .sniffing.metadataOnly) = false'
                    info "嗅探引擎减负已关闭，恢复深度探包。"
                else
                    _safe_jq_write '(.inbounds[]? | select(. != null) | select(.protocol=="vless") | .sniffing.metadataOnly) = true'
                    info "嗅探引擎减负已开启！"
                fi
                systemctl restart xray >/dev/null 2>&1 || true
                local _p=""; read -rp "按 Enter 继续..." _p || true
            ...
            10)
                if [ "$mph_state" = "true" ]; then
                    _safe_jq_write 'del(select(.routing != null) | .routing.domainMatcher)'
                    systemctl restart xray >/dev/null 2>&1 || true
                    info "MPH 路由匹配已关闭，恢复普通寻址。"
                else
                    _safe_jq_write 'select(.routing != null) | (.routing) |= (. // {}) | (.routing.domainMatcher) = "mph"'
                    systemctl restart xray >/dev/null 2>&1 || true
            ...
            28)
                if [ "$((app_off_count + sys_off_count))" -gt 0 ]; then
                    ...
                    systemctl restart xray >/dev/null 2>&1 || true
                    info "上帝之手：1-25 项全域极致优化已满血注入！(不支持的硬件已跳过)"
                else
...
do_sys_init_menu() {
    while true; do
        ...
        echo -e "  ${magenta}4) [极客锻造] 原生提档内核源码，动态直连拉取全速编译 (支持主线 / Xanmod 分支双选)${none}"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e12 - (The Apex Vanguard)${none}"
...
main_menu
# ==============================================================================
# EOF - Apex Vanguard V196e12 System Advanced Control Ready.
# ==============================================================================

# --- ✂️ Part 10 结束，所有代码拼接完毕。请运行 bash ex196e12.sh ---
