#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex108.sh (The Apex Vanguard - Project Genesis V108 [Absolute Zero Defect])
# 快捷方式: xrv
# 终极无瑕: 
#   1. 协议大一统：修复 Shadowsocks 单独安装时的“无限空转死锁”，将底层加速、嗅探与零拷贝旁路全量覆盖至 SS 协议，实现多协议同等待遇。
#   2. 语法防爆降维：彻底根除 jq 1.6+ 专属的 `//=` 语法炸弹，全面采用绝对向下兼容的 `(. // {})` 容错映射，确保在所有老旧 Linux 发行版上 100% 存活。
#   3. 抗阻断装甲：为核心拉取与探测模块加装严格的超时防线，免疫因 GitHub 或外部网络抽风导致的脚本线程假死。
#   4. 逻辑拨乱反正：维系 GSO/GRO 硬件卸载的探针反演正确性，死守零延迟电竞模式的纯粹性。
#   5. 大一统矩阵：全域 23 项极限微操 + 3 项一键统御，构建单节点路由的终极物理形态。
#   6. 不追求代码行数的精简，只追求工业级的绝对健壮，敬这单节点路由架构的巅峰。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex108.sh"
    exit 1
fi
...
check_gso_off_state() {
    ...
    if ethtool -k "$IFACE" 2>/dev/null | grep -q "rx-gro: off"; then echo "true"; else echo "false"; fi
}
check_irq_state() {
    local CORES=$(nproc)
...
_turn_on_app() {
    # 彻底重写 jq 数组赋值链，采用纯正的向上映射与合并算法，无视任何老版本 jq 的语法壁垒
    _safe_jq_write '
      .routing = (.routing // {}) |
...
do_app_level_tuning_menu() {
    while true; do
        ...
        local s22; if [ "$gso_off_state" = "true" ]; then s22="${cyan}已打散 (零延迟电竞模式)${none}"; else s22="${gray}未打散 (系统默认万兆聚合)${none}"; fi
        ...
        case "$app_opt" in
            ...
            22) toggle_gso_off; read -rp "按 Enter 继续..." _ ;;
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex108 The Apex Vanguard - Project Genesis V108${none}"
