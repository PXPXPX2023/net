#!/usr/bin/env bash
# ============================================================
# 脚本名称: g7g45.sh (The Omni-Ascendant V3 Edition)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 完美引入自定义节点备注 (利用 email 字段)，多用户管理一目了然。
#   2. 新增用户时支持自选专属节点名称，配套生成带名字的分发链接与二维码。
#   3. Reality 严苛质检: TLS 1.3 + ALPN h2 + OCSP 三重强制校验。
#   4. 商用级网卡流量计费，支持自定义账单周期与初装日追踪。
#   5. GFW 连通性物理探测，精准甄别 DNS 投毒与中国境内 CDN。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash g7g45.sh"
    exit 1
fi
...
do_user_manager() {
    while true; do
        title "用户管理 (增删用户备注、UUID、ShortId)"
        if ! test -f "$CONFIG"; then error "未发现配置"; return; fi

        # 采用竖线 | 分割，完美兼容含有空格的自定义节点名
        ...
        if test "$uopt" = "a"; then
            ...
            read -rp "请输入该用户的专属节点备注 (直接回车默认: User-${ns}): " u_remark
            ...
            info "用户创建成功！该用户专属节点信息如下："
            # 链接尾部的锚点直接使用新定义的 u_remark
...
EOF
    ...
    if test "$proto_choice" = "1" || test "$proto_choice" = "3"; then
        ...
        # 写入节点时，将用户输入的 REMARK_NAME 作为主用户的 email 备注
        local VLESS_INBOUND='{
            "tag": "vless-reality",
            "listen": "0.0.0.0",
            "port": '$LISTEN_PORT',
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray G7G45 The Omni-Ascendant V3 Edition${none}"
        ...
        echo "  2) 用户管理 (增删用户备注、UUID、ShortId)"
