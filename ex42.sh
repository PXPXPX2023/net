#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex42.sh (The Apex Vanguard - Project Genesis V15)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 完美融入官方预编译 XANMOD (main) 部署模块，根除低配机源码编译断连风险。
#   2. 植入 CPU 微架构 (x86-64-psABI) 智能探针，自动匹配 v1-v4 极速指令集内核。
...
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex42.sh"
    exit 1
fi
...
# -- 【新增】官方预编译 XANMOD (main) 部署模块 (彻底避免低配机编译假死) --
...
do_xanmod_compile() {
    ...
    warn "警告: 这是一个极其漫长的过程 (30-60分钟)，低配机极易引发死机断连！强烈建议使用菜单 32 的官方预编译版。"
...
do_sys_init_menu() {
    while true; do
        ...
        echo -e "  ${cyan}32) 安装 XANMOD (main) 官方预编译内核 (推荐/防断连/自动重启)${none}"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex42 The Apex Vanguard - Project Genesis V15${none}"
