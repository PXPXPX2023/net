#!/usr/bin/env bash
# ============================================================
# 脚本名称: dx28.sh (The Apex Vanguard - Project Genesis)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 拒绝任何妥协与折叠，JSON注入、内核编译等模块100%全量展开。
#   2. 引入 Swap 创建降级容错机制 (fallocate 失败则转 dd)，适配所有文件系统。
#   3. 补齐卸载模块的 chattr -i 解锁逻辑，确保 DNS 强锁体面退场不留残疾。
#   4. 深度融合 Xanmod 内核 + BBR3 编译，精准提取 Stable 稳定版源码。
#   5. TX Queue 队列调优与 Sysctl 百万并发压榨全量在线。
#   6. 130+ 纯净矩阵秒停、VLESS+SS 双规并存、商用汉化计费等核心神技全量融贯。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash dx28.sh"
    exit 1
fi
...
preflight() {
    ...
    # 新增 e2fsprogs(提供chattr) 和 pkg-config(编译备用) 确保万无一失
    local need="jq curl wget xxd unzip qrencode vnstat cron openssl coreutils sed e2fsprogs pkg-config"
    local install_list=""
    for i in $need; do 
...
do_change_dns() {
    ...
    # 彻底焊死 DNS 文件，防止 systemd-resolved 后台修改
    chattr +i /etc/resolv.conf 2>/dev/null || true
    ...
    cat /etc/resolv.conf | grep nameserver
    print_yellow "旧配置已自动备份至 /etc/resolv.conf.bak"
}
...
EOF
    ...
    echo "=== 检查并创建 Swap 虚拟内存 ==="
    if ! swapon --show | grep -q swapfile; then
        echo "自动创建 1GB Swap..."
        # 工业级容错：如果 fallocate 失败 (例如在 ZFS 上)，则降级使用 dd
        if ! fallocate -l 1024M /swapfile 2>/dev/null; then
            echo "fallocate 失败，正在使用 dd 降级创建..."
    ...
    apt update -y
    apt install -y build-essential bc bison flex libssl-dev libelf-dev git wget curl xz-utils ethtool numactl make pkg-config
    ...
    if ! tar -tJf $KERNEL_FILE >/dev/null 2>&1; then
        rm -f $KERNEL_FILE
        wget -q --show-progress $KERNEL_URL -O $KERNEL_FILE
        tar -tJf $KERNEL_FILE >/dev/null 2>&1 || { echo "下载或解压失败"; exit 1; }
    fi
...
[Unit]
Description=RPS RFS Network Optimization
After=network.target
...
EOF
    ...
    info "内核编译已全部就绪！系统将在 30 秒后自动重启应用更改..."
    sleep 30
    reboot
}
...
do_perf_tuning() {
    ...
    warn "警告: 该操作将写入百万级并发参数，执行后系统将自动重启！"
...
EOF
    ...
    info "极限参数注入完成！系统将在 30 秒后重启以应用更改..."
    sleep 30
    reboot
}
    ...
    while true; do
        ...
        case "$sys_opt" in
            1) 
                ...
                # 注入非交互式环境变量，防止 apt-get upgrade 卡死在系统弹窗上
                export DEBIAN_FRONTEND=noninteractive
                
                apt-get update -y
                apt-get -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" full-upgrade -y
                apt-get autoremove -y --purge
                apt-get install -y wget curl sudo socat ntpdate
...
# -- 安装主逻辑 (拒绝折叠，全量展开) --
do_install() {
    title "Apex Vanguard Ultimate Final V13: 核心部署"
    preflight
  ...
  "routing": {
    ...
    "rules": ${L_B}
      ...
      },
      {
        "outboundTag": "block",
        "_enabled": true,
        "domain": ${L_B} "geosite:category-ads-all" ${R_B}
      }
    ${R_B}
  },
  "inbounds": ${L_B} ${R_B},
...
EOF
    ...
    if test "$proto_choice" = "1" || test "$proto_choice" = "3"; then
        ...
        local VLESS_INBOUND='{
            ...
            "streamSettings": {
                ...
                "realitySettings": {
                    "dest": "'"$BEST_SNI"':443",
                    "serverNames": '${L_B}' '"$SNI_JSON_ARRAY"' '${R_B}',
                    "privateKey": "'"$priv"'",
                    "publicKey": "'"$pub"'",
                    "shortIds": '${L_B}' "'"$sid"'" '${R_B}'
                }
            },
            "sniffing": {
                "enabled": true,
                "destOverride": '${L_B}' "http", "tls", "quic" '${R_B}'
            }
        }'
    ...
    while true; do
        ...
        if [[ "$opt" == "b" || "$opt" == "B" ]]; then
            choose_sni
            if test $? -eq 0; then
                _safe_jq_write "(.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .streamSettings.realitySettings.serverNames) = ${L_B} $SNI_JSON_ARRAY ${R_B} | (.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .streamSettings.realitySettings.dest) = \"$BEST_SNI:443\""
                systemctl restart xray >/dev/null 2>&1
                do_summary
            else
                break
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray dx27 The Apex Vanguard Ultimate Final${none}"
        ...
        echo "  10) 初次安装、更新系统组件"
        echo "  0) 退出"
        echo -e "  ${red}88) 彻底卸载 (清空一切痕迹)${none}"
        hr
