#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex87.sh (The Apex Vanguard - Project Genesis V87)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 维度跃升：优化 Dnsmasq 缓存池 (21000条) 并采用 8.8.8.8 / 1.1.1.1 / OpenDNS 黄金并发组合。
#   2. 路由斩断：将 Xray domainStrategy 重置为 AsIs，彻底消除域名路由阶段的多余 DNS 解析动作，纯 IP 直接穿透。
#   3. 架构排爆：剔除原版系统 DNS 修改逻辑，防止与 Dnsmasq 产生 resolvconf 锁冲突。
#   4. Xray 轻量化：开启 Dnsmasq 后，自动剥离 Xray 沉重的 DoH 外壳，直连 127.0.0.1 享受 0.1ms 解析。
#   5. 七边形矩阵：扩容为 14 项极客双向微操枢纽 (应用层 7 项 + 系统底层 7 项)。
#   6. 智能反转引擎：15/16/17 项全量自适应硬件探测，缺省即补齐，全满即一键卸载复原。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex87.sh"
    exit 1
fi
...
do_install_xanmod_main_official() {
    ...
    print_magenta ">>> [3/4] 正在通过 APT 极极速拉取并安装专属内核: $pkg_name ..."
    apt-get update -y
...
# --- 新增的 Dnsmasq 本地极速内存缓存引擎 (第 8 项微操) ---
toggle_dnsmasq() {
...
do_app_level_tuning_menu() {
    while true; do
        ...
        # 缺省探测引擎 (系统层 - 8~14项 硬件自适应剔除)
        local sys_off_count=0
        [ "$(check_dnsmasq_state)" = "false" ] && sys_off_count=$((sys_off_count+1))
        ...
        local s8=$([ "$(check_dnsmasq_state)" = "true" ] && echo "${cyan}内存极速解析中 (0.1ms)${none}" || echo "${gray}未开启 (依赖系统原生)${none}")
        local s9=$([ "$(check_thp_state)" = "true" ] && echo "${cyan}已关闭 THP${none}" || ([ "$(check_thp_state)" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}系统默认${none}"))
        local s10=$([ "$(check_mtu_state)" = "true" ] && echo "${cyan}智能探测中${none}" || ([ "$(check_mtu_state)" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}未开启${none}"))
        local s11=$([ "$(check_cpu_state)" = "true" ] && echo "${cyan}全核火力全开${none}" || ([ "$(check_cpu_state)" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}节能待机${none}"))
        local s12=$([ "$(check_ring_state)" = "true" ] && echo "${cyan}已反向收缩${none}" || ([ "$(check_ring_state)" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}系统大缓冲${none}"))
        local s13=$([ "$(check_zram_state)" = "true" ] && echo "${cyan}已挂载 ZRAM${none}" || ([ "$(check_zram_state)" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}未启用${none}"))
        local s14=$([ "$(check_journal_state)" = "true" ] && echo "${cyan}纯内存极速化${none}" || ([ "$(check_journal_state)" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}磁盘 IO 写入中${none}"))
        local s15=$([ "$(check_process_priority_state)" = "true" ] && echo "${cyan}OOM免死 / IO抢占${none}" || echo "${gray}系统默认调度${none}")
        ...
        echo -e ""
        echo -e "  ${magenta}--- Linux 系统层与内核黑科技 (8-14) ---${none}"
        ...
        echo -e "  "
        echo -e "  ${cyan}16) 一键开启或关闭 1-6 项 应用层微操 (自动侦测并智能反转)${none}"
        ...
        case "$app_opt" in
            ...
            6)
                ...
                read -rp "按 Enter 继续..." _
                ;;
            8) toggle_dnsmasq; read -rp "按 Enter 继续..." _ ;;
            ...
            15) toggle_process_priority; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            16)
            ...
            17)
                if [ "$sys_off_count" -gt 0 ]; then
                    [ "$(check_dnsmasq_state)" = "false" ] && toggle_dnsmasq >/dev/null 2>&1
                    [ "$(check_thp_state)" = "false" ] && toggle_thp >/dev/null 2>&1
                    [ "$(check_mtu_state)" = "false" ] && toggle_mtu >/dev/null 2>&1
                    [ "$(check_cpu_state)" = "false" ] && toggle_cpu >/dev/null 2>&1
                    [ "$(check_ring_state)" = "false" ] && toggle_ring >/dev/null 2>&1
                    [ "$(check_zram_state)" = "false" ] && toggle_zram >/dev/null 2>&1
                    ...
                    [ "$(check_dnsmasq_state)" = "true" ] && toggle_dnsmasq >/dev/null 2>&1
                    [ "$(check_thp_state)" = "true" ] && toggle_thp >/dev/null 2>&1
                    [ "$(check_mtu_state)" = "true" ] && toggle_mtu >/dev/null 2>&1
                    [ "$(check_cpu_state)" = "true" ] && toggle_cpu >/dev/null 2>&1
                    [ "$(check_ring_state)" = "true" ] && toggle_ring >/dev/null 2>&1
                    [ "$(check_zram_state)" = "true" ] && toggle_zram >/dev/null 2>&1
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex87 The Apex Vanguard - Project Genesis V87${none}"
