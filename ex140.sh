#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex140.sh (The Apex Vanguard - Project Genesis V140 [Absolute Horizon])
# 快捷方式: xrv
# 巅峰重铸宣言: 
#   1. 绝杀编译死锁：彻底修复 Xanmod 编译中因宿主机配置缺失 CONFIG_X86_64_VERSION 导致的 -march 编译参数断裂 Bug；引入动态 CPU 级别探测与官方 scripts/config 静默注入。
#   2. 百万并发回归：系统底层连接池上限恢复至 LimitNOFILE=1048576 与 LimitNPROC=512000，拒绝任何性能缩水。
#   3. 补全幽灵拼图：实装第 8 号主菜单 (Reality 回落限速探针)，并在 config.json 写入时完整附带 Fallback 诱饵陷阱参数。
#   4. 军规级对齐：严格执行 8 行冒号阵列输出 (包含协议/IP/端口/SNI/公钥/ShortId/uTLS/UUID)。
#   5. 状态同步：完美继承 ax131 的 clients 与 shortIds 数组同步截断算法，根除删除用户导致的断流漂移。
#   6. 绝对不省代码：完整保留 ex118 的多行 jq 管道流与 17 项 BBR3 极限内核基线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex140.sh"
    exit 1
fi
...
# V140 绝杀修复: 完美实装全自动 Xanmod 源码编译，绝杀所有的架构断层和交互弹窗！
do_xanmod_compile() {
    ...
    print_magenta ">>> [3/4] 初始化 Xanmod 原生基底并静默注入 BBR3..."
    # 彻底废弃复制宿主机 config，防止破坏 Xanmod 定制的架构变量导致 -march 编译参数为空
    make defconfig
    
    # 动态探测 CPU x86-64 架构级别并写入配置，彻底绝杀 ‘x86-64-v’ 为空的致命 Bug
    local cpu_level_script="/tmp/check_x86-64_psabi.sh"
    wget -qO "$cpu_level_script" https://dl.xanmod.org/check_x86-64_psabi.sh
    local cpu_level=$(awk -f "$cpu_level_script" | grep -oE 'x86-64-v[1-4]' | grep -oE '[1-4]' | tail -n 1)
    rm -f "$cpu_level_script"
    ...
    # 强制静默推演所有缺失依赖项，彻底剥离交互式弹窗
    yes "" | make olddefconfig

    print_magenta ">>> [4/4] 全核全速编译中，请耐心等待 (切勿中断)..."
    # 加入物理熔断，一旦 make 失败立即退出，不再报假成功
...
do_perf_tuning() {
    ...
    # 100% 还原 ex118 的 17 项 BBR 参数配置，同时在末尾追加内存倾斜参数
...
do_app_level_tuning_menu() {
    while true; do
        ...
        local s21; if [ "$irq_state" = "true" ]; then s21="${cyan}多核 RPS / 单核绑死 自动激活${none}"; elif [ "$irq_state" = "unsupported" ]; then s21="${gray}不支持(单核)${none}"; else s21="${gray}默认平衡调度${none}"; fi
...
EOF
            ...
            if test "${total:-0}" -le 1; then error "必须至少保留一个核心用户！"
                ...
                if test -n "$target_uuid"; then
                    local idx=$((${dnum:-0}-1))
                    # -- 核心 Bug 修复：同步安全删除 clients 数组项与 shortIds 数组项，绝杀索引错位漂移 --
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex140 The Apex Vanguard - Project Genesis V140${none}"
