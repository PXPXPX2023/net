#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e103.sh (The Apex Vanguard - Project Genesis V198e103)
# 快捷方式: xrv
# 【V198e103 三协议稳态版：e97基线 + 内置BBR低资源硬熔断 + XanMod源码路线提示】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e103.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e103"
...
#==============================================================================
# V198e103 FINAL OVERRIDE LAYER
# 目标：
#   1) 保留 ex198e103 完整主菜单 1-20/88，不裁剪任何目录入口。
...
_e102_builtin_full_compile_precheck() {
    ...
    title "V198e103：内置型 BBR 完整内核编译硬门禁"
    ...
    if test "${XRAY_E102_ALLOW_BUILTIN_FULL_COMPILE:-0}" != "1" && test "${XRAY_E100_ALLOW_BUILTIN_FULL_COMPILE:-0}" != "1" && test "${XRAY_E98_ALLOW_BUILTIN_FULL_COMPILE:-0}" != "1"; then
        warn "为避免误触，V198e103 默认禁止内置型 BBR 完整编译。"
...
_e102_builtin_explain() {
    title "V198e103：内置型 BBR / XanMod 说明"
...
_e96_bbr_module_source_report() {
    ...
    cfgv="$(_e89_bbr_kconfig_value 2>/dev/null || echo unknown)"
    if test "$cfgv" = "y"; then
        title "V198e103：BBR 源码检查（内置型 y 专用提示）"
        echo "  当前内核：$(uname -r 2>/dev/null || echo unknown)"
    ...
    if declare -F _e96_bbr_module_source_report_e102_base >/dev/null 2>&1; then
        ...
        title "V198e103：BBR 模块源码 / headers 检查"
...
_e102_builtin_bbr_full_compile_menu() {
    ...
    while true; do
        cfgv="$(_e89_bbr_kconfig_value 2>/dev/null || echo unknown)"
        title "V198e103：内置型 BBR 完整源码补丁编译入口"
...
_e102_bbr3_flow() {
    ...
    while true; do
        cfgv="$(_e89_bbr_kconfig_value 2>/dev/null || echo unknown)"
        title "V198e103：BBR3 / XanMod → bbrx3_v1 参数中心"
        echo "  当前替换路线：$(_e102_bbr_mode_text)"
        if test "$cfgv" = "y"; then
...
_e102_bbrv1_flow() {
    ...
    while true; do
        cfgv="$(_e89_bbr_kconfig_value 2>/dev/null || echo unknown)"
        title "V198e103：普通 Linux 主线 BBR v1 → bbrx_v1 参数中心"
        echo "  当前替换路线：$(_e102_bbr_mode_text)"
        if test "$cfgv" = "y"; then
...
do_bbr_body_replace_menu_e89() {
    while true; do
        clear
        title "V198e103：BBR 本体替换 / 自定义编译中心"
        echo "  自动建议：$(_e89_bbr_detect 2>/dev/null || true)"
        echo "  当前替换路线：$(_e102_bbr_mode_text)"
        echo ""
        echo "  1) 自动检测当前 BBR 类型，并推荐编译方案"
...
hot_update_from_legacy() {
    title "V198e103：稳态热更新（完整主菜单保留 + BBR目录修复 + 真BBRv3源码/补丁1000MB门槛）"
    if declare -F hot_update_from_legacy_e102_base >/dev/null 2>&1; then
        hot_update_from_legacy_e102_base "$@" || true
    fi
    _e102_prepare_dirs
    echo "198e103" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    if test -f "${SCRIPT_PATH:-$0}"; then
        cp -f "${SCRIPT_PATH:-$0}" "$SYMLINK" 2>/dev/null || true
        cp -f "${SCRIPT_PATH:-$0}" "$SCRIPT_DIR/ex198e103.sh" 2>/dev/null || true
        cp -f "${SCRIPT_PATH:-$0}" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
        chmod +x "$SYMLINK" "$SCRIPT_DIR/ex198e103.sh" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
    fi
    hash -r 2>/dev/null || true
    info "V198e103 热更新完成。"
...
main_menu() {
    while true; do
        ...
        actual_qdisc="$(tc qdisc show dev "$iface" 2>/dev/null | head -n 1 || echo unknown)"
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e103 - (真BBRv3源码/补丁1000MB门槛 + 目录修复 + 防误编译)${none}"
        echo -e "  引擎态势: ${green}${running}${none} | 全局快捷指令: ${cyan}xrv${none} | 外部识别 IP: ${yellow}${ip}${none}"
        echo -e "  系统装配内核: ${cyan}$(uname -r 2>/dev/null || echo unknown)${none} | 微架构侦测: x64-v${yellow}$(detect_x86_64_level 2>/dev/null || echo '?')${none} | sysctl qdisc/cc: ${yellow}$(_e88_sysctl_get net.core.default_qdisc 2>/dev/null || sysctl -n net.core.default_qdisc 2>/dev/null || echo '?')/$(_e88_sysctl_get net.ipv4.tcp_congestion_control 2>/dev/null || sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null || echo '?')${none} | BBR配置:${yellow}${bbr_cfg}${none}"
        echo -e "  实际root队列: ${cyan}${actual_qdisc}${none}"
        echo -e "  DNS模式: ${cyan}$(cat "${E67_DNS_MODE_FILE:-/usr/local/etc/xray/dns_mode.txt}" 2>/dev/null || echo auto)${none} | 时间轴: ${cyan}${nowtz}${none} | notsent: ${yellow}$(_e88_current_notsent_lowat 2>/dev/null || echo '?')${none}/${cyan}$(_e88_persistent_notsent_lowat 2>/dev/null || echo '?')${none} | adv/app: ${yellow}$(_e88_sysctl_get net.ipv4.tcp_adv_win_scale 2>/dev/null || sysctl -n net.ipv4.tcp_adv_win_scale 2>/dev/null || echo '?')/$(_e88_sysctl_get net.ipv4.tcp_app_win 2>/dev/null || sysctl -n net.ipv4.tcp_app_win 2>/dev/null || echo '?')${none}"
        ...
        echo "  9) 全景网络连接审计日志与商用级别网络流量记账本"
        echo "  10) 高级发烧：内核双轨编译(APT/源码)/网络/应用层 60余项极限全控板"
        echo -e "  ${green}11) 从旧 ex198e 环境稳态热更新到 V198e103${none}"
        echo -e "  ${cyan}12) 清理空间（安全释放 / 快照收敛 / 显示硬盘与 Swap）${none}"
        echo -e "  ${cyan}13) 应用网络参数（缓冲/notsent/adv_win/app_win/txqueue/OOOX/MSS）${none}"
        echo -e "  ${cyan}14) TCP 重传 / 单连接诊断中心（只读排障）${none}"
        echo -e "  ${cyan}15) 检查现有配置并重新输出节点${none}"
...
_e102_bbr_module_source_report() {
    ...
    krel="$(uname -r 2>/dev/null || echo unknown)"
    kshort="$(_e102_kernel_shortver)"
    title "V198e103：BBR 模块源码 / headers 严格检查"
    echo "  当前内核：$krel"
    echo "  CONFIG_TCP_CONG_BBR：$cfgv"
    echo "  tcp_bbr 模块路径：$(_e89_bbr_mod_path 2>/dev/null || echo '')"
...
_e102_build_replace_bbr_module() {
    local kind="$1" conf="$2" cfgv modp src work newko bdir release_now timeout_s kbuild
    title "V198e103：模块型 BBR 严格快编热替换（只编 tcp_bbr.ko）"
    _e89_bbr_prepare_dirs
    _e102_prepare_dirs
...
EOF_ROLLBACK_E102
    ...
    info "V198e103 模块型 BBR 快编替换完成。备份目录：$bdir"
    warn "请验证：sysctl net.ipv4.tcp_congestion_control && modinfo tcp_bbr | head && ss -tin"
    return 0
}
...
_e89_bbr_detect() {
    title "V198e103：BBR 本体自动检测与编译建议"
    _e89_bbr_prepare_dirs
    local krel cfgv cc av modp src kind rec
    krel="$(uname -r 2>/dev/null || echo unknown)"
    cfgv="$(_e89_bbr_kconfig_value 2>/dev/null || echo unknown)"
...
hot_update_from_legacy() {
    title "V198e103：稳态热更新（BBR模块源码严格匹配 + 换行补丁修复 + 目录保留）"
    if declare -F hot_update_from_legacy_e102_base >/dev/null 2>&1; then
        hot_update_from_legacy_e102_base "$@" || true
    fi
    _e102_prepare_dirs
...
# V198e103 FINAL OVERRIDE LAYER
# 目标：
#   1) 移除菜单 13 里的 initcwnd/initrwnd=32 预设入口。
#   2) 热更新不再默认写入 initcwnd/initrwnd；已存在显式值会恢复为系统默认。
#   3) 关闭 Xray/DAT 自动更新定时器与相关 cron；保留手动更新入口。
#==============================================================================

if declare -F hot_update_from_legacy >/dev/null 2>&1 && ! declare -F hot_update_from_legacy_e103_base >/dev/null 2>&1; then
    eval "$(declare -f hot_update_from_legacy | sed '1s/hot_update_from_legacy/hot_update_from_legacy_e103_base/')" 2>/dev/null || true
fi
if declare -F do_update_core >/dev/null 2>&1 && ! declare -F do_update_core_e103_base >/dev/null 2>&1; then
    eval "$(declare -f do_update_core | sed '1s/do_update_core/do_update_core_e103_base/')" 2>/dev/null || true
fi
if declare -F _e88_set_init_window >/dev/null 2>&1 && ! declare -F _e88_set_init_window_e103_base >/dev/null 2>&1; then
    eval "$(declare -f _e88_set_init_window | sed '1s/_e88_set_init_window/_e88_set_init_window_e103_base/')" 2>/dev/null || true
fi

_e103_disable_xray_auto_update() {
    mkdir -p "$CONFIG_DIR" "$SCRIPT_DIR" "$LOG_DIR" 2>/dev/null || true

    # 关闭脚本曾经创建或常见命名的 Xray / dat 自动更新单元。
    local units=(
        xray-dat-update.timer xray-dat-update.service
        xray-update.timer xray-update.service
        xray-core-update.timer xray-core-update.service
        xray-geoip-update.timer xray-geoip-update.service
        xray-geosite-update.timer xray-geosite-update.service
    ...
    # 删除本脚本旧版生成的 dat 自动更新 timer/service，避免 daemon-reload 后再次被启用。
    rm -f /etc/systemd/system/xray-dat-update.timer /etc/systemd/system/xray-dat-update.service 2>/dev/null || true
    systemctl daemon-reload >/dev/null 2>&1 || true

    # 清理 cron 内 Xray 自动更新 / 规则库自动更新 / 旧版定时重启条目。
    local tmp_cron
    tmp_cron=$(mktemp /tmp/cron_e103_XXXXXX) || true
    if test -n "${tmp_cron:-}"; then
        crontab -l 2>/dev/null | awk '
            /update-dat\.sh/ {next}
            /xray-dat-update/ {next}
            /install-release\.sh/ {next}
            /XTLS\/Xray-install/ {next}
            /Xray-install/ {next}
            /[xX]ray/ && /[uU]pdate/ {next}
            $0 == "10 3 * * * /bin/systemctl restart xray >/dev/null 2>&1" {next}
            {print}
        ' > "$tmp_cron" || true
        crontab "$tmp_cron" 2>/dev/null || true
        rm -f "$tmp_cron" 2>/dev/null || true
    ...
    printf 'disabled_at=%s\n' "$(date '+%F %T')" > "$CONFIG_DIR/xray_auto_update_disabled_e103.txt" 2>/dev/null || true
}

install_update_dat() {
    # e103：保留手动更新脚本，不再创建/启用 systemd timer，不再写入 cron。
    mkdir -p "$SCRIPT_DIR" "$DAT_DIR" "$LOG_DIR" 2>/dev/null || true
    cat > "$UPDATE_DAT_SCRIPT" <<'UPDSH_E103'
#!/usr/bin/env bash
set -u
XRAY_DAT_DIR="/usr/local/share/xray"
LOG_FILE="/var/log/xray/update-dat.log"
...
dl_if_changed "https://github.com/Loyalsoldier/domain-list-custom/releases/latest/download/geosite.dat" "$XRAY_DAT_DIR/geosite.dat" || ok=1
if test -s /tmp/xray-dat-changed.$$ && systemctl is-active --quiet xray; then
    log_msg "[INFO] 手动 dat 更新后文件已变化，重启 Xray 使规则生效。"
    systemctl restart xray >/dev/null 2>&1 || true
fi
rm -f /tmp/xray-dat-changed.$$ 2>/dev/null || true
exit "$ok"
UPDSH_E103
    chmod +x "$UPDATE_DAT_SCRIPT" 2>/dev/null || true
    _e103_disable_xray_auto_update >/dev/null 2>&1 || true
    info "已关闭 Xray/DAT 自动更新定时器；仅保留手动规则库更新脚本：$UPDATE_DAT_SCRIPT"
}

_e103_reset_init_window_to_system_default() {
    local clean
    clean=$(_e88_default_route_clean 2>/dev/null || true)
    if test -n "$clean"; then
        ip route change $clean 2>/dev/null || true
    fi
    rm -f "$CONFIG_DIR/initcwnd.txt" "$CONFIG_DIR/initrwnd.txt" "$E88_INIT_WINDOW_STATE_FILE" 2>/dev/null || true
    systemctl disable --now xray-init-window-e88.service >/dev/null 2>&1 || true
    rm -f /etc/systemd/system/xray-init-window-e88.service /usr/local/bin/xray-init-window-e88.sh 2>/dev/null || true
    systemctl daemon-reload >/dev/null 2>&1 || true
    if declare -F update_hw_boot_script >/dev/null 2>&1; then update_hw_boot_script >/dev/null 2>&1 || true; fi
    ...
    {
        echo "time=$(date '+%F %T')"
        echo "mode=system-default"
        echo "note=e103 已移除热更新默认 initcwnd/initrwnd 写入，并清理开机恢复。"
        echo "route=$(ip route show default 2>/dev/null | head -n1 || true)"
    } > "$CONFIG_DIR/init_window_e103.txt" 2>/dev/null || true
}

_e88_set_init_window() {
    local cw="${1:-}" rw="${2:-$1}" mode="${3:-manual}"
    if test "$cw" = "32" && test "$rw" = "32" && [[ "$mode" =~ ^(fixed32|preset32|auto16-32)$ ]]; then
        warn "e103 已移除 initcwnd/initrwnd=32 预设入口；如确需实验，请走手动输入并自行验证丢包/重传。"
        return 0
    fi
    _e88_set_init_window_e103_base "$@"
}

_e88_show_init_window_state() {
    title "V198e103：TCP 起步窗口 initcwnd / initrwnd 当前状态"
    local route cw rw persisted_cw persisted_rw
    route=$(ip route show default 2>/dev/null | head -n1 || true)
    cw=$(printf '%s\n' "$route" | sed -n 's/.*[[:space:]]initcwnd[[:space:]]\+\([0-9]\+\).*/\1/p')
    rw=$(printf '%s\n' "$route" | sed -n 's/.*[[:space:]]initrwnd[[:space:]]\+\([0-9]\+\).*/\1/p')
    ...
    echo "  持久化 initrwnd: ${persisted_rw:-未记录}"
    echo ""
    echo "  e103 策略：热更新不再默认写入 initcwnd/initrwnd；39 号 32 预设已移除。"
    echo "  可用预设：18、22；自动检测封顶 22；更激进数值只能手动输入。"
}

_e88_reset_init_window_default20() { _e103_reset_init_window_to_system_default; _e88_show_init_window_state; }
_e88_reset_init_window_default21() { _e103_reset_init_window_to_system_default; _e88_show_init_window_state; }
_e88_reset_init_window_default()  { _e103_reset_init_window_to_system_default; _e88_show_init_window_state; }

_e88_manual_init_window() {
    title "V198e103：手动输入 initcwnd/initrwnd"
    _e88_show_init_window_state
    local cw rw
    echo ""
    read -rp "请输入 initcwnd（建议 18/22，默认 20；不建议日常写 32）: " cw || true
    cw=${cw:-20}
    read -rp "请输入 initrwnd（默认与 initcwnd 相同：$cw）: " rw || true
    rw=${rw:-$cw}
    _e88_set_init_window "$cw" "$rw" "manual"
...
_e103_auto_init_window_16_22() {
    title "V198e103：initcwnd/initrwnd 自动检测（16~22 保守封顶）"
    local rtt source retrans loss cw reason
    rtt=$(_e88_ss_rtt_avg_ms 2>/dev/null || true)
    source="ss -ti 活跃连接 RTT"
    if ! [[ "$rtt" =~ ^[0-9]+$ ]] || test "$rtt" -le 0 2>/dev/null; then
    ...
    if ! [[ "$rtt" =~ ^[0-9]+$ ]] || test "$rtt" -le 0 2>/dev/null; then
        ...
        cw=18; reason="RTT 80~179ms，选择 18，稳态偏快。"
    else
        cw=22; reason="RTT>=180ms，选择 22；e103 自动档不再跳到 32。"
    fi

    if [[ "$retrans" =~ ^[0-9]+$ ]] && test "$retrans" -gt 0 2>/dev/null && test "$cw" -gt 18 2>/dev/null; then
        reason="$reason 检测到 TCP retrans=$retrans，自动封顶到 18。"
        cw=18
    fi
...
_e88_auto_init_window_16_32() { _e103_auto_init_window_16_22; }

do_network_params_menu_e56() {
    while true; do
        clear
        title "网络参数应用中心（V198e103：移除 initcwnd=32 预设 + 关闭自动更新）"
        if declare -F show_network_buffer_params_e56 >/dev/null 2>&1; then show_network_buffer_params_e56; fi
        _e88_show_tcp_notsent_lowat_state
        _e88_show_core_network_state
        _e88_show_menu13_manual_notes
        ...
        echo "  34) 查看 RPS/RFS 当前状态"
        echo "  35) 查看 initcwnd/initrwnd 当前状态"
        echo "  36) 恢复 initcwnd/initrwnd 系统默认（移除显式值与开机恢复）"
        echo "  37) 设置 initcwnd/initrwnd = 18"
        echo "  38) 设置 initcwnd/initrwnd = 22"
        echo "  39) 自动检测 initcwnd/initrwnd（16~22，封顶22，不再写32）"
        echo "  40) 手动输入 initcwnd/initrwnd"
        echo "  41) IPv4/IPv6 出口检测 + gai.conf 优先级查看（只读，不禁 IPv6）"
        echo "  0) 返回主菜单"
        hr
        ...
        case "${opt:-}" in
            ...
            34) _e88_show_rps_rfs_state; _e52_pause ;;
            35) _e88_show_init_window_state; _e52_pause ;;
            36) _e103_reset_init_window_to_system_default; _e88_show_init_window_state; _e52_pause ;;
            37) _e88_set_init_window 18 18 "fixed18"; _e52_pause ;;
            38) _e88_set_init_window 22 22 "fixed22"; _e52_pause ;;
            39) _e103_auto_init_window_16_22; _e52_pause ;;
            40) _e88_manual_init_window; _e52_pause ;;
            41) _e88_show_ip_gai_state; _e52_pause ;;
            0) return ;;
            *) warn "无效操作。"; sleep 1 ;;
...
do_update_core() {
    title "V198e103：Xray Core 手动更新 / 修复（自动更新已关闭）"
    warn "e103 不创建任何 Xray 自动更新 timer/cron；本入口只在你确认后执行一次手动拉取。"
    local go=""
    read -rp "是否现在手动更新/修复 Xray Core？[y/N]: " go || true
    if [[ ! "$go" =~ ^[yY]$ ]]; then
        info "已取消；Xray Core 不会自动更新。"
        _e103_disable_xray_auto_update || true
        _e52_pause
        return 0
    fi
    do_update_core_e103_base "$@"
    _e103_disable_xray_auto_update || true
}

hot_update_from_legacy() {
    title "V198e103：稳态热更新（移除 initcwnd=32 预设 + 关闭 Xray 自动更新）"
    if declare -F hot_update_from_legacy_e103_base >/dev/null 2>&1; then
        hot_update_from_legacy_e103_base "$@" || true
    fi
    _e102_prepare_dirs 2>/dev/null || true
    echo "198e103" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    if test -f "${SCRIPT_PATH:-$0}"; then
        cp -f "${SCRIPT_PATH:-$0}" "$SYMLINK" 2>/dev/null || true
        cp -f "${SCRIPT_PATH:-$0}" "$SCRIPT_DIR/ex198e103.sh" 2>/dev/null || true
    ...
    hash -r 2>/dev/null || true
    _e103_disable_xray_auto_update || true
    _e103_reset_init_window_to_system_default || true
    install_update_dat || true
    info "V198e103 热更新完成。"
    info "已移除 13 菜单的 39) initcwnd/initrwnd=32 预设；39 现在改为 16~22 自动检测。"
    info "热更新不再默认写入 initcwnd/initrwnd；已清理旧 initcwnd/initrwnd 持久化与开机恢复服务。"
    info "已关闭 Xray/DAT 自动更新 timer/cron；只保留主菜单 4 的手动规则库更新和主菜单 5 的手动 Core 更新。"
}
...
#==============================================================================
# EOF - Apex Vanguard V198e103 Safety Hot Update Layer.
#==============================================================================
