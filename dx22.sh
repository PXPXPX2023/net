#!/usr/bin/env bash
# ============================================================
# 脚本名称: dx22.sh (The Apex Vanguard - Ultimate Final V13)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 彻底摒弃代码压缩折叠，全量展开底层逻辑，恢复千行级工业标准。
#   2. 深度集成 TX Queue (发送队列) 调优，自动配置 systemd 守护进程。
#   3. 全能系统初始化矩阵：Xanmod+BBR3 源码编译、Sysctl 极限并发压榨。
#   4. 物理级 DNS 强锁引擎，支持 IPv4 正则校验与 resolvconf 联动。
#   5. 保持所有 Reality 质检、130+纯净矩阵秒停、商用计费等高阶特性。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash dx22.sh"
    exit 1
fi
...
[Service]
Type=oneshot
ExecStart=/usr/sbin/ip link set $IFACE txqueuelen 12000
...
EOF
    ...
    echo -e "\n  ${cyan}当前网卡队列状态:${none}"
    ip link show "$IFACE" | grep -o 'qlen [0-9]*' | awk '{print "    " $0}'
    ...
    while true; do
        ...
        case "$sys_opt" in
            1) 
                ...
                apt autoremove -y
                apt update
                apt install wget curl sudo socat ntpdate -y
...
# -- 安装主逻辑 --
do_install() {
    title "Apex Vanguard Ultimate: 核心部署"
    preflight
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray dx22 The Apex Vanguard Ultimate Final V13${none}"
