#!/usr/bin/env bash
# ============================================================
# 脚本名称: g7g50x6.sh (The Matrix Nexus V2 Edition)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 彻底修复 awk 提取列位 Bug，绝杀提取数字进行 TLS 质检的乌龙。
#   2. 更新并清洗全局 130+ 域名矩阵库，彻底剔除测速与华硕等高危节点。
#   3. 引入 [矩阵模式]，支持多选/全选 Top 20 节点构建多维 SNI 容灾阵列。
#   4. 用户管理新增 [专属 SNI 挂载]，独立分配伪装域名，自动组装私有订阅。
#   5. vnstat 商业计费、BT/广告双轨拦截、半高无感二维码全量融合。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash g7g50x6.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray G7G50x6 The Matrix Nexus V2 Edition${none}"
