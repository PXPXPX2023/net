#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e24.sh (The Apex Vanguard - Project Genesis V198e24)
# 快捷方式: xrv
# 【V198e24 终极纯血版：全量UI + e12顶级jq解析链回归 + 编译级内核防爆驱动】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e24.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e24"
...
check_and_create_swap() {
    ...
    local swap_size=1050
    
    info "使用绝对安全的 dd 模式写入 1050MB Swap (防 fallocate 导致的开机挂载崩溃)..."
...
EOF
        ...
        resolvectl dns "$IFACE" 1.1.1.1 119.29.29.29 >/dev/null 2>&1 || true

        sleep 2

        if ping -c1 -W3 baidu.com >/dev/null 2>&1; then
            info "已自动切换为国内 DNS。"
        else
...
net.core.rmem_default = 1990577
net.core.wmem_default = 1955367
net.core.rmem_max = 21699928
net.core.wmem_max = 21699928
net.ipv4.tcp_rmem = 8192 1990577 21699928
net.ipv4.tcp_wmem = 8192 1955367 21699928
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e24 终极版)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e24 - (The Apex Vanguard)${none}"
...
#==============================================================================
# EOF - Apex Vanguard V198e24 System Advanced Control Ready.
#==============================================================================
