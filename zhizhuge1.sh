...
SERVICE_NAME="${TOPFLOW_SERVICE_NAME:-$DEFAULT_SERVICE_NAME}"
...
log() {
  printf '\033[1;36m[TopFlow]\033[0m %s\n' "$*"
}

warn() {
  printf '\033[1;33m[TopFlow]\033[0m %s\n' "$*" >&2
}

die() {
  printf '\033[1;31m[TopFlow]\033[0m %s\n' "$*" >&2
  exit 1
}
...
环境变量同样支持：
  TOPFLOW_PSK / TOPFLOW_LISTEN / TOPFLOW_DOWNLOAD_URL / TOPFLOW_DEBUG 等
EOF
}
...
install_dependencies() {
  ...
  if command -v apt-get >/dev/null 2>&1; then
    ...
    die "暂不支持当前发行版的软件包管理器，请手动安装 curl / openssl / systemd。"
  fi
}
...
check_current_install() {
  ...
  systemctl status "$SERVICE_NAME" >/dev/null 2>&1 || die "未找到 systemd 服务：$SERVICE_NAME"
}
...
常用命令:
  ...
  sudo bash $(realpath "$0") update
  sudo bash $(realpath "$0") uninstall --yes
EOF
}
