#!/usr/bin/env bash
# ============================================================
# 脚本名称: dx32.sh (The Apex Vanguard - Project Genesis V5)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 卸载逻辑明确边界：仅粉碎 Xray 及相关生态，永久保留系统级并发与网络调优。
#   2. Xray 进程并发上限精准设定为 131072，严防 Go Runtime 内存溢出 (OOM)。
#   3. 卸载时自动解锁 DNS、恢复备份并拉起系统原生解析服务 systemd-resolved。
#   4. 编译防爆护盾 Swap 维持在 1GB，加入 dd 命令底层降级创建机制适配所有文件系统。
#   5. 拒绝任何代码折叠，100% 结构化全量铺开，无损继承所有极限调优与纯净探测矩阵。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash dx32.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray dx31 The Apex Vanguard - Project Genesis V4${none}"
        ...
        echo "  10) 初次安装、更新系统组件"
        echo "  0) 退出"
        echo -e "  ${red}88) 彻底卸载 (安全复原并清空软件痕迹)${none}"
        hr
