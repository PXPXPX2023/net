#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e14.sh (The Apex Vanguard - Project Genesis V196e14)
# 快捷方式: xrv
#
# 【V196e14 终极勘误与真理重塑】
#   1. 修复菜单断层: 彻底对齐编译函数调用名称，根除 command not found 崩溃。
#   2. 三通道内核锻造: 新增 Mainline+BBR, Mainline+BBR3, Xanmod+BBR3 极客编译三重路线。
#   3. 斩断配置毒瘤: 完全信任内核原生 defconfig 推导架构，根除 -march 暴毙！
#   4. JQ 绝对绝缘: 全域覆盖 select(. != null) 护盾，杜绝任何数组/对象为空时的语法崩溃。
#   5. 完美融合: 全量继承 25 项系统微操及全域网络栈压榨引擎。
# ==============================================================================

# 必须用 bash 运行
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex196e14.sh"
    exit 1
fi
...
readonly SCRIPT_VERSION="196e14"
...
# ------------------------------------------------------------------------------
# 【V196e14 核心修复 1】：预编译官方 XANMOD 部署模块 (修复多重源与 404)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 【V196e14 核心修复 2】：真·双通道内核源码编译引擎 (Mainline 主线 vs Xanmod)
# ------------------------------------------------------------------------------
...
_compile_kernel_mainline() {
    ...
    title "系统飞升：编译 Linux 官方主线最新内核 (稳如老狗版 + $title_suffix)"
    ...
    info "=== 生成并清洗内核配置 (纯净主线 + $title_suffix 强开) ==="
...
_compile_kernel_xanmod() {
    title "系统飞升：编译 真·Xanmod 极客内核 (激进压榨版)"
    warn "警告: 此过程将极度消耗 CPU 与内存 (约 30-60 分钟)，低配机极易死机！"
...
# --- ✂️ 请将此行及其下方的 Part 13 紧接在 Part 12 结尾粘贴 ✂️ ---
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e14 - (The Apex Vanguard)${none}"
...
# ==============================================================================
# EOF - Apex Vanguard V196e14 System Advanced Control Ready.
# ==============================================================================
# --- ✂️ Part 15 结束，所有代码拼接完毕。请运行 bash ex196e14.sh ---
