#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex121.sh (The Apex Vanguard - Project Genesis V121 [The Immortal])
# 快捷方式: xrv
# 永生之境: 
#   1. JSON 崩塌绝杀：全面废除危险的 jq 数组重构语法，所有状态机的启停 (_turn_on_app/off_app/_update_matrix) 强制重写为 `|=` 原子化就地更新，彻底根除节点配置被误擦除的灭顶灾难。
#   2. 分发中心高可用重构：废弃高频读写 IO，引入内存极速解析与 `first` 协议探针，彻底免疫多节点或空指针造成的格式崩坏，恢复绝对稳定的二维码与节点流输出。
#   3. 时序护盾：强化 network-online.target 强锁与全局 PATH 注入，绝杀极简系统开机探针空转。
#   4. 港口管治完全体：继承 Oceanic 跨洋调度、Besteffort 盲走降负与 Bandwidth 硬件拆解补偿引擎。
#   5. 状态机绝杀：物理锚点池 (/etc/xray/flags) 深度护航 ack-filter/ecn/wash，跨越重启与重载，实现真正的绝对持久化。
#   6. 大一统矩阵扩容：全域扩张至 25 项极限微操 + 3 项上帝开关，构建史无前例的 28 项单节点路由绝对物理形态。
#   7. 极致工业级：不求代码极简，只求万里跨洋链路的坚不可摧。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex121.sh"
    exit 1
fi
...
do_app_level_tuning_menu() {
    while true; do
        clear
        title "全域 25 项极限微操 (Xray 提提速底牌 & 系统内核微操)"
        if ! test -f "$CONFIG"; then 
        ...
        # GSO 特判：如果不被支持（锁死），则不计入未优化项，防止上帝之手空转！
...
do_user_manager() {
    while true; do
        ...
        local clients=$(jq -r 'first(.inbounds[]? | select(.protocol=="vless")) | .settings?.clients[]? | .id + "|" + (.email // "无备注")' "$CONFIG" 2>/dev/null)
...
EOF
            ...
            local port=$(jq -r 'first(.inbounds[]? | select(.protocol=="vless")) | .port // empty' "$CONFIG" 2>/dev/null)
            local sni=$(jq -r 'first(.inbounds[]? | select(.protocol=="vless")) | .streamSettings.realitySettings.serverNames[0] // empty' "$CONFIG" 2>/dev/null)
            local pub=$(jq -r 'first(.inbounds[]? | select(.protocol=="vless")) | .streamSettings.realitySettings.publicKey // empty' "$CONFIG" 2>/dev/null)
            info "用户分配成功！"
...
EOF
            ...
            if test -n "$m_sni"; then
                ...
                m_sni=$(jq -r 'first(.inbounds[]? | select(.protocol=="vless")) | .streamSettings.realitySettings.serverNames[0] // empty' "$CONFIG" 2>/dev/null)
            fi
            ...
            if test -n "$target_uuid"; then
                ...
                if test -n "$u_sni"; then
                    ...
                    local vless_inbound=$(jq -c 'first(.inbounds[]? | select(.protocol=="vless"))' "$CONFIG" 2>/dev/null)
                    local port=$(echo "$vless_inbound" | jq -r '.port // empty' 2>/dev/null)
                    local idx=$((${snum:-0}-1))
                    local sid=$(echo "$vless_inbound" | jq -r ".streamSettings.realitySettings.shortIds[$idx] // empty" 2>/dev/null)
                    local pub=$(echo "$vless_inbound" | jq -r '.streamSettings.realitySettings.publicKey // empty' 2>/dev/null)
    ...
    while true; do
        ...
        case "$bc" in
            1) 
                ...
                # 安全的就地更新，杜绝数组重构崩溃
                jq --argjson nv "$nv" '(.routing.rules[]? | select(.protocol != null and (.protocol | index("bittorrent")))) |= (._enabled = $nv)' "$CONFIG" > "$CONFIG.tmp"
                mv -f "$CONFIG.tmp" "$CONFIG"
                fix_permissions
                systemctl restart xray
                info "BT 屏蔽切换为: $nv" 
            ...
            2) 
                ...
                # 安全的就地更新，杜绝数组重构崩溃
                jq --argjson nv "$nv" '(.routing.rules[]? | select(.domain != null and (.domain | index("geosite:category-ads-all")))) |= (._enabled = $nv)' "$CONFIG" > "$CONFIG.tmp"
                mv -f "$CONFIG.tmp" "$CONFIG"
                fix_permissions
                systemctl restart xray
                info "广告屏蔽切换为: $nv" 
...
do_summary() {
    ...
    # 绝对提取首个 vless 节点进入内存，杜绝多次遍历导致的数据重叠与语法崩溃
    local vless_inbound=$(jq -c 'first(.inbounds[]? | select(.protocol=="vless"))' "$CONFIG" 2>/dev/null)
    
    if [ -n "$vless_inbound" ] && [ "$vless_inbound" != "null" ]; then
        local client_count=$(echo "$vless_inbound" | jq -r '.settings.clients | length' 2>/dev/null)
        client_count=${client_count:-0}
        
        if [ "$client_count" -gt 0 ]; then
            local port=$(echo "$vless_inbound" | jq -r '.port // empty' 2>/dev/null)
            local pub=$(echo "$vless_inbound" | jq -r '.streamSettings.realitySettings.publicKey // empty' 2>/dev/null)
            local main_sni=$(echo "$vless_inbound" | jq -r '.streamSettings.realitySettings.serverNames[0] // empty' 2>/dev/null)
            
            for ((i=0; i<client_count; i++)); do
                local uuid=$(echo "$vless_inbound" | jq -r ".settings.clients[$i].id // empty" 2>/dev/null)
                local remark=$(echo "$vless_inbound" | jq -r ".settings.clients[$i].email // \"$REMARK_NAME\"" 2>/dev/null)
                local sid=$(echo "$vless_inbound" | jq -r ".streamSettings.realitySettings.shortIds[$i] // empty" 2>/dev/null)
    ...
    # 绝对提取首个 shadowsocks 节点进入内存
    local ss_inbound=$(jq -c 'first(.inbounds[]? | select(.protocol=="shadowsocks"))' "$CONFIG" 2>/dev/null)
    if [ -n "$ss_inbound" ] && [ "$ss_inbound" != "null" ]; then
        local s_port=$(echo "$ss_inbound" | jq -r '.port // empty' 2>/dev/null)
        local s_pass=$(echo "$ss_inbound" | jq -r '.settings.password // empty' 2>/dev/null)
        local s_method=$(echo "$ss_inbound" | jq -r '.settings.method // empty' 2>/dev/null)
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex121 The Apex Vanguard - Project Genesis V121${none}"
