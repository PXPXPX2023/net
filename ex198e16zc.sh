一：不对啊，你的ex198e15.sh代码还是不对啊，怎么比ex198e12.sh还短？？
二：ex198e15.sh代码【#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e15.sh (The Apex Vanguard - Project Genesis V198e15)
# 快捷方式: xrv
# 【V198e15 终极纯血版：e8完美内核编译系统 + 上帝视角动态拥塞扩容 + MSS/reusePort】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e15.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e15"
...
    local need="jq curl wget xxd unzip qrencode vnstat cron openssl coreutils sed e2fsprogs pkg-config iproute2 ethtool bc bison flex dwarves rsync python3 cpio apt-transport-https lz4 liblz4-tool gcc-multilib libc6-dev-i386 zstd rsync iptables"
    local missing=""
    for p in $need; do
...
    title "配置物理 Swap 防爆缓冲池"
    local SWAP_FILE="/swapfile"
    
    warn "严格执行指令，直接切辟固化 1130MB Swap 缓冲分区..."
    swapoff -a 2>/dev/null || true
    sed -i '/swapfile/d' /etc/fstab 2>/dev/null || true
    rm -f "$SWAP_FILE" 2>/dev/null || true
    
    local swap_size=1130
    
    if ! fallocate -l ${swap_size}M "$SWAP_FILE" 2>/dev/null; then
        warn "fallocate 创建失败，正使用 dd 建立 Swap (可能耗时较长)..."
...
        echo "  [已生效] 解封多核并发全火力编译，1130MB Swap 提供底层护航！"
        echo ""
...
    info "植入底层 MTU/MSS 钳制抗丢包规则 (iptables TCPMSS)..."
...
no-poll
EOF
        systemctl enable dnsmasq >/dev/null 2>&1 || true; systemctl restart dnsmasq >/dev/null 2>&1 || true; chattr -i /etc/resolv.conf 2>/dev/null || true; if [ ! -f /etc/resolv.conf.bak ]; then cp /etc/resolv.conf /etc/resolv.conf.bak 2>/dev/null || true; fi; rm -f /etc/resolv.conf 2>/dev/null || true; echo "nameserver 127.0.0.1" > /etc/resolv.conf; chattr +i /etc/resolv.conf 2>/dev/null || true; _safe_jq_write '.dns = {"servers":["127.0.0.1"], "queryStrategy":"UseIP"}'
    fi
}
...
      (.outbounds[]? | select(. != null) | select(.protocol=="freedom") | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt.tcpNoDelay) = true |
      (.outbounds[]? | select(. != null) | select(.protocol=="freedom") | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt.tcpFastOpen) = true |
      (.outbounds[]? | select(. != null) | select(.protocol=="freedom") | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt.tcpKeepAliveIdle) = 30 |
      (.outbounds[]? | select(. != null) | select(.protocol=="freedom") | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt.tcpKeepAliveInterval) = 15 |
      (.inbounds[]?  | select(. != null) | select(.protocol=="vless") | .streamSettings) |= (. // {}) |
      (.inbounds[]?  | select(. != null) | select(.protocol=="vless") | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt.tcpNoDelay) = true |
      (.inbounds[]?  | select(. != null) | select(.protocol=="vless") | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt.tcpFastOpen) = true |
      (.inbounds[]?  | select(. != null) | select(.protocol=="vless") | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt.reusePort) = true |
      (.inbounds[]?  | select(. != null) | select(.protocol=="vless") | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt.tcpKeepAliveIdle) = 30 |
      (.inbounds[]?  | select(. != null) | select(.protocol=="vless") | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt.tcpKeepAliveInterval) = 15
    '
...
      (.outbounds[]? | select(. != null) | select(.protocol=="freedom") | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt) |= del(.tcpNoDelay, .tcpFastOpen, .tcpKeepAliveIdle, .tcpKeepAliveInterval) | 
      (.inbounds[]?  | select(. != null) | select(.protocol=="vless")   | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt) |= del(.tcpNoDelay, .tcpFastOpen, .reusePort, .tcpKeepAliveIdle, .tcpKeepAliveInterval)
    '
...
        local s9=$([ "$affinity_state" = "true" ] && echo "${cyan}已启用智能 CPU 调优${none}" || echo "${gray}系统调度${none}")
...
        echo -e "  1) 双向并发提速 (NoDelay/FastOpen/reusePort)   | 状态: $s1"
...
        case "$app_opt" in
            1) if [ "$out_fastopen" = "true" ]; then _safe_jq_write '(.outbounds[]? | select(. != null) | select(.protocol=="freedom") | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt) |= del(.tcpNoDelay, .tcpFastOpen) | (.inbounds[]? | select(. != null) | select(.protocol=="vless") | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt) |= del(.tcpNoDelay, .tcpFastOpen, .reusePort)'; info "双向并发提速等特性已关闭。"; else _turn_on_app; info "双向并发提速 (FastOpen/reusePort) 等应用层已全域重置！"; fi; systemctl restart xray >/dev/null 2>&1 || true; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            2) if [ "$out_keepalive" = "30" ]; then _safe_jq_write '(.outbounds[]? | select(. != null) | select(.protocol=="freedom") | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt) |= del(.tcpKeepAliveIdle, .tcpKeepAliveInterval) | (.inbounds[]? | select(. != null) | select(.protocol=="vless") | select(.streamSettings?.sockopt != null) | .streamSettings.sockopt) |= del(.tcpKeepAliveIdle, .tcpKeepAliveInterval)'; info "Socket 智能保活心跳已关闭。"; else _turn_on_app; info "Socket 保活心跳等全域微操已写入！"; fi; systemctl restart xray >/dev/null 2>&1 || true; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
...
            8) if [ "$policy_status" = "60" ]; then _safe_jq_write 'del(.policy)'; info "Xray 策略组回收已关闭。"; else _safe_jq_write '.policy = {"levels":{"0":{"handshake":3,"connIdle":60}},"system":{"statsInboundDownlink":false,"statsInboundUplink":false}}'; info "策略组生命周期优化已开启。"; fi; systemctl restart xray >/dev/null 2>&1 || true; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
...
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e15 终极版)"
        echo "  1) [一键全清] 执行 Linux 强基更新、亚太时间轴校准并置入极客 1130MB 内存交换区"
...
rm -rf /tmp/* /var/cache/apt/archives/* 2>/dev/null || true
...
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e15 - (The Apex Vanguard)${none}"
        
...
#==============================================================================
# EOF - Apex Vanguard V198e15 System Advanced Control Ready.
#==============================================================================】

三：ex198e12.sh代码【#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e12.sh (The Apex Vanguard - Project Genesis V198e12)
# 快捷方式: xrv
# 【V198e12 终极纯血版：e8完美内核编译系统 + 上帝视角动态拥塞扩容】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e12.sh"; exit 1; fi
if test "$EUID" -ne 0; then echo -e "\033[31m[致命错误] 触及底层必须拥有最高权限，请使用 root 账户执行！\033[0m"; exit 1; fi
...
readonly SCRIPT_VERSION="198e12"
readonly XRAY_BIN="/usr/local/bin/xray"
readonly CONFIG_DIR="/usr/local/etc/xray"
readonly CONFIG="$CONFIG_DIR/config.json"
readonly PUBKEY_FILE="$CONFIG_DIR/public.key"
...
# 解决 cc1 x86-64-v1 非法参数暴毙的核心转换网关
_get_safe_march() {
    local lvl="${1:-1}"
    case "$lvl" in
        4) echo "x86-64-v4" ;;
        3) echo "x86-64-v3" ;;
        2) echo "x86-64-v2" ;;
        *) echo "x86-64" ;; # GCC 中没有 v1 这个参数，最基础的就是 x86-64
    esac
}
...
    local need="jq curl wget xxd unzip qrencode vnstat cron openssl coreutils sed e2fsprogs pkg-config iproute2 ethtool bc bison flex dwarves rsync python3 cpio apt-transport-https lz4 liblz4-tool gcc-multilib libc6-dev-i386 zstd rsync"
    local missing=""
    for p in $need; do
        if ! command -v "$p" >/dev/null 2>&1; then missing="$missing $p"; fi
    done
    
    if test -n "$missing"; then
        info "正在同步工业级依赖 (含32/64位全量编译内核套件): $missing"
        pkg_install $missing
        systemctl start vnstat  >/dev/null 2>&1 || true
        systemctl enable vnstat >/dev/null 2>&1 || true
        systemctl start cron    >/dev/null 2>&1 || systemctl start crond >/dev/null 2>&1 || true
...
        info ">>> 强制执行底层编译注入指令: $make_flags"
        
        # 动态验证 GCC 是否支持 CF-Protection 强关，避免 GCC 版本过老报错
        if gcc -E - -fcf-protection=none </dev/null >/dev/null 2>&1; then
            make_flags="$make_flags -fcf-protection=none"
        fi
        
...
    info "全局探针完成，CPU 等级锁定: x86-64-v${cpu_level}"
    
    # 补齐核心工业编译依赖 (加入 zstd 和 rsync 防 initramfs 崩盘)
    export DEBIAN_FRONTEND=noninteractive
    apt-get update -y >/dev/null 2>&1 || true
    apt-get install -y build-essential bc flex bison libssl-dev libelf-dev libncurses-dev dwarves git curl wget lz4 liblz4-tool gcc-multilib libc6-dev-i386 zstd rsync >/dev/null 2>&1 || true
...
    # 禁用32位兼容层（关键），解决 vdso32 架构冲突报错
    ./scripts/config --disable CONFIG_X86_X32
    ./scripts/config --disable CONFIG_IA32_EMULATION
    ./scripts/config --disable CONFIG_COMPAT

    # 关闭 IBT / CF protection (防 GCC 不兼容暴毙)
    ./scripts/config --disable CONFIG_CC_HAS_IBT
    ./scripts/config --disable CONFIG_X86_KERNEL_IBT
...
    # 物理斩断 Xanmod 源码中自带的 cc1 bad value 变量拼接毒瘤
    sed -i -E 's/-march=x86-64-v\$\([^)]+\)//g' arch/x86/Makefile 2>/dev/null || true
    sed -i -E 's/-march=x86-64-v\$\{[^}]+\}//g' arch/x86/Makefile 2>/dev/null || true
    
    # 强力注入 KCFLAGS 以绕过 Makefile 残缺语法报错并闭环 CF 保护
    _execute_compilation "" "-march=$final_march"
}
...
    # === 动态拥塞窗口 initcwnd 计算 (上帝视角) ===
    info ">>> 扒开内核态，提取正在活跃的 TCP 连接状态 (ss -ti)..."
    
    local latency=""
    # 暂时关闭 pipefail 以避免管道断裂引发意外退出
    set +e
    # 采用高阶 awk 纯血正则直接抓取，彻底斩断 grep 失败引发的双重 echo 污染陷阱
    latency=$(ss -ti 2>/dev/null | awk -F 'rtt:' 'NF>1 {split($2, a, " "); split(a[1], b, "/"); total+=b[1]; count++} END {if(count>0) print int(total/count); else print 150}')
    set -e
    
    # 双重保险：如果返回的不是纯数字（甚至为空），强制兜底到 150
    if ! [[ "$latency" =~ ^[0-9]+$ ]]; then latency=150; fi
    
    # 根据延迟设置初始拥塞窗口
    local tcp_init_cwnd=$((latency / 20 + 10))
...
toggle_dnsmasq() {
    if [ "$(check_dnsmasq_state)" = "true" ]; then
        systemctl stop dnsmasq >/dev/null 2>&1 || true; systemctl disable dnsmasq >/dev/null 2>&1 || true; chattr -i /etc/resolv.conf 2>/dev/null || true; rm -f /etc/resolv.conf 2>/dev/null || true; if [ -f /etc/resolv.conf.bak ]; then mv /etc/resolv.conf.bak /etc/resolv.conf; else echo "nameserver 8.8.8.8" > /etc/resolv.conf; fi; systemctl enable systemd-resolved >/dev/null 2>&1 || true; systemctl start systemd-resolved >/dev/null 2>&1 || true; _safe_jq_write '.dns = {"servers":["https://8.8.8.8/dns-query","https://1.1.1.1/dns-query","https://doh.opendns.com/dns-query"], "queryStrategy":"UseIP"}'
    else
        export DEBIAN_FRONTEND=noninteractive; apt-get update -y >/dev/null 2>&1 || yum makecache -y >/dev/null 2>&1 || true; apt-get install -y dnsmasq >/dev/null 2>&1 || yum install -y dnsmasq >/dev/null 2>&1 || true; systemctl stop systemd-resolved 2>/dev/null || true; systemctl disable systemd-resolved 2>/dev/null || true; systemctl stop resolvconf 2>/dev/null || true
        cat > /etc/dnsmasq.conf <<EOF
port=53
...
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e12 终极版)"
        echo "  1) [一键全清] 执行 Linux 强基更新、亚太时间轴校准并置入极客 1130MB 内存交换区"
        echo "  2) [系统防御] 强行修改源头 DNS 解析 (注入 resolvconf，免脱轨断联)"
        echo -e "  ${cyan}3) [重构内脏] 双轨飞升：官方 APT 预编译直装 或 极客全量源码锻造${none}"
        echo "  4) [网络底层] TX Queue 网卡出站队列防拥堵极限缩减 (配置为 12000 收缩)"
...
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e12 - (The Apex Vanguard)${none}"
        
        local svc=$(systemctl is-active xray 2>/dev/null || echo "inactive")
        if test "$svc" = "active"; then svc="${green}平稳在线 (Running)${none}"; else svc="${red}离线静默 (Stopped)${none}"; fi
        local sys_ver=$(uname -r 2>/dev/null || echo "未探测到数据")
...
#==============================================================================
# EOF - Apex Vanguard V198e12 System Advanced Control Ready.
#==============================================================================】

四：承上启下，使用3.1 Pro，请你检阅全部聊天记录和所有代码之后进行完整溯源、理清先后顺序、完整融合、完整修改、完整优化、完整增量，而且不要追求“代码行数少”而犯低级错误，但是你的无用part分的太多了，也绝对不能让底层引擎因为触发 Token 截断而吃掉我们的关键代码和 select(. != null) 绝缘层，不要乱动我的代码，最后再完整全量输出一份新的完整融合修复优化增量脚本代码（叫ex198e16.sh）。