#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex68.sh (The Apex Vanguard - Project Genesis V68)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 战神归位：OOM 免死与 I/O 抢占恢复【出厂默认强制注入】，依然受子菜单 11 持久化开关控制。
#   2. 智能继承升级：反向探针精准判别“初次安装的默认注入”与“老用户主动关闭后的状态继承”。
#   3. 极致提权：Xray 初始 CPU 调度抢占拉升至系统绝对物理最高值 (Nice=-20)。
#   4. 创世级重构：子菜单 7 【11项双向微操枢纽】，一键开启与关闭逻辑完美对齐。
...
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex68.sh"
    exit 1
fi
