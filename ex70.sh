#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex70.sh (The Apex Vanguard - Project Genesis V70)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 实装跨国链路 QoS 物理提权 (TOS/DSCP=184)，打通海底光缆免排队极速通道。
#   2. 根除应用层 JSON 注入暗雷：重构 jq 为无损嵌套赋值，绝不相互覆写覆盖。
#   3. 实装 Xray 路由漏斗引擎：将高频特征放行与拦截逻辑分层，彻底释放 CPU 算力。
#   4. 全景微操面板定型：12 项独立微操与 3 项全域批量控制开关完美对齐。
#   5. 战神级护城河：OOM 免死与 I/O 抢占保持出厂强制注入，跨版本完美状态继承。
#   6. 维持防断连生态：官方 XANMOD 预编译打底、阶梯飙车 GOGC、无损内存计费快照保留。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex70.sh"
    exit 1
fi
...
toggle_mtu() {
    ...
    if [ "$(check_mtu_state)" = "true" ]; then
        ...
        info "TCP PMTU 黑洞探测 已彻底关闭 (tcp_mtu_probing=0)。"
    else
        ...
        info "TCP PMTU 黑洞探测 已智能开启 (tcp_mtu_probing=1)，完美兼顾 GFW 环境！"
    fi
...
toggle_process_priority() {
    ...
    if grep -q "^OOMScoreAdjust=-500" "$limit_file"; then
        ...
        systemctl daemon-reload
        systemctl restart xray >/dev/null 2>&1
        info "进程防杀与 I/O 实时抢占特权已关闭，恢复系统默认调度。"
    else
        echo "OOMScoreAdjust=-500" >> "$limit_file"
        echo "IOSchedulingClass=realtime" >> "$limit_file"
        echo "IOSchedulingPriority=2" >> "$limit_file"
        systemctl daemon-reload
        systemctl restart xray >/dev/null 2>&1
        info "进程防杀 (OOM 免死) 与 I/O 实时抢占特权已强制开启！"
    fi
}

# -- [功能 10->7] 应用层与系统层 12 项双向设置枢纽 --
do_app_level_tuning_menu() {
    while true; do
        clear
        title "应用层 12 项极限微操 (Xray 提速底牌 & 系统级微操)"
        if ! test -f "$CONFIG"; then 
        ...
        local qos_status=$(jq -r '.inbounds[0].streamSettings.sockopt.tclass // "false"' "$CONFIG" 2>/dev/null)
        ...
        local s3=$([ "$qos_status" = "184" ] && echo "${cyan}已开启 (TOS 184)${none}" || echo "${gray}未开启${none}")
        local s4=$([ "$dns_status" = "UseIP" ] && echo "${cyan}已开启${none}" || echo "${gray}未开启${none}")
        local s6=$([ "$(check_thp_state)" = "true" ] && echo "${cyan}已关闭 THP${none}" || echo "${gray}系统默认${none}")
        local s7=$([ "$(check_mtu_state)" = "true" ] && echo "${cyan}智能黑洞探测中${none}" || echo "${gray}未开启${none}")
        local s8=$([ "$(check_cpu_state)" = "true" ] && echo "${cyan}全核火力全开${none}" || echo "${gray}节能降频中 / 不支持${none}")
        local s9=$([ "$(check_ring_state)" = "true" ] && echo "${cyan}已极限收缩${none}" || echo "${gray}系统大缓冲${none}")
        local s10=$([ "$(check_zram_state)" = "true" ] && echo "${cyan}已挂载 ZRAM${none}" || echo "${gray}未启用${none}")
        local s11=$([ "$(check_journal_state)" = "true" ] && echo "${cyan}纯内存极速化${none}" || echo "${gray}磁盘 IO 写入中${none}")
        local s12=$([ "$(check_process_priority_state)" = "true" ] && echo "${cyan}OOM免死 / IO抢占${none}" || echo "${gray}系统默认调度${none}")

        echo -e "  1) 开启或关闭 In/Out 双向 Socket 极限提速 (tcpNoDelay/FastOpen)   | 当前状态: $s1"
        echo -e "  2) 开启或关闭 嗅探引擎减负 (metadataOnly 解放 CPU)                | 当前状态: $s2"
        echo -e "  3) 开启或关闭 跨国链路 QoS 物理提权 (TOS/DSCP 标记)               | 当前状态: $s3"
        echo -e "  4) 开启或关闭 内置并发 DoH 引擎 (Xray Native DNS)                 | 当前状态: $s4"
        echo -e "  5) 执行 GOGC 内存阶梯飙车调优 (自动检测物理内存)                  | 当前设定: ${cyan}${gc_status}${none}"
        echo -e "  6) 开启或关闭【透明大页 (THP - Transparent Huge Pages)】          | 当前状态: $s6"
        echo -e "  7) 开启或关闭【TCP 路径 MTU 智能黑洞探测】                        | 当前状态: $s7"
        echo -e "  8) 开启或关闭【CPU 频率调度器锁定 (Performance)】                 | 当前状态: $s8"
        echo -e "  9) 开启或关闭【网卡硬件环形缓冲区 (Ring Buffer) 反向收缩】        | 当前状态: $s9"
        echo -e "  10) 开启或关闭【ZRAM 内存交换】(淘汰慢速 Swap，阶梯内存自动检测)  | 当前状态: $s10"
        echo -e "  11) 开启或关闭【日志系统 Journald 纯内存化】(斩断 I/O 羁绊)       | 当前状态: $s11"
        echo -e "  12) 开启或关闭【系统进程级防杀抢占 (OOM/IO 提权)】                | 当前状态: $s12"
        echo -e "  "
        echo -e "  ${magenta}13) 一键强力开启以上 1-4 项终极应用层微操${none}"
        echo -e "  ${red}14) 一键强制关闭以上 1-4 项应用层微操 (无损还原初始状态)${none}"
        echo -e "  ${magenta}15) 一键开启或关闭 6-12 项系统级微操 (批量自动化处理)${none}"
        echo "  0) 返回上一级"
        hr
        read -rp "请选择: " app_opt
        ...
        case "$app_opt" in
            1)
                if [ "$out_status" = "true" ]; then
                    _safe_jq_write '
                      del(.inbounds[].streamSettings.sockopt.tcpNoDelay,
                          .inbounds[].streamSettings.sockopt.tcpFastOpen,
                          .outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt.tcpNoDelay,
                          .outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt.tcpFastOpen,
                          .outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt.tcpKeepAliveInterval)
                    '
                    info "In/Out 双向 Socket 极限提速已关闭。"
                else
                    _safe_jq_write '
                      (.inbounds[].streamSettings.sockopt) |= (. // {}) |
                      (.inbounds[].streamSettings.sockopt.tcpNoDelay) = true |
                      (.inbounds[].streamSettings.sockopt.tcpFastOpen) = true |
                    ...
                    '
                    info "In/Out 双向 Socket 极限提速已无损开启！"
                fi
                systemctl restart xray >/dev/null 2>&1
                read -rp "按 Enter 继续..." _
                ;;
            2)
                ...
                ;;
            3)
                if [ "$qos_status" = "184" ]; then
                    _safe_jq_write '
                      del(.inbounds[].streamSettings.sockopt.tclass,
                          .outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt.tclass)
                    '
                    info "跨国链路 QoS 物理提权已关闭。"
                else
                    _safe_jq_write '
                      (.inbounds[].streamSettings.sockopt) |= (. // {}) |
                      (.inbounds[].streamSettings.sockopt.tclass) = 184 |
                      (.outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt) |= (. // {}) |
                      (.outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt.tclass) = 184
                    '
                    info "跨国链路 QoS 物理提权 (TOS=184) 已无损开启！"
                fi
                systemctl restart xray >/dev/null 2>&1
                read -rp "按 Enter 继续..." _
                ;;
            ...
            5)
                ...
                read -rp "按 Enter 继续..." _
                ;;
            6) toggle_thp; read -rp "按 Enter 继续..." _ ;;
            7) toggle_mtu; read -rp "按 Enter 继续..." _ ;;
            8) toggle_cpu; read -rp "按 Enter 继续..." _ ;;
            9) toggle_ring; read -rp "按 Enter 继续..." _ ;;
            10) toggle_zram; read -rp "按 Enter 继续..." _ ;;
            11) toggle_journal; read -rp "按 Enter 继续..." _ ;;
            12) toggle_process_priority; read -rp "按 Enter 继续..." _ ;;
            13)
                _safe_jq_write '
                  (.inbounds[].streamSettings.sockopt) |= (. // {}) |
                  (.inbounds[].streamSettings.sockopt.tcpNoDelay) = true |
            ...
            14)
                _safe_jq_write '
                  del(.inbounds[].streamSettings.sockopt.tcpNoDelay,
                      .inbounds[].streamSettings.sockopt.tcpFastOpen,
                      .inbounds[].streamSettings.sockopt.tclass,
                      .outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt.tcpNoDelay,
                      .outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt.tcpFastOpen,
                      .outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt.tcpKeepAliveInterval,
                      .outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt.tclass)
                ...
                systemctl restart xray >/dev/null 2>&1
                info "应用层 1-4 项微操已强制关闭，Xray 配置无损还原初始状态！"
                read -rp "按 Enter 继续..." _
                ;;
            15)
                echo -e "  ${cyan}1) 一键开启 6-12 项系统级底层微操${none}"
                echo -e "  ${red}2) 一键还原 6-12 项系统默认值${none}"
                ...
                if [ "$batch_act" = "1" ]; then
                    ...
                    [ "$(check_process_priority_state)" != "true" ] && toggle_process_priority
                    info "6-12 项底层提权黑科技已全部激活！"
...
do_sys_init_menu() {
    while true; do
        ...
        echo -e "  ${magenta}7) 应用层 12 项极限微操 (Xray 提速底牌 & 系统级黑科技)${none}"
        echo "  0) 返回主菜单"
        hr
        read -rp "请选择: " sys_opt
        case "$sys_opt" in
