...
VERSION="v3.0.0"
SCRIPT_NAME="ghost_ubuntu24_pro_v3.sh"
...
usage() {
  cat <<'USAGE'
Ghost Ubuntu 24 Pro One-Key Script v3.0.0

Usage:
  sudo bash ghost_ubuntu24_pro_v3.sh install --domain example.com --email admin@example.com
  sudo bash ghost_ubuntu24_pro_v3.sh --domain example.com --email admin@example.com
...
GitHub raw example:
  curl -fsSL https://raw.githubusercontent.com/USER/REPO/main/ghost_ubuntu24_pro_v3.sh -o ghost_ubuntu24_pro_v3.sh
  chmod +x ghost_ubuntu24_pro_v3.sh
  sudo ./ghost_ubuntu24_pro_v3.sh --domain example.com --email admin@example.com
...
load_config_if_exists() {
  if [[ -f "$CONFIG_FILE" ]]; then
    ...
    warn "Legacy config found: $LEGACY_CONFIG_FILE. It will be read and rewritten as v3 config later."
    # shellcheck source=/dev/null
...
validate_paths_and_names() {
  ...
  [[ "$DB_PASS" != *"'"* && "$DB_PASS" != *"\\"* && "$DB_PASS" != *$'\n'* ]] || die "--db-pass must not contain single quote, backslash, or newline"
}
...
get_public_ip() {
  ...
  [[ -n "$PUBLIC_IP" ]] || warn "Could not detect public IPv4. DNS check will only verify that A records exist."
}
...
META
  ...
  [[ -f "$CONFIG_FILE" ]] && cp -a "$CONFIG_FILE" "$TMP_WORKDIR/ghost-onekey-v3-config.env"
