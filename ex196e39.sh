...
_fetch_xanmod_tags() {
    info "正在连接 GitLab 检索最新的 Xanmod 内核分支/版本..."
    local api_url="https://gitlab.com/api/v4/projects/xanmod%2Flinux/repository/tags?per_page=15"
    local tags_json=""
    set +e
    ...
    if test -z "$tags_json" || echo "$tags_json" | grep -q "message"; then
        warn "API 检索失败，提供常用备用列表..."
        TAG_LIST=("6.13.4-xanmod1" "6.12.16-xanmod1" "6.6.78-xanmod1" "6.1.129-rt-xanmod1")
    else
        # 提取最新 Tag，过滤掉测试版 rc
        TAG_LIST=($(echo "$tags_json" | jq -r '.[].name' | grep -vE "rc|beta"))
    fi
    ...
    if test "$sel" = "0"; then
        read -rp "请精准输入 Tag 名称 (例如 6.1.129-rt-xanmod1): " LATEST_TAG
    else
        local idx=$((sel - 1))
        LATEST_TAG="${TAG_LIST[$idx]:-${TAG_LIST[0]}}"
    fi
...
_compile_kernel_xanmod() {
    ...
    if ! tar -tzf "$KERNEL_FILE" >/dev/null 2>&1; then
        ...
        rm -f "$KERNEL_FILE" 2>/dev/null || true
        set +e
        LATEST_TAG=$(curl -sL --connect-timeout 10 https://gitlab.com/api/v4/projects/xanmod%2Flinux/repository/tags | jq -r '.[].name' 2>/dev/null | grep -E "^6\.1[0-9]\.[0-9]+(-rt)?-xanmod[0-9]+$" | head -n 1 || echo "6.12.25-xanmod1")
        set -e
    ...
    # 【神级防爆 1】：彻底粉碎 vdso32 与非法 64 位指令集的冲突
    info ">> 正在植入 VDSO32 底层防爆盾：强制过滤非法的 64 位编译参数..."
    if test -f "arch/x86/entry/vdso/Makefile"; then
        # 强制将传递给 32 位编译器的 -march=x86-64-vX 与 -fcf-protection 过滤掉
        ...
        echo 'CFLAGS_REMOVE_vdso32-setup.o += -march=x86-64-v1 -march=x86-64-v2 -march=x86-64-v3 -march=x86-64-v4 -fcf-protection=branch' >> arch/x86/entry/vdso/Makefile
    fi

    # 【神级防爆 2】：精准限定 CONFIG_X86_64_VERSION，开启合法架构性能！
