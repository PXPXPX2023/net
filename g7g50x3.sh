#!/usr/bin/env bash
# ============================================================
# 脚本名称: g7g50x3.sh (The Apex Vanguard V4 Edition)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 130+ 矩阵哈希洗牌随机探测，支持随时按回车秒停跳过等待。
#   2. 节点导出新增 'b' 键快捷重选 SNI 钩子，交互丝滑。
#   3. 外部用户导入模块升级，支持同步强改全局 SNI 并智能匹配本机 pbk。
#   4. 彻底修复底层权限覆盖 Bug，绝杀 status 23 报错。
#   5. 完美保留 GFW/CDN 探测、TLS1.3 质检、半高二维码等尖端特性。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash g7g50x3.sh"
    exit 1
fi
...
do_user_manager() {
    while true; do
        ...
        cat "$tmp_users" | while IFS='|' read -r num uid remark; do
            echo -e "  $num) 备注: ${cyan}$remark${none} | UUID: ${yellow}$uid${none}"
        done
        hr
        ...
        if test "$uopt" = "a"; then
            ...
            if [[ "$rb" == "b" || "$rb" == "B" ]]; then
                choose_sni && _safe_jq_write "(.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .streamSettings.realitySettings.serverNames) = ${L_B} \"$BEST_SNI\" ${R_B} | (.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .streamSettings.realitySettings.dest) = \"$BEST_SNI:443\"" && systemctl restart xray >/dev/null 2>&1 && do_summary
            fi
            
        elif test "$uopt" = "m"; then
            hr
            echo -e " ${cyan}>>> 外部老用户平滑迁移向导 <<<${none}"
            echo -e " ${yellow}提示: 为了保证服务器能成功解密，脚本会自动将导入的 UUID 绑定到【本机公钥(pbk)】与【本机IP】！${none}"
            ...
            read -rp "是否要顺便修改全局伪装 SNI? (留空回车则保持不变): " m_sni
            
            local filter_add="(.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .settings.clients) += ${L_B} {\"id\":\"$m_uuid\",\"flow\":\"xtls-rprx-vision\",\"email\":\"$m_remark\"} ${R_B} | (.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .streamSettings.realitySettings.shortIds) += ${L_B} \"$m_sid\" ${R_B}"
            
            if test -n "$m_sni"; then
                filter_add="$filter_add | (.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .streamSettings.realitySettings.serverNames) = ${L_B} \"$m_sni\" ${R_B} | (.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .streamSettings.realitySettings.dest) = \"$m_sni:443\""
            fi
            
            _safe_jq_write "$filter_add"
            systemctl restart xray
            ...
            info "外部用户迁入成功！该用户在当前机器的全新分发信息如下："
            local link="vless://${m_uuid}@${SERVER_IP}:${port}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${sni}&fp=chrome&pbk=${pub}&sid=${m_sid}&type=tcp#${m_remark}"
            echo -e "\n  ${cyan}专属分发链接:${none} \n  $link\n"
            if command -v qrencode >/dev/null 2>&1; then 
                echo -e "  ${cyan}手机扫码导入:${none}"
                qrencode -m 2 -t UTF8 "$link"; 
...
do_summary() {
    ...
    if test "${v_count:-0}" -gt 0; then
        ...
        if test -n "$uuid" && test "$uuid" != "null"; then
            ...
            local link="vless://${uuid}@${SERVER_IP}:${port}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${sni}&fp=chrome&pbk=${pub}&sid=${sid}&type=tcp#${remark}"
            echo -e "\n  ${cyan}通用链接:${none} \n  $link\n"
            if command -v qrencode >/dev/null 2>&1; then
...
EOF
    ...
    while true; do
        read -rp "按 Enter 返回主菜单，或输入 b 重新选择 SNI: " opt
        if [[ "$opt" == "b" || "$opt" == "B" ]]; then
            choose_sni && _safe_jq_write "(.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .streamSettings.realitySettings.serverNames) = ${L_B} \"$BEST_SNI\" ${R_B} | (.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .streamSettings.realitySettings.dest) = \"$BEST_SNI:443\"" && systemctl restart xray >/dev/null 2>&1 && do_summary
        else
            break
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray G7G50x3 The Apex Vanguard V4 Edition${none}"
        ...
        echo "  2) 用户管理 (增删/导入 备注、UUID、ShortId)"
        echo "  3) 分发中心 (详情与半高二维码)"
        echo "  4) 手动更新 Geo 规则库 (已开启夜间自动热更)"
        echo "  5) 屏蔽规则管理 (BT/广告双轨拦截)"
        echo -e "  ${cyan}6) 无感热切 SNI 矩阵 (含 GFW探测/严苛质检)${none}"
        echo "  8) 彻底卸载 (清空一切痕迹)"
        ...
        case "$num" in
            ...
            6) 
               choose_sni && _safe_jq_write "(.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .streamSettings.realitySettings.serverNames) = ${L_B} \"$BEST_SNI\" ${R_B} | (.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .streamSettings.realitySettings.dest) = \"$BEST_SNI:443\"" && systemctl restart xray >/dev/null 2>&1 && do_summary
               while true; do
                   read -rp "按 Enter 返回主菜单，或输入 b 继续重新选择 SNI: " rb
                   if [[ "$rb" == "b" || "$rb" == "B" ]]; then
                       choose_sni && _safe_jq_write "(.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .streamSettings.realitySettings.serverNames) = ${L_B} \"$BEST_SNI\" ${R_B} | (.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .streamSettings.realitySettings.dest) = \"$BEST_SNI:443\"" && systemctl restart xray >/dev/null 2>&1 && do_summary
                   else
                       break
