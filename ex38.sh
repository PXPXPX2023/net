#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex38.sh (The Apex Vanguard - Project Genesis V11)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 全面正名为 ex 世代，优化分发中心详情展示顺序，符合顶层逻辑审美。
#   2. 深度植入 Burst 突发能力：动态 GOMEMLIMIT 防爆、Nice=-12 极限抢占。
#   3. Xray Socket 底层重构：强启 tcpNoDelay 与 tcpFastOpen，毫秒级响应。
#   4. 进程并发黄金线：精确锁定 65535，严防低配机 Go Runtime 内存溢出 (OOM)。
#   5. 物理级 DNS 强锁、全能生态复原引擎，所有逻辑 100% 展开零缩水。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex38.sh"
    exit 1
fi
...
do_install() {
    ...
    # 注入符合低配机极限性能的安全并发突破设置 (65535) 与 极限抗突发参数
    fix_xray_systemd_limits
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex38 The Apex Vanguard - Project Genesis V11${none}"
