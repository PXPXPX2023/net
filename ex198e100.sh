#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e100.sh (The Apex Vanguard - Project Genesis V198e100)
# 快捷方式: xrv
# 【V198e100 三协议稳态版：e97基线 + 内置BBR低资源硬熔断 + XanMod源码路线提示】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e100.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e100"
...
#==============================================================================
# V198e100 FINAL OVERRIDE LAYER
# 目标：
#   1) 修复内置型 CONFIG_TCP_CONG_BBR=y 时旧 e91 流程先弹 YES、后才熔断的问题。
#   2) 对低资源 VPS（Mem+Swap < 1000MB）直接拒绝完整内核编译，不再进入 route 选择。
#   3) 明确区分：模块型 m 可快编 tcp_bbr.ko；内置型 y 只能完整编译新内核。
#   4) XanMod 预编译内核默认提示“不要在小鸡上源码编译”，只保留专家强制开关。
#==============================================================================

if declare -F _e90_build_builtin_bbr_kernel >/dev/null 2>&1; then
    eval "$(declare -f _e90_build_builtin_bbr_kernel | sed '1s/_e90_build_builtin_bbr_kernel/_e90_build_builtin_bbr_kernel_e97_base/')"
fi
if declare -F hot_update_from_legacy >/dev/null 2>&1; then
    eval "$(declare -f hot_update_from_legacy | sed '1s/hot_update_from_legacy/hot_update_from_legacy_e97_base/')"
fi

readonly E98_AUDIT_DIR="$CONFIG_DIR/audit_e98" 2>/dev/null || E98_AUDIT_DIR="$CONFIG_DIR/audit_e98"

_e98_pause() { local _p=""; read -rp "按 Enter 继续..." _p || true; }
_e98_ts() { date +%Y%m%d_%H%M%S; }
_e98_total_mem_swap_mb() { free -m 2>/dev/null | awk '/^Mem:/{m=$2}/^Swap:/{s=$2} END{print int(m+s)}'; }
_e98_root_free_mb() { df -Pm / 2>/dev/null | awk 'NR==2{print int($4)}'; }
_e98_boot_free_mb() { if findmnt -rn /boot >/dev/null 2>&1; then df -Pm /boot 2>/dev/null | awk 'NR==2{print int($4)}'; else df -Pm / 2>/dev/null | awk 'NR==2{print int($4)}'; fi; }

_e98_prepare_dirs() {
    mkdir -p "$E98_AUDIT_DIR" "$E89_BBRX_BUILD_ROOT" "$E89_BBRX_BACKUP_ROOT" "$E89_BBRX_CURRENT" 2>/dev/null || true
}
...
_e98_bbr_mode_text() {
    ...
    case "$cfgv" in
        ...
        y) echo "内置型 y：不能热替换 .ko；低资源 VPS 默认禁止完整内核编译" ;;
        *) echo "未知：请先执行自动检测，禁止盲改" ;;
    esac
}
...
_e98_xanmod_hint() {
    ...
    if echo "$kr" | grep -qi xanmod; then
        ...
        echo "XanMod 的 CONFIG_TCP_CONG_BBR=y 时，BBR/BBR3 已经内置在 vmlinuz 内，不能像模块 m 那样替换 tcp_bbr.ko。"
        echo "如果要改 BBR3 源码参数，只能拿到匹配的完整 XanMod 源码树后重新编译新内核。"
    fi
}

_e98_builtin_full_compile_precheck() {
    local kind="${1:-}" conf="${2:-}" total root_free boot_free cfgv ok=1 log
    cfgv="$(_e89_bbr_kconfig_value 2>/dev/null || echo unknown)"
    total="$(_e98_total_mem_swap_mb 2>/dev/null || echo 0)"
    root_free="$(_e98_root_free_mb 2>/dev/null || echo 0)"
    boot_free="$(_e98_boot_free_mb 2>/dev/null || echo 0)"
    log="$E98_AUDIT_DIR/builtin-compile-precheck.$(_e98_ts).log"
    mkdir -p "$E98_AUDIT_DIR" 2>/dev/null || true

    title "V198e100：内置型 BBR 完整内核编译硬门禁"
    ...
    if test "$cfgv" != "y"; then
        warn "当前不是内置型 y，本门禁不处理。"
        return 0
    fi

    if test "${total:-0}" -lt 1000 2>/dev/null; then
        error "Mem+Swap < 1000MB。你当前约 ${total}MB，完整内核编译高概率 OOM/IO 卡死，已硬拒绝。"
        ok=0
    fi
    if test "${root_free:-0}" -lt 12000 2>/dev/null; then
    ...
    if test "$ok" -ne 1; then
        warn "这台 VPS 当前不适合完整内核编译。刚才卡在 i40e/iavf/ice，就是完整编译在编大量无关驱动。"
        warn "建议：保留当前 XanMod BBR3 + fq/qdisc/sysctl 优化；不要在 2GB 级资源上改内置 BBR 源码。"
        warn "审计日志：$log"
        echo ""
        echo "  如你坚持源码编译，请至少满足："
        ...
        echo "    3) 根分区 20GB+；"
        echo "    4) 拿到与当前 XanMod tag 匹配的完整源码。"
        echo ""
        echo "  强制继续需要同时设置两个环境变量后重新运行："
        echo "    export XRAY_E100_ALLOW_BUILTIN_FULL_COMPILE=1"
        echo "    export XRAY_E97_ALLOW_FULL_KERNEL_COMPILE_LOW_RESOURCE=1"
        echo "    # 兼容旧变量：export XRAY_E98_ALLOW_BUILTIN_FULL_COMPILE=1"
        if test "${XRAY_E100_ALLOW_BUILTIN_FULL_COMPILE:-0}" != "1" && test "${XRAY_E98_ALLOW_BUILTIN_FULL_COMPILE:-0}" != "1"; then return 1; fi
    fi

    if test "${XRAY_E100_ALLOW_BUILTIN_FULL_COMPILE:-0}" != "1" && test "${XRAY_E98_ALLOW_BUILTIN_FULL_COMPILE:-0}" != "1"; then
        warn "为避免误触，V198e100 默认禁止内置型 BBR 完整编译。"
        warn "确认资源充足并已快照后，可手动 export XRAY_E100_ALLOW_BUILTIN_FULL_COMPILE=1 再尝试。"
        warn "兼容旧变量：XRAY_E98_ALLOW_BUILTIN_FULL_COMPILE=1 仍可识别。"
        return 1
    fi
    return 0
}
...
_e90_build_builtin_bbr_kernel() {
    local kind="$1" conf="$2"
    _e98_prepare_dirs
    _e98_builtin_full_compile_precheck "$kind" "$conf" || { _e98_pause; return 1; }
    if declare -F _e90_build_builtin_bbr_kernel_e97_base >/dev/null 2>&1; then
        _e90_build_builtin_bbr_kernel_e97_base "$kind" "$conf"
    else
...
_e98_builtin_explain() {
    title "V198e100：内置型 BBR / XanMod 说明"
    cat <<'EOF_E98_EXPLAIN'
你现在看到 CONFIG_TCP_CONG_BBR=y，含义是：
  BBR/BBR3 已经被编进内核本体，不存在 tcp_bbr.ko 可热替换。

所以：
  - 17 → 7 检查 tcp_bbr.c，对 y 内置型没有直接热替换意义；
  - 选择 bbrx3 参数后，如果要真正替换，只能完整编译新内核；
  - 完整编译会编大量驱动，包括 i40e/iavf/ice，低配 VPS 很容易卡死；
  - e100 已按你的要求将放行门槛降到 1000MB，但低资源编译仍有 OOM/IO 卡死风险。

更现实的优化路线：
  1) 保留当前 XanMod BBR3；
  2) 用 fq/qdisc、notsent、adv_win/app_win、MSS、RPS/XPS 等参数优化；
  3) 如果一定要改 BBR3 源码，仍优先建议用 4GB~8GB 以上临时编译机，编好内核包再部署；
  4) 如果坚持在当前 VPS 上编译，必须先做快照/VNC/救援模式准备。
EOF_E98_EXPLAIN
}
...
_e96_bbr_module_source_report() {
    ...
    cfgv="$(_e89_bbr_kconfig_value 2>/dev/null || echo unknown)"
    if test "$cfgv" = "y"; then
        title "V198e100：BBR 源码检查（内置型 y 专用提示）"
        echo "  当前内核：$(uname -r 2>/dev/null || echo unknown)"
        echo "  CONFIG_TCP_CONG_BBR：y"
        echo "  tcp_bbr 模块路径：(builtin)"
        hr
        warn "这是内置型 BBR，不存在 tcp_bbr.ko 热替换路线。"
        warn "检查/补齐 headers 不能让内置 BBR 变成可热替换模块。"
        warn "只有完整内核源码编译才能修改 net/ipv4/tcp_bbr.c；当前低资源 VPS 不建议。"
        echo ""
        _e98_builtin_explain
        return 0
    fi
    if declare -F _e96_bbr_module_source_report_e97_base >/dev/null 2>&1; then
        _e96_bbr_module_source_report_e97_base
    else
        title "V198e100：BBR 模块源码 / headers 检查"
        _e96_bbr_find_or_prepare_source >/dev/null 2>&1 && info "已找到 tcp_bbr.c。" || warn "未找到 tcp_bbr.c。"
    fi
}

# 保存 e96 源码检查函数，避免上面覆盖时递归。
# 注意：这一段在函数定义后执行不影响 bash -n；运行时如果 base 不存在则降级。
if declare -F _e96_bbr_module_source_report >/dev/null 2>&1; then
    eval "$(declare -f _e96_bbr_module_source_report | sed '1s/_e96_bbr_module_source_report/_e96_bbr_module_source_report_e97_base/')" 2>/dev/null || true
fi
...
_e98_bbr3_flow() {
    ...
    while true; do
        cfgv="$(_e89_bbr_kconfig_value 2>/dev/null || echo unknown)"
        title "V198e100：BBR3 / XanMod → bbrx3_v1 参数中心"
        echo "  当前替换路线：$(_e98_bbr_mode_text)"
        if test "$cfgv" = "y"; then
            echo -e "  ${yellow}提示：当前是内置型 y。生成参数文件可以，但真正替换需要完整内核编译；低资源默认禁止。${none}"
        fi
        echo ""
        echo "  1) 查看参数表：参数 / 原版 BBR3 / bbrx3_v1 建议 / 说明"
        echo "  2) 使用 bbrx3_v1 推荐参数生成配置"
        ...
        read -rp "请选择: " m || true
        case "${m:-}" in
            1) _e89_show_bbr3_table; _e98_pause ;;
            2) _e89_collect_bbr3_params recommended || return; conf="$(_e89_write_bbr3_conf)"; info "已写入：$conf"; read -rp "是否立即替换 bbr？[y/N]: " go || true; [[ "$go" =~ ^[yY]$ ]] && _e89_build_replace_bbr bbr3 "$conf"; _e98_pause ;;
            3) _e89_collect_bbr3_params custom || return; conf="$(_e89_write_bbr3_conf)"; info "已写入：$conf"; read -rp "是否立即替换 bbr？[y/N]: " go || true; [[ "$go" =~ ^[yY]$ ]] && _e89_build_replace_bbr bbr3 "$conf"; _e98_pause ;;
            4) conf="$E89_BBRX_CURRENT/bbr3_replace.conf"; if test -f "$conf"; then _e89_build_replace_bbr bbr3 "$conf"; else warn "配置不存在，请先执行 2 或 3。"; fi; _e98_pause ;;
            7) _e98_builtin_explain; _e98_pause ;;
            0) return ;;
            *) warn "无效操作。"; sleep 1 ;;
        esac
    done
...
_e98_bbrv1_flow() {
    ...
    while true; do
        cfgv="$(_e89_bbr_kconfig_value 2>/dev/null || echo unknown)"
        title "V198e100：普通 Linux 主线 BBR v1 → bbrx_v1 参数中心"
        echo "  当前替换路线：$(_e98_bbr_mode_text)"
        if test "$cfgv" = "y"; then
            echo -e "  ${yellow}提示：当前是内置型 y。你现在内核像是 XanMod/BBR3，通常不建议用 BBR v1 模板。${none}"
        fi
        echo ""
        echo "  1) 查看参数表：参数 / 原版 BBR / bbrx_v1 建议 / 说明"
        echo "  2) 使用 bbrx_v1 推荐参数生成配置"
        ...
        read -rp "请选择: " m || true
        case "${m:-}" in
            1) _e89_show_bbrv1_table; _e98_pause ;;
            2) _e89_collect_bbrv1_params recommended || return; conf="$(_e89_write_bbrv1_conf)"; info "已写入：$conf"; read -rp "是否立即替换 bbr？[y/N]: " go || true; [[ "$go" =~ ^[yY]$ ]] && _e89_build_replace_bbr bbrv1 "$conf"; _e98_pause ;;
            3) _e89_collect_bbrv1_params custom || return; conf="$(_e89_write_bbrv1_conf)"; info "已写入：$conf"; read -rp "是否立即替换 bbr？[y/N]: " go || true; [[ "$go" =~ ^[yY]$ ]] && _e89_build_replace_bbr bbrv1 "$conf"; _e98_pause ;;
            4) conf="$E89_BBRX_CURRENT/bbrv1_replace.conf"; if test -f "$conf"; then _e89_build_replace_bbr bbrv1 "$conf"; else warn "配置不存在，请先执行 2 或 3。"; fi; _e98_pause ;;
            7) _e98_builtin_explain; _e98_pause ;;
            0) return ;;
            *) warn "无效操作。"; sleep 1 ;;
        esac
...
# 兼容 e96/e97 菜单名称：让旧菜单调用也进入 e98 flow。
_e96_bbr3_flow() { _e98_bbr3_flow; }
_e96_bbrv1_flow() { _e98_bbrv1_flow; }
...
do_bbr_body_replace_menu_e89() {
    while true; do
        clear
        title "V198e100：BBR 本体替换 / 自定义编译中心"
        echo "  自动建议：$(_e89_bbr_detect 2>/dev/null || true)"
        echo "  当前替换路线：$(_e98_bbr_mode_text)"
        echo ""
        echo "  1) 自动检测当前 BBR 类型，并推荐编译方案"
        ...
        read -rp "请选择: " opt || true
        case "${opt:-}" in
            1) _e89_bbr_detect; _e98_pause ;;
            2) _e98_bbrv1_flow ;;
            3) _e98_bbr3_flow ;;
            4) _e89_bbr_show_backups; _e98_pause ;;
            5) _e89_bbr_rollback_latest || warn "如果本次是内置型整内核编译，请通过云厂商 VNC/GRUB 选择旧内核回滚。"; _e98_pause ;;
            6) _e89_bbr_status; _e98_pause ;;
            7) _e98_builtin_explain; _e98_pause ;;
            8) if declare -F _e97_why_intel_compile_stuck >/dev/null 2>&1; then _e97_why_intel_compile_stuck; else _e98_builtin_explain; fi; _e98_pause ;;
            0) return ;;
            *) warn "无效操作。"; sleep 1 ;;
        esac
    done
...
hot_update_from_legacy() {
    title "V198e100：稳态热更新（完整主菜单保留 + 真BBRv3源码/补丁编译1000MB门槛）"
    if declare -F hot_update_from_legacy_e97_base >/dev/null 2>&1; then
        hot_update_from_legacy_e97_base "$@" || true
    fi
    _e98_prepare_dirs
    echo "198e100" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    if test -f "${SCRIPT_PATH:-$0}"; then
        cp -f "${SCRIPT_PATH:-$0}" "$SYMLINK" 2>/dev/null || true
        cp -f "${SCRIPT_PATH:-$0}" "$SCRIPT_DIR/ex198e100.sh" 2>/dev/null || true
        cp -f "${SCRIPT_PATH:-$0}" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
        chmod +x "$SYMLINK" "$SCRIPT_DIR/ex198e100.sh" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
    fi
    hash -r 2>/dev/null || true
    info "V198e100 热更新完成。"
    info "CONFIG_TCP_CONG_BBR=y 的内置型 BBR：完整内核编译门槛已降到 Mem+Swap >=1000MB，但仍需手动授权与快照/VNC准备。"
}
...
main_menu() {
    while true; do
        ...
        actual_qdisc="$(tc qdisc show dev "$iface" 2>/dev/null | head -n 1 || echo unknown)"
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e100 - (真BBRv3源码/补丁1000MB门槛 + 防误编译)${none}"
        echo -e "  引擎态势: ${green}${running}${none} | 全局快捷指令: ${cyan}xrv${none} | 外部识别 IP: ${yellow}${ip}${none}"
        echo -e "  系统装配内核: ${cyan}$(uname -r 2>/dev/null || echo unknown)${none} | 微架构侦测: x64-v${yellow}$(detect_x86_64_level 2>/dev/null || echo '?')${none} | sysctl qdisc/cc: ${yellow}$(_e88_sysctl_get net.core.default_qdisc 2>/dev/null || sysctl -n net.core.default_qdisc 2>/dev/null || echo '?')/$(_e88_sysctl_get net.ipv4.tcp_congestion_control 2>/dev/null || sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null || echo '?')${none} | BBR配置:${yellow}${bbr_cfg}${none}"
        echo -e "  实际root队列: ${cyan}${actual_qdisc}${none}"
        echo -e "  DNS模式: ${cyan}$(cat "${E67_DNS_MODE_FILE:-/usr/local/etc/xray/dns_mode.txt}" 2>/dev/null || echo auto)${none} | 时间轴: ${cyan}${nowtz}${none} | notsent: ${yellow}$(_e88_current_notsent_lowat 2>/dev/null || echo '?')${none}/${cyan}$(_e88_persistent_notsent_lowat 2>/dev/null || echo '?')${none} | adv/app: ${yellow}$(_e88_sysctl_get net.ipv4.tcp_adv_win_scale 2>/dev/null || sysctl -n net.ipv4.tcp_adv_win_scale 2>/dev/null || echo '?')/$(_e88_sysctl_get net.ipv4.tcp_app_win 2>/dev/null || sysctl -n net.ipv4.tcp_app_win 2>/dev/null || echo '?')${none}"
        ...
        echo "  9) 全景网络连接审计日志与商用级别网络流量记账本"
        echo "  10) 高级发烧：内核双轨编译(APT/源码)/网络/应用层 60余项极限全控板"
        echo -e "  ${green}11) 从旧 ex198e 环境稳态热更新到 V198e100${none}"
        echo -e "  ${cyan}12) 清理空间（安全释放 / 快照收敛 / 显示硬盘与 Swap）${none}"
        echo -e "  ${cyan}13) 应用网络参数（缓冲/notsent/adv_win/app_win/txqueue/OOOX/MSS）${none}"
        echo -e "  ${cyan}14) TCP 重传 / 单连接诊断中心（只读排障）${none}"
        echo -e "  ${cyan}15) 检查现有配置并重新输出节点${none}"
        ...
        case "${num:-}" in
            ...
            2) do_user_manager ;;
            3) do_summary; while true; do local rb=""; read -rp "输出完毕，按 Enter 退出或者按 b 发起特征替换: " rb || true; if test "$rb" = "b" || test "$rb" = "B"; then if choose_sni; then _update_matrix; do_summary; else break; fi; else break; fi; done ;;
            4) print_magenta ">>> 初始化云网络联通."; bash "$UPDATE_DAT_SCRIPT" >/dev/null 2>&1 || true; ensure_xray_is_alive; info "配置已穿透替换系统！当前路由库完成热加载！"; _e98_pause ;;
            5) do_update_core ;;
            6) if choose_sni; then _update_matrix; do_summary; fi ;;
            7) _global_block_rules ;;
            8) do_fallback_probe ;;
            ...
            14) do_tcp_diag_menu_e63 ;;
            15) do_install_repair_e77 ;;
            16) e77_audit_runtime_state; _e98_pause ;;
            17) do_bbr_body_replace_menu_e89 ;;
            18) do_e92_audit_menu ;;
            19) if declare -F do_xray_speed_diag_menu_e95 >/dev/null 2>&1; then do_xray_speed_diag_menu_e95; else do_xray_speed_diag_menu_e94; fi ;;
            20) if declare -F do_tc_bandwidth_menu_e95 >/dev/null 2>&1; then do_tc_bandwidth_menu_e95; else do_tc_bandwidth_menu_e94; fi ;;
...
#==============================================================================
# EOF - Apex Vanguard V198e100 Safety Hot Update Layer.
#==============================================================================
