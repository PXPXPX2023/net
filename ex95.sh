#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex95.sh (The Apex Vanguard - Project Genesis V95)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 视觉排爆：修复 vnstat 计费日正则多行捕获导致的 UI 换行撕裂问题，引入 head -n 1 与绝对覆写机制。
#   2. 阻塞解除：移除上帝开关中对于部分服务安装的静默抛弃，防止包管理器锁死引发的菜单假死。
#   3. 致命排雷：修复 Nano DB 与全球版数据库广告标签不兼容导致的崩溃，实现 config.json 标签动态无损切换。
#   4. 容灾升维：将自动热更的重启时间延迟至 3:10，提供 10 分钟黄金冗余下载期，斩断断流风险。
#   5. 三剑合璧：Nano DB + 物理绑核 (CPU Affinity) + Go 并发硬干预 (GOMAXPROCS) 完美缝合。
#   6. 十六边形战神矩阵：全域扩容为 16 项极限微操 (应用层 8 项 + 系统级 8 项)。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex95.sh"
    exit 1
fi
...
# -- 极简与错峰热更重构：自动热更 Geo 规则库设置 --
install_update_dat() {
    local is_nano="false"
    # 状态继承：检测当前是否已经开启了 Nano 极简库
    if grep -q "v2fly" "$UPDATE_DAT_SCRIPT" 2>/dev/null; then 
        is_nano="true"
    fi
    ...
    # 清除旧的混合任务，注入完美的 3:00 下载与 3:10 重启解耦逻辑
    ...
    info "已配置自动热更与错峰重启: 3:00 静默下载规则库，3:10 安全重载 Xray 进程。"
}

# -- 极简路由库核心无缝切换引擎 (含标签防崩适配) --
_toggle_nanodb_on() {
    # 斩断标签雪崩：将全球库的 category-ads-all 动态重写为极简库的 category-ads
    _safe_jq_write '(.routing.rules[]? | select(.domain != null) | .domain) |= map(if . == "geosite:category-ads-all" then "geosite:category-ads" else . end)'
...
_toggle_nanodb_off() {
    # 恢复原生形态：将极简库的 category-ads 无损还原为全球库的 category-ads-all
    _safe_jq_write '(.routing.rules[]? | select(.domain != null) | .domain) |= map(if . == "geosite:category-ads" then "geosite:category-ads-all" else . end)'
...
# -- [底层微操探针与热重载引擎] --
check_nanodb_state() {
    if grep -q "v2fly" "$UPDATE_DAT_SCRIPT" 2>/dev/null; then echo "true"; else echo "false"; fi
}
...
# -- [功能 10->7] 全域 16 项极限微操双向设置枢纽 --
do_app_level_tuning_menu() {
    while true; do
        clear
        title "全域 16 项极限微操 (Xray 提速底牌 & 系统内核微操)"
        if ! test -f "$CONFIG"; then 
        ...
        local nanodb_state=$(check_nanodb_state)
        ...
        [ "$nanodb_state" != "true" ] && app_off_count=$((app_off_count+1))
        [ "$affinity_state" != "true" ] && app_off_count=$((app_off_count+1))

        # 缺省探测引擎 (系统层 - 9~16项 硬件自适应剔除)
        ...
        local s7=$([ "$nanodb_state" = "true" ] && echo "${cyan}已开启 (极简库)${none}" || echo "${gray}默认 (全球库)${none}")
        local s8=$([ "$affinity_state" = "true" ] && echo "${cyan}已锁死单核 (零切换)${none}" || echo "${gray}默认 (系统调度)${none}")
        
        local s9=$([ "$dnsmasq_state" = "true" ] && echo "${cyan}内存极速解析中 (0.1ms)${none}" || echo "${gray}未开启 (依赖原生DoH)${none}")
        local s10=$([ "$thp_state" = "true" ] && echo "${cyan}已关闭 THP${none}" || ([ "$thp_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}系统默认${none}"))
        local s11=$([ "$mtu_state" = "true" ] && echo "${cyan}智能探测中${none}" || ([ "$mtu_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}未开启${none}"))
        local s12=$([ "$cpu_state" = "true" ] && echo "${cyan}全核火力全开${none}" || ([ "$cpu_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}节能待机${none}"))
        local s13=$([ "$ring_state" = "true" ] && echo "${cyan}已反向收缩${none}" || ([ "$ring_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}系统大缓冲${none}"))
        local s14=$([ "$zram_state" = "true" ] && echo "${cyan}已挂载 ZRAM${none}" || ([ "$zram_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}未启用${none}"))
        local s15=$([ "$journal_state" = "true" ] && echo "${cyan}纯内存极速化${none}" || ([ "$journal_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}磁盘 IO 写入中${none}"))
        local s16=$([ "$prio_state" = "true" ] && echo "${cyan}OOM免死 / IO抢占${none}" || echo "${gray}系统默认调度${none}")

        echo -e "  ${magenta}--- Xray Core 应用层内部调优 (1-8) ---${none}"
        ...
        echo -e "  7) 开启或关闭 Nano DB 极简路由规则库 (大幅降低 CPU Cache Miss)   | 当前状态: $s7"
        echo -e "  8) 开启或关闭 Xray 进程物理绑核 & GOMAXPROCS 并发锁 (零切换损耗) | 当前状态: $s8"
        echo -e ""
        echo -e "  ${magenta}--- Linux 系统层与内核黑科技 (9-16) ---${none}"
        echo -e "  9)  开启或关闭【Dnsmasq 本地极速内存缓存引擎 (21000并发/锁TTL)】 | 当前状态: $s9"
        echo -e "  10) 开启或关闭【透明大页 (THP - Transparent Huge Pages)】        | 当前状态: $s10"
        echo -e "  11) 开启或关闭【TCP PMTU 黑洞智能探测 (Probing=1)】              | 当前状态: $s11"
        echo -e "  12) 开启或关闭【CPU 频率调度器锁定 (Performance)】               | 当前状态: $s12"
        echo -e "  13) 开启或关闭【网卡硬件环形缓冲区 (Ring Buffer) 反向收缩】      | 当前状态: $s13"
        echo -e "  14) 开启或关闭【ZRAM】(淘汰慢速 Swap，阶梯内存自动检测)          | 当前状态: $s14"
        echo -e "  15) 开启或关闭【日志系统 Journald 纯内存化】(斩断 I/O 羁绊)      | 当前状态: $s15"
        echo -e "  16) 开启或关闭【系统进程级防杀抢占 (OOM/IO 提权)】               | 当前状态: $s16"
        echo -e "  "
        echo -e "  ${cyan}17) 一键开启或关闭 1-8 项 应用层微操 (自动侦测并智能反转)${none}"
        echo -e "  ${yellow}18) 一键开启或关闭 9-16 项 系统级微操 (自动避障侦测并反转)${none}"
        echo -e "  ${red}19) 上帝之手：一键开启或关闭 1-16 项 全域微操 (满血激活 / 无损还原)${none}"
        echo "  0) 返回上一级"
        hr
        read -rp "请选择: " app_opt
        ...
        case "$app_opt" in
            ...
            6)
                ...
                ;;
            7)
                if [ "$nanodb_state" = "true" ]; then
                    print_magenta ">>> 正在拉取大而全的全球版路由库，请稍候..."
                    _toggle_nanodb_off
                    systemctl restart xray >/dev/null 2>&1
                    info "全球版路由库及配套防崩标签已恢复并实时生效！"
                else
                    print_magenta ">>> 正在拉取 Nano 极简版路由库，为您削减内存与 Cache Miss，请稍候..."
                    _toggle_nanodb_on
                    systemctl restart xray >/dev/null 2>&1
                    info "Nano 极简版路由库及防崩标签已部署完毕，匹配速度已提升至纳秒级！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            8)
                if [ "$affinity_state" = "true" ]; then
                    ...
                    _toggle_affinity_on
                    systemctl restart xray >/dev/null 2>&1
                    info "Xray 进程已成功使用铁链锁死在目标物理核心，并发锁已实时激活！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            9) toggle_dnsmasq; systemctl restart xray >/dev/null 2>&1; read -rp "按 Enter 继续..." _ ;;
            10) toggle_thp; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            11) toggle_mtu; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            12) toggle_cpu; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            13) toggle_ring; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            14) toggle_zram; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            15) toggle_journal; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            16) toggle_process_priority; systemctl restart xray >/dev/null 2>&1; info "特权调度翻转完毕，进程已重启生效。"; read -rp "按 Enter 继续..." _ ;;
            17)
                if [ "$app_off_count" -gt 0 ]; then
                    print_magenta ">>> 正在全域强力开启 1-8 项，拉取数据库需数秒，请耐心等待..."
                    _turn_on_app
                    systemctl restart xray >/dev/null 2>&1
                    info "1-8 项 Xray 内部应用层微操已全域强力开启并实时生效！"
                else
                    print_magenta ">>> 正在全域恢复 1-8 项，拉取数据库需数秒，请耐心等待..."
                    _turn_off_app
                    systemctl restart xray >/dev/null 2>&1
                    info "1-8 项 Xray 内部应用层微操已全部强制关闭，无损还原官方初始状态！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            18)
                if [ "$sys_off_count" -gt 0 ]; then
                    ...
                    systemctl restart xray >/dev/null 2>&1
                    info "9-16 项系统内核底层黑科技已全部一键激活！(不支持的硬件已自动跳过)"
                else
            ...
            19)
                if [ "$((app_off_count + sys_off_count))" -gt 0 ]; then
                    ...
                    systemctl restart xray >/dev/null 2>&1
                    info "上帝降临！1-16 项全域极限微操已【全部满血激活】！(不支持的硬件已自动跳过)"
                else
                    ...
                    systemctl restart xray >/dev/null 2>&1
                    info "神威收敛！1-16 项全域极限微操已【全部强制关闭】，无损还原初始出厂状态！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            0)
...
do_sys_init_menu() {
    while true; do
        ...
        echo -e "  ${magenta}6) 全域 16 项极限微操 (Dnsmasq / Nano DB / 系统级黑科技)${none}"
        echo "  0) 返回主菜单"
        hr
        read -rp "请选择: " sys_opt
        case "$sys_opt" in
...
_global_block_rules() {
    while true; do
        ...
        # 兼容性侦测：无论配置里是 category-ads 还是 category-ads-all 都能精准探测状态
        local ad_en=$(jq -r '.routing.rules[]? | select(.domain != null) | select(.domain | join(" ") | contains("geosite:category-ads")) | ._enabled' "$CONFIG" 2>/dev/null | head -1)
        ...
        case "$bc" in
            ...
            2) 
                ...
                jq --argjson nv "$nv" '(.routing.rules[]? | select(.domain != null) | select(.domain | join(" ") | contains("geosite:category-ads")) | ._enabled) = $nv' "$CONFIG" > "$CONFIG.tmp"
                mv -f "$CONFIG.tmp" "$CONFIG"
                fix_permissions
                systemctl restart xray
                info "广告屏蔽已成功切换为: $nv"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex95 The Apex Vanguard - Project Genesis V95${none}"
