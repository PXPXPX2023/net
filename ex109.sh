#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex109.sh (The Apex Vanguard - Project Genesis V109 [The Final Truth])
# 快捷方式: xrv
# 终焉之境: 
#   1. 刺透虚妄：重构 GSO/GRO 探针 (check_gso_off_state)，新增对 [fixed] 硬件锁死标记的正则降维打击。彻底终结 Virtio 虚拟网卡因底层锁死导致的无限空转与 UI 假象。
#   2. 结构重铸：严厉废除所有单行折叠与 && 连写小聪明，将所有 jq 链式操作与状态控制流 100% 多行立体展开，回归工业级严谨。
#   3. 语法绝缘：完整保留 jq 的原子化赋值链 (.streamSettings //= {} | .sockopt //= {})，绝杀 JSON 结构残缺导致的空转。
#   4. 变量正名：精准维系 XRAY_RAY_BUFFER_SIZE=64，真正唤醒 Go Runtime 的万兆吞吐重卡池。
...
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex109.sh"
    exit 1
fi
...
# 刺透虚拟化屏障：深度正则匹配 fixed 状态，粉碎假死循环
check_gso_off_state() {
    local IFACE=$(ip route get 1.1.1.1 | awk '{print $5; exit}')
    ...
    # 很多云厂商的 Virtio 网卡强制锁死硬件卸载属性，标志为 [fixed]
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex109 The Apex Vanguard - Project Genesis V109${none}"
