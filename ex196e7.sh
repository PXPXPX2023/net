#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e7.sh (The Apex Vanguard - Project Genesis V196e7)
# 快捷方式: xrv
#
# 【V196e7 终极勘误与真理重塑】
#   1. 根绝编译血案: 彻底废弃对 CONFIG_X86_64_VERSION 及 GENERIC_CPU 的人工干预，根绝 -march= 暴毙！
#   2. 斩杀野蛮管道: 剥离 `yes ""` 的暴力传参，使用标准的 make olddefconfig。
#   3. 真·源码溯源: 直连 GitLab 官方仓库拉取正宗 7.x Xanmod 源码。
...
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex196e7.sh"
    exit 1
fi
...
do_xanmod_compile() {
    ...
    # 【核心修复】：彻底抛弃对 CONFIG_X86_64_VERSION 和 CONFIG_GENERIC_CPU 的人工干预！
    # Xanmod 源码自带完善的架构推导逻辑，人工篡改会导致 `-march=x86-64-v` 取值为空
    # 并且会引发 CONFIG_X86_L1_CACHE_SHIFT 丢失的致命断层！
    # 这里直接以官方 defconfig 为基准，仅注入网络层优化。
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e7 - (The Apex Vanguard)${none}"
...
main_menu
# ==============================================================================
# EOF - Apex Vanguard V196e7 System Advanced Control Ready.
# ==============================================================================
