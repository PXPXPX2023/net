#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e35.sh (The Apex Vanguard - Project Genesis V198e35)
# 快捷方式: xrv
# 【V198e35 极速隐稳版：1GB Swap 幂等检测 + Origin Guard 防 CDN 滥用 + Reality 指纹收敛】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e35.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e35"
...
check_and_clean_space() {
    ...
    # V198e35: 不再 rm -rf /tmp/*、不再删除全部 /var/log/*.log，避免误删 socket/锁文件/审计日志导致服务异常。
    rm -rf \
...
choose_best_qdisc() {
    # V198e35: 极速代理默认 fq + BBR。CAKE 不再作为默认项，避免弱 CPU VPS 因整形/分类开销反降速。
...
verify_sni_strict() {
    ...
    if ! echo "$out" | grep -qi "OCSP response:"; then warn "OCSP Stapling 未发现：这不是致命问题，V198e35 仅提示不强制淘汰。"; fi
...
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
IFACE="$IFACE"
# V198e35: 纯代理吞吐优先保留 GRO/GSO/TSO，降低 CPU 压力；仅关闭 LRO，避免破坏转发语义。
...
toggle_dnsmasq() {
    if [ "$(check_dnsmasq_state)" = "true" ]; then
        systemctl stop dnsmasq >/dev/null 2>&1 || true; systemctl disable dnsmasq >/dev/null 2>&1 || true; rm -f /etc/resolv.conf 2>/dev/null || true; if [ -f /etc/resolv.conf.bak ]; then mv /etc/resolv.conf.bak /etc/resolv.conf; else echo "nameserver 208.67.222.222" > /etc/resolv.conf; fi; systemctl enable systemd-resolved >/dev/null 2>&1 || true; systemctl start systemd-resolved >/dev/null 2>&1 || true; _safe_jq_write '.dns = {"servers":["https://1.1.1.1/dns-query","https://208.67.222.222/dns-query","https://doh.opendns.com/dns-query"], "queryStrategy":"UseIPv4"}'
    else
...
# cache-rr=ANY 在部分 Debian 旧版 dnsmasq 不支持，V198e35 默认关闭以避免服务启动失败。
EOF
...
do_app_level_tuning_menu() {
    while true; do
        clear
        title "全域 25 项系统级及应用层微操管理中心（V198e35 稳态极速）"
...
install_self_entrypoint() {
    ...
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e35.sh" 2>/dev/null || true
        chmod +x "$SYMLINK" "$SCRIPT_DIR/ex198e35.sh" 2>/dev/null || true
    fi
...
hot_patch_xray_config() {
    ...
    _safe_jq_write '
      ...
      .outbounds = (.outbounds // []) |
      if any(.outbounds[]?; .tag=="direct") then . else .outbounds += [{"protocol":"freedom","tag":"direct","settings":{"domainStrategy":"AsIs"},"streamSettings":{"sockopt":{"tcpNoDelay":true,"tcpFastOpen":true,"tcpKeepAliveIdle":30,"tcpKeepAliveInterval":15}}}] end |
      ...
      (.inbounds[]? | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.sockopt.tcpFastOpen) = true |
...
apply_hotupdate_fast_sysctl() {
    ...
    mkdir -p /etc/sysctl.d 2>/dev/null || true
    local adv app qdisc
    adv=$(sysctl -n net.ipv4.tcp_adv_win_scale 2>/dev/null || echo "1")
    app=$(sysctl -n net.ipv4.tcp_app_win 2>/dev/null || echo "31")
    [[ "$adv" =~ ^-?[0-9]+$ ]] || adv=1
    [[ "$app" =~ ^[0-9]+$ ]] || app=31
...
hot_update_from_legacy() {
    local mode="${1:-manual}"
    title "V198e35 旧版环境热更新迁移器（兼容 ex198e32/ex198e33/ex198e34）"
    ...
    if ensure_xray_is_alive; then
        install_origin_guard auto
        echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
        info "热更新完成：旧配置、旧用户和旧端口已保留，V198e35 防滥用与极速补丁已生效。"
        if test "$mode" != "auto"; then
            do_summary
            local _p=""; read -rp "按 Enter 返回主菜单..." _p || true
        fi
...
get_ssh_listen_ports() {
    local ports=""
    # V198e35: 优先从当前监听态识别 SSH 端口，避免 sshd_config 使用 Include/云镜像改名时误判为 22。
...
EOF
    ...
    if ensure_xray_is_alive; then harden_xray_reality_profile; install_origin_guard auto; info "所有架构配置装载确认生效！通讯网络已打开。"; do_summary; else error "贯通服务进程失败，请检查日志。"; return 1; fi
    finalize
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e35 稳态极速版)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e35 - (The Apex Vanguard)${none}"
        ...
        echo -e "  ${cyan}11) 从 ex198e32/ex198e33/ex198e34 旧环境补丁式热更新到 V198e35${none}"
...
#==============================================================================
# EOF - Apex Vanguard V198e35 System Advanced Control Ready.
#==============================================================================
