将ex132.sh代码对比一下你写的ex118.sh，你究竟偷工减料胡乱修改了多少代码和内容？？你不要乱动我的bbr优化参数啊！承上启下，使用3.1 Pro，请你检阅全部聊天记录和所有代码之后进行完整溯源、完整融合、完整修改、完整优化、完整增量，而且不要追求“代码行数少”而犯低级错误，最后再完整全量输出一份新的完整融合修复优化增量脚本代码（叫ex134.sh），ex118.sh代码见【#!/usr/bin/env bash

# ============================================================

# 脚本名称: ex118.sh (The Apex Vanguard - Project Genesis V118 [The Eternity])

# 快捷方式: xrv

# 永恒之境: 

#   1. 时序绝杀：将 xray-hw-tweaks.service 的挂载点强制升级为 network-online.target，彻底粉碎现代 Linux 快启动导致的网卡探针 (IFACE) 空转，确保 CAKE/IRQ 物理重启后 100% 命中。

#   2. 卸载防爆：补全 _turn_off_app 状态机中的 jq 原子化容错 (.sniffing = (.sniffing // {}))，免疫因用户手动删减 JSON 节点导致的一键关闭爆破。

#   3. 绝对路径锚定：为开机底盘注入全局 PATH 环境变量，免疫极简 OS 模版下 ethtool / tc 指令未入环境导致的静默执行失败。
...
#   7. 极致工业级：每一行逻辑都经历了最严酷的沙盘推演，不求代码极简，只求万里跨洋链路的坚不可摧。
...
if test -z "$BASH_VERSION"; then
    ...
    echo "错误: 请用 bash 运行此脚本: bash ex118.sh"
...
main_menu() {
    ...
    while true; do
        ...
        echo -e "  ${magenta}Xray ex118 The Apex Vanguard - Project Genesis V118${none}"
