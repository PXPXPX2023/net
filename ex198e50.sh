#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e50.sh (The Apex Vanguard - Project Genesis V198e50)
# 快捷方式: xrv
# 【V198e50 稳态极速版：Dnsmasq/ZRAM/Ring 三项热更新强落地】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e50.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e50"
...
check_and_clean_space() {
    ...
    # V198e50: 不再 rm -rf /tmp/*、不再删除全部 /var/log/*.log，避免误删 socket/锁文件/审计日志导致服务异常。
    rm -rf \
...
_e50_pause() { local _p=""; read -rp "按 Enter 继续..." _p || true; }
...
choose_best_qdisc() {
    # V198e50: 极速代理默认 fq + BBR。CAKE 不再作为默认项，避免弱 CPU VPS 因整形/分类开销反降速。
...
_zram_supported() {
    # e50: 支持三种情况：模块可加载、模块已加载、/sys/block/zram0 已存在。
    test -e /sys/block/zram0 && return 0
    lsmod 2>/dev/null | grep -q '^zram' && return 0
    modprobe -n zram >/dev/null 2>&1 && return 0
    return 1
}
...
_write_swap_sysctl_e50() {
    mkdir -p /etc/sysctl.d 2>/dev/null || true
    cat > /etc/sysctl.d/98-xray-swap.conf << 'EOF_SWAP_SYSCTL_E50'
vm.swappiness = 10
...
_setup_zram_1024_e50() {
    # e50：ZRAM 作为唯一默认 Swap；总 Swap 上限 1024MB。删除 /swapfile 等文件型 Swap。
    _disable_all_file_swaps
    cat > /usr/local/bin/xray-zram.sh <<'EOF_ZRAM_E50'
#!/usr/bin/env bash
set -u
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
ZRAM_SIZE_MB="1024"
...
if swapon --show=NAME --noheadings 2>/dev/null | awk '{print $1}' | grep -qx '/dev/zram0'; then
...
echo "${ZRAM_SIZE_MB}M" > /sys/block/zram0/disksize 2>/dev/null || exit 0
...
EOF_ZRAM_E50
    ...
    cat > /etc/systemd/system/xray-zram.service <<'EOF_ZRAM_SRV_E50'
[Unit]
...
enforce_swap_cap_1024_e50() {
    title "检查并执行 Swap 总量上限 1024MB（热更新默认优先 ZRAM，删除超额文件型 Swap）"
    mkdir -p "$BACKUP_DIR" 2>/dev/null || true
    cp -a /etc/fstab "$BACKUP_DIR/fstab.swapcap.e50.$(date +%Y%m%d_%H%M%S).bak" 2>/dev/null || true
    if _zram_supported; then
        info "检测到 ZRAM 支持：启用 /dev/zram0=1024MB，并删除 /swapfile/其它文件型 Swap。"
        _setup_zram_1024_e50
    else
    ...
    if test "${total:-0}" -gt 1024 2>/dev/null; then
        if test -n "$nonfile"; then
            warn "Swap 总量约 ${total}MB 仍超过 1024MB；检测到非文件型/非ZRAM Swap：$nonfile。为避免误伤磁盘分区，脚本不自动删除它们。"
        else
            warn "Swap 总量约 ${total}MB 仍超过 1024MB；请执行 swapon --show 复核。"
        fi
    else
...
EOF_TIMER_E44
    local tmp_cron
    tmp_cron=$(mktemp /tmp/cron_e50_XXXXXX) || true
...
verify_sni_strict() {
    ...
    if ! echo "$out" | grep -qi "OCSP response:"; then warn "OCSP Stapling 未发现：这不是致命问题，V198e50 仅提示不强制淘汰。"; fi
...
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
IFACE="$IFACE"
# V198e50: 纯代理吞吐优先保留 GRO/GSO/TSO，降低 CPU 压力；仅关闭 LRO，避免破坏转发语义。
...
do_perf_tuning() {
    ...
    info "配置系统高并发进程限制 (Limits，V198e50 稳态安全版)..."
...
check_dnsmasq_state() {
    # V198e50: 状态必须同时满足：dnsmasq active + 本机 127.0.0.1:53 监听 + resolv.conf 已接管。
    systemctl is-active --quiet dnsmasq 2>/dev/null || { echo "false"; return; }
    ss -H -lunpt 2>/dev/null | awk '{print $5}' | grep -Eq '(^127\.0\.0\.1:53$|^\[::1\]:53$|^0\.0\.0\.0:53$|^\*:53$|:53$)' || { echo "false"; return; }
    grep -Eq '^nameserver[[:space:]]+127\.0\.0\.1[[:space:]]*$' /etc/resolv.conf 2>/dev/null || { echo "false"; return; }
    echo "true"
}
...
check_ring_state() {
    local IFACE curr_rx target
    IFACE=$(ip route get 1.1.1.1 2>/dev/null | awk '{print $5; exit}' || echo "")
    if test -z "$IFACE" || ! command -v ethtool >/dev/null 2>&1 || ! ethtool -g "$IFACE" >/dev/null 2>&1; then echo "unsupported"; return; fi
    curr_rx=$(ethtool -g "$IFACE" 2>/dev/null | awk '/Current hardware settings/{flag=1; next} flag && /^[[:space:]]*RX:/{print $2; exit}')
    if ! [[ "${curr_rx:-}" =~ ^[0-9]+$ ]]; then echo "unsupported"; return; fi
    target=$(cat "$CONFIG_DIR/ring_target.txt" 2>/dev/null || echo "1024")
    if ! [[ "$target" =~ ^[0-9]+$ ]]; then target=1024; fi
    # e50：低延迟收缩不再只认 512；<=目标值即视为已收缩。256/512/1024 都属于可接受低延迟档。
    if test "$curr_rx" -le "$target" 2>/dev/null; then echo "true"; else echo "false"; fi
}

check_zram_state() {
    # e50：只要当前已启用 zram swap，且总 Swap 不超过 1024MB，即视为正确。
    if ! swapon --show=NAME,TYPE --noheadings 2>/dev/null | awk '$1 ~ /zram/ {found=1} END{exit found?0:1}'; then
        if ! modprobe -n zram >/dev/null 2>&1 && ! lsmod 2>/dev/null | grep -q '^zram'; then echo "unsupported"; else echo "false"; fi
        return
    fi
    local total
    total=$(_swap_total_mb 2>/dev/null || echo 0)
    if test "${total:-0}" -le 1024 2>/dev/null; then echo "true"; else echo "false"; fi
}
...
_apply_ring_target_value_e50() {
    local IFACE max_rx max_tx target
    IFACE=$(ip route get 1.1.1.1 2>/dev/null | awk '{print $5; exit}' || echo "")
    test -n "$IFACE" || return 1
    command -v ethtool >/dev/null 2>&1 || return 1
    ethtool -g "$IFACE" >/dev/null 2>&1 || return 1
    max_rx=$(ethtool -g "$IFACE" 2>/dev/null | awk '/Pre-set maximums/{flag=1; next} flag && /^[[:space:]]*RX:/{print $2; exit}')
    max_tx=$(ethtool -g "$IFACE" 2>/dev/null | awk '/Pre-set maximums/{flag=1; next} flag && /^[[:space:]]*TX:/{print $2; exit}')
    [[ "${max_rx:-}" =~ ^[0-9]+$ ]] || return 1
    [[ "${max_tx:-}" =~ ^[0-9]+$ ]] || max_tx="$max_rx"
    # 自适应低延迟档：超大 ring 用 1024；普通 ring 用 512；小于 512 的虚拟网卡保持最大值。
    if test "$max_rx" -ge 4096 2>/dev/null; then target=1024
    elif test "$max_rx" -ge 512 2>/dev/null; then target=512
    else target="$max_rx"; fi
    echo "$target"
}

apply_ring_low_latency_default() {
    local IFACE target tx_target max_tx curr_rx curr_tx
    IFACE=$(ip route get 1.1.1.1 2>/dev/null | awk '{print $5; exit}' || echo "")
    if test -z "$IFACE" || ! command -v ethtool >/dev/null 2>&1 || ! ethtool -g "$IFACE" >/dev/null 2>&1; then
        warn "当前网卡不支持 Ring Buffer 调整，已跳过。"
        return 0
    fi
    target=$(_apply_ring_target_value_e50 || echo "")
    if ! [[ "${target:-}" =~ ^[0-9]+$ ]]; then warn "未能识别 Ring Buffer 目标值，已跳过。"; return 0; fi
    max_tx=$(ethtool -g "$IFACE" 2>/dev/null | awk '/Pre-set maximums/{flag=1; next} flag && /^[[:space:]]*TX:/{print $2; exit}')
    [[ "${max_tx:-}" =~ ^[0-9]+$ ]] || max_tx="$target"
    tx_target="$target"; if test "$max_tx" -lt "$target" 2>/dev/null; then tx_target="$max_tx"; fi
    curr_rx=$(ethtool -g "$IFACE" 2>/dev/null | awk '/Current hardware settings/{flag=1; next} flag && /^[[:space:]]*RX:/{print $2; exit}')
    curr_tx=$(ethtool -g "$IFACE" 2>/dev/null | awk '/Current hardware settings/{flag=1; next} flag && /^[[:space:]]*TX:/{print $2; exit}')
    mkdir -p "$CONFIG_DIR" "$FLAGS_DIR" 2>/dev/null || true
    echo "$target" > "$CONFIG_DIR/ring_target.txt" 2>/dev/null || true
    touch "$FLAGS_DIR/ring_low_latency" 2>/dev/null || true
    ethtool -G "$IFACE" rx "$target" tx "$tx_target" 2>/dev/null || true
    info "Ring Buffer 低延迟档已尝试落地：$IFACE RX=${target}, TX=${tx_target}（原 RX=${curr_rx:-?}, TX=${curr_tx:-?}）。"
    update_hw_boot_script >/dev/null 2>&1 || true
}

toggle_ring() {
    # e50：菜单 16 不再反向切换；统一幂等开启低延迟 Ring Buffer。
    apply_ring_low_latency_default
}

toggle_zram() {
    # e50：废弃旧“按内存倍数创建 ZRAM”的 toggle，统一走 1024MB 总 Swap 上限策略。
    enforce_swap_cap_1024_e50
}
...
do_app_level_tuning_menu() {
    while true; do
        clear
        title "全域 25 项系统级及应用层微操管理中心（V198e50 稳态极速）"
        if ! test -f "$CONFIG"; then error "未发现配置，请先执行核心部署！"; _e50_pause; return; fi
        ...
        s16=$([ "$ring_state" = "true" ] && echo "${cyan}环形反向收缩${none}" || ([ "$ring_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}大缓冲(延迟高)${none}"))
        ...
        case "$app_opt" in
            1) apply_xray_nodelay_default; systemctl restart xray >/dev/null 2>&1 || true; info "TCP NoDelay 已幂等开启。"; _e50_pause ;;
            1f|1F) if [ "$out_fastopen" = "true" ]; then _safe_jq_write '(.outbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=false | (.inbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=false'; info "TCP FastOpen 已关闭。"; else _safe_jq_write '(.outbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=true | (.inbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=true'; warn "TCP FastOpen 已开启；如跨境握手不稳请关闭。"; fi; systemctl restart xray >/dev/null 2>&1 || true; _e50_pause ;;
            2|3|4|5|6) apply_xray_keep_2_to_6_defaults; remove_xray_env_buffer; systemctl restart xray >/dev/null 2>&1 || true; info "2-6 项已按稳态默认幂等固化。"; _e50_pause ;;
            7) set_xray_gogc_300; systemctl daemon-reload >/dev/null 2>&1 || true; systemctl restart xray >/dev/null 2>&1 || true; info "GOGC=300 已固化。"; _e50_pause ;;
            8) force_apply_xray_core_defaults_e50; remove_xray_env_buffer; systemctl restart xray >/dev/null 2>&1 || true; info "Policy=260s + Buffer=128KB 已固化。"; _e50_pause ;;
            9) apply_xray_cpu_schedule_default; systemctl daemon-reload >/dev/null 2>&1 || true; systemctl restart xray >/dev/null 2>&1 || true; info "CPU 调度已按单核/多核自动检测固化。"; _e50_pause ;;
            10) _safe_jq_write '.routing |= (. // {}) | .routing.domainMatcher = "mph"'; systemctl restart xray >/dev/null 2>&1 || true; info "MPH 已开启。"; _e50_pause ;;
            11) if [ -z "$has_reality" ] || [ "$has_reality" = "null" ]; then error "无 Reality 支持。"; else _safe_jq_write '(.inbounds[]? | select(. != null) | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.realitySettings.maxTimeDiff) = 60000'; systemctl restart xray >/dev/null 2>&1 || true; info "Reality 时间锁已开启。"; fi; _e50_pause ;;
            12) enable_dnsmasq_cache_safely; systemctl restart xray >/dev/null 2>&1 || true; _e50_pause ;;
            13) enable_thp_never_default; _e50_pause ;;
            14) apply_hotupdate_fast_sysctl; _e50_pause ;;
            15) enable_cpu_performance_default; _e50_pause ;;
            16) toggle_ring; _e50_pause ;;
            17) toggle_cake_qdisc; update_hw_boot_script; _e50_pause ;;
            18) toggle_cake_flag "ack_filter"; update_hw_boot_script; _e50_pause ;;
            19) toggle_cake_flag "ecn"; update_hw_boot_script; _e50_pause ;;
            20) toggle_cake_flag "wash"; update_hw_boot_script; _e50_pause ;;
            21) toggle_gso; _e50_pause ;;
            22) toggle_irq; _e50_pause ;;
            23) enable_zram_default; _e50_pause ;;
            24) enable_journald_volatile_default; _e50_pause ;;
            25) apply_process_priority_default; systemctl restart xray >/dev/null 2>&1 || true; _e50_pause ;;
            26) _turn_on_app; systemctl restart xray >/dev/null 2>&1 || true; info "应用层 1-11 已幂等激活。"; _e50_pause ;;
            27) apply_safe_turbo_defaults; systemctl restart xray >/dev/null 2>&1 || true; info "安全系统级微操已幂等落地。"; _e50_pause ;;
            28) _turn_on_app; apply_safe_turbo_defaults; systemctl restart xray >/dev/null 2>&1 || true; info "极速稳态一键打通完成。"; _e50_pause ;;
            0) return ;;
        esac
    done
}
...
install_self_entrypoint() {
    ...
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e50.sh" 2>/dev/null || true
        chmod +x "$SYMLINK" "$SCRIPT_DIR/ex198e50.sh" 2>/dev/null || true
    fi
...
apply_hotupdate_fast_sysctl() {
    ...
    mkdir -p /etc/sysctl.d 2>/dev/null || true
    local adv app qdisc
    # V198e50: 热更新默认固化你实测稳定的高 RTT 代理窗口组合。
    adv=1
    app=31
...
enable_dnsmasq_cache_safely() {
    # V198e50: 热更新默认开启 Dnsmasq，但必须安装、监听、解析全通过才接管 resolv.conf。
    # 不再覆盖 /etc/dnsmasq.conf；仅写 /etc/dnsmasq.d/xray-cache.conf，降低系统兼容风险。
    title "安全启用 Dnsmasq 本地极速缓存（conf.d 模式，失败自动回退）"
    repair_dns_if_broken
    export DEBIAN_FRONTEND=noninteractive
    ...
    if ! command -v dnsmasq >/dev/null 2>&1; then
        warn "dnsmasq 未安装成功，保持静态 DNS，避免解析断联。"
        write_static_resolv_conf
        return 0
    fi
    ...
    cp -a /etc/dnsmasq.conf "$BACKUP_DIR/dnsmasq.conf.before-e50.$(date +%Y%m%d_%H%M%S).bak" 2>/dev/null || true
    if test ! -f /etc/dnsmasq.conf; then touch /etc/dnsmasq.conf; fi
    ...
    cat > /etc/dnsmasq.d/xray-cache.conf <<'EOF_DNSMASQ_E50'
# Managed by ex198e50: local DNS cache for Xray VPS
port=53
listen-address=127.0.0.1
bind-interfaces
cache-size=20000
...
EOF_DNSMASQ_E50
    ...
    dnsmasq --test >/dev/null 2>&1 || {
        warn "dnsmasq 配置自检失败，自动回退静态 DNS。"
        rm -f /etc/dnsmasq.d/xray-cache.conf 2>/dev/null || true
        systemctl stop dnsmasq >/dev/null 2>&1 || true
        systemctl disable dnsmasq >/dev/null 2>&1 || true
        write_static_resolv_conf
        return 0
    ...
    if systemctl restart dnsmasq >/dev/null 2>&1 && dns_local_listener_ok; then
        chattr -i /etc/resolv.conf 2>/dev/null || true
        cp -a /etc/resolv.conf "$BACKUP_DIR/resolv.conf.before-dnsmasq.e50.$(date +%Y%m%d_%H%M%S).bak" 2>/dev/null || true
        echo "nameserver 127.0.0.1" > /etc/resolv.conf
        ...
        if getent hosts api.ipify.org >/dev/null 2>&1 || getent hosts google.com >/dev/null 2>&1; then
            ...
            info "dnsmasq 已启用、监听、解析验证通过，并接管本机解析。"
            return 0
        fi
    fi
...
apply_txqueue_3000_default() {
    # V198e50: 默认 3000，不再用 12000，避免队列过深导致排队延迟和 bufferbloat。
    local IP_CMD IFACE
...
enable_zram_default() {
    # e50：ZRAM 开启同时执行总 Swap 上限 1024MB；ZRAM 可用时删除 /swapfile，避免 2GB+ Swap。
    enforce_swap_cap_1024_e50
}
...
get_ssh_listen_ports() {
    local ports=""
    # V198e50: 优先从当前监听态识别 SSH 端口，避免 sshd_config 使用 Include/云镜像改名时误判为 22。
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e50 稳态极速版)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e50 - (The Apex Vanguard)${none}"
        ...
        echo -e "  ${cyan}11) 从 ex198e32/ex198e33/ex198e34/ex198e35/ex198e36/ex198e37/ex198e38/ex198e39/ex198e40/ex198e41/ex198e42/ex198e43 稳态热更新到 V198e50${none}"
...
rescue_after_v35_breakage() {
    title "V198e50 紧急修复：撤销旧版重启后断流风险项"
...
#==============================================================================
# V198e50 最终覆盖层：128KB Buffer 单位修正 + systemd 环境变量清理
...
toggle_buffer() {
    # V198e50：只在 config.json 的 KB 单位中切换 64/128，永不再写 systemd MB 环境变量。
    local v=""
...
#==============================================================================
# V198e50 覆盖修复层：修正 e40 残留状态/持久化问题
...
_safe_jq_write() {
    # V198e50：以退出码为准，不再强制搜索 Configuration OK 字符串。
...
force_apply_xray_core_defaults_e50() {
...
apply_xray_policy_260_default() { force_apply_xray_core_defaults_e50 >/dev/null 2>&1 || return 1; }
apply_xray_buffer_kb() { local kb="${1:-128}"; if test "$kb" != "128"; then warn "V198e50 默认只固化 128KB；如需其它值请手动改 config.json。"; fi; force_apply_xray_core_defaults_e50 >/dev/null 2>&1 || return 1; remove_xray_env_buffer; }
apply_xray_buffer_128_default() { force_apply_xray_core_defaults_e50 >/dev/null 2>&1 || return 1; remove_xray_env_buffer; }
apply_xray_nodelay_default() { force_apply_xray_core_defaults_e50 >/dev/null 2>&1 || return 1; }
apply_xray_fastopen_off_default() { force_apply_xray_core_defaults_e50 >/dev/null 2>&1 || return 1; }
apply_xray_keep_2_to_6_defaults() { force_apply_xray_core_defaults_e50 >/dev/null 2>&1 || return 1; }
hot_patch_xray_config() { force_apply_xray_core_defaults_e50; }
...
update_hw_boot_script() {
    cat > /usr/local/bin/xray-hw-tweaks.sh <<'EOF_E50_HW'
#!/bin/bash
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
...
if test -n "$IFACE"; then
    ...
    if test -f "$FLAGS_DIR/ring_low_latency" && command -v ethtool >/dev/null 2>&1 && ethtool -g "$IFACE" >/dev/null 2>&1; then
        TARGET=$(cat "$CONFIG_DIR/ring_target.txt" 2>/dev/null || echo "1024")
        MAX_TX=$(ethtool -g "$IFACE" 2>/dev/null | awk '/Pre-set maximums/{flag=1; next} flag && /^[[:space:]]*TX:/{print $2; exit}')
        if ! echo "$TARGET" | grep -Eq '^[0-9]+$'; then TARGET=1024; fi
        if ! echo "${MAX_TX:-}" | grep -Eq '^[0-9]+$'; then MAX_TX="$TARGET"; fi
        TX_TARGET="$TARGET"; if test "$MAX_TX" -lt "$TARGET" 2>/dev/null; then TX_TARGET="$MAX_TX"; fi
        ethtool -G "$IFACE" rx "$TARGET" tx "$TX_TARGET" 2>/dev/null || true
    fi
...
EOF_E50_HW
    ...
    cat > /etc/systemd/system/xray-hw-tweaks.service <<'EOF_E50_SVC'
[Unit]
...
apply_safe_turbo_defaults() {
    title "V198e50 稳态极速包：Dnsmasq + Ring低延迟 + ZRAM≤1024MB 强制热更新落地"
    repair_dns_if_broken
    check_and_create_swap
    fix_xray_systemd_limits
    apply_hotupdate_fast_sysctl
...
verify_e50_post_state() {
    ...
    if test "$ring_state" = "false"; then warn "Ring Buffer 未能收缩到低延迟目标；可能是云厂商虚拟网卡限制。"; fi
...
hot_update_from_legacy() {
    title "V198e50 稳态热更新：Dnsmasq/Ring/ZRAM 三项强落地 + Swap≤1024MB"
    ...
    local ts bdir
    ts=$(date +%Y%m%d_%H%M%S); bdir="$BACKUP_DIR/hotupdate_e50_${ts}"
    ...
    cp -a /etc/dnsmasq.conf "$bdir/dnsmasq.conf.bak" 2>/dev/null || true
    cp -a /etc/dnsmasq.d "$bdir/dnsmasq.d.bak" 2>/dev/null || true
    ...
    fix_permissions
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    if ! verify_e50_post_state; then error "V198e50 关键状态校验发现异常，已保留快照：$bdir"; return 1; fi
    ensure_xray_is_alive
    info "V198e50 热更新完成：Dnsmasq/Ring/ZRAM 已细化落地，Swap≤1024MB、Buffer=128KB、Policy=260s、FQ默认、GRO/GSO/TSO保留。"
}
