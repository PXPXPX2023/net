#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e71.sh (The Apex Vanguard - Project Genesis V198e71)
# 快捷方式: xrv
# 【V198e71 DNS双模式修正版：静态DNS急救 / Dnsmasq稳态分离 + Asia/Singapore时间轴】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e71.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e71"
...
#==============================================================================
# V198e71 DNS / Timezone / Section-5 final hardening layer
# 目标：
...
write_static_resolv_conf() {
    # V198e71：静态 DNS 回退也安装 guard，避免 DHCP/cloud-init 重启后把 resolv.conf 改回坏状态。
...
check_and_clean_space() {
    title "安全空间清理（V198e71：不默认删除 linux-headers）"
    echo "===== 清理前硬盘 / Swap ====="
    df -h / /boot 2>/dev/null || df -h /
    free -h 2>/dev/null || true
    swapon --show 2>/dev/null || true
...
kernel_pre_space_clean() {
    title "内核操作前安全空间清理（V198e71）"
    check_and_clean_space || true
}

_e65_apply_section5_stack() {
    title "V198e71 Section-5：Limits + Sysctl + MSS 持久化重塑"
    write_safe_global_limits
    fix_xray_systemd_limits >/dev/null 2>&1 || true
    cleanup_network_sysctl_conflicts_e58 >/dev/null 2>&1 || true
    write_full_network_optimized_conf_e60 1 31
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e71 DNS分离 + Asia/Singapore)"
...
install_self_entrypoint() {
    ...
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e71.sh" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
        chmod +x "$SYMLINK" "$SCRIPT_DIR/ex198e71.sh" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
    fi
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    hash -r 2>/dev/null || true
}
...
hot_update_from_legacy() {
    title "V198e71 稳态热更新：DNS双模式分离 + Asia/Singapore + Section-5持久化"
    if test ! -f "$CONFIG"; then die "未找到 $CONFIG，不能热更新。请先确认 Xray 已安装。"; fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then die "现有 config.json 不是合法 JSON，拒绝热更新，避免节点损坏。"; fi
    trap _e66_interrupt_guard INT TERM
    local ts bdir
    ...
    trap - INT TERM
    if ! verify_e66_post_state; then
        error "V198e71 关键状态校验失败，已保留快照：$bdir"
        warn "可以再次执行：bash ex198e71.sh --hot-update，或进入 10→5 手动应用。"
        return 1
    fi
    ensure_xray_is_alive || true
    info "V198e71 热更新完成：xrv 已指向最新总界面；默认使用 Dnsmasq 稳态缓存，选项 2 可随时切换静态 DNS 急救。"
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e71 - (The Apex Vanguard)${none}"
        local svc sys_ver
        svc=$(systemctl is-active xray 2>/dev/null || echo "inactive")
        if test "$svc" = "active"; then svc="${green}平稳在线 (Running)${none}"; else svc="${red}离线静默 (Stopped)${none}"; fi
        sys_ver=$(uname -r 2>/dev/null || echo "未探测到数据")
        ...
        echo "  9) 全景网络连接审计日志与商用级别网络流量记账本"
        echo "  10) 高级发烧：内核双轨编译(APT/源码)/网络/应用层 60余项极限全控板"
        echo -e "  ${cyan}11) 从旧 ex198e 环境稳态热更新到 V198e71${none}"
        echo -e "  ${cyan}12) 清理空间（安全释放 / 快照收敛 / 显示硬盘与 Swap）${none}"
        echo -e "  ${cyan}13) 应用网络参数（均衡/高吞吐/手动，显示永久配置）${none}"
        echo -e "  ${cyan}14) TCP 重传 / 单连接诊断中心（只读排障）${none}"
        echo "  0) 折叠命令控制台，返回底层终端"
...
#==============================================================================
# V198e71 final overlay：11 热更新强制执行选项1时区 + 选项6/28 一键稳态打通
# 设计目标：
...
#   3) xrv 在热更新一开始就被替换到最新脚本，热更新后直接 xrv 进入 V198e71 总界面。
...
e68_apply_micro_28_defaults() {
    title "V198e71：执行 25项微操 28 号极速稳态一键打通"
...
verify_e68_post_state() {
    ...
    if ! command -v xrv >/dev/null 2>&1; then
        ...
        echo "$xrv_head" | grep -q 'ex198e71' || warn "xrv 已存在，但文件头未明确显示 ex198e71；请确认 /usr/local/bin/xrv 是否为最新脚本。"
    fi
    return "$fail"
}

hot_update_from_legacy() {
    title "V198e71 稳态热更新：强制时区 + 明确执行 25项微操 28 号 + xrv 最新入口"
    if test ! -f "$CONFIG"; then die "未找到 $CONFIG，不能热更新。请先确认 Xray 已安装。"; fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then die "现有 config.json 不是合法 JSON，拒绝热更新，避免节点损坏。"; fi
    ...
    if ! verify_e68_post_state; then
        error "V198e71 状态校验失败，已保留快照：$bdir"
        warn "可再次执行：bash ex198e71.sh --hot-update，或进入 10→6→28 / 10→5 手动应用。"
        return 1
    fi

    ensure_xray_is_alive || true
    info "V198e71 热更新完成：选项1 Asia/Singapore 与选项6→28 已明确执行；xrv 已指向最新总界面。"
    info "现在可直接输入：xrv"
}


#==============================================================================
# V198e71 final overlay：e30/e32 老环境稳态热更新防翻车版
# 目标：
...
e69_enable_dnsmasq_or_static() {
    title "V198e71 DNS 稳态：优先 dnsmasq 缓存，失败回退静态 DNS"
...
#==============================================================================
# Xray / VPS Network Optimized Sysctl Profile - V198e71 high-throughput stable
#==============================================================================
net.core.default_qdisc = fq
net.ipv4.tcp_congestion_control = bbr
net.ipv4.neigh.default.base_reachable_time_ms = 60000
...
[Unit]
Description=Xray Hardware Tweaks Safe Defaults V198e71
Wants=network-online.target
After=network-online.target
Before=xray.service
...
e69_apply_micro_28_safe() {
    title "V198e71：安全等价执行 25项微操 28 号"
...
e69_verify_post_state() {
    ...
    if command -v xrv >/dev/null 2>&1; then head -n 8 "$(command -v xrv)" 2>/dev/null | grep -q 'ex198e71' || warn "xrv 未明确显示 ex198e71，但可能仍为本脚本。"; fi
    if ! getent hosts api.ipify.org >/dev/null 2>&1 && ! getent hosts google.com >/dev/null 2>&1; then warn "DNS 解析验证未通过。"; fail=1; fi
    return "$fail"
}

hot_update_from_legacy() {
    title "V198e71：从 V198e30/e32 旧环境安全热更新到稳态极速版"
    if test ! -f "$CONFIG"; then die "未找到 $CONFIG，不能热更新。请先确认 Xray 已安装。"; fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then die "现有 config.json 不是合法 JSON，拒绝热更新，避免节点损坏。"; fi
    ...
    if ! e69_verify_post_state; then
        error "V198e71 状态校验仍有未匹配项，快照目录：$bdir"
        warn "可再次执行：bash ex198e71.sh --hot-update；如 Xray 未运行，执行 journalctl -u xray --no-pager -n 80。"
        return 1
    fi
    ...
    info "V198e71 热更新完成：已兼容 V198e30 旧环境；xrv 已指向最新总界面。"
    info "现在可直接输入：xrv"
}
...
do_micro_control_menu() {
    while true; do
        clear
        title "全域 25 项系统级及应用层微操管理中心（V198e71 稳态修复版）"
...
# 更稳的入口安装：即使从 /root/ex198e71.sh 执行，也确保 xrv 被替换。
install_self_entrypoint() {
    mkdir -p "$SCRIPT_DIR" 2>/dev/null || true
    local src="${SCRIPT_PATH:-$0}"
    if test -f "$src"; then
        cp -f "$src" "$SYMLINK" 2>/dev/null || true
        cp -f "$src" "$SCRIPT_DIR/ex198e71.sh" 2>/dev/null || true
...
#==============================================================================
# V198e71 absolute final overlay 2026-07-01
...
do_micro_control_menu() {
    while true; do
        clear
        title "全域 25 项系统级及应用层微操管理中心（V198e71 e30兼容修复版）"
        echo "  12) Dnsmasq 本地极速内存缓存引擎（验证通过才接管 resolv.conf）"
        echo "  23) ZRAM / Swap 总量 1024MB 稳态控制"
        echo "  24) Journald 纯内存化"
        echo "  25) Xray 进程级 Limits / OOM / Nice / GOGC / Buffer 稳态"
...
hot_update_from_legacy() {
    ...
    set -e
    info "V198e71 热更新完成：已兼容 V198e30/e32 旧环境；xrv 已指向最新总界面。"
    info "现在可直接输入：xrv"
    return 0
}
...
#==============================================================================
# V198e71 final overlay 2026-07-01
...
install_self_entrypoint() {
    ...
    echo "198e71" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    hash -r 2>/dev/null || true
}
...
e69_apply_critical_stack() {
    ...
    systemctl daemon-reload >/dev/null 2>&1 || true
    info "V198e71 落地关键网络栈 [$phase]：Nice=-20 / GOGC=300 / tcp_notsent_lowat=131072"
}
...
# 最终热更新入口。
hot_update_from_legacy() {
    title "V198e71：Nice=-20 + GOGC=300 + tcp_notsent_lowat=131072 稳态热更新"
    if test ! -f "$CONFIG"; then die "未找到 $CONFIG，不能热更新。请先确认 Xray 已安装。"; fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then die "现有 config.json 不是合法 JSON，拒绝热更新，避免节点损坏。"; fi
    ...
    set -e
    info "V198e71 热更新完成：Nice=-20 / GOGC=300 / tcp_notsent_lowat=131072 已落地；xrv 已指向最新总界面。"
    info "现在可直接输入：xrv"
    return 0
}
...
main_menu() {
    while true; do
        ...
        echo -e "  ${cyan}13) 应用网络参数（均衡/高吞吐/手动，显示永久配置）${none}"
        echo -e "  ${cyan}14) TCP 重传 / 单连接诊断中心（只读排障）${none}"
        echo -e "  ${gray}0) 折叠命令控制台，返回底层终端${none}"
        hr
        local opt=""; read -rp "请输入操作代码指令: " opt || true
        case "${opt:-}" in
            11) hot_update_from_legacy; read -rp "按 Enter 继续..." _ || true ;;
            12) kernel_pre_space_clean 2>/dev/null || check_and_clean_space || true; read -rp "按 Enter 继续..." _ || true ;;
            13) network_param_center 2>/dev/null || { e69_apply_critical_stack manual; read -rp "按 Enter 继续..." _ || true; } ;;
            14) tcp_diag_center 2>/dev/null || { echo "诊断中心不可用。"; read -rp "按 Enter 继续..." _ || true; } ;;
            0) exit 0 ;;
            *) warn "无效指令。"; sleep 1 ;;
        esac
    done
...
#==============================================================================
# EOF - Apex Vanguard V198e71 System Advanced Control Ready.
#==============================================================================
