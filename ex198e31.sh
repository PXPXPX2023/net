...
# 快捷方式: xrv
# 【V198e31 终极纯血版：全量UI + Dns/MTU/动态内存自适应深度修复防爆版】
#==============================================================================
...
check_and_clean_space() {
    ...
    rm -rf /var/log/*.log /var/log/*/*.log /tmp/xray* /tmp/sni* /usr/src/linux* /usr/src/bbr* /usr/src/xanmod* /compile/* /root/linux* /root/*.tar* /root/*.gz /root/*.xz /var/cache/apt/archives/* 2>/dev/null || true
    sync
}
...
do_change_dns() {
    ...
    if [ "$(check_dnsmasq_state)" = "true" ]; then
        warn "检测到 Dnsmasq 内存加速引擎已开启！请先在菜单中关闭 Dnsmasq (选项12) 再修改主网 DNS，以免冲突断网。"
        sleep 3; return
    fi

    if ! systemctl list-unit-files | grep -q "^systemd-resolved"; then warn "系统未检测到 systemd-resolved，跳过 DNS 配置。"; return; fi
    local IFACE
    IFACE=$(ip route | awk '/default/ {print $5}' | head -n1)
    if [ -z "$IFACE" ]; then warn "无法检测默认网卡，DNS 配置终止。"; return; fi
    ...
    while true; do
        ...
        if [[ $nameserver =~ ^([0-9]{1,2}|1[0-9][0-9]|2[0-4][0-9]|25[0-5])\.([0-9]{1,2}|1[0-9][0-9]|2[0-4][0-9]|25[0-5])\.([0-9]{1,2}|1[0-9][0-9]|2[0-4][0-9]|25[0-5])\.([0-9]{1,2}|1[0-9][0-9]|2[0-4][0-9]|25[0-5])$ ]]; then break; else print_red "DNS IP 格式错误，请重新输入。"; fi
    done
...
EOF
        ...
        sleep 2
        if ping -c1 -W3 google.com >/dev/null 2>&1; then info "已自动切换 DNS。"; else warn "DNS 仍异常，请检查 VPS 网络环境。"; fi
    fi
    info "当前 DNS 状态："
...
do_kernel_compile_menu() {
    while true; do
        ...
        echo "  [已生效] 解封多核并发全火力编译，1050MB Swap 提供底层护航！"
        echo ""
...
do_perf_tuning() {
    ...
    # 修复：防止 ss -ti 返回奇怪字符导致 Bash 算术崩溃
...
EOF
    ...
    # 修复: 把夸张的 tcp_rmem 26MB 降至安全的 16MB 工业级防爆阈值
...
do_txqueuelen_opt() {
    ...
    local tq_val=12000
    # 修复：阻止 Virtio 网卡强吃 12000 队列导致系统网络底层崩溃断联
    if ethtool -i "$IFACE" 2>/dev/null | grep -qi "virtio"; then
        warn "检测到 Virtio 虚拟网卡，设置过大发送队列将导致宿主机断网，已回缩至安全自适应值 5000。"
        tq_val=5000
    fi
    
    info "正在修改 $IFACE 发送队列长度至 $tq_val ..."
    $IP_CMD link set "$IFACE" txqueuelen $tq_val 2>/dev/null || true
...
[Service]
Type=oneshot
ExecStart=$IP_CMD link set $IFACE txqueuelen $tq_val
...
EOF
    ...
    if test "$CHECK_QLEN" = "$tq_val"; then info "已成功将网卡底层并发队列长度扩容至 $tq_val 级。"; else warn "当前虚拟机或网卡驱动不支持调节 txqueuelen。"; fi
...
EOF
    ...
    # 修复：防止 Virtio 遇到硬件强开导致失联崩溃
    if [ "$(check_ring_state)" = "true" ]; then echo "if ! ethtool -i \"\$IFACE\" 2>/dev/null | grep -qi 'virtio'; then ethtool -G \$IFACE rx 512 tx 512 2>/dev/null || true; fi" >> /usr/local/bin/xray-hw-tweaks.sh; fi
    if [ "$(check_gso_off_state)" = "true" ]; then echo "if ! ethtool -i \"\$IFACE\" 2>/dev/null | grep -qi 'virtio'; then ethtool -K \$IFACE gro off gso off tso off 2>/dev/null || true; fi" >> /usr/local/bin/xray-hw-tweaks.sh; elif [ "$(check_gso_off_state)" = "false" ]; then echo "ethtool -K \$IFACE gro on gso on tso on 2>/dev/null || true" >> /usr/local/bin/xray-hw-tweaks.sh; fi
...
toggle_dnsmasq() {
    # 修复：极易导致 53 端口冲突炸网与 systemd-resolved 软链接死亡的 Bug
    if [ "$(check_dnsmasq_state)" = "true" ]; then
        ...
        if ! grep -q "nameserver" /etc/resolv.conf 2>/dev/null; then echo "nameserver 8.8.8.8" > /etc/resolv.conf; fi
...
toggle_mtu() { 
    # 修复：防止 sed 丢失以及重启未生效问题，强化实时注入和持久化
...
toggle_ring() { 
    ...
    if [ "$(check_ring_state)" = "true" ]; then 
        ...
        if [ -n "$max_rx" ]; then 
            if ! ethtool -i "$IFACE" 2>/dev/null | grep -qi 'virtio'; then ethtool -G "$IFACE" rx "$max_rx" tx "$max_rx" 2>/dev/null || true; fi
        fi
    else 
        if ! ethtool -i "$IFACE" 2>/dev/null | grep -qi 'virtio'; then ethtool -G "$IFACE" rx 512 tx 512 2>/dev/null || true; fi
    fi
    update_hw_boot_script
}
...
toggle_zram() { 
    ...
    if [ "$(check_zram_state)" = "true" ]; then 
        swapoff /dev/zram0 2>/dev/null || true; rmmod zram 2>/dev/null || true; systemctl disable xray-zram.service --now 2>/dev/null || true; rm -f /etc/systemd/system/xray-zram.service /usr/local/bin/xray-zram.sh
    else 
        # 修复：防止 ZRAM=内存*2 导致小鸡系统瞬间完全死锁 OOM
        local TOTAL_MEM=$(free -m 2>/dev/null | awk '/Mem/{print $2}' || echo "1024"); local ZRAM_SIZE
        if [ "$TOTAL_MEM" -lt 1024 ]; then ZRAM_SIZE=$((TOTAL_MEM / 2)); else ZRAM_SIZE=512; fi
...
WantedBy=multi-user.target
EOFZ
        systemctl daemon-reload >/dev/null 2>&1 || true; systemctl enable xray-zram.service >/dev/null 2>&1 || true; systemctl start xray-zram.service >/dev/null 2>&1 || true
    fi
}
...
toggle_gso() { local IFACE=$(ip route get 1.1.1.1 2>/dev/null | awk '{print $5; exit}' || echo ""); if [ -z "$IFACE" ]; then return; fi; if [ "$(check_gso_off_state)" = "true" ]; then ethtool -K "$IFACE" gro on gso on tso on 2>/dev/null || true; else if ! ethtool -i "$IFACE" 2>/dev/null | grep -qi "virtio"; then ethtool -K "$IFACE" gro off gso off tso off 2>/dev/null || true; fi; fi; update_hw_boot_script; }
...
_turn_on_app() {
    ...
    if [ "$(check_dnsmasq_state)" = "true" ]; then _safe_jq_write '.dns = {"servers":["127.0.0.1"], "queryStrategy":"UseIP"}'; else _safe_jq_write '.dns = {"servers":["https://8.8.8.8/dns-query","https://1.1.1.1/dns-query"], "queryStrategy":"UseIP"}'; fi
    ...
    if [ -f "$limit_file" ]; then
        ...
        # 修复：防止 GOGC 设置过高（如1000）导致 VPS 小内存瞬间塞满 OOM 强杀进程
        local TOTAL_MEM=$(free -m 2>/dev/null | awk '/Mem/{print $2}' || echo "1024"); local DYNAMIC_GOGC=100
        if [ "$TOTAL_MEM" -ge 3800 ]; then DYNAMIC_GOGC=300; elif [ "$TOTAL_MEM" -ge 1800 ]; then DYNAMIC_GOGC=150; elif [ "$TOTAL_MEM" -ge 900 ]; then DYNAMIC_GOGC=100; else DYNAMIC_GOGC=50; fi
...
_turn_off_app() {
    _safe_jq_write '
      ...
      del(.inbounds[]?  | select(. != null) | select(.protocol=="vless")   | .streamSettings?.sockopt?.tcpNoDelay, .streamSettings?.sockopt?.tcpFastOpen, .streamSettings?.sockopt?.reusePort, .streamSettings?.sockopt?.tcpKeepAliveIdle, .streamSettings?.sockopt?.tcpKeepAliveInterval)
    '
...
do_app_level_tuning_menu() {
    while true; do
        ...
        case "$app_opt" in
            1) if [ "$out_fastopen" = "true" ]; then _safe_jq_write 'del(.outbounds[]? | select(. != null) | select(.protocol=="freedom") | .streamSettings?.sockopt?.tcpNoDelay, .streamSettings?.sockopt?.tcpFastOpen) | del(.inbounds[]? | select(. != null) | select(.protocol=="vless") | .streamSettings?.sockopt?.tcpNoDelay, .streamSettings?.sockopt?.tcpFastOpen, .streamSettings?.sockopt?.reusePort)'; info "双向并发提速等特性已关闭。"; else _turn_on_app; info "双向并发提速 (FastOpen/reusePort) 等应用层已全域重置！"; fi; systemctl restart xray >/dev/null 2>&1 || true; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e31 终极纯血版)"
        echo "  1) [一键全清] 执行 Linux 强基更新、亚太时间轴校准并置入极客 1050MB 内存交换区"
        echo "  2) [系统防御] 强行修改源头 DNS 解析 (注入 resolvconf，免脱轨断联)"
        echo -e "  ${cyan}3) [重构内脏] 双轨飞升：官方 APT 预编译直装 或 极客全量源码锻造${none}"
        echo "  4) [网络底层] TX Queue 网卡出站队列防拥堵极限缩减 (自适应 5000/12000 收缩)"
        echo "  5) [极限压榨] 全域系统底层网络栈结构重塑 (Limits + Sysctl 内存钳制防爆)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e31 - (The Apex Vanguard)${none}"
...
#==============================================================================
# EOF - Apex Vanguard V198e31 System Advanced Control Ready.
#==============================================================================
