#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex111.sh (The Apex Vanguard - Project Genesis V111 [The Horizon])
# 快捷方式: xrv
# 巅峰视界: 
#   1. 港口管治：新增 CAKE 带宽、Overhead 封包补偿与 MPU 精准控制引擎。完美配合 ack-filter，彻底解决 GSO 强制卸载环境下的调度失真问题。
#   2. 逻辑粉碎：修复 do_summary 中由于 jq length 语法特性导致的 Shadowsocks 节点信息偶发性蒸发 BUG，改为绝对安全的协议探针判空。
#   3. 陷阱免疫：绝对拒绝使用旧版残缺代码，全量继承 V109 中所有无折叠多行赋值、virtio 锁死探针、双协议平权等神级特性。
#   4. 语法绝缘：完整保留 jq 的原子化赋值链 (.streamSettings //= {} | .sockopt //= {})，绝杀 JSON 结构残缺导致的空转。
#   5. 大一统矩阵：全域 23 项极限微操 + CAKE 深度高阶参数面板，构建单节点路由的终极物理形态。
#   6. 不追求代码行数的精简，只追求工业级的绝对健壮，敬这单节点路由架构的最后丰碑。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex111.sh"
    exit 1
fi
...
config_cake_advanced() {
    clear
    title "CAKE 高阶调度参数配置 (解决虚拟网卡 GSO/GRO 锁死时的缓冲膨胀)"
