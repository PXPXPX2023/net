#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e22.sh (The Apex Vanguard - Project Genesis V196e22)
# 快捷方式: xrv
#
# 【V196e22 终极勘误与真理重塑：降维打击版】
#   1. 揭开 dx28.sh 伪造 Xanmod 骗局，本脚本将为你编译 100% 纯正的真·Xanmod + BBR3。
#   2. 彻底斩除 Xanmod 编译暴毙: 动用正则物理切除 Makefile 中的 -march=x86-64-v 动态断层。
#   3. 开放极客自选版本: 允许任意手工指定 Git Tag (例如 6.18.25-rt-xanmod1)。
#   4. 融合 dx28 容错机制: 引入 Swap 降级创建 (fallocate -> dd)，适配所有底层文件系统。
#   5. 全量单文件输出，告别截断切片。
# ==============================================================================

# 必须用 bash 运行
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex196e22.sh"
    exit 1
fi
...
readonly SCRIPT_VERSION="196e22"
...
do_kernel_compile_menu() {
    while true; do
        clear
        title "极客内核源码锻造中心 (V196e22 真理降维打击版)"
        echo "  针对上个版本中由于过度兼容而引发的 bad value (‘x86-64-v’) 报错，"
        echo "  现已彻底抛弃斯文的配置注入，动用 sed 物理抹除 Makefile 中所有的动态断层！"
        echo ""
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e22 - (The Apex Vanguard)${none}"
...
# ==============================================================================
# EOF - Apex Vanguard V196e22 System Advanced Control Ready.
# ==============================================================================
# --- ✂️ Part 12 结束，所有代码拼接完毕。请运行 bash ex196e22.sh ---
