#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex40.sh (The Apex Vanguard - Project Genesis V13)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 彻底转型【极低延迟/小包秒开】架构：Xray 并发精简至 4096，TX 队列收缩至 2000。
#   2. 重塑 NAPI 软中断配额：dev_weight=64，bias=1，杜绝算力独占带来的微观卡顿。
#   3. 激活内核态忙轮询 (Busy Polling): net.core.busy_poll=50，死盯网卡极限提速。
#   4. 终结 TCP Bufferbloat: 引入精准 16KB (16384) 的 notsent_lowat 发送缓冲防膨胀。
#   5. 剥离网卡中断聚合 (Interrupt Coalescing): 强制 ethtool -C 做到 0 延迟硬件触发。
#   6. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex40.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex40 The Apex Vanguard - Project Genesis V13${none}"
