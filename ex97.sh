#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex97.sh (The Apex Vanguard - Project Genesis V97)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 逻辑重构：修复 jq 在处理缺失路径 (streamSettings) 时的致命 Silent Crash，实装多级自愈初始化引擎，确保提速参数 100% 物理落盘。
#   2. 时序排爆：为单点优先级提权 (OOM/IO) 补齐实时的进程重载指令，做到所见即所得。
#   3. 进程透明：撕除上帝之手中繁重任务 (Dnsmasq 部署) 的静默面罩，解除 UI 假死，过程全息可见。
#   4. 架构回归：剥离 Nano 极简库，全面换回大而全的全球版路由库 (Loyalsoldier)，恢复全球分流与全量广告拦截能力。
#   5. 阵列重组：全域极客微操收拢为 15 项 (应用层 7 项 + 系统级 8 项)，物理绑核与并发干预底牌完美保留。
#   6. 容灾升维：继承 3:00 下载与 3:10 重启的错峰热更机制，提供 10 分钟黄金冗余下载期。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex97.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex97 The Apex Vanguard - Project Genesis V97${none}"
