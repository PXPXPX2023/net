#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e21.sh (The Apex Vanguard - Project Genesis V196e21)
# 快捷方式: xrv
#
# 【V196e21 终极勘误与真理重塑】
#   1. 解决 Xanmod 编译暴毙: 根除 -march=x86-64-v 残缺指令，加入 Makefile 物理防爆盾。
#   2. 开放极客自选版本: 允许用户在编译 Xanmod 时手工指定 Git Tag (例如 6.12 稳定版)。
#   3. 三通道内核锻造: 完美融合 Mainline+BBR, Xanmod+BBR3, Mainline+BBR3 极客三重路线。
#   4. 斩断配置毒瘤: 完全信任内核原生 defconfig 推导架构，不再污染 .config。
#   5. JQ 绝对绝缘: 全域覆盖 select(. != null) 护盾，杜绝任何数组为空时的语法崩溃。
# ==============================================================================

# 必须用 bash 运行
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex196e21.sh"
    exit 1
fi
...
readonly SCRIPT_VERSION="196e21"
...
do_kernel_compile_menu() {
    while true; do
        clear
        title "极客内核源码锻造中心 (V196e21 真理修复版)"
        echo "  针对上个版本中由于过度兼容而引发的 bad value (‘x86-64-v’) 报错，"
        echo "  现已彻底更正底层写入逻辑，并植入 Makefile 物理防爆盾。"
        echo ""
        echo -e "  ${cyan}1) [稳如老狗] 编译 Linux 官方主线最新内核 (Mainline + BBR) ${none}"
        echo "     - 源码直连 kernel.org，100% 原生纯净，编译零报错"
        echo ""
        echo -e "  ${magenta}2) [激进压榨] 编译 Xanmod 官方极客内核 (Xanmod 稳定分支 + BBR3) ${none}"
        echo "     - 源码直连 GitLab，支持人工手填分支 Tag 以规避前沿 Bug"
        echo "     - 内置强力防爆盾，强制剥离冲突指令，兼容全部 x86_64 服务器"
        echo ""
...
_compile_kernel_xanmod() {
    title "系统飞升：编译 真·Xanmod 极客内核 (全版本防爆兼容模式)"
    warn "警告: 此过程将极度消耗 CPU 与内存 (约 30-60 分钟)，低配机极易死机！"
    local confirm=""; read -rp "确定要开始极客源码编译吗？(y/n): " confirm || true
    if [[ ! "$confirm" =~ ^[yY]$ ]]; then return; fi
    
    local default_tag="6.12.25-xanmod1"
    info "您可以手动指定内核的 GitLab Release Tag 以进行精确编译 (例如 6.12.25-xanmod1 或 6.18.25-rt-xanmod1)。"
    ...
    info ">> 注入物理防爆盾：锁定 CONFIG_X86_64_VERSION，拦截空变量暴毙 Bug..."
    ...
    info ">> 正在直接修正 arch/x86/Makefile，移除危险的动态 march 拼接..."
...
EOF
    ...
    if test "$CHECK_QLEN" = "2000"; then info "已成功将网卡底层并发队列长度扩容至 2000 级。"; else warn "当前虚拟机或网卡驱动不支持调节 txqueuelen。"; fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e21 - (The Apex Vanguard)${none}"
...
# ==============================================================================
# EOF - Apex Vanguard V196e21 System Advanced Control Ready.
# ==============================================================================
# --- ✂️ Part 12 结束，所有代码拼接完毕。请运行 bash ex196e21.sh ---
