#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex43.sh (The Apex Vanguard - Project Genesis V16)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 重构系统初始化子菜单逻辑顺序：强制预编译打底 -> 源码编译 -> 网卡队列 -> 内核栈，打造完美防坑流水线。
#   2. 维持多用户分发中心 (do_summary) 动态循环解析，全量展示专属节点明细。
#   3. 维持【极低延迟/小包秒开】架构：Xray 并发 4096，TX 队列 2000，Busy Poll 50。
#   4. 维持 NAPI 软中断配额：dev_weight=64，bias=1，杜绝算力独占引发微观抖动。
#   5. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex43.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex43 The Apex Vanguard - Project Genesis V16${none}"
