#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: exex196e38.sh (The Apex Vanguard - Project Genesis V196e38)
# 快捷方式: xrv
#
# 【V196e38 终极融合不朽版：零缩水、零截断、斩断 32 位死结】
#   1. 粉碎编译暴毙: 物理关闭 CONFIG_IA32_EMULATION，彻底解决 vdso32 与 x86-64-v3 参数的致命冲突。
#   2. 满血复原: 100% 恢复应用层微操全控板、JQ 安全写入护盾与全景雷达网络。
#   3. 双轨驱动: 兼容官方 APT Xanmod 预编译（极稳）与 GitLab 全源码手工锻造（极致发烧）。
#   4. 智能架构: 融合第三方优秀 AWK 逻辑，精准匹配 CPU psABI 微架构 (v1-v4)。
# ==============================================================================

if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash exex196e38.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="196e38"
...
_compile_kernel_mainline() {
    ...
    # 【终极防爆】：斩除 32 位兼容层，防止 x86-64-vX 编译 32 位目标时暴毙
...
_compile_kernel_xanmod() {
    ...
    # 【终极防爆】：斩除 32 位兼容层，防止 x86-64-v3 编译 32 位目标 (vdso32) 时当场暴毙
    info ">> 正在斩除 32位 (IA32) 兼容层，彻底粉碎 vdso32 与指令集的编译冲突..."
    ./scripts/config --disable CONFIG_IA32_EMULATION
    ./scripts/config --disable CONFIG_X86_X32
    ./scripts/config --disable CONFIG_X86_X32_ABI
    ...
    # 精准限定 CONFIG_X86_64_VERSION，废除会破坏 Xanmod 原生性能的危险正则 sed
