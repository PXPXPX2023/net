#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e43.sh (The Apex Vanguard - Project Genesis V198e43)
# 快捷方式: xrv
# 【V198e43 稳态极速热更新版：安全加速包默认启用 + DNS 防翻车 + Reality 指纹收敛】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e43.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e43"
...
check_and_clean_space() {
    ...
    # V198e43: 不再 rm -rf /tmp/*、不再删除全部 /var/log/*.log，避免误删 socket/锁文件/审计日志导致服务异常。
    rm -rf \
...
detect_x86_64_level() {
    ...
    if curl -fsSL --connect-timeout 5 https://dl.xanmod.org/check_x86-64_psabi.sh -o "$script" 2>/dev/null; then
        ...
        level=$(bash "$script" 2>/dev/null | grep -oE 'x86-64-v[1-4]' | grep -oE '[1-4]' | tail -n1 || echo "")
        rm -f "$script" 2>/dev/null || true
    fi

    if [[ ! "$level" =~ ^[1-4]$ ]]; then
        local flags="$(LC_ALL=C lscpu 2>/dev/null | awk -F: '/Flags|Flags/{print $2}' | head -n1)"
...
choose_best_qdisc() {
    # V198e43: 极速代理默认 fq + BBR。CAKE 不再作为默认项，避免弱 CPU VPS 因整形/分类开销反降速。
...
check_and_create_swap() {
    title "检查并配置 1GB Swap 防爆缓冲池（幂等防重复）"

    local SWAP_FILE="/swapfile"
    local TARGET_SWAP_MB=1024
    local MIN_OK_MB=1000

    mkdir -p "$BACKUP_DIR/sysctl" 2>/dev/null || true

    local total_swap_mb
    total_swap_mb=$(free -m 2>/dev/null | awk '/^Swap:/ {print int($2)}' || echo "0")
    if ! [[ "$total_swap_mb" =~ ^[0-9]+$ ]]; then total_swap_mb=0; fi

    local swapfile_enabled_mb=0
    if swapon --show=NAME,SIZE --bytes 2>/dev/null | awk '{print $1}' | grep -qx "$SWAP_FILE"; then
        swapfile_enabled_mb=$(swapon --show=NAME,SIZE --bytes 2>/dev/null | awk -v f="$SWAP_FILE" '$1==f {print int($2/1024/1024); exit}' || echo "0")
        if ! [[ "$swapfile_enabled_mb" =~ ^[0-9]+$ ]]; then swapfile_enabled_mb=0; fi
    fi

    if test "$total_swap_mb" -ge "$MIN_OK_MB" 2>/dev/null; then
        info "检测到系统当前 Swap 总量 ${total_swap_mb}MB，已满足 1GB 目标，不重复创建。"
        if test "$swapfile_enabled_mb" -gt 0 2>/dev/null; then
            info "/swapfile 当前已启用 ${swapfile_enabled_mb}MB，保持现状。"
        fi
    ...
    if test "$swapfile_enabled_mb" -gt 0 2>/dev/null; then
        warn "检测到 /swapfile 已启用但只有 ${swapfile_enabled_mb}MB，不足 1GB，准备安全重建。"
    ...
    if test "$root_free_mb" -lt 1300 2>/dev/null; then
        warn "根分区可用空间约 ${root_free_mb}MB，不足以安全创建 1GB Swap，已跳过。"
        return 0
    fi

    info "开始创建 1GB Swap: $SWAP_FILE"
    ...
    if ! mkswap "$SWAP_FILE" >/dev/null 2>&1; then
        warn "mkswap 失败，已删除损坏 swapfile。"
        rm -f "$SWAP_FILE" 2>/dev/null || true
        return 0
    fi

    if ! swapon "$SWAP_FILE" >/dev/null 2>&1; then
        warn "swapon 启动失败，可能是云厂商或容器限制。"
        rm -f "$SWAP_FILE" 2>/dev/null || true
        return 0
    fi
    ...
    cat > /etc/sysctl.d/98-xray-swap.conf << 'EOF'
vm.swappiness = 10
...
EOF
    ...
    local after_swap_mb
    after_swap_mb=$(free -m 2>/dev/null | awk '/^Swap:/ {print int($2)}' || echo "0")
    info "1GB Swap 防爆缓冲池配置完成，当前系统 Swap 总量约 ${after_swap_mb}MB。"
}


write_safe_global_limits() {
    # V198e43: 修复 V198e35/V198e36 残留的 100000KB stack 问题。
    # getaddrinfo()/curl 会创建解析线程，过大的默认 stack 在小内存 VPS 上会导致：
    # curl: (6) getaddrinfo() thread failed to start
    mkdir -p /etc/security/limits.d /etc/systemd/system.conf.d 2>/dev/null || true

    cp -a /etc/security/limits.d/99-xray-limits.conf \
        "$BACKUP_DIR/99-xray-limits.conf.$(date +%Y%m%d_%H%M%S).bak" 2>/dev/null || true
    cp -a /etc/systemd/system.conf.d/99-xray-limits.conf \
        "$BACKUP_DIR/systemd-99-xray-limits.conf.$(date +%Y%m%d_%H%M%S).bak" 2>/dev/null || true

    cat > /etc/security/limits.d/99-xray-limits.conf <<'EOF_LIMITS'
...
EOF_LIMITS
    ...
    cat > /etc/systemd/system.conf.d/99-xray-limits.conf <<'EOF_MANAGER_LIMITS'
[Manager]
...
fix_xray_systemd_limits() {
    ...
    local current_nice="-10"
    local current_gogc="200"
    ...
    if test -f "$limit_file"; then
        if grep -q "^Nice=" "$limit_file" 2>/dev/null; then current_nice=$(awk -F'=' '/^Nice=/{print $2}' "$limit_file" | head -n 1 || echo "-10"); fi
        if grep -q "^Environment=\"GOGC=" "$limit_file" 2>/dev/null; then current_gogc=$(awk -F'=' '/^Environment="GOGC=/{print $3}' "$limit_file" | tr -d '"' | head -n 1 || echo "200"); fi
    ...
    local go_mem_limit=$(( total_mem * 80 / 100 ))
    if test "$go_mem_limit" -lt 128 2>/dev/null; then go_mem_limit=128; fi
...
verify_sni_strict() {
    ...
    if ! echo "$out" | grep -qi "OCSP response:"; then warn "OCSP Stapling 未发现：这不是致命问题，V198e43 仅提示不强制淘汰。"; fi
...
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
IFACE="$IFACE"
# V198e43: 纯代理吞吐优先保留 GRO/GSO/TSO，降低 CPU 压力；仅关闭 LRO，避免破坏转发语义。
...
do_perf_tuning() {
    ...
    info "配置系统高并发进程限制 (Limits，V198e43 稳态安全版)..."
    write_safe_global_limits
...
MASK=$(python3 - "$CORES" <<'PY' 2>/dev/null || echo ffffffff
import sys
print(format((1 << int(sys.argv[1])) - 1, 'x'))
PY
)
for irq in $(grep "$IFACE" /proc/interrupts 2>/dev/null | awk '{print $1}' | tr -d ':' || true); do if test -w "/proc/irq/$irq/smp_affinity"; then echo "$MASK" > "/proc/irq/$irq/smp_affinity" 2>/dev/null || true; fi; done
EOF
    fi

    cat >> /usr/local/bin/xray-hw-tweaks.sh << 'EOF'
if test -f "/usr/local/etc/xray/initcwnd.txt"; then
    CWND_VAL=$(cat "/usr/local/etc/xray/initcwnd.txt" 2>/dev/null || echo "10")
    DEF_ROUTE=$(ip route show default 2>/dev/null | head -n 1)
    if [[ -n "$DEF_ROUTE" ]]; then
        CLEAN_ROUTE=$(echo "$DEF_ROUTE" | sed 's/ initcwnd [0-9]*//g' | sed 's/ initrwnd [0-9]*//g')
        ip route change $CLEAN_ROUTE initcwnd $CWND_VAL initrwnd $CWND_VAL 2>/dev/null || true
    fi
fi
modprobe iptable_mangle >/dev/null 2>&1 || true
modprobe xt_TCPMSS >/dev/null 2>&1 || true
iptables -t mangle -C POSTROUTING -p tcp -m tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu 2>/dev/null || iptables -t mangle -A POSTROUTING -p tcp -m tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu 2>/dev/null || true
EOF
...
_toggle_affinity_on() {
    ...
    if [ -f "$limit_file" ]; then 
        ...
        if [ "$CORES" -eq 2 ]; then 
            ...
            TARGET_CPU="1-$((CORES-1))"
            GO_PROCS=$((CORES-1))
        fi
        echo "CPUAffinity=$TARGET_CPU" >> "$limit_file"
        echo "Environment=\"GOMAXPROCS=$GO_PROCS\"" >> "$limit_file"
        systemctl daemon-reload >/dev/null 2>&1 || true
    fi
}
...
_turn_on_app() {
    _safe_jq_write '
      ...
      (.outbounds[]? | select(. != null) | select(.protocol=="freedom") | .streamSettings.sockopt.tcpFastOpen) = false |
      ...
      (.inbounds[]?  | select(. != null) | select(.protocol=="vless") | .streamSettings.sockopt.tcpFastOpen) = false |
    ...
    if [ "$(check_dnsmasq_state)" = "true" ]; then _safe_jq_write '.dns = {"servers":["127.0.0.1"], "queryStrategy":"UseIPv4"}'; else _safe_jq_write '.dns = {"servers":["https://1.1.1.1/dns-query","https://208.67.222.222/dns-query","https://doh.opendns.com/dns-query"], "queryStrategy":"UseIPv4"}'; fi
    _safe_jq_write '.policy = {"levels":{"0":{"handshake":3,"connIdle":260,"uplinkOnly":2,"downlinkOnly":5}},"system":{"statsInboundDownlink":false,"statsInboundUplink":false}}'
...
do_app_level_tuning_menu() {
    while true; do
        clear
        title "全域 25 项系统级及应用层微操管理中心（V198e43 稳态极速）"
        ...
        local out_nodelay=$(jq -r '.outbounds[]? | select(. != null) | select(.protocol=="freedom") | .streamSettings?.sockopt?.tcpNoDelay // "false"' "$CONFIG" 2>/dev/null | head -n 1)
        ...
        [ "$out_nodelay" != "true" ] && app_off_count=$((app_off_count+1))
        ...
        [ "$dns_status" != "UseIPv4" ] && app_off_count=$((app_off_count+1))
        if [ "$gc_status" = "默认 100" ] || [ "$gc_status" = "100" ]; then app_off_count=$((app_off_count+1)); fi
        [ "$policy_status" != "260" ] && app_off_count=$((app_off_count+1))
        ...
        local s1=$([ "$out_nodelay" = "true" ] && echo "${cyan}已开启${none}" || echo "${gray}未开启${none}")
        local s1f=$([ "$out_fastopen" = "true" ] && echo "${yellow}已开启（不建议默认）${none}" || echo "${gray}默认关闭${none}")
        ...
        local s5=$([ "$buffer_state" = "true" ] && echo "${cyan}已收缩 (128KB)${none}" || echo "${gray}系统默认${none}")
        local s6=$([ "$dns_status" = "UseIPv4" ] && echo "${cyan}已开启 IPv4 优先${none}" || echo "${gray}未开启${none}")
        local s8=$([ "$policy_status" = "260" ] && echo "${cyan}已开启 (闲置260s/握手3s)${none}" || echo "${gray}默认/非260s${none}")
        ...
        local s21=$([ "$gso_state" = "true" ] && echo "${cyan}已关闭卸载 (排障/低延迟测试)${none}" || ([ "$gso_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}未开启${none}"))
        ...
        echo -e "  16) 【网卡硬件环形缓冲区 (Ring Buffer)】(可选：低延迟收缩)    | 状态: $s16"
        ...
        case "$app_opt" in
            1) if [ "$out_nodelay" = "true" ]; then _safe_jq_write 'del(.outbounds[]? | select(. != null) | select(.protocol=="freedom") | .streamSettings?.sockopt?.tcpNoDelay) | del(.inbounds[]? | select(. != null) | select(.protocol=="vless") | .streamSettings?.sockopt?.tcpNoDelay)'; info "TCP NoDelay 已关闭。"; else _safe_jq_write '(.outbounds[]? | select(. != null) | select(.protocol=="freedom") | .streamSettings) |= (. // {}) | (.outbounds[]? | select(. != null) | select(.protocol=="freedom") | .streamSettings.sockopt) |= (. // {}) | (.inbounds[]? | select(. != null) | select(.protocol=="vless") | .streamSettings) |= (. // {}) | (.inbounds[]? | select(. != null) | select(.protocol=="vless") | .streamSettings.sockopt) |= (. // {}) | (.outbounds[]? | select(. != null) | select(.protocol=="freedom") | .streamSettings.sockopt.tcpNoDelay)=true | (.inbounds[]? | select(. != null) | select(.protocol=="vless") | .streamSettings.sockopt.tcpNoDelay)=true'; info "TCP NoDelay 已开启，FastOpen 保持独立。"; fi; systemctl restart xray >/dev/null 2>&1 || true; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            1f|1F) if [ "$out_fastopen" = "true" ]; then _safe_jq_write '(.outbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=false | (.inbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=false'; info "TCP FastOpen 已关闭。"; else _safe_jq_write '(.outbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=true | (.inbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=true'; info "TCP FastOpen 已开启；如跨境握手不稳请立即关闭。"; fi; systemctl restart xray >/dev/null 2>&1 || true; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            ...
            5) force_apply_xray_core_defaults_e43; remove_xray_env_buffer; systemctl restart xray >/dev/null 2>&1 || true; info "Buffer 已幂等固化为 config.json 的 128KB，不再使用 systemd MB 环境变量。"; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            6) toggle_dnsmasq; systemctl restart xray >/dev/null 2>&1 || true; info "DNS 引擎流向已重构。"; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            7) if [ -f "$limit_file" ]; then if [ "$gc_status" = "300" ]; then sed -i 's/^Environment="GOGC=.*/Environment="GOGC=100"/' "$limit_file" 2>/dev/null || true; info "GOGC 已恢复为 100。"; else set_or_replace_line "$limit_file" '^Environment="GOGC=' 'Environment="GOGC=300"'; info "GOGC 已固定为 300。"; fi; systemctl daemon-reload >/dev/null 2>&1 || true; systemctl restart xray >/dev/null 2>&1 || true; else error "未找到配置环境！"; fi; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            8) force_apply_xray_core_defaults_e43; systemctl restart xray >/dev/null 2>&1 || true; info "Policy 已幂等固化为 connIdle=260，并保留 Buffer=128KB。"; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            9) if [ "$affinity_state" = "true" ]; then _toggle_affinity_off; systemctl restart xray >/dev/null 2>&1 || true; info "智能 CPU 调优已解除。"; else _toggle_affinity_on; systemctl restart xray >/dev/null 2>&1 || true; info "智能绑定核心已激活，适配多核复用！"; fi; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            10) if [ "$mph_state" = "true" ]; then _safe_jq_write 'del(.routing.domainMatcher)'; systemctl restart xray >/dev/null 2>&1 || true; info "MPH 算法关闭。"; else _safe_jq_write '.routing |= (. // {}) | .routing.domainMatcher = "mph"'; systemctl restart xray >/dev/null 2>&1 || true; info "MPH 极速匹配启用！"; fi; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            ...
            26) print_magenta ">>> 幂等开启 1-11 项应用层微操..."; _turn_on_app; systemctl restart xray >/dev/null 2>&1 || true; info "应用层微操已幂等激活，不再自动反向关闭。"; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            27) apply_safe_turbo_defaults; systemctl restart xray >/dev/null 2>&1 || true; info "V198e43 安全系统级微操已幂等落地。"; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            28) _turn_on_app; apply_safe_turbo_defaults; systemctl restart xray >/dev/null 2>&1 || true; info "V198e43 极速稳态一键打通完成：应用层 + 安全系统级微操已幂等落地。"; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            0) return ;;
        esac
    done
}
...
install_self_entrypoint() {
    ...
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e43.sh" 2>/dev/null || true
        chmod +x "$SYMLINK" "$SCRIPT_DIR/ex198e43.sh" 2>/dev/null || true
    fi
...
hot_patch_xray_config() {
    if test ! -f "$CONFIG"; then return 1; fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then
        error "现有 config.json 不是合法 JSON，拒绝热更新。"
        return 1
    fi
    ...
    _safe_jq_write '
      ...
      if any(.routing.rules[]?; (.protocol // []) | index("bittorrent")) then . else .routing.rules += [{"outboundTag":"block","_enabled":true,"protocol":["bittorrent"]}] end |
      if any(.routing.rules[]?; (.domain // []) | index("geosite:category-ads-all")) then . else .routing.rules += [{"outboundTag":"block","_enabled":true,"domain":["geosite:category-ads-all"]}] end |
      ...
      .outbounds = (.outbounds // []) |
      if any(.outbounds[]?; .tag=="direct") then . else .outbounds += [{"protocol":"freedom","tag":"direct","settings":{"domainStrategy":"AsIs"},"streamSettings":{"sockopt":{"tcpNoDelay":true,"tcpFastOpen":false,"tcpKeepAliveIdle":30,"tcpKeepAliveInterval":15}}}] end |
      if any(.outbounds[]?; .tag=="block") then . else .outbounds += [{"protocol":"blackhole","tag":"block"}] end |
      ...
      (.inbounds[]? | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.network) = "tcp" |
      (.inbounds[]? | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.sockopt.tcpNoDelay) = true |
      (.inbounds[]? | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.sockopt.tcpFastOpen) = false |
      (.inbounds[]? | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.sockopt.tcpKeepAliveIdle) = 30 |
      (.inbounds[]? | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.sockopt.tcpKeepAliveInterval) = 15 |
      (.inbounds[]? | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.realitySettings) |= (
          . as $rs |
          .show = false |
          .maxTimeDiff = 60000 |
          .serverNames = (($rs.serverNames // []) | if type=="array" and length>0 then . else [(($rs.dest // "www.microsoft.com:443") | split(":")[0])] end) |
      ...
      (.inbounds[]? | select(.protocol=="vless" or .protocol=="shadowsocks") | .sniffing) = {"enabled":true,"destOverride":["http","tls","quic"],"metadataOnly":true,"routeOnly":true}
    '
}
...
apply_hotupdate_fast_sysctl() {
    ...
    mkdir -p /etc/sysctl.d 2>/dev/null || true
    local adv app qdisc
    # V198e43: 热更新默认固化你实测稳定的高 RTT 代理窗口组合。
    adv=1
    app=31
...
enable_dnsmasq_cache_safely() {
    # V198e43: 默认启用 dnsmasq，但必须通过监听与解析验证后才接管 resolv.conf。
...
apply_txqueue_3000_default() {
    # V198e43: 默认 3000，不再用 12000，避免队列过深导致排队延迟和 bufferbloat。
    local IP_CMD IFACE
...
enable_zram_default() {
    # 已挂载则跳过；不支持则跳过。保持 swapfile 作为低优先级兜底，ZRAM 高优先级。
    if swapon --show 2>/dev/null | grep -q 'zram'; then return 0; fi
    if ! modprobe -n zram >/dev/null 2>&1 && ! lsmod 2>/dev/null | grep -q zram; then return 0; fi
    local TOTAL_MEM ZRAM_SIZE
    TOTAL_MEM=$(free -m 2>/dev/null | awk '/Mem/{print int($2)}' || echo "1024")
    if test "$TOTAL_MEM" -lt 500 2>/dev/null; then ZRAM_SIZE=$((TOTAL_MEM * 2));
    elif test "$TOTAL_MEM" -lt 1024 2>/dev/null; then ZRAM_SIZE=$((TOTAL_MEM * 3 / 2));
    else ZRAM_SIZE=$TOTAL_MEM; fi
    cat > /usr/local/bin/xray-zram.sh <<EOF_ZRAM_V38
#!/usr/bin/env bash
set -e
modprobe zram num_devices=1 2>/dev/null || true
if [ -e /sys/block/zram0/reset ]; then swapoff /dev/zram0 2>/dev/null || true; echo 1 > /sys/block/zram0/reset 2>/dev/null || true; fi
...
EOF_ZRAM_V38
    ...
    cat > /etc/systemd/system/xray-zram.service <<'EOF_ZRAM_SRV_V38'
[Unit]
Description=Xray ZRAM Setup
...
apply_process_priority_default() {
    # 幂等写入，避免重复追加 OOM/Nice 项。
    local limit_file="/etc/systemd/system/xray.service.d/limits.conf"
    mkdir -p "$(dirname "$limit_file")" 2>/dev/null || true
    touch "$limit_file" 2>/dev/null || true
    sed -i '/^OOMScoreAdjust=/d;/^IOSchedulingClass=/d;/^IOSchedulingPriority=/d' "$limit_file" 2>/dev/null || true
    cat >> "$limit_file" <<'EOF_PRIO_V38'
OOMScoreAdjust=-500
IOSchedulingClass=best-effort
IOSchedulingPriority=0
EOF_PRIO_V38
...
apply_safe_turbo_defaults() {
    # V198e43: 热更新默认启用“安全极速包”，但不启用高风险项：Origin Guard / CAKE / ECN / TFO / 禁 IPv6 / 源码内核编译。
    title "V198e43 安全极速包：随热更新自动启用"
    repair_dns_if_broken
    check_and_create_swap
    fix_xray_systemd_limits
    apply_hotupdate_fast_sysctl
...
hot_update_from_legacy() {
    local mode="${1:-manual}"
    title "V198e43 稳态极速热更新迁移器（兼容 ex198e32-ex198e37）"

    if test ! -f "$CONFIG"; then
        error "未发现 $CONFIG，当前 VPS 可能尚未安装旧版环境。请先使用菜单 1 新装，或确认配置路径。"
        return 1
    fi
    repair_dns_if_broken
    if ! command -v jq >/dev/null 2>&1; then pkg_install jq >/dev/null 2>&1 || true; fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then
        error "现有 Xray config.json 解析失败。为避免破坏旧节点，热更新已中止。"
        return 1
    fi

    warn "热更新模式会保留现有 UUID / privateKey / shortId / 端口 / 用户，不会重建节点。"
    warn "不要在旧环境上选择菜单 1 重新安装，除非你准备让旧客户端全部失效。"
    if test "$mode" != "auto"; then
        local confirm=""
        read -rp "确认开始补丁式热更新？(y/N): " confirm || true
        if [[ ! "$confirm" =~ ^[yY]$ ]]; then return 0; fi
    fi

    local snap
    snap=$(backup_hotupdate_snapshot)
    info "已创建热更新快照: $snap"

    install_self_entrypoint
    repair_dns_if_broken
    install_update_dat
    # V198e43: 先移除旧版遗留的自动入口守卫，避免重启后链路被 iptables 误伤。
    remove_origin_guard auto >/dev/null 2>&1 || true
    apply_safe_turbo_defaults
    migrate_legacy_user_maps
    ensure_public_key_cache

    if ! hot_patch_xray_config; then
        error "配置补丁失败，已保留原快照: $snap"
        return 1
    fi

    fix_permissions
    if ensure_xray_is_alive; then
        # V198e43: 热更新不再自动启用 Origin Guard。V198e35/V198e36 的自动守卫会在重启后误伤部分真实链路。
        remove_origin_guard auto >/dev/null 2>&1 || true
        echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
        info "热更新完成：旧配置、旧用户和旧端口已保留，V198e43 安全极速包已生效。Origin Guard 仍默认关闭，可在防火墙菜单中手动开启。"
        if test "$mode" != "auto"; then
            do_summary
            local _p=""; read -rp "按 Enter 返回主菜单..." _p || true
        fi
    ...
    error "Xray 重启验证失败。已尝试回滚最新 config 备份；完整快照位于: $snap"
    return 1
}
...
get_ssh_listen_ports() {
    local ports=""
    # V198e43: 优先从当前监听态识别 SSH 端口，避免 sshd_config 使用 Include/云镜像改名时误判为 22。
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e43 稳态极速版)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e43 - (The Apex Vanguard)${none}"
        ...
        echo -e "  ${cyan}11) 从 ex198e32/ex198e33/ex198e34/ex198e35/ex198e36/ex198e37/ex198e38/ex198e39/ex198e40/ex198e41 稳态热更新到 V198e43${none}"
...
rescue_after_v35_breakage() {
    title "V198e43 紧急修复：撤销旧版重启后断流风险项"
...
apply_xray_cpu_schedule_default() {
    ...
    cores=$(nproc 2>/dev/null || echo 1)
    if ! [[ "$cores" =~ ^[0-9]+$ ]] || test "$cores" -lt 1 2>/dev/null; then cores=1; fi
    cpus=$(xray_cpu_list "$cores")
    sed -i '/^CPUAffinity=/d;/^Environment="GOMAXPROCS=/d' "$limit_file" 2>/dev/null || true
    # 单核：锁 0；多核：锁全核心集合，避免旧版误锁单核导致瓶颈。
    echo "CPUAffinity=$cpus" >> "$limit_file"
...
check_affinity_state() {
    local limit_file="/etc/systemd/system/xray.service.d/limits.conf"
    if [ -f "$limit_file" ] && grep -q '^CPUAffinity=' "$limit_file" 2>/dev/null && grep -q '^Environment="GOMAXPROCS=' "$limit_file" 2>/dev/null; then echo "true"; else echo "false"; fi
}
...
apply_xray_fastopen_off_default() {
    test -f "$CONFIG" || return 0
    _safe_jq_write '
      (.outbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen) = false |
      (.inbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen) = false
...
fix_xray_systemd_limits() {
    ...
    mkdir -p "$override_dir" 2>/dev/null || true
    local limit_file="$override_dir/limits.conf"
    local total_mem go_mem_limit
    total_mem=$(free -m 2>/dev/null | awk '/Mem/{print $2}' || echo "1024")
    go_mem_limit=$(( total_mem * 80 / 100 ))
    if test "$go_mem_limit" -lt 128 2>/dev/null; then go_mem_limit=128; fi
    cat > "$limit_file" <<EOF_LIMITS_E40
[Service]
LimitNOFILE=1048576
LimitNPROC=262144
LimitMEMLOCK=infinity
...
apply_process_priority_default() {
    ...
    mkdir -p "$(dirname "$limit_file")" 2>/dev/null || true
    touch "$limit_file" 2>/dev/null || true
    sed -i '/^OOMScoreAdjust=/d;/^IOSchedulingClass=/d;/^IOSchedulingPriority=/d;/^Nice=/d;/^LimitNOFILE=/d;/^LimitSTACK=/d' "$limit_file" 2>/dev/null || true
    cat >> "$limit_file" <<'EOF_PRIO_E40'
LimitNOFILE=1048576
LimitSTACK=8M
Nice=-10
OOMScoreAdjust=-500
...
apply_safe_turbo_defaults() {
    title "V198e43 安全极速包：e37/e38 一键热更新默认落地"
    repair_dns_if_broken
    check_and_create_swap
    fix_xray_systemd_limits
    apply_hotupdate_fast_sysctl
...
hot_update_from_legacy() {
    title "V198e43 稳态极速热更新：保留节点参数，一键落地默认加速项"
    if test ! -f "$CONFIG"; then
        die "未找到 $CONFIG，不能热更新。请先确认 Xray 已安装。"
    fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then
        die "现有 config.json 不是合法 JSON，拒绝热更新，避免节点损坏。"
    fi
    local ts bdir
    ts=$(date +%Y%m%d_%H%M%S)
    bdir="$BACKUP_DIR/hotupdate_e40_${ts}"
    ...
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    ensure_xray_is_alive
    info "V198e43 热更新完成：NoDelay 已开启，FastOpen 默认关闭，GOGC=300，Policy=260s，Dnsmasq/THP/ZRAM/OOM 默认按安全条件落地。"
}



#==============================================================================
# V198e43 最终覆盖层：128KB Buffer 单位修正 + systemd 环境变量清理
...
remove_xray_env_buffer() {
    local paths="/etc/systemd/system/xray.service /etc/systemd/system/xray.service.d"
    for p in $paths; do
        if test -e "$p"; then
            grep -RIl 'XRAY_RAY_BUFFER_SIZE' "$p" 2>/dev/null | while read -r f; do
                cp -a "$f" "$BACKUP_DIR/$(basename "$f").remove_buffer_env.$(date +%Y%m%d_%H%M%S).bak" 2>/dev/null || true
...
apply_xray_buffer_kb() {
    local kb="${1:-128}"
    if ! [[ "$kb" =~ ^[0-9]+$ ]]; then kb=128; fi
    if test "$kb" -lt 16 2>/dev/null; then kb=16; fi
    if test "$kb" -gt 512 2>/dev/null; then kb=512; fi
    test -f "$CONFIG" || { remove_xray_env_buffer; return 0; }
    _safe_jq_write --argjson kb "$kb" '
      .policy = (.policy // {}) |
      .policy.levels = (.policy.levels // {}) |
      .policy.levels["0"] = (.policy.levels["0"] // {}) |
      .policy.levels["0"].bufferSize = $kb
    ' >/dev/null 2>&1 || true
    remove_xray_env_buffer
}

apply_xray_buffer_128_default() {
...
check_buffer_state() {
    ...
    if test "$v" = "128" && ! systemctl cat xray 2>/dev/null | grep -qi 'XRAY_RAY_BUFFER_SIZE'; then
        echo "true"
    else
        echo "false"
    fi
...
toggle_buffer() {
    # V198e43：只在 config.json 的 KB 单位中切换 64/128，永不再写 systemd MB 环境变量。
    local v=""
    v=$(jq -r '.policy.levels["0"].bufferSize // empty' "$CONFIG" 2>/dev/null || echo "")
    if test "$v" = "128"; then
...
apply_process_priority_default() {
    ...
    mkdir -p "$(dirname "$limit_file")" 2>/dev/null || true
    touch "$limit_file" 2>/dev/null || true
    sed -i '/^OOMScoreAdjust=/d;/^IOSchedulingClass=/d;/^IOSchedulingPriority=/d;/^Nice=/d;/^LimitNOFILE=/d;/^LimitSTACK=/d;/XRAY_RAY_BUFFER_SIZE/d' "$limit_file" 2>/dev/null || true
    cat >> "$limit_file" <<'EOF_PRIO_E40'
LimitNOFILE=1048576
LimitSTACK=8M
Nice=-10
...
apply_safe_turbo_defaults() {
    title "V198e43 安全极速包：默认启用 128KB Buffer，并清理 systemd MB 单位变量"
    repair_dns_if_broken
    check_and_create_swap
    fix_xray_systemd_limits
    apply_hotupdate_fast_sysctl
...
hot_update_from_legacy() {
    title "V198e43 稳态极速热更新：保留节点参数，默认修正 Buffer=128KB"
    if test ! -f "$CONFIG"; then
        die "未找到 $CONFIG，不能热更新。请先确认 Xray 已安装。"
    fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then
    ...
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    ensure_xray_is_alive
    info "V198e43 热更新完成：config.json bufferSize=128KB 已落地，systemd XRAY_RAY_BUFFER_SIZE 已清理，NoDelay 开启，FastOpen 关闭，GOGC=300，Policy=260s。"
}



#==============================================================================
# V198e43 覆盖修复层：修正 e40 残留状态/持久化问题
    ...
    for p in /etc/systemd/system/xray.service /etc/systemd/system/xray.service.d; do
        if test -e "$p"; then
            grep -RIl 'XRAY_RAY_BUFFER_SIZE' "$p" 2>/dev/null | while read -r f; do
                ...
                cp -a "$f" "$BACKUP_DIR/$(basename "$f").remove_buffer_env.e41.$(date +%Y%m%d_%H%M%S).bak" 2>/dev/null || true
                sed -i '/XRAY_RAY_BUFFER_SIZE/d' "$f" 2>/dev/null || true
            done
        fi
    done
...
apply_xray_buffer_128_default() { apply_xray_buffer_kb 128; }
...
check_cake_state() {
    local IFACE q default_qdisc
    IFACE=$(_get_default_iface)
    if test -n "$IFACE" && command -v tc >/dev/null 2>&1; then
        q=$(tc qdisc show dev "$IFACE" 2>/dev/null || true)
...
check_ackfilter_state() { if test "$(check_cake_state)" = "true" && test -f "$FLAGS_DIR/ack_filter"; then echo "true"; else echo "false"; fi; }
check_ecn_state()       { if test "$(check_cake_state)" = "true" && test -f "$FLAGS_DIR/ecn"; then echo "true"; else echo "false"; fi; }
check_wash_state()      { if test "$(check_cake_state)" = "true" && test -f "$FLAGS_DIR/wash"; then echo "true"; else echo "false"; fi; }

clear_cake_child_flags_default() {
    # 极速默认是 FQ；CAKE 子项 flag 不应在 CAKE 未启用时污染状态显示。
...
check_gso_off_state() {
    local IFACE eth_info gro gso tso fixed_count field_count off_count
    IFACE=$(_get_default_iface)
    if test -z "$IFACE" || ! command -v ethtool >/dev/null 2>&1; then echo "unsupported"; return; fi
    eth_info=$(ethtool -k "$IFACE" 2>/dev/null || echo "")
    test -n "$eth_info" || { echo "unsupported"; return; }
    field_count=$(echo "$eth_info" | grep -Ec '^(generic-receive-offload|generic-segmentation-offload|tcp-segmentation-offload):')
    test "$field_count" -gt 0 2>/dev/null || { echo "unsupported"; return; }
    fixed_count=$(echo "$eth_info" | grep -E '^(generic-receive-offload|generic-segmentation-offload|tcp-segmentation-offload):' | grep -c '\[fixed\]' || true)
    if test "$fixed_count" -ge "$field_count" 2>/dev/null; then echo "unsupported"; return; fi
    off_count=$(echo "$eth_info" | grep -E '^(generic-receive-offload|generic-segmentation-offload|tcp-segmentation-offload):' | grep -v '\[fixed\]' | grep -c ' off' || true)
    if test "$off_count" -gt 0 2>/dev/null; then echo "true"; else echo "false"; fi
}
...
apply_gso_gro_on_default() {
    ...
    if test -n "$IFACE" && command -v ethtool >/dev/null 2>&1; then
        ...
        ethtool -K "$IFACE" rx-gro-hw on tx-udp-segmentation on 2>/dev/null || true
    fi
    if test -f /usr/local/bin/xray-hw-tweaks.sh; then
        sed -i 's/gro off gso off tso off/gro on gso on tso on/g' /usr/local/bin/xray-hw-tweaks.sh 2>/dev/null || true
    fi
}
...
update_hw_boot_script() {
    local IFACE
    IFACE=$(_get_default_iface)
    cat > /usr/local/bin/xray-hw-tweaks.sh <<'EOF_E41_HW'
#!/bin/bash
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
IFACE=$(ip route get 1.1.1.1 2>/dev/null | awk '{print $5; exit}' || true)
if test -n "$IFACE"; then
    # 极速默认保留 GRO/GSO/TSO；仅关闭 LRO，避免破坏转发语义。
    ethtool -K "$IFACE" gro on gso on tso on lro off rx-gro-hw on tx-udp-segmentation on 2>/dev/null || true
    ethtool -C "$IFACE" adaptive-rx on 2>/dev/null || true
fi
...
# CPU governor: VPS 不支持则静默跳过。
for cpu in /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor; do
    test -f "$cpu" && echo performance > "$cpu" 2>/dev/null || true
done

sysctl -w net.core.default_qdisc=fq >/dev/null 2>&1 || true
if test -n "$IFACE" && command -v tc >/dev/null 2>&1; then tc qdisc replace dev "$IFACE" root fq 2>/dev/null || true; fi

if test -n "$IFACE"; then ip link set dev "$IFACE" txqueuelen 3000 2>/dev/null || true; fi

if test -f "/usr/local/etc/xray/initcwnd.txt"; then
    CWND_VAL=$(cat "/usr/local/etc/xray/initcwnd.txt" 2>/dev/null || echo "10")
    DEF_ROUTE=$(ip route show default 2>/dev/null | head -n 1)
    if test -n "$DEF_ROUTE"; then
        CLEAN_ROUTE=$(echo "$DEF_ROUTE" | sed 's/ initcwnd [0-9]*//g' | sed 's/ initrwnd [0-9]*//g')
        ip route change $CLEAN_ROUTE initcwnd "$CWND_VAL" initrwnd "$CWND_VAL" 2>/dev/null || true
    fi
fi
...
EOF_E41_HW
    chmod +x /usr/local/bin/xray-hw-tweaks.sh 2>/dev/null || true
    cat > /etc/systemd/system/xray-hw-tweaks.service <<'EOF_E41_SVC'
[Unit]
...
apply_xray_cpu_schedule_default() {
    ...
    touch "$limit_file" 2>/dev/null || true
    sed -i '/^CPUAffinity=/d;/^Environment="GOMAXPROCS=/d' "$limit_file" 2>/dev/null || true
    local CORES TARGET_CPU GO_PROCS
    CORES=$(nproc 2>/dev/null || echo 1)
    if test "$CORES" -le 1 2>/dev/null; then
        TARGET_CPU="0"; GO_PROCS="1"
    elif test "$CORES" -eq 2 2>/dev/null; then
        TARGET_CPU="0-1"; GO_PROCS="2"
    else
        TARGET_CPU="0-$((CORES-1))"; GO_PROCS="$CORES"
    fi
    echo "CPUAffinity=$TARGET_CPU" >> "$limit_file"
    echo "Environment=\"GOMAXPROCS=$GO_PROCS\"" >> "$limit_file"
    systemctl daemon-reload >/dev/null 2>&1 || true
...
apply_safe_turbo_defaults() {
    title "V198e43 安全极速包：修正 e40 状态残留，默认 FQ + 128KB Buffer + GSO/GRO 保留"
    repair_dns_if_broken
    check_and_create_swap
    fix_xray_systemd_limits
    apply_hotupdate_fast_sysctl
...
hot_update_from_legacy() {
    title "V198e43 稳态极速热更新：修复 e40 状态残留并保留节点参数"
    if test ! -f "$CONFIG"; then
        die "未找到 $CONFIG，不能热更新。请先确认 Xray 已安装。"
    fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then
    ...
    local ts bdir
    ts=$(date +%Y%m%d_%H%M%S)
    bdir="$BACKUP_DIR/hotupdate_e41_${ts}"
    mkdir -p "$bdir" 2>/dev/null || true
    cp -a "$CONFIG" "$bdir/config.json.bak" 2>/dev/null || true
    cp -a "$PUBKEY_FILE" "$bdir/public.key.bak" 2>/dev/null || true
    cp -a "$USER_SNI_MAP" "$bdir/user_sni.txt.bak" 2>/dev/null || true
    ...
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    ensure_xray_is_alive
    info "V198e43 热更新完成：Policy=260、Buffer=128KB、systemd Buffer 环境变量清理、CAKE 子项残留清理、GSO/GRO/TSO 保留。"
}



#==============================================================================
# V198e43 最终覆盖层：去重与幂等化修正
# 目标：
...
check_affinity_state() {
    local limit_file="/etc/systemd/system/xray.service.d/limits.conf"
    if test -f "$limit_file" && grep -q '^CPUAffinity=' "$limit_file" 2>/dev/null && grep -q '^Environment="GOMAXPROCS=' "$limit_file" 2>/dev/null; then echo "true"; else echo "false"; fi
}
...
fix_xray_systemd_limits() {
    ...
    local total_mem go_mem_limit
    total_mem=$(free -m 2>/dev/null | awk '/Mem/{print $2}' || echo "1024")
    if ! [[ "$total_mem" =~ ^[0-9]+$ ]]; then total_mem=1024; fi
    go_mem_limit=$(( total_mem * 80 / 100 ))
    if test "$go_mem_limit" -lt 128 2>/dev/null; then go_mem_limit=128; fi
    cat > "$limit_file" <<EOF_E43_LIMITS
[Service]
LimitNOFILE=1048576
LimitNPROC=262144
LimitMEMLOCK=infinity
...
apply_process_priority_default() {
    # 统一交给 fix_xray_systemd_limits 重写，避免重复追加 LimitNOFILE/LimitSTACK/Nice/OOM。
    fix_xray_systemd_limits
}
...
check_process_priority_state() {
    ...
    if grep -q '^OOMScoreAdjust=-500' "$limit_file" 2>/dev/null \
       && grep -q '^Nice=-10' "$limit_file" 2>/dev/null \
       && grep -q '^LimitNOFILE=1048576' "$limit_file" 2>/dev/null \
       && grep -q '^LimitSTACK=8M' "$limit_file" 2>/dev/null; then
        echo "true"
    else
        echo "false"
    fi
...
check_gso_off_state() {
    local IFACE eth_info field_count fixed_count off_count
    IFACE=$(_get_default_iface)
    if test -z "$IFACE" || ! command -v ethtool >/dev/null 2>&1; then echo "unsupported"; return; fi
    eth_info=$(ethtool -k "$IFACE" 2>/dev/null || echo "")
    test -n "$eth_info" || { echo "unsupported"; return; }
...
update_hw_boot_script() {
    cat > /usr/local/bin/xray-hw-tweaks.sh <<'EOF_E43_HW'
#!/bin/bash
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
IFACE=$(ip route get 1.1.1.1 2>/dev/null | awk '{print $5; exit}' || true)
if test -n "$IFACE"; then
...
EOF_E43_HW
    chmod +x /usr/local/bin/xray-hw-tweaks.sh 2>/dev/null || true
    cat > /etc/systemd/system/xray-hw-tweaks.service <<'EOF_E43_SVC'
[Unit]
Description=Xray Hardware Tweaks Safe Defaults
After=network.target
...
hot_patch_xray_config() {
    if test ! -f "$CONFIG"; then return 1; fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then error "现有 config.json 不是合法 JSON，拒绝热更新。"; return 1; fi
    _safe_jq_write '
      .log = (.log // {}) |
      .log.access = "none" |
      .log.error = "none" |
      ...
      .policy.system.statsInboundUplink = false |
      .outbounds = (.outbounds // []) |
      if any(.outbounds[]?; .tag=="direct") then . else .outbounds += [{"protocol":"freedom","tag":"direct","settings":{"domainStrategy":"AsIs"},"streamSettings":{"sockopt":{}}}] end |
      if any(.outbounds[]?; .tag=="block") then . else .outbounds += [{"protocol":"blackhole","tag":"block"}] end
    ' >/dev/null 2>&1 || true
    apply_xray_keep_2_to_6_defaults
}
...
apply_safe_turbo_defaults() {
    title "V198e43 安全极速包：幂等开启 + 默认 FQ + 128KB Buffer + GSO/GRO 保留"
    repair_dns_if_broken
    check_and_create_swap
    fix_xray_systemd_limits
    apply_hotupdate_fast_sysctl
...
hot_update_from_legacy() {
    title "V198e43 稳态极速热更新：修复 e41 重复/残留并保留节点参数"
    ...
    local ts bdir
    ts=$(date +%Y%m%d_%H%M%S)
    bdir="$BACKUP_DIR/hotupdate_e43_${ts}"
    mkdir -p "$bdir" 2>/dev/null || true
    cp -a "$CONFIG" "$bdir/config.json.bak" 2>/dev/null || true
    cp -a "$PUBKEY_FILE" "$bdir/public.key.bak" 2>/dev/null || true
    cp -a "$USER_SNI_MAP" "$bdir/user_sni.txt.bak" 2>/dev/null || true
    ...
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    ensure_xray_is_alive
    info "V198e43 热更新完成：Policy=260、Buffer=128KB、systemd Buffer 环境变量清理、FQ 默认、CAKE 子项残留清理、GRO/GSO/TSO 保留、26 号菜单不再反向关闭。"
}



#==============================================================================
# V198e43 最终覆盖层：强校验热更新 + 重启后持久化修正
# 设计目标：
...
_safe_jq_write() {
    # V198e43：以退出码为准，不再强制搜索 Configuration OK 字符串。
...
write_safe_global_limits() {
    ...
    cat > /etc/security/limits.d/99-xray-limits.conf <<'EOF_E43_LIMITS'
root soft nofile 1048576
root hard nofile 1048576
root soft nproc 65535
root hard nproc 65535
...
EOF_E43_LIMITS
    ...
    cat > /etc/systemd/system.conf.d/99-xray-limits.conf <<'EOF_E43_MANAGER_LIMITS'
[Manager]
DefaultLimitNOFILE=1048576
DefaultLimitNPROC=65535
DefaultLimitSTACK=8M
DefaultTasksMax=80%
EOF_E43_MANAGER_LIMITS
}
...
fix_xray_systemd_limits() {
    ...
    cat > "$limit_file" <<EOF_E43_XRAY_LIMITS
[Service]
LimitNOFILE=1048576
LimitNPROC=262144
LimitMEMLOCK=infinity
...
force_apply_xray_core_defaults_e43() {
...
apply_xray_policy_260_default() { force_apply_xray_core_defaults_e43 >/dev/null 2>&1 || return 1; }
apply_xray_buffer_kb() { local kb="${1:-128}"; if test "$kb" != "128"; then warn "V198e43 默认只固化 128KB；如需其它值请手动改 config.json。"; fi; force_apply_xray_core_defaults_e43 >/dev/null 2>&1 || return 1; remove_xray_env_buffer; }
apply_xray_buffer_128_default() { force_apply_xray_core_defaults_e43 >/dev/null 2>&1 || return 1; remove_xray_env_buffer; }
apply_xray_nodelay_default() { force_apply_xray_core_defaults_e43 >/dev/null 2>&1 || return 1; }
apply_xray_fastopen_off_default() { force_apply_xray_core_defaults_e43 >/dev/null 2>&1 || return 1; }
apply_xray_keep_2_to_6_defaults() { force_apply_xray_core_defaults_e43 >/dev/null 2>&1 || return 1; }
hot_patch_xray_config() { force_apply_xray_core_defaults_e43; }
...
apply_fq_qdisc_default() {
    local IFACE conf
    IFACE=$(_get_default_iface)
    conf="/etc/sysctl.d/99-xray-hotupdate-fast.conf"
...
check_gso_off_state() {
    # V198e43：菜单 21 表示“脚本是否主动关闭卸载”。驱动固定 off 不再误判为用户已关闭。
    if test -f "$FLAGS_DIR/gso_off"; then echo "true"; else echo "false"; fi
}
...
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
IFACE=$(ip route get 1.1.1.1 2>/dev/null | awk '{print $5; exit}' || true)
FLAGS_DIR="/usr/local/etc/xray/flags"
if test -n "$IFACE"; then
    if test -f "$FLAGS_DIR/gso_off"; then
        ethtool -K "$IFACE" gro off gso off tso off 2>/dev/null || true
    else
...
apply_safe_turbo_defaults() {
    title "V198e43 安全极速包：强校验写入 + 重启持久化"
    repair_dns_if_broken
    check_and_create_swap
    fix_xray_systemd_limits
    apply_hotupdate_fast_sysctl
...
verify_e43_post_state() {
    local fail=0 buf pol env
    ...
    if test "$fail" -ne 0; then return 1; fi
    return 0
}

hot_update_from_legacy() {
    title "V198e43 稳态极速热更新：强校验修复 e42 重启后失效"
    if test ! -f "$CONFIG"; then die "未找到 $CONFIG，不能热更新。请先确认 Xray 已安装。"; fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then die "现有 config.json 不是合法 JSON，拒绝热更新，避免节点损坏。"; fi
    local ts bdir
    ts=$(date +%Y%m%d_%H%M%S)
    ...
    if ! verify_e43_post_state; then
        error "V198e43 关键状态校验失败，已保留快照：$bdir"
        return 1
    fi

    ensure_xray_is_alive
    info "V198e43 热更新完成：Buffer=128KB、Policy=260s 已强校验落地；systemd Buffer 环境变量已清理；GRO/GSO/TSO 默认保留。"
}
...
#==============================================================================
# EOF - Apex Vanguard V198e43 System Advanced Control Ready.
#==============================================================================
