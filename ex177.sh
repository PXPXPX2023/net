...
#    ╚═╝   ╚═╝  ╚═╝╚══════╝    ╚═╝  ╚═╝╚═╝     ╚══════╝╚═╝  ╚═╝
# ==============================================================================
# 脚本名称: ex177.sh (The Apex Vanguard - Project Genesis V177 [The Infinity Pure])
# 快捷方式: xrv
#
# V177 终极纯净重铸宣言:
#   1. 纯粹至上: 剔除冗余的 Caddy/NaiveProxy 和 Hysteria2，回归极简 VLESS/SS 双引擎。
#   2. 容错大一统: set -euo pipefail 严格模式 + 全局 ERR 探针 + 10 份历史快照轮转回滚。
#   3. 极客不妥协: 130+ SNI 单行垂直展开，60+ 项 Sysctl 参数满血注入，拒绝面条代码。
#   4. 编译防砖: kernel.org 原核裸装引擎，严密继承宿主驱动，确保护航 0 宕机。
#   5. 状态持久化: 物理 flags 锚点 + network-online.target 彻底消灭开机探针空转。
#   6. 语法免疫: jq "$@" 全量传参修复，.sniffing = (.sniffing // {}) 原子级护航。
#   7. 探针补全: 全量恢复 Reality 回落黑洞雷达扫描仪 (do_fallback_probe)。
# ==============================================================================

# 强制 Bash 运行环境检测
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行: bash ex177.sh"
    exit 1
fi
...
mkdir -p "$CONFIG_DIR" "$DAT_DIR" "$SCRIPT_DIR" "$FLAGS_DIR" \
         "$LOG_DIR" "$BACKUP_DIR" 2>/dev/null || true
...
run_sni_scanner() {
    ...
    # 军规级要求：严禁同行嵌套！每一发弹药都必须清晰占领单独的一行，绝不精简行数！
    local sni_list=(
        "www.apple.com"
        "support.apple.com"
        "developer.apple.com"
...
# ==============================================================================
# [ 区块 VIII: 状态机探针与开机底层锚点生成引擎 (容错自闭环展开版) ]
# ==============================================================================
...
check_mtu_state() {
    ...
    if [[ "$(sysctl -n net.ipv4.tcp_mtu_probing 2>/dev/null || echo "0")" == "1" ]]; then
        echo "true"
    else
        echo "false"
    fi
...
check_zram_state() {
    if ! modprobe -n zram >/dev/null 2>&1 && ! lsmod 2>/dev/null | grep -q zram 2>/dev/null; then
        echo "unsupported"
        return
    fi
    if swapon --show 2>/dev/null | grep -q 'zram' 2>/dev/null; then echo "true"; else echo "false"; fi
}
...
check_cake_state() {
    if sysctl net.core.default_qdisc 2>/dev/null | grep -q 'cake' 2>/dev/null; then echo "true"; else echo "false"; fi
}
...
# (上帝开关 Toggle 体系、UI 控制台以及主入口将于下一段无缝送出！)
# ==============================================================================
# ==============================================================================
...
do_app_level_tuning_menu() {
    while true; do
        ...
        [[ "$out_fastopen" != "true" ]] && ((app_off_count++))
        [[ "$out_keepalive" != "30" ]] && ((app_off_count++))
        [[ "$sniff_status" != "true" ]] && ((app_off_count++))
        [[ "$dns_status" != "UseIP" ]] && ((app_off_count++))
        [[ "$gc_status" == "默认 100" || "$gc_status" == "100" ]] && ((app_off_count++))
        [[ "$policy_status" != "60" ]] && ((app_off_count++))
        [[ "$affinity_state" != "true" ]] && ((app_off_count++))
        [[ "$mph_state" != "true" ]] && ((app_off_count++))
        [[ "$routeonly_status" != "true" ]] && ((app_off_count++))
        [[ "$buffer_state" != "true" ]] && ((app_off_count++))
        
        local has_reality
        has_reality=$(jq -r '.inbounds[]? | select(.protocol=="vless" and .streamSettings?.security=="reality") | .protocol' "$CONFIG" 2>/dev/null | head -n 1 || echo "")
        if [[ -n "$has_reality" ]]; then 
            [[ "$maxtime_state" != "true" ]] && ((app_off_count++))
        fi

        local sys_off_count=0
        [[ "$dnsmasq_state" == "false" ]] && ((sys_off_count++))
        [[ "$thp_state" == "false" ]] && ((sys_off_count++))
        [[ "$mtu_state" == "false" ]] && ((sys_off_count++))
        [[ "$cpu_state" == "false" ]] && ((sys_off_count++))
        [[ "$ring_state" == "false" ]] && ((sys_off_count++))
        [[ "$zram_state" == "false" ]] && ((sys_off_count++))
        [[ "$journal_state" == "false" ]] && ((sys_off_count++))
        [[ "$prio_state" == "false" ]] && ((sys_off_count++))
        [[ "$cake_state" == "false" ]] && ((sys_off_count++))
        [[ "$irq_state" == "false" ]] && ((sys_off_count++))
        [[ "$gso_off_state" == "false" ]] && ((sys_off_count++))
        [[ "$ackfilter_state" == "false" ]] && ((sys_off_count++))
        [[ "$ecn_state" == "false" ]] && ((sys_off_count++))
        [[ "$wash_state" == "false" ]] && ((sys_off_count++))

        # ==========================================
        # 终端渲染大屏
        # ==========================================
        local s1; [[ "$out_fastopen" == "true" ]] && s1="${cyan}已开启${none}" || s1="${gray}未开启${none}"
        local s2; [[ "$out_keepalive" == "30" ]] && s2="${cyan}已开启 (30s/15s)${none}" || s2="${gray}系统默认${none}"
        local s3; [[ "$sniff_status" == "true" ]] && s3="${cyan}已开启${none}" || s3="${gray}未开启${none}"
        local s4; [[ "$dns_status" == "UseIP" ]] && s4="${cyan}已开启${none}" || s4="${gray}未开启${none}"
        local s6; [[ "$policy_status" == "60" ]] && s6="${cyan}已开启 (闲置60s/握手3s)${none}" || s6="${gray}默认 300s 慢回收${none}"
        local s7; [[ "$affinity_state" == "true" ]] && s7="${cyan}已锁死单核 (零切换)${none}" || s7="${gray}默认 (系统调度)${none}"
        local s8; [[ "$mph_state" == "true" ]] && s8="${cyan}O(1) 预编译算法就绪${none}" || s8="${gray}默认 (Linear/AC机)${none}"
        
        local s9
        if [[ -z "$has_reality" ]]; then 
            s9="${gray}无 Reality (已跳过)${none}"
        else 
            [[ "$maxtime_state" == "true" ]] && s9="${cyan}绝对防线 (60s)${none}" || s9="${gray}默认 (不设防)${none}"
        fi
        
        local s10; [[ "$routeonly_status" == "true" ]] && s10="${cyan}盲走快车道已通车${none}" || s10="${gray}默认全量嗅探${none}"
        local s11; [[ "$buffer_state" == "true" ]] && s11="${cyan}巨型重卡池 (64K)${none}" || s11="${gray}默认轻型分配${none}"
        
        local s12; [[ "$dnsmasq_state" == "true" ]] && s12="${cyan}极速内存解析中 (0.1ms)${none}" || s12="${gray}依赖原生 DoH${none}"
        local s13; [[ "$thp_state" == "true" ]] && s13="${cyan}已关闭 THP${none}" elif [[ "$thp_state" == "unsupported" ]] && s13="${gray}不支持${none}" else s13="${gray}系统默认${none}"
        local s14; [[ "$mtu_state" == "true" ]] && s14="${cyan}智能探测中${none}" elif [[ "$mtu_state" == "unsupported" ]] && s14="${gray}不支持${none}" else s14="${gray}未开启${none}"
        local s15; [[ "$cpu_state" == "true" ]] && s15="${cyan}全核火力全开${none}" elif [[ "$cpu_state" == "unsupported" ]] && s15="${gray}不支持${none}" else s15="${gray}节能待机${none}"
        local s16; [[ "$ring_state" == "true" ]] && s16="${cyan}已反向收缩${none}" elif [[ "$ring_state" == "unsupported" ]] && s16="${gray}不支持${none}" else s16="${gray}系统大缓冲${none}"
        local s17; [[ "$zram_state" == "true" ]] && s17="${cyan}已挂载 ZRAM${none}" elif [[ "$zram_state" == "unsupported" ]] && s17="${gray}不支持${none}" else s17="${gray}未启用${none}"
        local s18; [[ "$journal_state" == "true" ]] && s18="${cyan}纯内存极速化${none}" elif [[ "$journal_state" == "unsupported" ]] && s18="${gray}不支持${none}" else s18="${gray}磁盘 IO 写入中${none}"
        local s19; [[ "$prio_state" == "true" ]] && s19="${cyan}OOM免死 / IO抢占${none}" else s19="${gray}系统默认调度${none}"
        local s20; [[ "$cake_state" == "true" ]] && s20="${cyan}CAKE 削峰填谷中${none}" else s20="${gray}默认 FQ 队列${none}"
        local s21; [[ "$irq_state" == "true" ]] && s21="${cyan}已锁死 Core 0${none}" elif [[ "$irq_state" == "unsupported" ]] && s21="${gray}不支持(单核)${none}" else s21="${gray}默认平衡调度${none}"
        ...
        local s23; [[ "$ackfilter_state" == "true" ]] && s23="${cyan}绞杀空 ACK 释放上行${none}" else s23="${gray}默认不干预${none}"
        local s24; [[ "$ecn_state" == "true" ]] && s24="${cyan}显式拥塞警告 (0 丢包平滑降速)${none}" else s24="${gray}默认 (暴力丢包)${none}"
        local s25; [[ "$wash_state" == "true" ]] && s25="${cyan}强力清除干扰乱码${none}" else s25="${gray}默认不干预${none}"
        ...
        case "$app_opt" in
            ...
            27)
                if ((sys_off_count > 0)); then
                    ...
                    [[ "$cake_state" == "false" ]] && toggle_cake
                    [[ "$irq_state" == "false" ]] && toggle_irq
                    [[ "$gso_off_state" == "false" ]] && toggle_gso_off
                    [[ "$ackfilter_state" == "false" ]] && toggle_ackfilter
                    [[ "$ecn_state" == "false" ]] && toggle_ecn
                    [[ "$wash_state" == "false" ]] && toggle_wash
                    info "12-25 系统级满血激活！"
                else
                    ...
                    [[ "$cake_state" == "true" ]] && toggle_cake
                    [[ "$irq_state" == "true" ]] && toggle_irq
                    [[ "$gso_off_state" == "true" ]] && toggle_gso_off
                    [[ "$ackfilter_state" == "true" ]] && toggle_ackfilter
                    [[ "$ecn_state" == "true" ]] && toggle_ecn
                    [[ "$wash_state" == "true" ]] && toggle_wash
                    info "12-25 系统级已卸载！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray ex177 The Apex Vanguard - Project Genesis V177 (极客纯净版)${none}"
...
# ==============================================================================
# EOF: 极客工业级底层标志，本行如果存在即代表 V177 核心引擎全系防爆版输出圆满完成！
# ==============================================================================
