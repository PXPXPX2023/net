#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex52.sh (The Apex Vanguard - Project Genesis V18)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 深度重构实时 IP 探针 (选项9 -> 4)：启用全域嗅探 ss -ntupa，破除闲置状态下的雷达盲区。
#   2. 强化探针正则清洗引擎：无视 TCP/UDP 混合位移，精准剥离回环地址与监听态冗余 IP。
#   3. 深度勘误：修复 /tmp 目录 noexec 挂载导致的 CPU 探针失效问题。
...
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex52.sh"
    exit 1
fi
    ...
    while true; do
        ...
        case "$s" in
            ...
            4)
                ...
                # xargs 去除幽灵空格，彻底阻断正则管道异常
                local x_pids=$(pidof xray | xargs | tr ' ' '|')
                if [ -n "$x_pids" ]; then
                    echo -e "  ${cyan}【底层进程与连接池状态分布】${none}"
                    # 开启全域嗅探 (-ntupa)，提取出完整的协议状态 (ESTAB/LISTEN/UNCONN)
                    ss -ntupa | grep -E "pid=($x_pids)(,|$)" | awk '{print $1"_"$2}' | sort | uniq -c | sort -nr | awk '{printf "    %-15s : %s\n", $2, $1}'
                    
                    echo -e "\n  ${cyan}【连接来源独立 IP 排行 (TOP 10)】${none}"
                    # 精准切片第 6 列 Peer IP，忽略 LISTEN 待机态与本地 IPv4/IPv6 回环数据
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex52 The Apex Vanguard - Project Genesis V18${none}"
