#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e77.sh (The Apex Vanguard - Project Genesis V198e77)
# 快捷方式: xrv
# 【V198e77 三协议建仓版：Reality Vision / VLESSENC Vision / SS2022】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e77.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e77"
...
hot_update_from_legacy() {
    ...
    if ! verify_e66_post_state; then
        ...
        warn "可以再次执行：bash ex198e77.sh --hot-update，或进入 10→5 手动应用。"
        return 1
    fi
    ensure_xray_is_alive || true
...
hot_update_from_legacy() {
    ...
    if ! verify_e68_post_state; then
        ...
        warn "可再次执行：bash ex198e77.sh --hot-update，或进入 10→6→28 / 10→5 手动应用。"
        return 1
    fi
...
hot_update_from_legacy() {
    ...
    if ! e69_verify_post_state; then
        ...
        warn "可再次执行：bash ex198e77.sh --hot-update；如 Xray 未运行，执行 journalctl -u xray --no-pager -n 80。"
        return 1
    fi
...
e77_install_self_entrypoint() {
    ...
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e77.sh" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
        chmod +x "$SYMLINK" "$SCRIPT_DIR/ex198e77.sh" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
        hash -r 2>/dev/null || true
    fi
    echo "198e77" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
}
...
do_install() {
    title "V198e77：三协议完整建仓（Reality Vision / VLESSENC Vision / SS2022）"
    preflight
    e77_install_self_entrypoint
...
e77_audit_runtime_state() {
    title "V198e77 状态隔离审计：不会读取失败旧脚本"
    echo "  当前执行脚本: $SCRIPT_PATH"
    echo "  xrv 入口文件:  $SYMLINK"
    echo "  当前版本文件:  $(cat "$SCRIPT_VERSION_FILE" 2>/dev/null || echo unknown)"
    echo "  协议能力:"
    ...
    echo ""
    echo "  说明：V198e77 不会 source 或读取 /root/ex198e63.sh、/root/ex198e73.sh、/root/ex198e75.sh 等失败脚本。"
    echo "        热更新只处理固定运行态路径：$CONFIG_DIR、$SCRIPT_DIR、$SYMLINK。"
    echo ""
    if e76_config_semantic_ok "$CONFIG" >/dev/null 2>&1; then
        info "当前 config.json 通过 JSON / Xray / inbound 语义检查，可作为现有节点保留。"
...
hot_update_from_legacy() {
    title "V198e77：稳态热更新（三协议建仓能力 + 旧状态隔离）"
    set +e
    trap - ERR
    ...
    if e76_config_semantic_ok "$CONFIG"; then
        ...
        warn "未发现有效现网节点配置。V198e77 已安装为 xrv；请执行 xrv → 1 重新搭建。"
    fi

    echo "198e77" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    set -e
    trap '_err_handler $? $LINENO "$BASH_COMMAND"' ERR
    info "V198e77 热更新完成。现在可输入：xrv"
    return 0
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e77 - (Reality / VLESSENC / SS2022)${none}"
        local svc sys_ver arch_level dns_mode tz_line
        svc=$(systemctl is-active xray 2>/dev/null || echo "inactive")
        if test "$svc" = "active"; then svc="${green}平稳在线 (Running)${none}"; else svc="${red}离线静默 (Stopped)${none}"; fi
        sys_ver=$(uname -r 2>/dev/null || echo "未探测到数据")
        ...
        echo "  9) 全景网络连接审计日志与商用级别网络流量记账本"
        echo "  10) 高级发烧：内核双轨编译(APT/源码)/网络/应用层 60余项极限全控板"
        echo -e "  ${cyan}11) 从旧 ex198e 环境稳态热更新到 V198e77${none}"
        echo -e "  ${cyan}12) 清理空间（安全释放 / 快照收敛 / 显示硬盘与 Swap）${none}"
        echo -e "  ${cyan}13) 应用网络参数（均衡/高吞吐/手动，显示永久配置）${none}"
        echo -e "  ${cyan}14) TCP 重传 / 单连接诊断中心（只读排障）${none}"
        echo -e "  ${cyan}15) 检查现有配置并重新输出节点${none}"
...
#==============================================================================
# EOF - Apex Vanguard V198e77 Three Protocol Builder Ready.
#==============================================================================
