#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e24.sh (The Apex Vanguard - Project Genesis V196e24)
# 快捷方式: xrv
#
# 【V196e24 真理终结版：完美告别失联与卡死】
#   1. 修复内核交互死锁: 引入 make olddefconfig 自动跳过新内核功能提问，解决卡死。
#   2. 驱动基因继承: 拷贝宿主机 /boot/config，保留所有显卡(DRM)与网卡驱动，告别无头黑屏。
#   3. 维持防爆降维打击: 锁定 arch/x86 目录，sed 抹除 Makefile march 断层机制。
#   4. 开放极客自选版本: 支持任意手工指定 Git Tag (例如 6.18.25-rt-xanmod1)。
# ==============================================================================

if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex196e24.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="196e24"
...
do_kernel_compile_menu() {
    while true; do
        clear
        title "极客内核源码锻造中心 (V196e24 真理告别卡死版)"
        echo "  针对之前重启黑屏 (无显卡驱动) 和隐形死锁的问题，"
        echo "  现已引入宿主机驱动基因完美继承 (cp .config + olddefconfig)，彻底防失联！"
        echo ""
...
_compile_kernel_mainline() {
    ...
    # 【核心修复】：继承宿主机当前驱动，不再使用瞎子 defconfig，且修复交互卡死死锁
    if test -f "/boot/config-$(uname -r)"; then
        info "正在继承当前宿主机驱动配置以保证完美兼容..."
        cp "/boot/config-$(uname -r)" .config
        # ★ 致命修复：必须立刻执行 olddefconfig！否则后面的 make 会陷入隐形交互模式等待回车！
...
_compile_kernel_xanmod() {
    ...
    if test -f "/boot/config-$(uname -r)"; then
        info "正在继承当前宿主机驱动配置以保证完美兼容..."
        cp "/boot/config-$(uname -r)" .config
        # ★ 致命修复：必须立刻执行 olddefconfig！跳过新特性的 Y/n 提问，避免隐形卡死
        make olddefconfig >/dev/null 2>&1 || true
    else
        warn "未找到宿主机驱动配置文件，启用回退 defconfig 模式..."
        make defconfig >/dev/null 2>&1 || true
    ...
    # 【终极后门防线】: 彻底斩断 Makefile 里引发 error 1 的隐患参数！
    # 直接抹除动态架构检测逻辑，强行将残废的 -march=x86-64-v$(XXX) 全局替换为最通用的 -march=x86-64
    info ">> 启动上帝模式：物理抹除 arch/x86 目录下的动态架构拼写断层..."
    find arch/x86/ -type f -name "Makefile" -exec sed -i 's/-march=x86-64-v[^ \t]*/-march=x86-64/g' {} + 2>/dev/null || true

    info ">> 正在清洗配置项，移除危险的微架构开关..."
    ./scripts/config --disable CONFIG_DEBUG_INFO
    ./scripts/config --disable CONFIG_DEBUG_INFO_BTF
    ./scripts/config --disable CONFIG_MODULE_SIG
    ./scripts/config --disable CONFIG_SYSTEM_TRUSTED_KEYS
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e24 - (The Apex Vanguard)${none}"
...
# ==============================================================================
# EOF - Apex Vanguard V196e24 System Advanced Control Ready.
# ==============================================================================
# --- ✂️ Part 12 结束，所有代码拼接完毕。请运行 bash ex196e24.sh ---
