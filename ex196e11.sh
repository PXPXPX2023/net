#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e11.sh (The Apex Vanguard - Project Genesis V196e11)
# 快捷方式: xrv
#
# 【V196e11 终极勘误与真理重塑】
#   1. 彻底斩断宿主机 config 污染：抛弃复用旧内核配置，强制使用 7.x 源码原生的 make defconfig 生成底座，根治汇编层错乱。
#   2. JQ 绝对绝缘: 全域覆盖 select(. != null) 护盾，杜绝任何数组/对象为空时的语法崩溃。
#   3. 真·源码溯源: 直连 GitLab 官方仓库拉取正宗 7.x Xanmod 源码，精准剥离冗余组件。
...
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex196e11.sh"
    exit 1
fi
...
readonly SCRIPT_VERSION="196e11"
...
# ------------------------------------------------------------------------------
# 【V196e11 核心修复 1】：预编译官方 XANMOD 部署模块 (修复多重源与 404)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 【V196e11 核心修复 2】：真·Xanmod 源码极客编译引擎 (抛弃人工篡改架构)
# ------------------------------------------------------------------------------
...
do_xanmod_compile() {
    ...
    # 【核心防御闭环】：在此处彻底抛弃宿主机的 config，绝不复用，防止旧宏污染！
    # 强制让 Xanmod 的 Kconfig 跑原生的 defconfig，让它自己补全所有的架构宏和 L1_CACHE_SHIFT。
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e11 - (The Apex Vanguard)${none}"
...
main_menu
# ==============================================================================
# EOF - Apex Vanguard V196e11 System Advanced Control Ready.
# ==============================================================================
