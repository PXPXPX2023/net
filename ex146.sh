#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex146.sh (The Apex Vanguard - Project Genesis V146 [Absolute Horizon])
# 快捷方式: xrv
# 终极修复宣言: 
#   1. 1GB Swap 信仰：实装纯正 1GB 永久 Swap 管理器 (自动校验、卸载溢出、写死 fstab)。
#   2. 内核编译防爆：彻底修复 Xanmod Makefile 中的 -march=x86-64-v 空指针 Bug，精准注入 CONFIG_X86_64_VERSION。
...
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex146.sh"
    exit 1
fi
...
do_install_xanmod_main_official() {
    ...
    if [ -z "$cpu_level" ]; then cpu_level=1; fi
...
# V146 绝杀修复: 彻底解决 Xanmod 编译 -march 空指针 Bug
do_xanmod_compile() {
    ...
    # 核心绝杀：强制赋予 Xanmod 专属的 CONFIG_X86_64_VERSION 值，防止 cc1 收到空的 -march 参数！
    ./scripts/config --set-val CONFIG_X86_64_VERSION ${cpu_level}
    ...
    # 二次物理断点保护，确保 VERSION 变量没有被 olddefconfig 洗掉
    if ! grep -q "CONFIG_X86_64_VERSION=${cpu_level}" .config; then
        echo "CONFIG_X86_64_VERSION=${cpu_level}" >> .config
    fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex146 The Apex Vanguard - Project Genesis V146${none}"
