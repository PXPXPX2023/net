#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e58.sh (The Apex Vanguard - Project Genesis V198e58)
# 快捷方式: xrv
# 【V198e58 热更新自执行版：自动执行空间清理 + 默认网络参数应用】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e58.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e58"
...
# V198e58: 主菜单 12 / 内核实验区前置安全空间清理 + 快照收敛。
...
# V198e58: 主菜单 13 /etc/sysctl.d/99-network-optimized.conf 网络缓冲参数管理。
...
# V198e58: 清理旧热更新遗留的 rmem/wmem 冲突定义。
# 原因：/etc/sysctl.d/99-xray-hotupdate-fast.conf 排序晚于 99-network-optimized.conf，
...
cleanup_network_sysctl_conflicts_e58() {
    title "清理旧版 rmem/wmem Sysctl 冲突源（V198e58）"
    local f found=0
    # 默认只清理脚本已知会覆盖 13 菜单的旧文件，不动用户自定义 sysctl.conf。
    for f in "$LEGACY_HOTUPDATE_SYSCTL_CONF" "/etc/sysctl.d/99-sysctl.conf"; do
        test -f "$f" || continue
        if grep -Eq '^[[:space:]]*(net\.core\.(rmem|wmem)_(default|max)|net\.ipv4\.tcp_[rw]mem)[[:space:]]*=' "$f" 2>/dev/null; then
            found=1
            warn "发现旧 rmem/wmem 定义：$f，已备份并移除这些键，避免覆盖 $NETWORK_OPT_CONF。"
...
# V198e58 manual network buffer profile
...
manual_network_buffer_params_e56() {
    title "手动输入网络缓冲参数"
    warn "输入为空则使用当前永久文件值；若当前未写入，则使用 V198e58 默认值。"
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e58 空间清理强化版)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e58 - (The Apex Vanguard)${none}"
        ...
        echo -e "  ${cyan}11) 从 ex198e32/ex198e33/ex198e34/ex198e35/ex198e36/ex198e37/ex198e38/ex198e39/ex198e40/ex198e41/ex198e42/ex198e43/ex198e54/ex198e55/ex198e56/ex198e57 稳态热更新到 V198e58${none}"
...
hot_update_from_legacy() {
    title "V198e58 稳态热更新：自动执行 12 清理空间 + 13 默认网络参数"
    ...
    local ts bdir
    ts=$(date +%Y%m%d_%H%M%S); bdir="$BACKUP_DIR/hotupdate_e57_${ts}"
    ...
    # V198e58：热更新时自动执行主菜单 12 的安全空间清理。
    ...
    # V198e58：热更新时自动执行主菜单 13 的默认网络参数写入与 sysctl 应用。
    ...
    fix_permissions
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    if ! verify_e58_post_state; then error "V198e58 关键状态校验失败，已保留快照：$bdir"; return 1; fi
    ensure_xray_is_alive
    info "V198e58 热更新完成：已自动执行 12 清理空间、13 默认网络参数，并保留第 9 项 IP/单连接审计中心。"
}
...
do_status_menu() {
    while true; do
        clear
        title "主控台: 监控、账单及 IP 审计核心（V198e58）"
        echo "  1) [系统守护] 追踪主线程服务健康率"
        echo "  2) [全网穿透] 检测基础外网及监听位态"
        echo "  3) [总计账单] 调取网卡出入核算账册 (vnstat)"
...
# V198e58: 热更新后额外写入版本号；不默认启用任何 IP 限制规则。
...
verify_e58_post_state() {
    ...
    if grep -Eq '^[[:space:]]*(net\.core\.(rmem|wmem)_(default|max)|net\.ipv4\.tcp_[rw]mem)[[:space:]]*=' "$LEGACY_HOTUPDATE_SYSCTL_CONF" 2>/dev/null; then
        error "旧冲突文件仍包含 rmem/wmem 定义：$LEGACY_HOTUPDATE_SYSCTL_CONF"; fail=1
    fi
    local rv
...
#==============================================================================
# EOF - Apex Vanguard V198e58 System Advanced Control Ready.
#==============================================================================
