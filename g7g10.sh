#!/usr/bin/env bash
# ============================================================
# 脚本名称: g7g10.sh (Ultimate Bulletproof Edition)
# 核心增量与终极修复: 
#   1. 修复双节点安装时，端口冲突逻辑死结的问题。
#   2. 重构 UUID 生成逻辑，优先使用 xray 内置引擎，解决精简容器无 /proc 报错。
#   3. 新增防自爆机制：禁止删除 VLESS 下的最后一个用户，防止 JSON 结构违规。
#   4. 修复纯净环境直接执行 Geo 更新时的目录写入报错。
#   5. 修复卸载时 systemd 多实例 (@.service) 残留问题。
# ============================================================
...
LOG_FILE="/var/log/xray_g7g10_install.log"
...
do_install() {
    ...
    if [[ "$choice" == "2" || "$choice" == "3" ]]; then
        ...
        if [[ "$choice" == "3" && "$sp" == "$p" ]]; then
            if [[ "$p" == "8388" ]]; then sp=8389; else sp=8388; fi
            log_warn "检测到 SS 端口与 VLESS 冲突，SS 已自动分配为 $sp"
        fi
    ...
    while true; do
        ...
        case "$uopt" in
            1) 
               ...
               ;;
            2)
               # 修复 Bug 3: 防自爆机制，禁止删除最后一个用户
               local count=$(jq ".inbounds[$vidx].settings.clients | length" "$CONFIG")
               if [[ "$count" -le 1 ]]; then
                   print_yellow "警告: 这是节点下的最后一个 UUID！"
                   print_red "禁止删除！VLESS 协议要求至少存在一个客户端，否则 Xray 服务将崩溃重启失败。"
                   print_yellow "请先选择 1) 新增一个用户，然后再来删除旧的。"
                   continue
               fi
...
main_menu() {
    ...
    while true; do
        clear
        echo -e "\033[94m===================================================\033[0m"
        echo -e "     \033[96mXray G7G10 究极防弹版 (输入 xrv 唤醒面板)\033[0m"
        echo -e "\033[94m===================================================\033[0m"
        ...
        case "$opt" in
            ...
            6) do_upgrade_core ;;
            7) 
               # 修复 Bug 4: 提前创建规则库文件夹，防止 curl 报错
               mkdir -p "$DAT_DIR"
            ...
            8) 
               if [[ "$SERVICE_MANAGER" == "systemctl" ]]; then 
                   ...
                   # 修复 Bug 5: 彻底粉碎所有以 xray 开头的 service 文件（含多实例）
