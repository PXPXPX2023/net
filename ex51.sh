#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex51.sh (The Apex Vanguard - Project Genesis V17)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 深度勘误：修复 /tmp 目录 noexec 挂载导致的 CPU 探针失效问题 (改用 bash 解析执行)。
#   2. 深度勘误：剥离已被现代系统淘汰的 gnupg1 依赖，确保 Debian 12 完美部署预编译内核。
#   3. 深度勘误：强化“实时 IP 统计雷达”的 PID 正则捕获机制，杜绝僵尸进程引发的管道报错。
#   4. 完整继承 88 彻底卸载时的“初次安装计费日”内存级无损保留。
#   5. 完整继承【极低延迟/小包秒开】架构：Xray 并发 4096，TX 队列 2000，Busy Poll 50。
#   6. 完整继承 NAPI 软中断配额：dev_weight=64，bias=1，杜绝算力独占。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex51.sh"
    exit 1
fi
    ...
    while true; do
        ...
        case "$s" in
            ...
            4)
                ...
                # 使用 tr -s 严谨清洗，防止多余空格带来尾随管道符
                local x_pids=$(pidof xray | tr -s ' ' '|')
                if [ -n "$x_pids" ]; then
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex51 The Apex Vanguard - Project Genesis V17${none}"
