#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e20.sh (The Apex Vanguard - Project Genesis V198e20)
# 快捷方式: xrv
# 【V198e20 终极纯血版：全量UI找回 + e8完美内核 + 上帝视角拥塞 + MSS/reusePort】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e20.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e20"
...
check_and_create_swap() {
    ...
    info "使用绝对安全的 dd 模式写入 1130MB Swap (防 fallocate 导致的开机挂载崩溃)..."
    if ! dd if=/dev/zero of="$SWAP_FILE" bs=1M count=${swap_size} status=none 2>/dev/null; then
        warn "dd 写入异常，放弃配置 Swap。"
        return
    fi
    ...
    if mkswap "$SWAP_FILE" >/dev/null 2>&1; then
        ...
        echo "$SWAP_FILE none swap sw,nofail 0 0" >> /etc/fstab 2>/dev/null || true
        info "1130MB Swap 防爆缓冲池组建并固化完成。"
    else
        warn "mkswap 失败，跳过 Swap 配置。"
    fi
}
...
do_change_dns() {
    ...
    if test -f /etc/resolv.conf.bak; then cp /etc/resolv.conf.bak /etc/resolv.conf 2>/dev/null || true; fi
    ...
    info "DNS 已被软锁定为：$nameserver (避免死锁)"
}
...
net.core.rmem_default = 960576
net.core.wmem_default = 960576
net.core.rmem_max = 21699928
net.core.wmem_max = 21699928
net.ipv4.tcp_rmem = 16380 960576 21699928
net.ipv4.tcp_wmem = 16380 960576 21699928
...
toggle_dnsmasq() {
    if [ "$(check_dnsmasq_state)" = "true" ]; then
        systemctl stop dnsmasq >/dev/null 2>&1 || true; systemctl disable dnsmasq >/dev/null 2>&1 || true; rm -f /etc/resolv.conf 2>/dev/null || true; if [ -f /etc/resolv.conf.bak ]; then mv /etc/resolv.conf.bak /etc/resolv.conf; else echo "nameserver 8.8.8.8" > /etc/resolv.conf; fi; systemctl enable systemd-resolved >/dev/null 2>&1 || true; systemctl start systemd-resolved >/dev/null 2>&1 || true; _safe_jq_write 'select(.dns != null) | .dns = {"servers":["https://8.8.8.8/dns-query","https://1.1.1.1/dns-query","https://doh.opendns.com/dns-query"], "queryStrategy":"UseIP"}'
    else
...
no-poll
EOF
        systemctl enable dnsmasq >/dev/null 2>&1 || true; systemctl restart dnsmasq >/dev/null 2>&1 || true; if [ ! -f /etc/resolv.conf.bak ]; then cp /etc/resolv.conf /etc/resolv.conf.bak 2>/dev/null || true; fi; rm -f /etc/resolv.conf 2>/dev/null || true; echo "nameserver 127.0.0.1" > /etc/resolv.conf; _safe_jq_write 'select(.dns != null) | .dns = {"servers":["127.0.0.1"], "queryStrategy":"UseIP"}'
    fi
}
...
do_app_level_tuning_menu() {
    while true; do
        ...
        echo -e "  8) Policy 策略组优化 (连接生命周期极极速回收)          | 状态: $s8"
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e20 终极版)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e20 - (The Apex Vanguard)${none}"
...
#==============================================================================
# EOF - Apex Vanguard V198e20 System Advanced Control Ready.
#==============================================================================
