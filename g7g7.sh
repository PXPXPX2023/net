#!/usr/bin/env bash
# ============================================================
# 脚本名称: g7g7.sh (Grandmaster Industrial Fusion Edition)
# 论坛支持: https://xp.day
# 核心增量与融合亮点: 
#   1. 工业级底层：融合教科书级的包管理器防锁死、多源高可用 IP 获取、依赖下载重试机制。
#   2. 交互式面板：保留大一统管理面板（装/卸/更新/探针/规则/升降级）。
#   3. 极致安全：剔除所有 "1024" 字眼与写死的 "88" ShortId，全面随机化 + 600 严格权限。
#   4. XTLS 巅峰：强制 Vision 流控 + uTLS 伪装 + Vision Seed (Padding) 多用户级动态下发。
# ============================================================
...
LOG_FILE="/var/log/xray_g7g7_install.log"
...
get_server_ip_silent() {
    ...
    exit_with_error "无法获取服务器 IP，请检查网络出口"
}
...
do_install() {
    ...
    if [[ "$choice" == "1" || "$choice" == "3" ]]; then
        ...
        [[ -z "$p" || ! $(validate_port "$p") ]] && p=443
        
        read -r -t 30 -p "目标伪装域名(Dest) [回车默认 www.amazon.com]: " d
        [[ -z "$d" || ! $(validate_domain "$d") ]] && d="www.amazon.com"
...
do_upgrade_core() {
    ...
    [[ -z "$versions" ]] && { log_err "获取列表失败，请检查网络"; read -rp "按 Enter 返回..."; return; }
...
do_summary() {
    ...
    if [[ "$sidx" != "null" && -n "$sidx" ]]; then
        ...
        print_green " ss://${b64}@${SERVER_IP}:${sport}#xp-ss"
        echo ""
    fi
}
...
main_menu() {
    ...
    while true; do
        clear
        echo -e "\033[94m===================================================\033[0m"
        echo -e "     \033[96mXray G7G7 工业大成版 (任意位置输入 xrv 唤醒)\033[0m"
        echo -e "\033[94m===================================================\033[0m"
        ...
        case "$opt" in
            ...
            8) 
               if [[ "$SERVICE_MANAGER" == "systemctl" ]]; then systemctl stop xray; systemctl disable xray; else service xray stop; fi
