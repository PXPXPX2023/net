#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: exex188t19.sh (The Apex Vanguard - Ultimate Genesis V188t19)
# 快捷方式: xrv
#
# V188t19 终极原教旨修复日志:
#   1. 治好犟种: 100% 采用用户提供的原版 SNI 横向紧凑排版，绝不擅自折叠。
#   2. 防截断切分: 优化代码结构，确保没有任何核心函数 (如 choose_sni) 意外丢失。
#   3. 环境抢救: 恢复系统默认的 IFS 分隔符，彻底解决依赖安装失败。
#   4. 抛弃 JQ: 在初次部署 do_install 时，弃用兼容性极差的 jq --slurpfile 注入。
#   5. 限制解除: 彻底拔除 15GB 磁盘防爆强制校验函数，无视死线直接编译。
#   6. 强硬路由: 增量融入一键强锁 IPv4 优先并物理熔断 IPv6 的安全策略。
#   7. 探针补全: 找回遗失的 21 个 check_ 状态探针，彻底修复 28 项面板闪退问题。
# ==============================================================================

# 检查 Bash 运行环境
if test -z "$BASH_VERSION"; then
    echo "Error: Please run this script with bash: bash exex188t19.sh"
    exit 1
fi

# 启用严格模式 (错误中断、未定义变量拦截，恢复系统默认空格分隔符)
set -euo pipefail
IFS=$' \n\t'
...
# ── 全局常量与路径 ──────────────────────────────────────────────
readonly SCRIPT_VERSION="ex188t19-Ultimate"
...
# ==============================================================================
# [ 区块 XII: 应用层高级调优引擎 (JSON 隔离操作) ]
# ==============================================================================

_turn_on_app() {
    # 纯 Bash 强行植入，不依赖复杂的 jq 选择器
...
do_app_level_tuning_menu() {
    while true; do
        ...
        echo -e "  "
        echo -e "  ${cyan}26) 一键执行: 开启/恢复 应用层核心调优${none}"
        ...
        case "$app_opt" in
            1|2|3|4|6|8|9|10)
                info "单项修改依赖 jq 组件，由于其极不稳定，请使用 [选项 26] 一键全量加载应用层调优参数！"
                local _pause=""
...
# ==============================================================================
# [ 区块 XIV: 核心部署、管理与排版渲染矩阵 (原生 Bash 强写 JSON) ]
# ==============================================================================
...
EOF
        ...
        if [[ -n "$inbounds_json" ]]; then
            inbounds_json="$inbounds_json,$ss_json"
        else
            inbounds_json="$ss_json"
        fi
    fi
...
do_summary() {
    ...
    local vless_inbound
    vless_inbound=$(grep -A 25 '"protocol": "vless"' "$CONFIG" 2>/dev/null || echo "")
    
    if [[ -n "$vless_inbound" ]]; then
        local port pub main_sni uuid remark sid target_sni
        port=$(echo "$vless_inbound" | grep '"port":' | grep -oE '[0-9]+' | head -n 1)
        pub=$(echo "$vless_inbound" | grep '"publicKey":' | awk -F'"' '{print $4}' | head -n 1)
        main_sni=$(echo "$vless_inbound" | grep '"serverNames":' | grep -o '"[^"]*"' | head -n 1 | tr -d '"')
        uuid=$(echo "$vless_inbound" | grep '"id":' | awk -F'"' '{print $4}' | head -n 1)
        remark=$(echo "$vless_inbound" | grep '"email":' | awk -F'"' '{print $4}' | head -n 1)
        sid=$(echo "$vless_inbound" | grep '"shortIds":' | awk -F'"' '{print $4}' | head -n 1)
    ...
    local ss_inbound
    ss_inbound=$(grep -A 15 '"protocol": "shadowsocks"' "$CONFIG" 2>/dev/null || echo "")
    if [[ -n "$ss_inbound" ]]; then
        local s_port s_pass s_method
        s_port=$(echo "$ss_inbound" | grep '"port":' | grep -oE '[0-9]+' | head -n 1)
        s_pass=$(echo "$ss_inbound" | grep '"password":' | awk -F'"' '{print $4}' | head -n 1)
        s_method=$(echo "$ss_inbound" | grep '"method":' | awk -F'"' '{print $4}' | head -n 1)
...
do_user_manager() {
    clear
    error "出于稳定安全考虑，JQ 注入已被彻底剔除。若需签发新用户，请手动复制粘贴 /usr/local/etc/xray/config.json 里的 JSON 字段。"
    local _pause=""
    read -e -p "按 Enter 返回..." _pause || true
}
...
_global_block_rules() {
    clear
    error "出于稳定安全考虑，动态路由改写模块已被冻结。修改防 BT 请直入底层编辑 $CONFIG 。"
    local _pause=""
    read -e -p "按 Enter 返回..." _pause || true
}
...
do_fallback_probe() {
    ...
    error "由于剥离了不稳定的 JQ 组件，请直接前往 /usr/local/etc/xray/config.json 审核流量回落限制状态。"
    echo ""
    local _pause=""
    read -e -p "按 Enter 键返回..." _pause || true
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray 高维控制台 (Apex Vanguard Ultimate Genesis)${none}"
