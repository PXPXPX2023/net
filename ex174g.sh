...
#    ╚═╝   ╚═╝  ╚═╝╚══════╝    ╚═╝  ╚═╝╚═╝     ╚══════╝╚═╝  ╚═╝
# ==============================================================================
# 脚本名称: ex174g.sh (The Apex Vanguard - Project Genesis V174g [The Infinity])
# 快捷方式: xrv
#
# V174g 全量融合重铸大一统宣言:
#   1. 协议大一统: 完美整合 VLESS-Reality / Shadowsocks / NaiveProxy(Caddy) / Hysteria2。
#   2. 容错大一统: 全局 ERR 探针 + Xray原生 -test 质检 + 10 份历史备份轮转回滚。
#   3. 极客绝不妥协: 严禁压缩！全面恢复 130+ SNI 单行展开与 60+ 项 Sysctl 满血阵列。
#   4. 编译防砖复活: 找回 kernel.org 原核裸装引擎，严密继承宿主驱动，确保 0 宕机。
#   5. 状态持久化: 保留 flags 物理锚点与 network-online.target 时序绝杀。
#   6. 语法免疫: 完美修复 jq "$@" 传参，.sniffing = (.sniffing // {}) 原子级护航。
# ==============================================================================

# 强制 Bash 运行环境检测
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行: bash ex174g.sh"; exit 1
fi

# 严格模式 (开启错误中断与未定义变量拦截，同时使用 || true 处理合法非零返回)
set -euo pipefail
IFS=$'\n\t'
...
# 【核心修复】完美支持 $@ 参数透传的 JSON 原子化写入系统
_safe_jq_write() {
    backup_config
    local tmp; tmp=$(mktemp) || return 1
    
    # 将用户传进来的全量参数 (包含 --argjson) 投入 jq 管道
...
run_sni_scanner() {
    ...
    if [[ -s "$tmp_sni" ]]; then
        ...
        if ((count < 20)); then
            grep " CN_CDN$" "$tmp_sni" | sort -n | head -n $((20 - count)) | awk '{print $2, $1}' >> "$SNI_CACHE_FILE"
        fi
    else
...
do_xanmod_compile() {
    ...
    export DEBIAN_FRONTEND=noninteractive
    apt-get update -y >/dev/null 2>&1 || true
    apt-get install -y build-essential bc bison flex libssl-dev libelf-dev libncurses-dev zstd git wget curl xz-utils ethtool numactl make pkg-config dwarves rsync python3 libdw-dev cpio >/dev/null 2>&1
    ...
    local KERNEL_URL
    KERNEL_URL=$(curl -s https://www.kernel.org/releases.json | jq -r '.releases[] | select(.type=="stable") | .tarball' | head -1)
    ...
    # 否则重启后无法挂载系统盘，引发惨绝人寰的 Kernel Panic (VFS: Unable to mount root fs)！
    ...
    NEW_KERNEL_VER=$(make -s kernelrelease)
...
EOF
    ...
    # 【时序绝杀修复】强制使用 network-online.target，防止网卡未获取 IP 时命令抛空报错！
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray ex174g The Apex Vanguard - Project Genesis V174g (终极持久闭环版)${none}"
...
# ==============================================================================
# EOF: 极客工业级底层标志，本行如果存在即代表 V174g 核心引擎全系防爆版输出圆满完成！
# ==============================================================================
