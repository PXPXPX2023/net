#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex61.sh (The Apex Vanguard - Project Genesis V61)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 新增极客特权：系统初始化菜单 (10) 中嵌入子菜单 (7) 【4项应用层极限微操】。
#   2. 底牌全开：出站双向提速、嗅探减负 (metadataOnly)、内置 DoH 并发引擎一键注入。
#   3. 阶梯飙车：实装 GOGC 动态内存阶梯容差算法 (支持 100~1000 动态阀门调优)。
...
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex61.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex61 The Apex Vanguard - Project Genesis V61${none}"
