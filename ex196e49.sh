#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e49.sh (The Apex Vanguard - Project Genesis V196e49)
# 快捷方式: xrv
#
# 【V196e49 终极不朽版：彻底解决编译暴毙、OOM、防变砖断联】
#   1. 防变砖核心: 强制注入 Virtio/KVM 虚拟化网卡及硬盘驱动，重启 100% 秒连。
#   2. 编译不暴毙: 废除所有侵入式 Makefile Hack，补全 gcc-multilib/lz4 底层依赖。
#   3. OOM 绝对免疫: 固化 1280MB Swap，无论配置多高，链接阶段强制锁死单核编译。
#   4. 7.x 动态天花板: 引入 sort -V -r，完美抓取并置顶最新分支。
# ==============================================================================

if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex196e49.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="196e49"
...
preflight() {
    ...
    # 重点：新增 lz4, liblz4-tool, gcc-multilib, libc6-dev-i386 防止编译组件缺失
    ...
    if test -n "$missing"; then
        info "正在同步工业级依赖 (含32位/64位全量编译套件): $missing"
        pkg_install $missing
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e49 - (The Apex Vanguard)${none}"
...
# ==============================================================================
# EOF - Apex Vanguard V196e49 System Advanced Control Ready.
# ==============================================================================
# --- ✂️ Part 15 结束，所有代码拼接完毕。请运行 bash ex196e49.sh ---
