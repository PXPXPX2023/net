#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex189.sh (The Apex Vanguard - Project Genesis V189)
# 快捷方式: xrv
#
# 【V189 终极大一统修补增量版】
#   1. 作用域修复: 全量寻回 17 个 toggle_* 函数，修复 28 项全域微操断层。
#   2. 排版重铸: 100% 完美复现 8 行对齐的节点链接分发界面。
#   3. 绝缘护盾: JQ 全量采用 |= 与 select(. != null) 语法，_safe_jq_write 引入 "$@" 传参。
#   4. 反代穿墙: Geo 数据与 Xray 核心更新强制挂载 ghp.ci 等免翻镜像矩阵。
#   5. 原生展开: 彻底废弃 [ ] && 短路语法，全量多行 if-then-else 规避 set -e 故障。
#   6. 探针修复: 注入自带缓存的 _get_ip，彻底解决 command not found 及菜单卡顿。
# ==============================================================================
...
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 严重错误！本脚本深度依赖 Bash 高级特性，请执行: bash ex189.sh"
    exit 1
fi
...
_get_ip() {
    ...
    if test -z "${GLOBAL_IP:-}"; then
        local temp_ip=""
        # 脱离极严格模式执行外部网络请求，防止由于超时被 pipefail 直接切断脚本
        set +e
...
verify_xray_config() {
    ...
    local test_result
    test_result=$("$XRAY_BIN" run -test -config "$target_config" 2>&1 || true)
...
# ------------------------------------------------------------------------------
# [ 0x07: 绝对核心：百万并发 Limits 守护进程与系统提权 ]
# ------------------------------------------------------------------------------
...
fix_xray_systemd_limits() {
    ...
    # 临时脱离严格的 pipefail，防止 grep 失败导致提权链中断
    set +o pipefail
...
EOF
    ...
    if ! systemctl daemon-reload >/dev/null 2>&1; then
        ...
        info "Systemd 提权指令下发完毕，百万并发防线已重新闭环。"
    fi
}

# ------------------------------------------------------------------------------
# [ 0x08: 物理 1GB Swap 信仰卫士 ]
# ------------------------------------------------------------------------------
...
check_and_create_1gb_swap() {
    ...
    if test -n "$CURRENT_SWAP"; then
        if test "$CURRENT_SWAP" -ge 1000000 2>/dev/null; then
            info "系统已配置足量的 Swap 分区 (≥1GB)，内存缓冲池完美通过基线校验。"
            return 0
        fi
    fi
    
    warn "未检测到足量 Swap，为了防止编译与运行期 OOM (内存溢出) 崩盘，正在强制重置并分配 1GB 永久 Swap 分区..."
    ...
    if dd if=/dev/zero of="$SWAP_FILE" bs=1M count=1024 status=none 2>/dev/null; then
        ...
        info "1GB 永久 Swap 已重建并钉入系统 fstab 骨架。"
    else
        error "分配 Swap 失败，物理硬盘可能存在故障或空间已满！"
    fi
}
...
preflight() {
    ...
    # 唤醒 0x04.5 区块中加载的 _get_ip 探针
    local tmp_ip
    tmp_ip=$(_get_ip || echo "获取失败")
    
    if test "$tmp_ip" = "获取失败" || test "$tmp_ip" = "外网探针离线"; then
        warn "探针发散失败，机器的公网 IPv4 寻址暂时被阻断或处于遮蔽状态！"
        SERVER_IP="获取失败"
    else
        SERVER_IP="$tmp_ip"
...
UPDSH
    ...
    local temp_cron
    temp_cron=$(mktemp /tmp/current_cron_XXXXXX)
    ...
    info "Geo 路由流防断更机制已部署: 每日 03:00 穿墙同步规则，03:10 触发引擎平滑重载。"
}
# ------------------------------------------------------------------------------
# [ 0x0B: 物理 DNS 解析锁死引擎 (基于 resolvconf) ]
# ------------------------------------------------------------------------------
...
do_change_dns() {
    ...
    if test -f /etc/redhat-release; then
        ...
        if cat /etc/issue 2>/dev/null | grep -Eqi "debian"; then
            release="debian"
        else
            if cat /etc/issue 2>/dev/null | grep -Eqi "ubuntu"; then
                release="ubuntu"
            else
                if cat /etc/issue 2>/dev/null | grep -Eqi "centos|red hat|redhat"; then
                    release="centos"
                else
                    if cat /proc/version 2>/dev/null | grep -Eqi "debian"; then
                        release="debian"
                    else
                        if cat /proc/version 2>/dev/null | grep -Eqi "ubuntu"; then
                            release="ubuntu"
                        else
                            if cat /proc/version 2>/dev/null | grep -Eqi "centos|red hat|redhat"; then
                                release="centos"
                            fi
                        fi
                    fi
    ...
    if test ! -e '/usr/sbin/resolvconf'; then
        if test ! -e '/sbin/resolvconf'; then
            info "系统底层缺少 resolvconf 守护进程，准备强行安装..."
    ...
    while test "$IPcheck" = "0"; do
        read -rp "长官，请给出需要死锁的新 Nameserver 独立 IP (例如抗污染的 8.8.8.8 或 1.1.1.1): " nameserver || true
...
verify_sni_strict() {
    ...
    # 临时脱离严格模式，防止 openssl 超时抛出异常错误码直接切断整个脚本
    set +e
    local out
    ...
    # 逐级审查协议特征，拒绝一切 && 压行，清爽展现逻辑
    if ! echo "$out" | grep -qi "TLSv1.3"; then 
        warn "质检拦截: 目标服务器未开启 TLSv1.3 协议，Reality 握手将会完全暴露特征！"
        pass=0
    fi
    
    if ! echo "$out" | grep -qiE "ALPN, server accepted to use h2|ALPN.*h2"; then 
        warn "质检拦截: 目标服务器不支持 ALPN h2 协商，不适合作为高并发伪装目标！"
        pass=0
    fi
    
    if ! echo "$out" | grep -qi "OCSP response:"; then 
        warn "质检拦截: 目标服务器未配置 OCSP Stapling 证书状态装订，极易被防火墙主动探测阻断！"
        pass=0
    fi
    
    if test "$pass" -eq 0; then
        error "结论：该目标指纹残缺，易遭墙探！"
        return 1
    else
    ...
    while true; do
        if test -f "$SNI_CACHE_FILE"; then
            ...
            if verify_sni_strict "$BEST_SNI"; then
                info "质检完毕：主控目标 $BEST_SNI 完美通过特征审查！"
                break
            else
                warn "危险预警：该域名协议支持不达标，若强行部署会导致 Reality 防线极易被墙或断流！请尽量重新选择。"
...
validate_port() {
    ...
    local check
    check=$(echo "$p" | tr -d '0-9' || true)
    
    if test -n "$check"; then 
        return 1
    fi
    
    if test "${p:-0}" -ge 1 2>/dev/null; then
        if test "${p:-0}" -le 65535 2>/dev/null; then
...
gen_x25519() {
    ...
    if test -z "$X25519_PRIV"; then
        die "X25519 椭圆曲线私钥生成遭遇致命断层！"
    fi
    
    if test -z "$X25519_PUB"; then
        die "X25519 椭圆曲线公钥生成遭遇致命断层！"
    fi
}
...
EOF
    ...
    # 管道流防爆处理：临时关闭 pipefail 并采用绝对安全测试
    set +o pipefail
...
if test "${RX_QUEUES:-0}" -gt 0 2>/dev/null; then
    FLOW_PER_QUEUE=$((65535 / RX_QUEUES))
    for RX in /sys/class/net/"$IFACE"/queues/rx-*; do
        if test -w "$RX/rps_flow_cnt"; then
            echo "$FLOW_PER_QUEUE" > "$RX/rps_flow_cnt" 2>/dev/null || true
...
# ------------------------------------------------------------------------------
# [ 0x14: 动态控制引擎 (Toggle Switches 全域展示版) ]
# ------------------------------------------------------------------------------
...
toggle_gso_off() {
    ...
    if test "$(check_gso_off_state)" = "unsupported"; then
        warn "硬件卸载已被底层虚拟机强制锁死 (fixed)。为防止断网，已安全跳过 GSO/GRO 干预！"
        sleep 2
        return
    fi
...
# ------------------------------------------------------------------------------
# [ 0x15: 终极绝缘体：应用层全量激活 / 剥离中心 (杜绝 JSON 清空) ]
# ------------------------------------------------------------------------------
...
EOF
                ...
                local temp_cron
                temp_cron=$(mktemp /tmp/cron_XXXXXX)
                crontab -l 2>/dev/null | grep -v "cc1.sh" > "$temp_cron" || true
                echo "0 4 */10 * * /usr/local/bin/cc1.sh >/dev/null 2>&1" >> "$temp_cron"
                crontab "$temp_cron" 2>/dev/null || true
                rm -f "$temp_cron" 2>/dev/null || true
...
do_install() {
    ...
    print_magenta ">>> 正在全域连接云端，多轨自旋拉取最新版 Xray 核心引擎..."
    ...
    for proxy_prefix in "https://ghfast.top/" "https://ghproxy.net/" "https://mirror.ghproxy.com/" "https://ghp.ci/" ""; do
        ...
        if curl -kfsSL --connect-timeout 10 --max-time 60 -o /tmp/install-release.sh "$inst_url" 2>/dev/null; then
            ...
            if bash /tmp/install-release.sh @ install >/dev/null 2>&1; then
                if test -x "$XRAY_BIN"; then
                    xray_installed=1
                    info "Xray 核心底层部署成功！桥接源：${proxy_prefix:-直连}"
                    break
                fi
            fi
            unset PROXY
        fi
        warn "链路源 ${proxy_prefix:-直连} 遭墙或断流，自旋切换下一穿墙节点..."
    done
    ...
    if test "$xray_installed" -eq 0; then
        die "核心拉取全线溃败！所有 CDN 穿墙镜像均遭到严重网络阻断，核心未能安装！"
    fi
            ...
            3) 
                ...
                case "${vn_opt:-}" in
                    1) 
                        ...
                        if test "${d_day:-0}" -ge 1 2>/dev/null; then
                            if test "${d_day:-0}" -le 31 2>/dev/null; then
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray ex189 The Apex Vanguard - Project Genesis (终极大一统修补版)${none}"
...
main_menu
# ==============================================================================
# EOF: 代码末尾标记，本行存在即代表 V189 真核大一统版全量下发，未遭 Token 截断
# ==============================================================================
