#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex90.sh (The Apex Vanguard - Project Genesis V90)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 致命勘误：修复由于过度精简导致的 vnstat 计费日修改功能缺失，完整复活交互式正则修改引擎。
#   2. 时序排爆：重构状态机，引入快照级变量锁，彻底治愈“上帝之手”因状态读取冲突导致的死锁罢工。
...
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex90.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex90 The Apex Vanguard - Project Genesis V90${none}"
