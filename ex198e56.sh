#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e56.sh (The Apex Vanguard - Project Genesis V198e56)
# 快捷方式: xrv
# 【V198e56 空间清理+网络参数强化版：快照收敛 + rmem/wmem手动管理】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e56.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e56"
...
# V198e56: 主菜单 12 / 内核实验区前置安全空间清理 + 快照收敛。
...
# V198e56: 主菜单 13 /etc/sysctl.d/99-network-optimized.conf 网络缓冲参数管理。
...
# V198e56 manual network buffer profile
...
manual_network_buffer_params_e56() {
    title "手动输入网络缓冲参数"
    warn "输入为空则使用当前永久文件值；若当前未写入，则使用 V198e56 默认值。"
...
do_network_params_menu_e56() {
    while true; do
        ...
        show_network_buffer_params_e56
        echo ""
        echo "  1) 应用 V198e56 默认 rmem/wmem 参数"
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e56 空间清理强化版)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e56 - (The Apex Vanguard)${none}"
        ...
        echo -e "  ${cyan}11) 从 ex198e32/ex198e33/ex198e34/ex198e35/ex198e36/ex198e37/ex198e38/ex198e39/ex198e40/ex198e41/ex198e42/ex198e43/ex198e54/ex198e55 稳态热更新到 V198e56${none}"
...
hot_update_from_legacy() {
    title "V198e54 稳态热更新：安全极速默认 + IP/单连接审计中心"
    ...
    local ts bdir
    ts=$(date +%Y%m%d_%H%M%S); bdir="$BACKUP_DIR/hotupdate_e54_${ts}"
    ...
    fix_permissions
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    if ! verify_e55_post_state; then error "V198e54 关键状态校验失败，已保留快照：$bdir"; return 1; fi
    ensure_xray_is_alive
    info "V198e54 热更新完成：稳态参数已落地，IP/单连接审计中心已接入第 9 项菜单。"
}
...
do_status_menu() {
    while true; do
        clear
        title "主控台: 监控、账单及 IP 审计核心（V198e56）"
        echo "  1) [系统守护] 追踪主线程服务健康率"
        echo "  2) [全网穿透] 检测基础外网及监听位态"
        echo "  3) [总计账单] 调取网卡出入核算账册 (vnstat)"
...
# V198e56: 热更新后额外写入版本号；不默认启用任何 IP 限制规则。
...
#==============================================================================
# EOF - Apex Vanguard V198e56 System Advanced Control Ready.
#==============================================================================
