#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e10.sh (The Apex Vanguard - Project Genesis V196e10)
# 快捷方式: xrv
#
# 【V196e10 终极勘误与真理重塑】
#   1. 根绝编译血案: 敬畏内核，放弃手工篡改 CPU 架构宏，全权交由 defconfig 推导，根治 -march 暴毙！
#   2. JQ 绝对绝缘: 全域覆盖 select(. != null) 护盾，杜绝任何数组/对象为空时的语法崩溃。
#   3. 真·源码溯源: 直连 GitLab 官方仓库拉取正宗 7.x Xanmod 源码，剔除臃肿的 DEBUG 信息。
...
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex196e10.sh"
    exit 1
fi
...
readonly SCRIPT_VERSION="196e10"
...
# ------------------------------------------------------------------------------
# 【V196e10 核心修复 1】：预编译官方 XANMOD 部署模块 (修复多重源与 404)
# ------------------------------------------------------------------------------
...
do_install_xanmod_main_official() {
    ...
    set +e
    # 物理清除历史遗留的重复源配置，绝杀 "configured multiple times" 报错
    ...
    # 修复 404 Bug：调用官方正确的 archive.key 接口
...
# ------------------------------------------------------------------------------
# 【V196e10 核心修复 2】：真·Xanmod 源码极客编译引擎 (抛弃人工篡改架构)
# ------------------------------------------------------------------------------

do_xanmod_compile() {
    title "系统飞升：编译安装 真·Xanmod 源码 + BBR3 (极客锻造版)"
    warn "警告: 此过程将极度消耗 CPU 与内存 (约 30-60 分钟)，低配机极易死机！强烈建议优先使用菜单 3 的官方预编译版。"
    
    local confirm=""
    read -rp "确定要执意开始极客源码编译吗？(y/n): " confirm || true
    ...
    # 【终极防爆闭环】：提取宿主机 config 以保全所有驱动，避免 defconfig 丢失组件
    # 绝对不再使用 scripts/config 篡改任何与 CPU 微架构 (GENERIC_CPU) 有关的宏！
    # 将推导权全部交还给 Kconfig，让其自行补全 L1_CACHE_SHIFT。
    ...
    # 注入网络优化项
    info "正在向 .config 中注入原生 BBR3 拥塞控制组件..."
    ...
    if test -n "$COMPILED_VER"; then
        info "真·Xanmod 内核 ($COMPILED_VER) 已注入宿主机核心。"
    fi
...
EOF_RPS_SRV
    ...
    info "真·Xanmod 极客内核编译与优化已全部就绪！系统将在 30 秒后强制重启..."
    sleep 30
    reboot
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e10 - (The Apex Vanguard)${none}"
...
main_menu
# ==============================================================================
# EOF - Apex Vanguard V196e10 System Advanced Control Ready.
# ==============================================================================

# --- ✂️ Part 7 结束，脚本完全拼接完毕 ✂️ ---
