#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex150.sh (The Apex Vanguard - Project Genesis V150 [Absolute Horizon])
# 快捷方式: xrv
# 终极修复宣言 (绝对防截断全量版): 
#   1. 绝杀编译死锁：彻底移除对 Xanmod 架构变量的鲁莽干预，完全信任原生 defconfig，彻底解决 -march 空指针报错。
#   2. 1GB 永久 Swap：彻底废弃临时 Swap，强制引入规范的 1GB 永久 Swap 管理器，保护物理洁癖。
#   3. 硬件绝对保护：探测到网卡底层限制，永久锁死 GSO/GRO 强制修改逻辑，绝不执行危险指令。
#   4. 物理装甲回归：完美复刻 V62 的 100万并发锁 (limits.conf)、60+ 行极限 Sysctl 参数与 resolvconf 底层锁死。
#   5. 核心大一统：保留 28 项全域微操矩阵、回落探针 (菜单 8)、8 行军规排版、ShortId 防漂移联动删除。
# ============================================================

if test -z "$BASH_VERSION"; then echo "错误: 请用 bash 运行此脚本"; exit 1; fi

red='\033[31m'; yellow='\033[33m'; gray='\033[90m'; green='\033[92m'; blue='\033[94m'; magenta='\033[95m'; cyan='\033[96m'; none='\033[0m'
XRAY_BIN="/usr/local/bin/xray"; CONFIG_DIR="/usr/local/etc/xray"; CONFIG="$CONFIG_DIR/config.json"; PUBKEY_FILE="$CONFIG_DIR/public.key"; SNI_CACHE_FILE="$CONFIG_DIR/sni_cache.txt"; INSTALL_DATE_FILE="$CONFIG_DIR/install_date.txt"; USER_SNI_MAP="$CONFIG_DIR/user_sni.txt"; USER_TIME_MAP="$CONFIG_DIR/user_time.txt"; CAKE_OPTS_FILE="$CONFIG_DIR/cake_opts.txt"; FLAGS_DIR="$CONFIG_DIR/flags"; DAT_DIR="/usr/local/share/xray"; SCRIPT_DIR="/usr/local/etc/xray-script"; UPDATE_DAT_SCRIPT="$SCRIPT_DIR/update-dat.sh"; SYMLINK="/usr/local/bin/xrv"; SCRIPT_PATH=$(readlink -f "$0")
GLOBAL_IP=""; SERVER_IP=""; REMARK_NAME="xp-reality"; BEST_SNI="www.microsoft.com"; SNI_JSON_ARRAY="\"www.microsoft.com\""; LISTEN_PORT=443

mkdir -p "$CONFIG_DIR" "$DAT_DIR" "$SCRIPT_DIR" "$FLAGS_DIR" 2>/dev/null; touch "$USER_SNI_MAP" "$USER_TIME_MAP"

print_red() { echo -e "${red}$*${none}"; }; print_green() { echo -e "${green}$*${none}"; }; print_yellow() { echo -e "${yellow}$*${none}"; }; print_magenta() { echo -e "${magenta}$*${none}"; }; print_cyan() { echo -e "${cyan}$*${none}"; }
info() { echo -e "${green}✓${none} $*"; }; warn() { echo -e "${yellow}!${none} $*"; }; error() { echo -e "${red}✗${none} $*"; }; die() { echo -e "\n${red}致命错误${none} $*\n"; exit 1; }
title() { echo -e "\n${blue}===================================================${none}"; echo -e "  ${cyan}$*${none}"; echo -e "${blue}===================================================${none}"; }
hr() { echo -e "${gray}---------------------------------------------------${none}"; }

_get_ip() { if [ -z "$GLOBAL_IP" ]; then GLOBAL_IP=$( (curl -s -4 --connect-timeout 5 https://api.ipify.org || curl -s -4 --connect-timeout 5 https://ifconfig.me || echo "获取失败") | tr -d '\r\n' ); fi; echo "$GLOBAL_IP"; }

fix_permissions() {
    if test -f "$CONFIG"; then chmod 644 "$CONFIG" >/dev/null 2>&1; fi
    if test -d "$CONFIG_DIR"; then chmod 755 "$CONFIG_DIR" >/dev/null 2>&1; fi
    chown root:root "$CONFIG" >/dev/null 2>&1 || true; chown -R root:root "$CONFIG_DIR" >/dev/null 2>&1 || true
}

_safe_jq_write() {
    local filter="$1"; local tmp=$(mktemp)
    if jq "$filter" "$CONFIG" > "$tmp" 2>/dev/null; then mv "$tmp" "$CONFIG" >/dev/null 2>&1; fix_permissions; return 0; fi
    rm -f "$tmp" >/dev/null 2>&1; return 1
}

fix_xray_systemd_limits() {
    local override_dir="/etc/systemd/system/xray.service.d"; mkdir -p "$override_dir" 2>/dev/null; local limit_file="$override_dir/limits.conf"
    local current_nice="-20"; local current_gogc="100"; local current_oom="true"; local current_affinity=""; local current_gomaxprocs=""; local current_buffer=""
    if [ -f "$limit_file" ]; then
        if grep -q "^Nice=" "$limit_file"; then current_nice=$(awk -F'=' '/^Nice=/ {print $2}' "$limit_file" | head -1); fi
        if grep -q "^Environment=\"GOGC=" "$limit_file"; then current_gogc=$(awk -F'=' '/^Environment="GOGC=/ {print $3}' "$limit_file" | tr -d '"' | head -1); fi
        if ! grep -q "^OOMScoreAdjust=" "$limit_file"; then current_oom="false"; fi
        if grep -q "^CPUAffinity=" "$limit_file"; then current_affinity=$(awk -F'=' '/^CPUAffinity=/ {print $2}' "$limit_file" | head -1); fi
        if grep -q "^Environment=\"GOMAXPROCS=" "$limit_file"; then current_gomaxprocs=$(awk -F'=' '/^Environment="GOMAXPROCS=/ {print $3}' "$limit_file" | tr -d '"' | head -1); fi
        if grep -q "^Environment=\"XRAY_RAY_BUFFER_SIZE=" "$limit_file"; then current_buffer=$(awk -F'=' '/^Environment="XRAY_RAY_BUFFER_SIZE=/ {print $3}' "$limit_file" | tr -d '"' | head -1); fi
    fi
    local TOTAL_MEM=$(free -m | awk '/Mem/{print $2}'); local GO_MEM_LIMIT=$(( TOTAL_MEM * 85 / 100 ))
    cat > "$limit_file" << EOF
[Service]
LimitNOFILE=1048576
LimitNPROC=512000
...
EOF
    if [ "$current_oom" = "true" ]; then echo "OOMScoreAdjust=-500" >> "$limit_file"; echo "IOSchedulingClass=realtime" >> "$limit_file"; echo "IOSchedulingPriority=2" >> "$limit_file"; fi
...
check_and_create_1gb_swap() {
    ...
    local SWAP_FILE="/swapfile"; local CURRENT_SWAP=$(swapon --show=NAME,KBYTES --noheadings 2>/dev/null | grep "$SWAP_FILE" | awk '{print $2}')
    if [[ -n "$CURRENT_SWAP" ]] && [[ "$CURRENT_SWAP" =~ ^1048 ]]; then info "系统已存在规范的 1GB 永久 Swap，基线校验通过。"
    else
        warn "检测到 Swap 缺失或容量不符，正在重置并强制分配 1GB 永久 Swap 空间..."
        swapoff -a 2>/dev/null || true; sed -i '/swapfile/d' /etc/fstab; rm -f "$SWAP_FILE" 2>/dev/null
        dd if=/dev/zero of="$SWAP_FILE" bs=1M count=1024 status=none; chmod 600 "$SWAP_FILE"; mkswap "$SWAP_FILE" >/dev/null 2>&1; swapon "$SWAP_FILE" >/dev/null 2>&1
        echo "$SWAP_FILE none swap sw 0 0" >> /etc/fstab; info "1GB 永久 Swap 已重建并写入系统 fstab 底盘。"
    fi
}
...
preflight() {
    ...
    local install_list=""
    for i in $need; do if ! command -v "$i" >/dev/null 2>&1; then install_list="$install_list $i"; fi; done
    if test -n "$install_list"; then
        info "同步工业级依赖: $install_list"
        export DEBIAN_FRONTEND=noninteractive; (apt-get update -y || yum makecache -y) >/dev/null 2>&1; (apt-get install -y $install_list || yum install -y $install_list) >/dev/null 2>&1
        systemctl start vnstat >/dev/null 2>&1 || true; systemctl start cron >/dev/null 2>&1 || systemctl start crond >/dev/null 2>&1 || true
    fi
    if test -f "$SCRIPT_PATH"; then \cp -f "$SCRIPT_PATH" "$SYMLINK" >/dev/null 2>&1; chmod +x "$SYMLINK" >/dev/null 2>&1; hash -r 2>/dev/null; fi
    SERVER_IP=$(_get_ip)
}
...
UPDSH
    ...
    (crontab -l 2>/dev/null | grep -v "$UPDATE_DAT_SCRIPT" | grep -v "/bin/systemctl restart xray"; echo "0 3 * * * $UPDATE_DAT_SCRIPT >/dev/null 2>&1"; echo "10 3 * * * /bin/systemctl restart xray >/dev/null 2>&1") | crontab -
    info "已配置自动热更: 每天凌晨 3:00 更新 Geo 库"
}

do_change_dns() {
    title "修改系统 DNS 解析 (底层锁定防漂移)"
    local release=""; if [ -f /etc/redhat-release ]; then release="centos"; elif cat /etc/issue | grep -Eqi "debian"; then release="debian"; elif cat /etc/issue | grep -Eqi "ubuntu"; then release="ubuntu"; elif cat /etc/issue | grep -Eqi "centos|red hat|redhat"; then release="centos"; elif cat /proc/version | grep -Eqi "debian"; then release="debian"; elif cat /proc/version | grep -Eqi "ubuntu"; then release="ubuntu"; elif cat /proc/version | grep -Eqi "centos|red hat|redhat"; then release="centos"; fi
    if [ ! -e '/usr/sbin/resolvconf' ] && [ ! -e '/sbin/resolvconf' ]; then
        print_yellow "正在安装必要的环境依赖 resolvconf..."
        if [ "${release}" == "centos" ]; then yum -y install resolvconf > /dev/null 2>&1; else export DEBIAN_FRONTEND=noninteractive; apt-get update > /dev/null 2>&1; apt-get -y install resolvconf > /dev/null 2>&1; fi
    fi
    systemctl stop systemd-resolved 2>/dev/null || true; systemctl disable systemd-resolved 2>/dev/null || true; systemctl stop resolvconf.service > /dev/null 2>&1 || true; systemctl start resolvconf.service > /dev/null 2>&1 || true; systemctl enable resolvconf.service > /dev/null 2>&1 || true
    local nameserver=""; local IPcheck="0"
    while [ "$IPcheck" == "0" ]; do
        read -rp "请输入要强行锁定的新 Nameserver IP (例如 8.8.8.8): " nameserver
        if [[ $nameserver =~ ^([0-9]{1,2}|1[0-9][0-9]|2[0-4][0-9]|25[0-5])\.([0-9]{1,2}|1[0-9][0-9]|2[0-4][0-9]|25[0-5])\.([0-9]{1,2}|1[0-9][0-9]|2[0-4][0-9]|25[0-5])\.([0-9]{1,2}|1[0-9][0-9]|2[0-4][0-9]|25[0-5])$ ]]; then IPcheck="1"; else print_red "IPv4 格式错误！"; fi
    done
    chattr -i /etc/resolv.conf 2>/dev/null || true; rm -f /etc/resolv.conf 2>/dev/null || true; mv /etc/resolv.conf /etc/resolv.conf.bak 2>/dev/null || true; echo "nameserver $nameserver" > /etc/resolv.conf; chattr +i /etc/resolv.conf 2>/dev/null || true
    mkdir -p /etc/resolvconf/resolv.conf.d 2>/dev/null; echo "nameserver $nameserver" > /etc/resolvconf/resolv.conf.d/head 2>/dev/null; systemctl restart resolvconf.service >/dev/null 2>&1
...
run_sni_scanner() {
    ...
    local sni_list=("www.apple.com" "support.apple.com" "developer.apple.com" "id.apple.com" "icloud.apple.com" "www.microsoft.com" "login.microsoftonline.com" "portal.azure.com" "support.microsoft.com" "office.com" "www.intel.com" "downloadcenter.intel.com" "ark.intel.com" "www.amd.com" "drivers.amd.com" "www.dell.com" "support.dell.com" "www.hp.com" "support.hp.com" "developers.hp.com" "www.bmw.com" "www.mercedes-benz.com" "global.toyota" "www.honda.com" "www.volkswagen.com" "www.nike.com" "www.adidas.com" "www.zara.com" "www.ikea.com" "www.shell.com" "www.bp.com" "www.ge.com" "www.hsbc.com" "www.morganstanley.com" "www.msc.com" "www.sony.com" "www.canon.com" "www.nintendo.com" "www.unilever.com" "www.loreal.com" "www.hermes.com" "www.louisvuitton.com" "www.dior.com" "www.gucci.com" "www.coca-cola.com" "www.tesla.com" "s0.awsstatic.com" "www.nvidia.com" "www.samsung.com" "www.oracle.com" "addons.mozilla.org" "www.airbnb.com.sg" "mit.edu" "stanford.edu" "www.lufthansa.com" "www.singaporeair.com" "www.specialized.com" "www.logitech.com" "www.razer.com" "www.corsair.com")
    local sni_string=$(printf "%s\n" "${sni_list[@]}"); if command -v shuf >/dev/null 2>&1; then sni_string=$(echo "$sni_string" | shuf); else sni_string=$(echo "$sni_string" | awk 'BEGIN{srand()} {print rand()"\t"$0}' | sort -n | cut -f2-); fi
    local tmp_sni=$(mktemp /tmp/sni_test.XXXXXX)
    for sni in $sni_string; do
        read -t 0.1 -n 1 key; if test $? -eq 0; then echo -e "\n${yellow}已手动中止...${none}"; break; fi
        ...
        if test "${ms:-0}" -gt 0; then
            ...
            local status_cn=""; local p_type="NORM"
            if test -z "$dns_cn" || test "$dns_cn" = "127.0.0.1" || test "$dns_cn" = "0.0.0.0" || test "$dns_cn" = "::1"; then status_cn="${red}阻断${none}"; p_type="BLOCK"
            else
                local loc=$(curl -s --connect-timeout 2 "https://ipinfo.io/$dns_cn/country" 2>/dev/null | tr -d ' \n')
                if test "$loc" = "CN"; then status_cn="${green}直通${none} (CDN)"; p_type="CN_CDN"; else status_cn="${green}直通${none} (原生)"; p_type="NORM"; fi
            fi
    ...
    if test -s "$tmp_sni"; then
        grep "NORM" "$tmp_sni" | sort -n | head -n 20 | awk '{print $2, $1}' > "$SNI_CACHE_FILE"; local count=$(wc -l < "$SNI_CACHE_FILE" 2>/dev/null)
        if test "${count:-0}" -lt 20; then grep "CN_CDN" "$tmp_sni" | sort -n | head -n $((20 - ${count:-0})) | awk '{print $2, $1}' >> "$SNI_CACHE_FILE"; fi
    else print_red "探测全灭，采用微软保底。"; echo "www.microsoft.com 999" > "$SNI_CACHE_FILE"; fi
    rm -f "$tmp_sni"
}
...
verify_sni_strict() {
    ...
    local pass=1
    if ! echo "$out" | grep -qi "TLSv1.3"; then pass=0; fi; if ! echo "$out" | grep -qi "ALPN, server accepted to use h2"; then pass=0; fi; if ! echo "$out" | grep -qi "OCSP response:"; then pass=0; fi
    if [ "$pass" -eq 0 ]; then print_red " ✗ 质检未通过"; else print_green " ✓ 质检通过"; fi
    return $pass
}
...
choose_sni() {
    while true; do
        if test -f "$SNI_CACHE_FILE"; then
            echo -e "\n  ${cyan}【战备缓存：极速 Top 20】${none}"; local idx=1
            while read -r s t; do echo -e "  $idx) $s (${cyan}${t}ms${none})"; idx=$((idx + 1)); done < "$SNI_CACHE_FILE"
            echo -e "  ${yellow}r) 重新扫描矩阵${none}"; echo "  m) 矩阵模式 (多选防封)"; echo "  0) 手动输入域名"; read -rp "  请选择: " sel; sel=${sel:-1}
            if test "$sel" = "q"; then return 1; fi; if test "$sel" = "r"; then run_sni_scanner; continue; fi
            if test "$sel" = "m"; then
                read -rp "输入序号 (空格分隔): " m_sel; local arr=()
                for i in $m_sel; do local picked=$(awk "NR==$i {print \$1}" "$SNI_CACHE_FILE" 2>/dev/null); if test -n "$picked"; then arr+=("$picked"); fi; done
                if test ${#arr[@]} -eq 0; then error "无效"; continue; fi
                BEST_SNI="${arr[0]}"; local jq_args=(); for s in "${arr[@]}"; do jq_args+=("\"$s\""); done; SNI_JSON_ARRAY=$(IFS=,; echo "${jq_args[*]}")
            elif test "$sel" = "0"; then read -rp "自定义域名: " d; BEST_SNI=${d:-www.microsoft.com}; SNI_JSON_ARRAY="\"$BEST_SNI\""
            else
                local picked=$(awk "NR==$sel {print \$1}" "$SNI_CACHE_FILE" 2>/dev/null)
                if test -n "$picked"; then BEST_SNI="$picked"; else BEST_SNI=$(awk "NR==1 {print \$1}" "$SNI_CACHE_FILE" 2>/dev/null); fi
                SNI_JSON_ARRAY="\"$BEST_SNI\""
            fi
            if verify_sni_strict "$BEST_SNI"; then break; else continue; fi
        else run_sni_scanner; fi
    done
    return 0
}

validate_port() {
    local p="$1"; if test -z "$p"; then return 1; fi; local check=$(echo "$p" | tr -d '0-9'); if test -n "$check"; then return 1; fi
    if test "${p:-0}" -ge 1 2>/dev/null && test "${p:-0}" -le 65535 2>/dev/null; then
        if ss -tuln | grep -q ":${p} "; then print_red "端口占用！"; return 1; fi
        return 0
    fi
    return 1
}
...
do_update_core() {
    ...
    fix_xray_systemd_limits; systemctl restart xray >/dev/null 2>&1
    local cur_ver=$($XRAY_BIN version 2>/dev/null | head -n1 | awk '{print $2}'); info "已更新至: ${cyan}$cur_ver${none}"; read -rp "按 Enter 继续..." _
}

gen_ss_pass() { head -c 24 /dev/urandom | base64 | tr -d '=/+\n' | head -c 24; }

_select_ss_method() {
    echo -e "  ${cyan}选择 SS 加密方式：${none}" >&2; echo "  1) aes-256-gcm  2) aes-128-gcm  3) chacha20-ietf-poly1305" >&2; read -rp "  编号: " mc >&2
...
do_install_xanmod_main_official() {
    ...
    if [ "$(uname -m)" != "x86_64" ]; then error "仅支持 x86_64"; return; fi
    local cpu_level_script="/tmp/check_x86-64_psabi.sh"; wget -qO "$cpu_level_script" https://dl.xanmod.org/check_x86-64_psabi.sh; local cpu_level=$(bash "$cpu_level_script" | grep -oE 'x86-64-v[1-4]' | grep -oE '[1-4]' | tail -n 1); rm -f "$cpu_level_script"; if [ -z "$cpu_level" ]; then cpu_level=1; fi
    local pkg_name="linux-xanmod-x64v${cpu_level}"
    export DEBIAN_FRONTEND=noninteractive; apt-get update -y >/dev/null 2>&1; apt-get install -y gnupg gnupg2 curl sudo wget e2fsprogs >/dev/null 2>&1
    ...
    info "XANMOD 部署就绪！10秒后重启..."; sleep 10; reboot
}

do_xanmod_compile() {
    title "源码编译安装 Xanmod 内核 + TCP BBRv3"
    warn "警告: 源码编译耗时 30分钟 ~ 2小时，此操作具有较高风险！"
    read -rp "确定要继续吗？(y/n): " confirm; if [[ "$confirm" != "y" ]]; then return; fi
    
    print_magenta ">>> [1/4] 安装底层编译核心依赖包..."
    ...
    print_magenta ">>> [2/4] 获取 Xanmod 源码 (内置 BBR3 补丁)..."
    mkdir -p /usr/src; cd /usr/src; rm -rf linux-xanmod; git clone --depth=1 https://gitlab.com/xanmod/linux.git linux-xanmod; cd linux-xanmod

    print_magenta ">>> [3/4] 初始化原生基底并执行防爆破注入..."
    make defconfig
    ...
    print_magenta ">>> [4/4] 全核全速编译中，请耐心等待 (切勿中断)..."
    local THREADS=$(nproc); local TOTAL_MEM=$(free -m | awk '/^Mem:/{print $2}')
    if [ "$TOTAL_MEM" -lt 2000 ] && [ "$THREADS" -gt 2 ]; then THREADS=2; fi
    if ! make -j${THREADS} bindeb-pkg; then error "内核编译遭遇致命错误，流程已熔断！请上翻查看 make 报错信息。"; read -rp "按 Enter 返回主菜单..." _; return 1; fi

    cd /usr/src; dpkg -i linux-image-*xanmod*.deb linux-headers-*xanmod*.deb 2>/dev/null || true; update-grub 2>/dev/null || true
    info "源码编译已封顶！底层驱动架构已焕然一新。"; info "请务必重启服务器以应用崭新的 BBR3 协议栈！"; read -rp "按 Enter 返回主菜单..." _
}

do_perf_tuning() {
    title "极限压榨：低延迟系统底层网络栈调优"
    warn "警告: 注入全域极限参数后将重启！"; read -rp "确定要继续吗？(y/n): " confirm; if [[ "$confirm" != "y" ]]; then return; fi
    
    local current_scale=$(sysctl -n net.ipv4.tcp_adv_win_scale 2>/dev/null || echo "1"); local current_app=$(sysctl -n net.ipv4.tcp_app_win 2>/dev/null || echo "31")
    echo -e "  当前 tcp_adv_win_scale: ${cyan}${current_scale}${none} (建议 1)"
    echo -e "  当前 tcp_app_win: ${cyan}${current_app}${none} (建议 31)"
    read -rp "请输入 tcp_adv_win_scale (-2 到 2，回车默认): " new_scale; new_scale=${new_scale:-$current_scale}
    read -rp "请输入 tcp_app_win (1 到 31，回车默认): " new_app; new_app=${new_app:-$current_app}

    systemctl stop net-speeder >/dev/null 2>&1 || true; killall net-speeder >/dev/null 2>&1 || true; systemctl disable net-speeder >/dev/null 2>&1 || true; rm -f /etc/systemd/system/net-speeder.service; systemctl daemon-reload >/dev/null 2>&1; rm -rf /root/net-speeder
    truncate -s 0 /etc/sysctl.conf 2>/dev/null || true; truncate -s 0 /etc/sysctl.d/99-sysctl.conf 2>/dev/null || true; truncate -s 0 /etc/sysctl.d/99-network-optimized.conf 2>/dev/null || true
...
EOF
    ...
    local target_qdisc="fq"; if [ "$(check_cake_state)" = "true" ]; then target_qdisc="cake"; fi
...
EOF
    ...
    info "极致低延迟参数注入完成！系统将在 30 秒后重启..."; sleep 30; reboot
}

do_txqueuelen_opt() {
    title "TX Queue 调优"
    local IP_CMD=$(command -v ip); local IFACE=$(ip route get 1.1.1.1 | awk '{print $5; exit}')
    if [ -z "$IFACE" ]; then error "无法识别网卡。"; return 1; fi
...
WantedBy=multi-user.target
EOF
    systemctl daemon-reload; systemctl enable txqueue >/dev/null 2>&1; systemctl start txqueue
    info "已应用 txqueuelen 2000"; read -rp "Enter 继续..." _
}

config_cake_advanced() {
    clear; title "CAKE 高阶调度配置"
    local current_opts="无"; if [ -f "$CAKE_OPTS_FILE" ]; then current_opts=$(cat "$CAKE_OPTS_FILE"); fi
    ...
    echo "  RTT模式: 1) internet  2) oceanic  3) satellite"; read -rp "  选择(默认2): " rtt_sel
    case "${rtt_sel:-2}" in 1) c_rtt="internet" ;; 2) c_rtt="oceanic" ;; 3) c_rtt="satellite" ;; *) c_rtt="oceanic" ;; esac
    
    echo "  识别模式: 1) diffserv4  2) besteffort"; read -rp "  选择(默认2): " diff_sel
    case "${diff_sel:-2}" in 1) c_diff="diffserv4" ;; 2) c_diff="besteffort" ;; *) c_diff="besteffort" ;; esac
    ...
    final_opts="$final_opts $c_rtt $c_diff"; final_opts=$(echo "$final_opts" | sed 's/^ *//')
    
    if [ -z "$final_opts" ]; then 
        rm -f "$CAKE_OPTS_FILE"; info "已清除 CAKE 参数。"
    else 
        echo "$final_opts" > "$CAKE_OPTS_FILE"; info "已保存: $final_opts"
    fi
    
    _apply_cake_live; read -rp "Enter 继续..." _
}

check_mph_state() { local state=$(jq -r '.routing.domainMatcher // "false"' "$CONFIG" 2>/dev/null); if [ "$state" = "mph" ]; then echo "true"; else echo "false"; fi; }
check_maxtime_state() { local state=$(jq -r '.inbounds[]? | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.realitySettings.maxTimeDiff // "false"' "$CONFIG" 2>/dev/null | head -n 1); if [ "$state" = "60000" ]; then echo "true"; else echo "false"; fi; }
check_routeonly_state() { local state=$(jq -r '.inbounds[]? | select(.protocol=="vless" or .protocol=="shadowsocks") | .sniffing.routeOnly // "false"' "$CONFIG" 2>/dev/null | head -n 1); if [ "$state" = "true" ]; then echo "true"; else echo "false"; fi; }
check_sniff_state() { local state=$(jq -r '.inbounds[]? | select(.protocol=="vless" or .protocol=="shadowsocks") | .sniffing.metadataOnly // "false"' "$CONFIG" 2>/dev/null | head -n 1); if [ "$state" = "true" ]; then echo "true"; else echo "false"; fi; }
check_affinity_state() { local limit_file="/etc/systemd/system/xray.service.d/limits.conf"; if grep -q "^CPUAffinity=" "$limit_file" 2>/dev/null; then echo "true"; else echo "false"; fi; }
check_buffer_state() { local limit_file="/etc/systemd/system/xray.service.d/limits.conf"; if grep -q "^Environment=\"XRAY_RAY_BUFFER_SIZE=64\"" "$limit_file" 2>/dev/null; then echo "true"; else echo "false"; fi; }
...
check_ring_state() { local IFACE=$(ip route get 1.1.1.1 | awk '{print $5; exit}'); if [ -z "$IFACE" ] || ! command -v ethtool >/dev/null 2>&1 || ! ethtool -g "$IFACE" >/dev/null 2>&1; then echo "unsupported"; return; fi; local curr_rx=$(ethtool -g "$IFACE" 2>/dev/null | grep -A4 "Current hardware settings" | grep "RX:" | head -1 | awk '{print $2}'); if [ -z "$curr_rx" ]; then echo "unsupported"; return; fi; if [ "$curr_rx" = "512" ]; then echo "true"; else echo "false"; fi; }
check_zram_state() { if ! modprobe -n zram >/dev/null 2>&1 && ! lsmod | grep -q zram; then echo "unsupported"; return; fi; if swapon --show | grep -q 'zram'; then echo "true"; else echo "false"; fi; }
...
check_ackfilter_state() { if [ -f "$FLAGS_DIR/ack_filter" ]; then echo "true"; else echo "false"; fi; }
check_ecn_state() { if [ -f "$FLAGS_DIR/ecn" ]; then echo "true"; else echo "false"; fi; }
check_wash_state() { if [ -f "$FLAGS_DIR/wash" ]; then echo "true"; else echo "false"; fi; }
...
_apply_cake_live() {
    ...
    if [ "$(check_cake_state)" = "true" ]; then
        ...
        local f_ack=""; if [ "$(check_ackfilter_state)" = "true" ]; then f_ack="ack-filter"; fi
        local f_ecn=""; if [ "$(check_ecn_state)" = "true" ]; then f_ecn="ecn"; fi
        local f_wash=""; if [ "$(check_wash_state)" = "true" ]; then f_wash="wash"; fi
...
do_summary() {
    ...
    if [ -n "$vless_inbound" ] && [ "$vless_inbound" != "null" ]; then
        ...
        while read -r client; do
            ...
            # 使用索引从 shortIds 数组中精准提取对应的 sid，防止漂移错位
...
# -- 核心突破: 用户管理系统 (完美同步 8 行对齐 + ax131 索引防漂移) --
do_user_manager() {
    while true; do
...
EOF
            ...
            if test "${total:-0}" -le 1; then error "必须至少保留一个核心用户！"
                ...
                if test -n "$target_uuid"; then
                    local idx=$((${dnum:-0}-1))
                    # -- ax131 核心 Bug 修复：同步安全删除 clients 数组项与 shortIds 数组项，绝杀索引错位漂移 --
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex144 The Apex Vanguard - Project Genesis V144${none}"
