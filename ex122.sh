#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex122.sh (The Apex Vanguard - Project Genesis V122 [The Immortal])
# 快捷方式: xrv
# 永生之境: 
#   1. jq 降级防爆：全面清洗高危的 first() 语法，改用绝对安全的管道 head -n 1，彻底免疫老旧 Linux 系统下 jq 版本的空指针崩溃，修复分发中心与用户管理的死寂 BUG。
#   2. 传参黑洞修复：修正 _update_matrix 中错用 _safe_jq_write 导致传参失效、洗白配置的致命低级错误，恢复矩阵的无感热切。
#   3. JSON 崩塌绝杀：全面废除危险的 jq 数组重构语法，所有状态机的启停强制重写为 `|=` 原子化就地更新，彻底根除节点配置被误擦除的灭顶灾难。
...
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex122.sh"
    exit 1
fi
...
EOF
            ...
            local port=$(jq -r '.inbounds[]? | select(.protocol=="vless") | .port // empty' "$CONFIG" 2>/dev/null | head -n 1)
            local sni=$(jq -r '.inbounds[]? | select(.protocol=="vless") | .streamSettings.realitySettings.serverNames[0] // empty' "$CONFIG" 2>/dev/null | head -n 1)
            local pub=$(jq -r '.inbounds[]? | select(.protocol=="vless") | .streamSettings.realitySettings.publicKey // empty' "$CONFIG" 2>/dev/null | head -n 1)
            info "用户分配成功！"
...
do_summary() {
    ...
    # 绝对管道流提取节点，杜绝多次遍历与老旧 jq first() 空指针崩溃
    local vless_inbound=$(jq -c '.inbounds[]? | select(.protocol=="vless")' "$CONFIG" 2>/dev/null | head -n 1)
...
# -- 矩阵切换注入核心 (绝对绕过 _safe_jq_write 传参黑洞) --
_update_matrix() {
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex122 The Apex Vanguard - Project Genesis V122${none}"
