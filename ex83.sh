#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex83.sh (The Apex Vanguard - Project Genesis V83)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 逻辑闭环：修复虚拟化环境缺失硬件支持导致的一键反转死锁，实装【硬件能力自适应剥离】侦测引擎。
#   2. 漏洞查杀：修复 jq 写入不存在路径时的 null 报错，引入 |= (. // {}) 空对象自愈初始化逻辑。
#   3. 上帝开关：新增子菜单 14/15/16 项，一键横跨应用层与系统层，全域 13 项黑科技无损秒切。
#   4. 终极勘误：全域清理历史版本残影，实现表里如一的 V83 视觉与内核统一。
#   5. 战神归位：OOM 免死与 I/O 抢占【出厂默认强制注入】，升级核心时自动继承探取状态。
#   6. 极致提权：Xray 初始 CPU 调度抢占拉升至系统绝对物理最高值 (Nice=-20)。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex83.sh"
    exit 1
fi
...
do_app_level_tuning_menu() {
    while true; do
        ...
        local s7=$([ "$(check_thp_state)" = "true" ] && echo "${cyan}已关闭 THP${none}" || ([ "$(check_thp_state)" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}系统默认${none}"))
        local s8=$([ "$(check_mtu_state)" = "true" ] && echo "${cyan}智能探测中${none}" || ([ "$(check_mtu_state)" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}未开启${none}"))
        local s9=$([ "$(check_cpu_state)" = "true" ] && echo "${cyan}全核火力全开${none}" || ([ "$(check_cpu_state)" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}节能待机${none}"))
        local s10=$([ "$(check_ring_state)" = "true" ] && echo "${cyan}已反向收缩${none}" || ([ "$(check_ring_state)" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}系统大缓冲${none}"))
        local s11=$([ "$(check_zram_state)" = "true" ] && echo "${cyan}已挂载 ZRAM${none}" || ([ "$(check_zram_state)" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}未启用${none}"))
        local s12=$([ "$(check_journal_state)" = "true" ] && echo "${cyan}纯内存极速化${none}" || ([ "$(check_journal_state)" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}磁盘 IO 写入中${none}"))
        ...
        case "$app_opt" in
            ...
            6)
                ...
                read -rp "按 Enter 继续..." _
                ;;
            7) toggle_thp; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            8) toggle_mtu; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            9) toggle_cpu; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            10) toggle_ring; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            11) toggle_zram; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            12) toggle_journal; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            13) toggle_process_priority; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            14)
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex83 The Apex Vanguard - Project Genesis V83${none}"
