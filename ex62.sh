#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex62.sh (The Apex Vanguard - Project Genesis V62)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 新增【应用层一键关闭】(选项10->7->6): 实现出站提速、嗅探减负、DoH并发的秒级还原。
#   2. 维持极客特权：系统初始化菜单 (10) 中嵌入子菜单 (7) 【4项应用层极限微操】。
#   3. 维持阶梯飙车：实装 GOGC 动态内存阶梯容差算法 (支持 100~1000 动态阀门调优)。
#   4. 维持跨代级核心：雷达守望模式 (选项9->4)、动态 Nice 优先级提权 (选项9->5)。
#   5. 维持防断连生态：官方 XANMOD 预编译打底、无损内存计费快照、极限 TCP 栈参数。
#   6. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex62.sh"
    exit 1
fi
...
do_install_xanmod_main_official() {
    ...
    # 极客级绕过 noexec 挂载: 直接 bash 内存解析，无需赋予执行权限
    ...
    # 修正 Debian 12 (Bookworm) 的兼容性: 彻底移除淘汰的 gnupg1，保留标准依赖
...
# -- [功能 10->7] 应用层 4 项普通极客设置 --
do_app_level_tuning_menu() {
    while true; do
        clear
        title "应用层 4 项极限微操 (Xray 提速底牌)"
        if ! test -f "$CONFIG"; then 
        ...
        echo -e "  1) 开启 Outbound 出站双向提速 (tcpNoDelay/FastOpen) | 当前状态: ${cyan}${out_status}${none}"
        echo -e "  2) 开启 嗅探引擎减负 (metadataOnly 解放 CPU)        | 当前状态: ${cyan}${sniff_status}${none}"
        echo -e "  3) 开启 内置并发 DoH 引擎 (Xray Native DNS)         | 当前状态: ${cyan}$(if [ "$dns_status" = "UseIP" ]; then echo "true"; else echo "false"; fi)${none}"
        echo -e "  4) 执行 GOGC 内存阶梯飙车调优 (自动检测物理内存)    | 当前设定: ${cyan}${gc_status}${none}"
        echo -e "  ${magenta}5) 一键强力开启以上 4 项终极微操${none}"
        echo -e "  ${red}6) 一键强制关闭以上 1-3 项微操 (无损还原初始状态)${none}"
        echo "  0) 返回上一级"
        hr
        read -rp "请选择: " app_opt
        ...
        case "$app_opt" in
            ...
            4)
                if [ -f "$limit_file" ]; then
                    ...
                    info "GOGC 动态阶梯调优完成！检测到内存 ${TOTAL_MEM}MB，阈值已设为: ${DYNAMIC_GOGC}"
                else
            ...
            6)
                ...
                systemctl restart xray >/dev/null 2>&1
                info "应用层 1-3 项微操已强制关闭，Xray 配置已无损还原！"
                read -rp "按 Enter 继续..." _
                ;;
            0)
                return
...
do_sys_init_menu() {
    while true; do
        ...
        echo -e "  ${magenta}7) 应用层 4 项普通设置 (Xray 极限微操)${none}"
        echo "  0) 返回主菜单"
        hr
        read -rp "请选择: " sys_opt
        case "$sys_opt" in
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex62 The Apex Vanguard - Project Genesis V62${none}"
