#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex39.sh (The Apex Vanguard - Project Genesis V12)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 彻底转型【极低延迟/小包秒开】架构：Xray 并发精简至 4096，TX 队列收缩至 2000。
#   2. 激活内核态忙轮询 (Busy Polling): net.core.busy_poll=50，死盯网卡极限提速。
#   3. 终结 TCP Bufferbloat: 引入精准 16KB (16384) 的 notsent_lowat 发送缓冲防膨胀。
#   4. 剥离网卡中断聚合 (Interrupt Coalescing): 强制 ethtool -C 做到 0 延迟硬件触发。
#   5. 完美适配 ex37/ex38 热升级，仅需重跑菜单 4 和 5 即可完成新架构底层注入。
#   6. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex39.sh"
    exit 1
fi
...
do_sys_init_menu() {
    while true; do
        ...
        echo "  4) 系统内核网络栈极限调优 (低延迟特化版 / 自动重启)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex39 The Apex Vanguard - Project Genesis V12${none}"
