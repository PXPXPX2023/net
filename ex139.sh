#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex139.sh (The Apex Vanguard - Project Genesis V139 [Absolute Unity])
# 快捷方式: xrv
# 巅峰重铸: 
#   1. 找回百万并发：修复 Systemd 资源限制退化问题，重新拉满 LimitNOFILE=1048576 与 LimitNPROC=512000。
#   2. 补全幽灵拼图：实装第 8 号主菜单选项 (Reality 回落限速探针)，并在 VLESS 生成时完整注入 Fallback 陷阱参数。
#   3. 硬件安全熔断：彻底锁死 GSO/GRO 强制修改逻辑，防范底层网卡不支持导致的内核断流与空转。
#   4. 母本守护：全量保留 ex118 的 17 项 BBR3/内核调优基线，完美嵌入 tcp_adv_win_scale 内存倾斜与 BQL 强压引擎。
#   5. 军规排版：严格执行 8 行冒号对齐展示，完美整合 ax131 数组同步截断算法，绝杀 ShortId 错位。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex139.sh"
    exit 1
fi
...
fix_xray_systemd_limits() {
    ...
    # 恢复极限参数：1048576 文件描述符与 512000 进程数
    cat > "$limit_file" << EOF
[Service]
LimitNOFILE=1048576
LimitNPROC=512000
...
# 彻底修复 V139: 完美实装全自动 Xanmod 源码编译，绝杀所有依赖陷阱和交互弹窗！
do_xanmod_compile() {
    ...
    print_magenta ">>> [3/4] 继承当前系统基座配置并静默注入 BBR3..."
...
do_perf_tuning() {
    ...
    # 100% 还原 ex118 的 BBR 参数配置，同时在末尾追加 V139 内存倾斜
...
# [V139 物理熔断] 强行跳过不支持的机器环境，保护网络不中断
check_gso_off_state() {
    echo "unsupported"
    return
}
...
# V139 完整保留多核散列和 BQL 强压 (移除 GSO 强干预)
...
toggle_gso_off() {
    warn "当前机器环境硬件卸载已锁死，为了防止断网，已安全跳过 GSO/GRO 强制干预。"
    sleep 2
}
...
do_fallback_probe() {
    ...
    jq -r '.inbounds[]? | select(.protocol=="vless" and .streamSettings.security=="reality") | .streamSettings.realitySettings | "  [上传方向 (Upload)]\n    诱饵大小 (afterBytes) : \(.limitFallbackUpload.afterBytes // "未设置 (不限速)")\n    基准限速 (bytesPerSec) : \(.limitFallbackUpload.bytesPerSec // "未设置 (不限速)")\n  [下载方向 (Download)]\n    诱饵大小 (afterBytes) : \(.limitFallbackDownload.afterBytes // "未设置 (不限速)")\n    基准限速 (bytesPerSec) : \(.limitFallbackDownload.bytesPerSec // "未设置 (不限速)")"' "$CONFIG" 2>/dev/null || echo -e "  \033[31m读取失败：未找到配置文件或 jq 解析错误\033[0m"
    echo ""
    read -rp "按 Enter 继续..." _
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex139 The Apex Vanguard - Project Genesis V139${none}"
