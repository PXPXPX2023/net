#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex93.sh (The Apex Vanguard - Project Genesis V93)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 时序排雷：修复单点开启绑核与规则库未显式重启的漏洞，确保所有微操 100% 实时生效。
#   2. 运维升维：重构定时热更引擎，实现【下载(3:00)与重启(3:05)物理级解耦错峰】，彻底杜绝断流风险。
#   3. 状态锁死：优化核心重装逻辑，确保用户开启的 Nano DB 选择在热更时不被暴力覆盖。
#   4. 三剑合璧：Nano DB + 物理绑核 (CPU Affinity) + Go 并发硬干预 (GOMAXPROCS) 完美缝合。
#   5. 十六边形战神矩阵：全域扩容为 16 项极限微操 (应用层 8 项 + 系统级 8 项)。
#   6. 智能反转引擎：17/18/19 项上帝开关全量适配新架构，遵循缺省即补、满血即卸、硬件避障的铁律。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex93.sh"
    exit 1
fi
...
install_update_dat() {
    ...
    # 清除旧的混合任务，注入完美的 3:00 下载与 3:05 重启解耦逻辑
    (crontab -l 2>/dev/null | grep -v "$UPDATE_DAT_SCRIPT" | grep -v "/bin/systemctl restart xray"; 
     echo "0 3 * * * $UPDATE_DAT_SCRIPT >/dev/null 2>&1"; 
     echo "5 3 * * * /bin/systemctl restart xray >/dev/null 2>&1") | crontab -
     
    info "已配置自动热更与错峰重启: 3:00 静默下载规则库，3:05 安全重载 Xray 进程。"
}
        ...
        case "$app_opt" in
            ...
            7)
                if [ "$nanodb_state" = "true" ]; then
                    print_magenta ">>> 正在恢复大而全的全球版路由库，请稍候..."
                    _toggle_nanodb_off
                    systemctl restart xray >/dev/null 2>&1
                    info "全球版路由库已恢复并应用！"
                else
                    ...
                    _toggle_nanodb_on
                    systemctl restart xray >/dev/null 2>&1
                    info "Nano 极简版路由库已部署完毕，匹配速度已提升至纳秒级！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            8)
                if [ "$affinity_state" = "true" ]; then
                    ...
                    _toggle_affinity_on
                    systemctl restart xray >/dev/null 2>&1
                    info "Xray 进程已成功使用铁链锁死在目标物理核心，并发锁已激活！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            ...
            17)
                if [ "$app_off_count" -gt 0 ]; then
                    ...
                    _turn_on_app
                    systemctl restart xray >/dev/null 2>&1
                    info "1-8 项 Xray 内部应用层微操已全域强力开启！"
                else
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex93 The Apex Vanguard - Project Genesis V93${none}"
