#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex196e56.sh (The Apex Vanguard - Project Genesis V196e56)
# 快捷方式: xrv
# 【V196e56 终极融合版：官方APT完美降维探针、源码编译斩断cc1、死锁1380M Swap】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex196e56.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="196e56"
...
do_install_xanmod_official_fusion() {
    ...
    info "正在连接 Xanmod 官方检测 CPU 指令集等级..."
    if wget -qO "$cpu_level_script" https://dl.xanmod.org/check_x86-64_psabi.sh; then
        chmod +x "$cpu_level_script"
        cpu_level=$(awk -f "$cpu_level_script" 2>/dev/null | grep -oE 'x86-64-v[1-4]' | grep -oE '[1-4]' | tail -n1 || echo "")
        rm -f "$cpu_level_script" 2>/dev/null || true
    fi

    if [[ ! "$cpu_level" =~ ^[1-4]$ ]]; then
        warn "官方 API 检测受阻或无法识别，回落至绝对安全的 v1 基础架构..."
        cpu_level="1"
    fi
...
_execute_compilation() {
    ...
    local c_lvl="$(_detect_psabi_level)"
    c_lvl="${c_lvl:-2}"
    local safe_march="$(_get_safe_march "$c_lvl")"
    
    info "=== 开启单核极限稳定编译 (指令集: $safe_march, Swap: 1380MB) ==="
    if ! make -j"$THREADS" KCFLAGS="-march=$safe_march" $extra_make_args; then
...
_compile_kernel_xanmod() {
    ...
    ./scripts/config --enable CONFIG_X86_64_V1
...
do_kernel_compile_menu() {
    while true; do
        ...
        echo "  [已修复] cc1 bad value 编译暴毙问题 (降维兼容，拦截残缺拼接参数)"
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V196e56 最终版)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e56 - (The Apex Vanguard)${none}"
...
#==============================================================================
# EOF - Apex Vanguard V196e56 System Advanced Control Ready.
#==============================================================================
