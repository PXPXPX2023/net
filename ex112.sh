#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex112.sh (The Apex Vanguard - Project Genesis V112 [The Last Stand])
# 快捷方式: xrv
# 终极圆满: 
#   1. 港口管治完全体：CAKE 高阶参数引擎扩容，新增 Oceanic (跨洋高延迟免杀) 与 Besteffort (纯代理盲走降负) 动态配置。
#   2. 单兵极致：剔除无效的多路调度消耗，专为单人/高延迟/加密隧道环境打造极限拥塞防线。
#   3. 逻辑粉碎：完美继承 V111 修复的 SS 节点提取漏洞，确立绝对安全的协议探针判空。
#   4. 陷阱免疫：绝对拒绝使用旧版残缺代码，全量继承 V109/V111 中所有无折叠多行赋值、virtio 锁死探针、双协议平权。
#   5. 语法绝缘：完整保留 jq 的原子化赋值链 (.streamSettings //= {} | .sockopt //= {})，绝杀 JSON 结构残缺导致的空转。
#   6. 大一统矩阵：全域 23 项极限微操 + CAKE 5 大高阶透视参数面板，构建单节点路由的终极物理形态。
#   7. 不追求代码行数的精简，只追求工业级的绝对健壮，这是跨洋代理调优的最终丰碑。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex112.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex111 The Apex Vanguard - Project Genesis V111${none}"
