#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex99.sh (The Apex Vanguard - Project Genesis V99)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 重启阵列：为“上帝之手 (全域微操)”加入 6 秒强制倒计时重启，确保系统级底层变更 (ZRAM/THP/CPU/队列) 绝对干净地落盘与生效。
#   2. 逻辑归一：修复 vnstat UI 割裂问题，计费日修改无缝融入流量统计主界面，正则穿透完美覆盖。
#   3. 深度清剿：彻底切除前期版本残留的 Nano DB 死代码与异构探针，保持底层脚本的绝对洁净，回归全球版。
#   4. 绝缘排爆：为所有 jq 查询加装 select(. != null) 绝缘层，彻底断绝因 JSON 字段缺失导致的闪退。
#   5. 十五边形战神矩阵：全域极客微操收拢为 15 项 (应用层 7 项 + 系统级 8 项)。
#   6. 智能反转引擎：16/17/18 项上帝开关全量适配新架构，遵循缺省即补、满血即卸、硬件避障的铁律。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex99.sh"
    exit 1
fi
...
# -- [功能 10->7] 全域 15 项极限微操双向设置枢纽 --
do_app_level_tuning_menu() {
    while true; do
        clear
        title "全域 15 项极限微操 (Xray 提速底牌 & 系统内核微操)"
        if ! test -f "$CONFIG"; then 
        ...
        local s8=$([ "$dnsmasq_state" = "true" ] && echo "${cyan}内存极速解析中 (0.1ms)${none}" || echo "${gray}未开启 (依赖原生DoH)${none}")
        local s9=$([ "$thp_state" = "true" ] && echo "${cyan}已关闭 THP${none}" || ([ "$thp_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}系统默认${none}"))
        local s10=$([ "$mtu_state" = "true" ] && echo "${cyan}智能探测中${none}" || ([ "$mtu_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}未开启${none}"))
        local s11=$([ "$cpu_state" = "true" ] && echo "${cyan}全核火力全开${none}" || ([ "$cpu_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}节能待机${none}"))
        local s12=$([ "$ring_state" = "true" ] && echo "${cyan}已反向收缩${none}" || ([ "$ring_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}系统大缓冲${none}"))
        local s13=$([ "$zram_state" = "true" ] && echo "${cyan}已挂载 ZRAM${none}" || ([ "$zram_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}未启用${none}"))
        local s14=$([ "$journal_state" = "true" ] && echo "${cyan}纯内存极速化${none}" || ([ "$journal_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}磁盘 IO 写入中${none}"))
        local s15=$([ "$prio_state" = "true" ] && echo "${cyan}OOM免死 / IO抢占${none}" || echo "${gray}系统默认调度${none}")
        ...
        echo -e "  13) 开启或关闭【ZRAM】(淘汰慢速 Swap，阶梯内存自动检测)          | 当前状态: $s13"
        echo -e "  14) 开启或关闭【日志系统 Journald 纯内存化】(斩断 I/O 羁绊)      | 当前状态: $s14"
        echo -e "  15) 开启或关闭【系统进程级防杀抢占 (OOM/IO 提权)】               | 当前状态: $s15"
        echo -e "  "
        echo -e "  ${cyan}16) 一键开启或关闭 1-7 项 应用层微操 (自动侦测并智能反转)${none}"
        echo -e "  ${yellow}17) 一键开启或关闭 8-15 项 系统级微操 (自动避障侦测并反转)${none}"
        echo -e "  ${red}18) 上帝之手：一键开启或关闭 1-15 项 全域微操 (执行后自动重启系统)${none}"
        echo "  0) 返回上一级"
        hr
        read -rp "请选择: " app_opt
        ...
        case "$app_opt" in
            ...
            7)
                ...
                read -rp "按 Enter 继续..." _
                ;;
            8) toggle_dnsmasq; systemctl restart xray >/dev/null 2>&1; read -rp "按 Enter 继续..." _ ;;
            9) toggle_thp; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            10) toggle_mtu; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            11) toggle_cpu; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            12) toggle_ring; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            13) toggle_zram; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            14) toggle_journal; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            15) toggle_process_priority; systemctl restart xray >/dev/null 2>&1; info "操作已下发，进程已重启生效。"; read -rp "按 Enter 继续..." _ ;;
            16)
                if [ "$app_off_count" -gt 0 ]; then
                    print_magenta ">>> 正在全域强力开启 1-7 项，请耐心等待..."
                    _turn_on_app
                    systemctl restart xray >/dev/null 2>&1
                    info "1-7 项 Xray 内部应用层微操已全域强力开启并实时生效！"
                else
                    print_magenta ">>> 正在全域恢复 1-7 项，请耐心等待..."
                    _turn_off_app
                    systemctl restart xray >/dev/null 2>&1
                    info "1-7 项 Xray 内部应用层微操已全部强制关闭，无损还原官方初始状态！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            17)
                if [ "$sys_off_count" -gt 0 ]; then
                    ...
                    systemctl restart xray >/dev/null 2>&1
                    info "8-15 项系统内核底层黑科技已全部一键激活！(不支持的硬件已自动跳过)"
                else
            ...
            18)
                if [ "$((app_off_count + sys_off_count))" -gt 0 ]; then
                    ...
                    info "上帝降临！1-15 项全域极限微操已【全部满血激活】！(不支持的硬件已自动跳过)"
                else
                    ...
                    [ "$journal_state" = "true" ] && toggle_journal >/dev/null 2>&1
                    [ "$prio_state" = "true" ] && toggle_process_priority >/dev/null 2>&1
                    info "神威收敛！1-15 项全域极限微操已【全部强制关闭】，无损还原初始出厂状态！"
                fi
...
do_sys_init_menu() {
    while true; do
        ...
        echo -e "  ${magenta}6) 全域 15 项极限微操 (Dnsmasq / Xray 提速底牌 / 系统级黑科技)${none}"
        echo "  0) 返回主菜单"
        hr
        read -rp "请选择: " sys_opt
        case "$sys_opt" in
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex99 The Apex Vanguard - Project Genesis V99${none}"
