#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex188t21.sh (The Apex Vanguard - Enterprise Ultimate Genesis)
# 快捷方式: xrv
# 版本号: V188t21.Final.Unabridged
#
# 【V188t21 终极重构与增量日志】
#   1. 拒绝缩水: 代码全量展开，引入企业级日志记录与 Trap 异常捕获追溯系统。
#   2. 绝对自检: Xray 配置落盘前强制触发 `xray -test` 引擎预检，拒绝暴毙。
#   3. 持续在线: 将 ethtool/rps/cake 硬件层调优全部封装为 Systemd 守护进程，重启持久化。
#   4. 变量净化: 修复 x25519 密钥生成中的隐藏 \r\n，杜绝 JQ 语法解析崩溃。
#   5. 探针与调度: 21项 check_ 探针与 28项调度面板全量物理植入，变量绝对安全。
#   6. 强硬路由: 一键强锁 IPv4 优先，物理级熔断 IPv6 泄露风险。
#   7. 原装伪装: 100% 采用用户指定的 60 个高防 SNI 横向排版。
# ==============================================================================
...
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 本系统底层依赖 Bash 特性，请执行: bash ex188t21.sh"
    exit 1
fi
...
break_end() {
    ...
    info "当前序列操作已执行完毕。"
    local _pause=""
    read -e -p "请按任意键继续并返回控制中枢..." _pause || true
    echo ""
}
...
readonly SCRIPT_VERSION="ex188t21-Enterprise-Genesis"
...
fix_permissions() {
    # 强制修正 Xray 配置目录的权限，防止 Xray 核心启动拒绝
    if [[ -f "$CONFIG" ]]; then
...
_safe_jq_write() {
    ...
    rm -f "$tmp" 2>/dev/null || true
    error "JQ 节点解析器发生内核级异常，写入指令已主动阻断。"
    log_error "JQ 安全写入失败，注入参数: $*"
    restore_latest_backup
    return 1
}
...
# ------------------------------------------------------------------------------
# [ 0x07: 进程级护盾与系统资源限额 (Systemd Limits & Swap) ]
# ------------------------------------------------------------------------------

fix_xray_systemd_limits() {
    info "正在重构 Xray 守护进程的资源配额 (Systemd Limits)..."
...
run_sni_scanner() {
    ...
    # 原教旨主义：100% 采用用户指定的 60 个高防 SNI 横向排版，绝不压缩
    local sni_list=(
...
do_install_xanmod_main_official() {
    ...
    info "开始注流 Xanmod APT 软件源与签名验证密钥..."
    export DEBIAN_FRONTEND=noninteractive
    apt-get update -y >/dev/null 2>&1 || true
    ...
    if ! wget -qO - https://dl.xanmod.org/gpg.key | gpg --dearmor --yes -o /etc/apt/trusted.gpg.d/xanmod-kernel.gpg; then
        error "从远端导入 GPG 密钥链发生错误，链路可能被污染！"
        return 1
    fi
    
    info "指令已下发，正在并行安装: $pkg_name"
    apt-get update -y >/dev/null 2>&1 || true
...
# ------------------------------------------------------------------------------
# [ 0x0F: 应用层高级状态调控面板 (The 28-Panel Center) ]
# ------------------------------------------------------------------------------
...
do_app_level_tuning_menu() {
    while true; do
        ...
        if [[ "$out_fastopen" != "false" && -n "$out_fastopen" ]]; then 
            out_fastopen="true"
        else 
            out_fastopen="false"
        fi
        ...
        if [[ "$out_keepalive" != "false" && -n "$out_keepalive" ]]; then 
            out_keepalive="30"
        else 
            out_keepalive="false"
        fi
        ...
        if [[ "$dns_status" != "false" && -n "$dns_status" ]]; then 
            dns_status="UseIP"
        else 
            dns_status="false"
        fi
        ...
        if [[ "$policy_status" != "false" && -n "$policy_status" ]]; then 
            policy_status="60"
        else 
            policy_status="false"
        fi
        ...
        case "$app_opt" in
            ...
            5)
                ...
                local _pause=""
                read -e -p "内核级内存态势配置已更新生效，按 Enter 键返回..." _pause || true
                ;;
            7)
                ...
                systemctl restart xray >/dev/null 2>&1 || true
                local _pause=""
                read -e -p "CPU 绑核与进程调度锁配置已更新，按 Enter 键返回..." _pause || true
                ;;
            11) 
                toggle_buffer
                systemctl restart xray >/dev/null 2>&1 || true
                local _pause=""
                read -e -p "内存池架构配置已应用生效，按 Enter 继续..." _pause || true 
                ;;
            12) toggle_dnsmasq; local _pause=""; read -e -p "缓存引擎配置已应用生效，按 Enter 继续..." _pause || true ;;
            13) toggle_thp; local _pause=""; read -e -p "内存大页架构配置已应用生效，按 Enter 继续..." _pause || true ;;
            14) toggle_mtu; local _pause=""; read -e -p "黑洞防堵配置已应用生效，按 Enter 继续..." _pause || true ;;
            15) toggle_cpu; local _pause=""; read -e -p "锁频控制器配置已应用生效，按 Enter 继续..." _pause || true ;;
            16) toggle_ring; local _pause=""; read -e -p "环形通道控制配置已应用生效，按 Enter 继续..." _pause || true ;;
            17) toggle_zram; local _pause=""; read -e -p "ZRAM 压缩挂载配置已应用生效，按 Enter 继续..." _pause || true ;;
            18) toggle_journal; local _pause=""; read -e -p "日志系统 IO 配置已应用生效，按 Enter 继续..." _pause || true ;;
            19) 
                toggle_process_priority
                systemctl restart xray >/dev/null 2>&1 || true
                local _pause=""
                read -e -p "系统级提权防杀机制配置已应用生效，按 Enter 继续..." _pause || true 
                ;;
            20) toggle_cake; local _pause=""; read -e -p "CAKE 系统配置已应用生效，按 Enter 继续..." _pause || true ;;
            21) toggle_irq; local _pause=""; read -e -p "中断隔离控制器配置已应用生效，按 Enter 继续..." _pause || true ;;
            22) 
                if [[ "$gso_off_state" == "unsupported" ]]; then
                    warn "宿主机当前所挂载的硬件驱动在底层拒绝了状态改写请求。"
                    sleep 2
                else
                    toggle_gso_off
                    local _pause=""
                    read -e -p "配置已应用生效，按 Enter 继续..." _pause || true 
                fi
                ;;
            23) toggle_ackfilter; local _pause=""; read -e -p "控制配置已应用生效，按 Enter 继续..." _pause || true ;;
            24) toggle_ecn; local _pause=""; read -e -p "控制配置已应用生效，按 Enter 继续..." _pause || true ;;
            25) toggle_wash; local _pause=""; read -e -p "控制配置已应用生效，按 Enter 继续..." _pause || true ;;
            26)
                if ((app_off_count > 0)); then
                    info "触发总线控制：正在全量级下发应用层加速核心..."
                    _turn_on_app
                    systemctl restart xray >/dev/null 2>&1 || true
                else
                    info "触发总线控制：正在回滚剥离应用层高级参数集..."
                    _turn_off_app
                    systemctl restart xray >/dev/null 2>&1 || true
                fi
                local _pause=""
                read -e -p "指令序列执行完毕，按 Enter 继续..." _pause || true
                ;;
            27)
            ...
            28)
                ...
                echo ""
                info "所有应用层协议与系统底层调优参数已全数注满！"
                warn "为了使硬中断等核心模块重新载入驱动，系统将在 5 秒后强制重启..."
                sleep 5
                sync
                reboot
                ;;
...
# ------------------------------------------------------------------------------
# [ 0x10: 核心通信协议框架部署 (纯原生 Bash 强写架构) ]
# ------------------------------------------------------------------------------

gen_ss_pass() { 
    # 强制切除 base64 转化过程中可能夹带的所有不可见回车符
...
do_install() {
    ...
    if ! bash -c "$(curl -fsSL --connect-timeout 10 https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install >/dev/null 2>&1; then
        warn "主用官方脚本连接失败或遭遇劫持，配置仍将继续，但内核获取存在异常！"
    fi
    
    install_update_dat
    fix_xray_systemd_limits

    info "触发引擎级别 JSON 配置装配中心 (纯 Bash 安全编译)..."
...
EOF
    ...
    if ensure_xray_is_alive; then
        ...
        error "预警！守护进程激活失败，请通过报错堆栈排查核心配置！"
        return 1
    fi
...
# ------------------------------------------------------------------------------
# [ 0x11: 节点分发与数据终端 (Data Render & Summary) ]
# ------------------------------------------------------------------------------
...
do_summary() {
    ...
    # 通过原生 jq 精准提取（因为只做只读提取，极其安全，不会破坏 JSON）
    local vless_inbound
...
do_user_manager() {
    clear
    title "多用户与认证管理中心"
    warn "出于绝对的系统稳定性考虑，利用 JQ 动态重写多用户的模块已被物理剥离。"
    info "若您需要扩展用户表阵列，请通过 nano 或 vim 直接直入编辑底层数据链："
    echo -e "  ${cyan}# nano $CONFIG${none}"
    local _pause=""
    read -e -p "指令接收完毕，按 Enter 退回主系统..." _pause || true
}

_global_block_rules() {
    clear
    title "全局防火墙与探针阻断策略"
    warn "出于绝对的系统稳定性考虑，利用 JQ 动态修改路由黑洞的模块已被物理剥离。"
    info "系统的底层已经内置部署了 GeoIP:CN、BT 下载与全局恶意广告等物理阻断防线。"
    info "若您需要增改规则，请直接编辑底层数据链："
    echo -e "  ${cyan}# nano $CONFIG${none}"
    local _pause=""
    read -e -p "指令接收完毕，按 Enter 退回主系统..." _pause || true
}
...
do_fallback_probe() {
    ...
    warn "为了杜绝 JQ 的读取漏洞带来的断层风险，自动呈现功能已被休眠。"
    info "防盗扫回落触发预警值 (limitFallbackUpload/Download) 配置已安全保存。"
    info "您可以键入以下指令，强行调取底层配置文件进行人工比对审计："
    echo -e "  ${cyan}# cat $CONFIG${none}"
    echo ""
    local _pause=""
    ...
    while true; do
        ...
        case "${s:-}" in
            ...
            3) 
                ...
                echo "  2) 向数据库回溯查询特定历史年月的详细跑量图谱 (如: 2026-04)"
                echo "  0) 折叠流量面板并退出"
