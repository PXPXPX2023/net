#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex185.sh (Xray Advanced Management Script V185)
# 快捷方式: xrv
#
# V185 核心重构日志:
#   1. 规范升级: 彻底肃清所有非专业修辞，全篇统一采用工业级 [INFO]/[WARN]/[ERROR] 日志标准。
#   2. JSON 重构: 淘汰危险的 jq input 流合并，强制采用 --slurpfile 隔离写入，根除格式损坏。
#   3. 变量补齐: 修复 magenta 缺失问题，完全适配 set -euo pipefail 极严苛容错标准。
#   4. 底盘固化: 无损保留 130+ SNI 测速与 60+ Sysctl 参数，确保大并发业务稳定运行。
# ==============================================================================

# 检查 Bash 运行环境
if test -z "$BASH_VERSION"; then
    echo "Error: Please run this script with bash: bash ex185.sh"
    exit 1
fi
...
# 安全 JSON 写入接口 (支持 --slurpfile 等高级传参)
_safe_jq_write() {
    backup_config
    local tmp
    tmp=$(mktemp) || return 1
...
EOF
    ...
    if [[ -n "$IFACE" ]]; then
        info "配置网卡硬件卸载与 CPU 软中断分发 ($IFACE)..."
...
EOF
    ...
    if [[ "$CHECK_QLEN" == "2000" ]]; then
        ...
        warn "修改未生效，当前网卡驱动可能不支持调整 txqueuelen。"
    fi
    read -rp "按 Enter 返回..." _
}
...
config_cake_advanced() {
    ...
    local current_opts="未配置 (系统自适应)"
    if [[ -f "$CAKE_OPTS_FILE" ]]; then
        current_opts=$(cat "$CAKE_OPTS_FILE" 2>/dev/null || echo "")
    fi
    echo -e "  当前已配置参数: ${cyan}${current_opts}${none}\n"
    
    read -rp "  [1] 服务器可用带宽 (格式如 900Mbit, 输入 0 不限制): " c_bw
    read -rp "  [2] 封装报文开销补偿 Overhead (建议 VPN/代理输入 48, 输入 0 废弃): " c_oh
    read -rp "  [3] 最小数据单元 MPU (建议 64 或 84, 输入 0 废弃): " c_mpu
...
# ==============================================================================
# [ 区块 IX: Xray 核心安装与网络部署 ]
# ==============================================================================

do_install() {
    title "Xray 核心安装与网络架构初始化"
    preflight
...
EOF
    ...
    # === V185 核心重构: JQ 使用 --slurpfile 安全隔离并合并配置 ===
    if [[ "$proto_choice" == "1" || "$proto_choice" == "3" ]]; then
...
do_summary() {
    ...
    hr
    echo -e "  ${gray}配置文档: $CONFIG | 备份目录: $BACKUP_DIR${none}"
}
...
do_sys_init_menu() {
    while true; do
        ...
        echo "  3) 编译部署 Linux Kernel 官方主线内核并开启 BBR3"
        echo "  4) 配置网卡发送队列 (TX Queue) 并发加速"
        echo "  5) 写入系统内核网络栈高速调优参数集"
        echo "  6) 网络应用层与系统级高级参数全量控制面板"
        echo "  7) 配置高级 CAKE 队列与网络流量调度参数"
        echo "  0) 返回系统主界面"
        hr
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray 高级管理系统 (Industrial Base V185)${none}"
        ...
        case "$num" in
            ...
            3) 
                do_summary
                while true; do 
                    read -rp "按 Enter 返回主菜单，或输入 b 重启伪装验证策略: " rb
                    if [[ "$rb" == "b" || "$rb" == "B" ]]; then 
                        if choose_sni; then 
                            _update_matrix
                            do_summary
