#!/usr/bin/env bash
# ============================================================
# 脚本名称: dx36.sh (The Apex Vanguard - Project Genesis V8)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 融入上古遗毒清理引擎，彻底绞杀 net-speeder 进程及残留，防冲突。
#   2. 升级精准网卡识别逻辑 (ip route get)，并在调优后强制接管网卡 fq 队列。
#   3. 彻底重构 JSON 注入引擎，采用纯净 HereDoc 与 jq input 机制，杜绝截断报错。
#   4. 卸载逻辑明确边界：仅粉碎 Xray 生态，永久保留系统级并发与网络调优。
#   5. Xray 进程并发上限精准设定为 131072，严防低配机 Go Runtime OOM。
#   6. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash dx36.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray dx36 The Apex Vanguard - Project Genesis V8${none}"
