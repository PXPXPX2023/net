#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198.sh (The Apex Vanguard - Project Genesis V198)
# 快捷方式: xrv
# 【V198 终极版：拦截 cc1 x86-64-v1 非法参数、彻底消灭 VDSO 冲突、1380M Swap死锁】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198.sh"; exit 1; fi
...
_get_safe_march() {
    ...
    case "$lvl" in
        ...
        3) echo "x86-64-v3" ;;
        2) echo "x86-64-v2" ;;
        *) echo "x86-64" ;; # GCC 中没有 v1 这个参数，v1 对应的参数名就是 x86-64
    esac
}
...
_execute_compilation() {
    ...
    # 坚守防爆底线：无视机器核心数，为了 1GB/1380MB 环境绝对防 OOM 暴毙，死锁单线程
    THREADS=1
    
    info "=== 开启单核极限稳定编译 (死锁线程数: 1，彻底杜绝 OOM 暴毙) ==="
    
    if test -n "$make_flags"; then
        info ">>> 强制执行底层编译注入指令: $make_flags"
        if ! eval make -j"$THREADS" "$make_flags" "$extra_make_args"; then
    ...
    if test -n "$COMPILED_VER"; then 
        ...
        if command -v update-initramfs >/dev/null 2>&1; then
            info "正在重建 initramfs 引导镜像，确保 Virtio 驱动被正确装载..."
            update-initramfs -c -k "$COMPILED_VER" >/dev/null 2>&1 || true
        elif command -v dracut >/dev/null 2>&1; then
            info "正在通过 dracut 重建引导镜像..."
...
_compile_kernel_mainline() {
    ...
    if test -f "/boot/config-$(uname -r)"; then
        ...
        warn "未找到宿主机配置！启用 defconfig 并强制注入 KVM/Virtio 驱动防变砖..."
    ...
    # 写入架构优化配置
    ./scripts/config --set-str CONFIG_MARCH "x86-64-v${cpu_level}" 2>/dev/null || true
    ...
    _execute_compilation "" "KCFLAGS=\"-march=$final_march -fcf-protection=none\""
}
...
do_kernel_compile_menu() {
    while true; do
        clear
        title "极客内核源码锻造中心 (防变砖·防OOM单核版)"
        echo "  [已修复] vdso32 / cc1 编译架构冲突问题 (彻底禁用 32位兼容层)"
        echo "  [已修复] 重启断网变砖问题 (驱动 100% 继承并强注 Virtio)"
        echo "  [已生效] 强制锁死单线程编译，1380MB Swap 绝对免疫 OOM！"
        echo ""
        ...
        echo "     - 源码直连 GitLab API，自动选择最安全配置进行单线程慢烘"
        echo ""
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198 - (The Apex Vanguard)${none}"
...
#==============================================================================
# EOF - Apex Vanguard V198 System Advanced Control Ready.
#==============================================================================
