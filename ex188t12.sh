#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: exex188t12.sh (The Apex Vanguard - Pure Genesis V188t12)
# 快捷方式: xrv
#
# V188t12 终极原教旨修复日志:
#   1. 灵魂归位: 100% 还原 ex188.sh 中写死的 1GB 极简 Swap。
#   2. 参数归位: 100% 还原 ex188.sh 中强大的 do_perf_tuning (BBR/Sysctl 暴力写入集)。
#   3. 原装伪装: SNI 探针库完全替换为 ex188.sh 原版的 129 个顶级域名。
#   4. 限制解除: 移除源码编译阶段的 15GB 磁盘防爆强制校验。
#   5. 强硬路由: 增量融入一键强锁 IPv4 优先并物理熔断 IPv6 的安全策略。
#   6. 终极防御: 全面修复 set -euo pipefail 下 read 变量未绑定引发的终端闪退问题。
# ==============================================================================

# 检查 Bash 运行环境
if test -z "$BASH_VERSION"; then
    echo "Error: Please run this script with bash: bash exex188t12.sh"
    exit 1
fi
...
# ── 全局常量与路径 ──────────────────────────────────────────────
readonly SCRIPT_VERSION="ex188t12-Ultimate"
