#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex155.sh (The Apex Vanguard - Project Genesis V155 [Absolute Horizon])
# 快捷方式: xrv
# 终极溯源重铸宣言: 
#   1. 完美复刻 ex55.sh 编译逻辑：彻底抛弃 Xanmod 魔改源码的架构陷阱，改回从 kernel.org 拉取纯净主线内核。
#   2. 抛弃打包陷阱：使用最暴力的 make && make install 直接裸装，绝杀所有 Debian 证书与 dpkg-buildpackage 报错！
#   3. 1GB 永久 Swap 信仰：全量实装纯正 1GB 永久 Swap 管理器 (自动校验、多退少补、写死 fstab)。
#   4. 物理装甲全量回归：百万并发锁 (limits.conf)、60+ 行极限 Sysctl 网络栈参数、cc1.sh 守护进程与 resolvconf 底层锁死完整驻留。
#   5. 核心大一统：保留 28 项全域微操矩阵、回落探针 (菜单 8)、8 行军规排版、ShortId 防漂移联动删除。
#   6. 拒绝代码阉割：所有 jq 管道流与复杂的 if-else 逻辑全面恢复多行可读结构，严禁任何降低可读性的单行压缩。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex155.sh"
    exit 1
fi
...
check_and_create_1gb_swap() {
    ...
    if [[ -n "$CURRENT_SWAP" ]] && [[ "$CURRENT_SWAP" =~ ^1048 ]]; then
        ...
        info "1GB 永久 Swap 已重建并写入系统 fstab 底盘。"
    fi
}
...
# V153 绝杀修复: 彻底解决 Xanmod 编译打包与空指针陷阱，采用 make install 裸装！
do_xanmod_compile() {
    title "系统飞升：编译安装最新主线内核 + BBR3 (原装防爆版)"
    warn "警告: 这是一个漫长的过程 (30-60分钟)，低配机极易引发死机断连！"
    read -rp "确定要开始源码编译吗？(y/n): " confirm
    if [[ "$confirm" != "y" ]]; then 
        return
    fi

    print_magenta ">>> [1/6] 执行深度清理与初始化编译环境..."
    export DEBIAN_FRONTEND=noninteractive
    apt-get update -y
    ...
    # 强制 1GB 永久 Swap 校验，防止 OOM 宕机
    check_and_create_1gb_swap

    print_magenta ">>> [2/6] 拉取 Kernel 最新 Stable 稳定版源码 (完全取代出错的魔改版)..."
    ...
    if ! tar -tJf $KERNEL_FILE >/dev/null 2>&1; then
        rm -f $KERNEL_FILE
        wget -q --show-progress $KERNEL_URL -O $KERNEL_FILE
        tar -tJf $KERNEL_FILE >/dev/null 2>&1 || { error "下载或解压验证失败，已中止。"; return 1; }
    fi
    ...
    print_magenta ">>> [3/6] 注入原生防爆内核配置参数..."
    make defconfig
    make scripts
    ...
    print_magenta ">>> [4/6] 启动多线程源码裸编译 (彻底绕开 dpkg 打包陷阱)..."
    local CPU=$(nproc)
    ...
    # 复刻 ex55.sh：直接 make，抛弃容易报错的 make bindeb-pkg
    if ! make -j$THREADS; then
        error "内核编译遭遇致命错误，流程已熔断！"
        read -rp "按 Enter 返回主菜单..." _
        return 1
    fi

    print_magenta ">>> [5/6] 正在执行模块植入与内核挂载 (make install)..."
    make modules_install
    make install

    print_magenta ">>> [6/6] 清理编译扫尾工作..."
...
do_sys_init_menu() {
    while true; do
        ...
        echo "  4) 先完成3），编译安装最新主线内核 + BBR3 (原装防爆版)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex153 The Apex Vanguard - Project Genesis V153${none}"
