#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex188t34.sh (The Apex Vanguard - Ultimate Genesis 3000+)
# 快捷方式: xrv
# 版本号: V188t34.Mega.Unabridged.Perfect.Fusion
#
# 【V188t34 终极溯源全量排爆融合版】
#   1. 内核寻址修复: 完美融入 ex99.sh 预编译逻辑，新增 APT 动态智能寻址，彻底绝杀 Unable to locate package。
#   2. 源码编译重构: 完美继承 ex99.sh 极简 make install 源码编译引擎，剔除 bindeb-pkg 依赖断层。
#   3. JQ 绝缘屏障: 所有的 jq 数组操作全量加装 select(. != null)，根除回车炸弹与键值缺失闪退。
#   4. 绝对不压行: 所有的逻辑链条、Sysctl 与 SNI 矩阵垂直铺展，放弃代码长度换取绝对健壮性。
# ==============================================================================
...
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 本脚本深度依赖 Bash 高级特性，请执行: bash ex188t34.sh"
    exit 1
fi
...
# ------------------------------------------------------------------------------
# [ 0x08: Systemd 特种兵级提权 (强制 User=root 与百万并发防线) ]
# ------------------------------------------------------------------------------
...
do_install_xanmod_main_official() {
    ...
    if test -f "$cpu_level_script"; then
        # 极客级绕过 noexec 挂载: 直接 awk 解析，无需赋予执行权限。加入 || true 防止管道流截断熔断
    ...
    # ---------------------------------------------------------
    # 【智能动态包名寻址引擎】：事前探伤，免疫仓库改名报错
    # ---------------------------------------------------------
    if ! apt-cache show "$pkg_name" >/dev/null 2>&1; then
        warn "异常拦截：APT 仓库探伤失败，官方源目前未找到独立的 $pkg_name 安装包或包名已被重构。"
        warn "正在为您触发智能保护系统，安全回退至极度兼容的 linux-xanmod-x64v1 保底版本..."
        pkg_name="linux-xanmod-x64v1"
    fi
    
    # 容错降级循环安装，即使 v1 也加上管道容错
...
# ------------------------------------------------------------------------------
# [ 0x12: 编译安装原生 Linux 主线内核与 BBR3 强制封装 (内核原生直驱版) ]
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# [ 0x18: 系统级黑科技反转控制器 (Toggle Engines) ]
# ------------------------------------------------------------------------------
...
toggle_ring() {
    ...
    if test "$(check_ring_state)" = "true"; then
        local max_rx
        max_rx=$(ethtool -g "$IFACE" 2>/dev/null | grep -A4 "Pre-set maximums" | grep "RX:" | head -1 | awk '{print $2}' || echo "")
...
_turn_off_app() {
    # 即使关闭，也要加入 .sniffing = (.sniffing // {}) 容错，免疫因用户手动删减 JSON 节点导致的一键关闭爆破
    _safe_jq_write '
...
do_app_level_tuning_menu() {
    while true; do
        ...
        local gc_status="未知"
        if test -f "$limit_file"; then
            gc_status=$(awk -F'=' '/^Environment="GOGC=/ {print $3}' "$limit_file" 2>/dev/null | tr -d '"' | head -1 || echo "")
        ...
        local gso_off_state="unsupported" # 已被底层物理熔断
        ...
        if echo "$gc_status" | grep -q "100"; then app_off_count=$((app_off_count+1)); fi
        ...
        # ==========================================
        local s1
        if test "$out_fastopen" = "true"; then s1="${cyan}已开启${none}"; else s1="${gray}未开启${none}"; fi
        
        local s2
        if test "$out_keepalive" = "30"; then s2="${cyan}已开启 (30s/15s)${none}"; else s2="${gray}系统默认${none}"; fi
        
        local s3
        if test "$sniff_status" = "true"; then s3="${cyan}已开启${none}"; else s3="${gray}未开启${none}"; fi
        
        local s4
        if test "$dns_status" = "UseIP"; then s4="${cyan}已开启${none}"; else s4="${gray}未开启${none}"; fi
        
        local s6
        if test "$policy_status" = "60"; then s6="${cyan}已开启 (闲置60s/握手3s)${none}"; else s6="${gray}默认 300s 慢回收${none}"; fi
        
        local s7
        if test "$affinity_state" = "true"; then s7="${cyan}已锁死单核 (零切换)${none}"; else s7="${gray}默认 (系统调度)${none}"; fi
        
        local s8
        if test "$mph_state" = "true"; then s8="${cyan}O(1) 预编译算法就绪${none}"; else s8="${gray}默认 (Linear/AC机)${none}"; fi
        ...
        local s10
        if test "$routeonly_status" = "true"; then s10="${cyan}盲走快车道已通车${none}"; else s10="${gray}默认全量嗅探${none}"; fi
        
        local s11
        if test "$buffer_state" = "true"; then s11="${cyan}巨型重卡池 (64K)${none}"; else s11="${gray}默认轻型内存分配${none}"; fi
        
        local s12
        if test "$dnsmasq_state" = "true"; then s12="${cyan}极速内存解析中 (0.1ms)${none}"; else s12="${gray}依赖原生 DoH${none}"; fi
        
        local s13
        if test "$thp_state" = "true"; then s13="${cyan}已关闭 THP${none}"; elif test "$thp_state" = "unsupported"; then s13="${gray}不支持${none}"; else s13="${gray}系统默认${none}"; fi
        
        local s14
        if test "$mtu_state" = "true"; then s14="${cyan}智能探测中${none}"; elif test "$mtu_state" = "unsupported"; then s14="${gray}不支持${none}"; else s14="${gray}未开启${none}"; fi
        
        local s15
        if test "$cpu_state" = "true"; then s15="${cyan}全核火力全开${none}"; elif test "$cpu_state" = "unsupported"; then s15="${gray}不支持${none}"; else s15="${gray}节能待机${none}"; fi
        
        local s16
        if test "$ring_state" = "true"; then s16="${cyan}已反向收缩${none}"; elif test "$ring_state" = "unsupported"; then s16="${gray}不支持${none}"; else s16="${gray}系统大缓冲${none}"; fi
        
        local s17
        if test "$zram_state" = "true"; then s17="${cyan}已挂载 ZRAM${none}"; elif test "$zram_state" = "unsupported"; then s17="${gray}不支持${none}"; else s17="${gray}未启用${none}"; fi
        
        local s18
        if test "$journal_state" = "true"; then s18="${cyan}纯内存极速化${none}"; elif test "$journal_state" = "unsupported"; then s18="${gray}不支持${none}"; else s18="${gray}磁盘 IO 写入中${none}"; fi
        
        local s19
        if test "$prio_state" = "true"; then s19="${cyan}OOM免死 / IO抢占${none}"; else s19="${gray}系统默认调度${none}"; fi
        
        local s20
        if test "$cake_state" = "true"; then s20="${cyan}CAKE 削峰填谷中${none}"; else s20="${gray}默认 FQ 队列${none}"; fi
        
        local s21
        if test "$irq_state" = "true"; then s21="${cyan}多核 RPS / 单核绑死 自动激活${none}"; elif test "$irq_state" = "unsupported"; then s21="${gray}不支持(单核)${none}"; else s21="${gray}默认平衡调度${none}"; fi
        
        local s22="${gray}不支持 (底层驱动限制，已安全跳过)${none}"
        
        local s23
        if test "$ackfilter_state" = "true"; then s23="${cyan}绞杀空 ACK 释放上行${none}"; else s23="${gray}默认不干预${none}"; fi
        
        local s24
        if test "$ecn_state" = "true"; then s24="${cyan}显式拥塞警告 (0 丢包平滑降速)${none}"; else s24="${gray}默认 (传统暴力丢包)${none}"; fi
        
        local s25
        if test "$wash_state" = "true"; then s25="${cyan}强力清除干扰乱码${none}"; else s25="${gray}默认不干预${none}"; fi
        ...
        case "${app_opt:-}" in
            ...
            11)
                ...
                local _p=""; read -rp "按 Enter 继续..." _p || true
                ;;
            12) toggle_dnsmasq; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            13) toggle_thp; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            14) toggle_mtu; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            15) toggle_cpu; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            16) toggle_ring; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            17) toggle_zram; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            18) toggle_journal; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            19) 
                toggle_process_priority
                systemctl restart xray >/dev/null 2>&1 || true
                local _p=""; read -rp "按 Enter 继续..." _p || true 
                ;;
            20) toggle_cake; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            21) toggle_irq; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            22) check_gso_off_state >/dev/null 2>&1 || true; local _p=""; read -rp "该功能已被底层物理熔断保护，按 Enter 继续..." _p || true ;;
            23) toggle_ackfilter; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            24) toggle_ecn; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            25) toggle_wash; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            26)
...
# ------------------------------------------------------------------------------
# [ 0x1B: 伪装矩阵动态偏移重组核心 (安全 JQ 防爆重切引擎) ]
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# [ 0x1C: 核心通信协议框架部署 (完美匹配 UI 界面的纯原生 Bash 安装流) ]
# ------------------------------------------------------------------------------
...
do_install() {
    ...
    local proto_choice=""
    read -rp "  请输入编号 (默认 1): " proto_choice
...
do_summary() {
    ...
    local client_count
    client_count=$(jq '.inbounds[]? | select(.protocol=="vless" and .settings != null) | .settings.clients | length' "$CONFIG" 2>/dev/null || echo 0)
    
    if test "${client_count:-0}" -gt 0; then
        local port
        port=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .port // empty" "$CONFIG" 2>/dev/null || echo "")
        
        local pub
        pub=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .streamSettings.realitySettings.publicKey // empty" "$CONFIG" 2>/dev/null || echo "")
        
        local all_snis
        all_snis=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .streamSettings.realitySettings.serverNames | join(\", \")" "$CONFIG" 2>/dev/null || echo "")
        
        local main_sni
        main_sni=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .streamSettings.realitySettings.serverNames[0] // empty" "$CONFIG" 2>/dev/null || echo "")
        
        for ((i=0; i<client_count; i++)); do
            local uuid
            uuid=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .settings.clients[$i].id // empty" "$CONFIG" 2>/dev/null || echo "")
            
            local remark
            remark=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .settings.clients[$i].email // \"$REMARK_NAME\"" "$CONFIG" 2>/dev/null || echo "")
            
            local sid
            sid=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .streamSettings.realitySettings.shortIds[$i] // empty" "$CONFIG" 2>/dev/null || echo "")
    ...
    local s_count
    s_count=$(jq ".inbounds[]? | select(.protocol==\"shadowsocks\") | length" "$CONFIG" 2>/dev/null || echo 0)
    
    if test "${s_count:-0}" -gt 0; then
        local s_port
        s_port=$(jq -r ".inbounds[]? | select(.protocol==\"shadowsocks\") | .port // empty" "$CONFIG" 2>/dev/null || echo "")
        
        local s_pass
        s_pass=$(jq -r ".inbounds[]? | select(.protocol==\"shadowsocks\") | .settings.password // empty" "$CONFIG" 2>/dev/null || echo "")
        
        local s_method
        s_method=$(jq -r ".inbounds[]? | select(.protocol==\"shadowsocks\") | .settings.method // empty" "$CONFIG" 2>/dev/null || echo "")
...
# ------------------------------------------------------------------------------
# [ 0x1E: 用户管理 (增删/导入/专属 SNI 挂载) - 绝缘防爆重构版 ]
# ------------------------------------------------------------------------------
...
do_user_manager() {
    while true; do
        ...
        # ---------------------------------------------------------
        local clients
        clients=$(jq -r ".inbounds[]? | select(.protocol==\"vless\" and .settings != null) | .settings.clients[]? | select(.id != null) | .id + \"|\" + (.email // \"无备注\")" "$CONFIG" 2>/dev/null || echo "")
...
EOF
            if jq '(.inbounds[]? | select(.protocol=="vless" and .settings != null) | .settings.clients) += [input]' "$CONFIG" /tmp/new_client.json > "$CONFIG.tmp1" 2>/dev/null; then
                if jq --arg sid "$ns" '(.inbounds[]? | select(.protocol=="vless" and .streamSettings.realitySettings != null) | .streamSettings.realitySettings.shortIds) += [$sid]' "$CONFIG.tmp1" > "$CONFIG" 2>/dev/null; then
                    echo "$nu|$ctime" >> "$USER_TIME_MAP"
                    fix_permissions
                    systemctl restart xray >/dev/null 2>&1 || true
                    
                    local port
                    port=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .port // empty" "$CONFIG" 2>/dev/null | head -1 || echo "")
                    local sni
                    sni=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .streamSettings.realitySettings.serverNames[0] // empty" "$CONFIG" 2>/dev/null | head -1 || echo "")
                    local pub
                    pub=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .streamSettings.realitySettings.publicKey // empty" "$CONFIG" 2>/dev/null | head -1 || echo "")
...
EOF
            if jq '(.inbounds[]? | select(.protocol=="vless" and .settings != null) | .settings.clients) += [input]' "$CONFIG" /tmp/new_client.json > "$CONFIG.tmp1" 2>/dev/null; then
                if jq --arg sid "$m_sid" '(.inbounds[]? | select(.protocol=="vless" and .streamSettings.realitySettings != null) | .streamSettings.realitySettings.shortIds) += [$sid]' "$CONFIG.tmp1" > "$CONFIG" 2>/dev/null; then
                    ...
                    if test -n "$m_sni"; then
                        if jq --arg sni "$m_sni" '(.inbounds[]? | select(.protocol=="vless" and .streamSettings.realitySettings != null) | .streamSettings.realitySettings.serverNames) += [$sni] | (.inbounds[]? | select(.protocol=="vless" and .streamSettings.realitySettings != null) | .streamSettings.realitySettings.serverNames) |= unique' "$CONFIG" > "$CONFIG.tmp2" 2>/dev/null; then
                            mv -f "$CONFIG.tmp2" "$CONFIG" 2>/dev/null || true
            ...
            rm -f /tmp/new_client.json "$CONFIG.tmp1" "$CONFIG.tmp2" 2>/dev/null || true
            local _pause=""; read -rp "按 Enter 继续..." _pause || true
            ...
            if test -n "$target_uuid"; then
                ...
                if test -n "$u_sni"; then
                    if jq --arg sni "$u_sni" '(.inbounds[]? | select(.protocol=="vless" and .streamSettings.realitySettings != null) | .streamSettings.realitySettings.serverNames) += [$sni] | (.inbounds[]? | select(.protocol=="vless" and .streamSettings.realitySettings != null) | .streamSettings.realitySettings.serverNames) |= unique' "$CONFIG" > "$CONFIG.tmp" 2>/dev/null; then
                        ...
                        local idx=$(( ${snum:-0} - 1 ))
                        local sid
                        sid=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .streamSettings.realitySettings.shortIds[$idx] // empty" "$CONFIG" 2>/dev/null || echo "")
                        local pub
                        pub=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .streamSettings.realitySettings.publicKey // empty" "$CONFIG" 2>/dev/null | head -1 || echo "")
    ...
    while true; do
        ...
        case "${b_opt:-}" in
            1)
                ...
                if jq --argjson nv "$nv" '(.routing.rules[]? | select(.protocol != null) | select(.protocol | index("bittorrent")) | ._enabled) = $nv' "$CONFIG" > "$CONFIG.tmp" 2>/dev/null; then
                    mv -f "$CONFIG.tmp" "$CONFIG" 2>/dev/null || true
                    fix_permissions
                    systemctl restart xray >/dev/null 2>&1 || true
                    info "BT 屏蔽已成功切换为: $nv"
            ...
            2)
                ...
                if jq --argjson nv "$nv" '(.routing.rules[]? | select(.domain != null) | select(.domain | index("geosite:category-ads-all")) | ._enabled) = $nv' "$CONFIG" > "$CONFIG.tmp" 2>/dev/null; then
                    mv -f "$CONFIG.tmp" "$CONFIG" 2>/dev/null || true
                    fix_permissions
                    systemctl restart xray >/dev/null 2>&1 || true
                    info "广告屏蔽已成功切换为: $nv"
...
do_status_menu() {
    while true; do
        ...
        case "${s:-}" in
            1) 
                local current_day
                current_day=$(grep -E "^MonthRotate" /etc/vnstat.conf 2>/dev/null | awk '{print $2}' | head -n 1 | tr -d '\r\n' || echo "")
            ...
            4) 
                ...
                local m_day
                m_day=$(grep -E "^MonthRotate" /etc/vnstat.conf 2>/dev/null | awk '{print $2}' | head -n 1 | tr -d '\r\n' || echo "")
                m_day=${m_day:-"1 (默认)"}
                
                echo -e "  每月重置流量的计费日: ${cyan}每月 $m_day 号${none}"
                hr
                (vnstat -m 3 2>/dev/null || vnstat -m 2>/dev/null || true) | sed -e 's/estimated/估计流量/ig' -e 's/rx/接收/ig' -e 's/tx/发送/ig' -e 's/total/总计/ig' -e 's/daily/按天/ig' -e 's/monthly/按月/ig'
                hr
                ...
                case "${vn_opt:-}" in
                    1)
                        ...
                        if test -z "$d_month"; then
                            vnstat -d 2>/dev/null | sed -e 's/estimated/估计流量/ig' -e 's/rx/接收/ig' -e 's/tx/发送/ig' -e 's/total/总计/ig' -e 's/daily/按天/ig' -e 's/monthly/按月/ig' || true
                        else
                            if ! vnstat -d 2>/dev/null | grep -iE "($d_month| day |estimated|--)" | sed -e 's/estimated/估计流量/ig' -e 's/rx/接收/ig' -e 's/tx/发送/ig' -e 's/total/总计/ig' -e 's/daily/按天/ig' -e 's/monthly/按月/ig'; then
            ...
            6)
                while true; do
                    ...
                    if test -f "$limit_file"; then
                        if grep -q "^Nice=" "$limit_file" 2>/dev/null; then
                            current_nice=$(awk -F'=' '/^Nice=/ {print $2}' "$limit_file" | head -1 || echo "")
                        fi
                    fi
...
# ------------------------------------------------------------------------------
# [ 0x22: 最高防线控制中枢主屏幕展示区 (The Genesis Hub) ]
# ------------------------------------------------------------------------------
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex188t34 The Apex Vanguard - Project Genesis V188${none}"
