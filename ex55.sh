#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex55.sh (The Apex Vanguard - Project Genesis V20)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 深度勘误：修复服务状态查询 (选项9->1) 视图重叠失灵，强制禁用分页器并优化UI。
#   2. 极客新增：实装【动态 Nice 优先级提权】(选项9->5)，支持 -20 至 -10 档位调节与5秒重启。
#   3. 维持雷达守望模式 (选项9->4)：全域嗅探 ss -ntupa，2秒心跳自动刷新与 r 键瞬间强刷。
#   4. 维持跨代级架构：强制预编译打底防断连、4096 并发精简、Busy Poll 50 盲等。
#   5. 维持 NAPI 软中断配额：dev_weight=64，bias=1，杜绝算力独占引发的微观抖动。
#   6. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex55.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex55 The Apex Vanguard - Project Genesis V20${none}"
