#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex153.sh (The Apex Vanguard - Project Genesis V153 [Absolute Horizon])
# 快捷方式: xrv
# 终极溯源重铸宣言: 
#   1. 完美复刻 ex55.sh 编译逻辑：彻底抛弃 dpkg-buildpackage 打包流程，使用最暴力的 make && make install 直接裸装，绝杀所有 Debian 证书与依赖报错！
#   2. 绝杀架构空指针：彻底销毁对 CONFIG_X86_64_VERSION 的人为干预，信任内核原生 defconfig，根除 -march=x86-64-v 编译崩塌 Bug。
#   3. 1GB 永久 Swap 信仰：全量实装纯正 1GB 永久 Swap 管理器 (自动校验、卸载溢出、写死 fstab)。
#   4. 物理装甲全量回归：百万并发锁 (limits.conf)、60+ 行极限 Sysctl 网络栈参数、cc1.sh 守护进程与 do_change_dns 底层锁死完整驻留。
...
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex153.sh"
    exit 1
fi
...
do_xanmod_compile() {
    ...
    # 核心绝杀：只生成基础配置，绝不干预 CPU 架构选择，信任 Xanmod 默认配置！
    make defconfig
    make scripts
    ...
    print_magenta ">>> [4/5] 全核全速编译中 (绕过 Deb 打包，执行最稳定的裸装编译)..."
    local THREADS=$(nproc)
    local TOTAL_MEM=$(free -m | awk '/^Mem:/{print $2}')
    ...
    print_magenta ">>> [5/5] 正在执行模块植入与内核挂载 (make install)..."
