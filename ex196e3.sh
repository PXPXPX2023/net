#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e3.sh (The Apex Vanguard - Project Genesis V196e3)
# 快捷方式: xrv
#
# 【V196e3 终极勘误与真理重塑】
...
#   5. 完美融合: 全量继承 V193 的 JQ 绝缘护盾 (select(. != null))、严格模式防爆以及 25 项微操。
# ==============================================================================

# 必须用 bash 运行
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex196e3.sh"
    exit 1
fi
...
do_install_xanmod_main_official() {
    ...
    if test -f "$cpu_level_script"; then
        ...
        cpu_level=$("$cpu_level_script" 2>/dev/null | grep -oE 'x86-64-v[1-4]' | grep -oE '[1-4]' | tail -n 1 || echo "")
...
do_xanmod_compile() {
    ...
    # [核心级防御修复]: 绝杀 x86-64-v missing 参数 Bug，斩杀 yes "" 强制清空值！
    local arch_level="1"
    ...
    if wget -qO "$psabi_script" https://dl.xanmod.org/check_x86-64_psabi.sh 2>/dev/null; then
        ...
        arch_level=$("$psabi_script" 2>/dev/null | grep -oE 'x86-64-v[1-4]' | grep -oE '[1-4]' | tail -n 1 || echo "1")
        rm -f "$psabi_script" 2>/dev/null || true
    fi
    if test -z "$arch_level"; then arch_level="1"; fi
    set -e

    info "锁定宿主机 CPU 微架构级别为: x86-64-v${arch_level} (防御编译期段错误)"
    
    # 显式注入架构指令，保证 make olddefconfig 能抓取到合法值
...
do_txqueuelen_opt() {
    ...
    if test -z "$IP_CMD"; then
        error "系统缺失 iproute2 核心组件，无法调节网卡。"
        local _p=""; read -rp "按 Enter 返回..." _p || true
        return 1
    fi
    local IFACE=$(ip route get 1.1.1.1 2>/dev/null | awk '{print $5; exit}' || echo "")
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e3 - (The Apex Vanguard)${none}"
...
main_menu
# ==============================================================================
# EOF - Apex Vanguard V196e3 System Advanced Control Ready.
# ==============================================================================
