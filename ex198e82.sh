#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e82.sh (The Apex Vanguard - Project Genesis V198e82)
# 快捷方式: xrv
# 【V198e82 三协议稳态版：Reality Vision / VLESSENC Vision / SS2022 + tcp_notsent_lowat=26214 热更新默认】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e82.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e82"
...
hot_update_from_legacy() {
    ...
    if ! verify_e66_post_state; then
        ...
        warn "可以再次执行：bash ex198e82.sh --hot-update，或进入 10→5 手动应用。"
        return 1
    fi
    ensure_xray_is_alive || true
...
hot_update_from_legacy() {
    ...
    if ! verify_e68_post_state; then
        ...
        warn "可再次执行：bash ex198e82.sh --hot-update，或进入 10→6→28 / 10→5 手动应用。"
        return 1
    fi
...
hot_update_from_legacy() {
    ...
    if ! e69_verify_post_state; then
        ...
        warn "可再次执行：bash ex198e82.sh --hot-update；如 Xray 未运行，执行 journalctl -u xray --no-pager -n 80。"
        return 1
    fi
...
_e82_install_self_entrypoint() {
    ...
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        chmod +x "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e82.sh" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
        chmod +x "$SCRIPT_DIR/ex198e82.sh" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
    fi
    echo "198e82" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    hash -r 2>/dev/null || true
    info "V198e82 已安装/刷新快捷入口：$SYMLINK"
}
...
# V198e82 ${label}: TCP unsent write queue low watermark. 常用：26214=极低排队；32768=低延迟；65536=均衡；131072=高吞吐。
...
do_network_params_menu_e56() {
    while true; do
        clear
        title "网络参数应用中心（V198e82：均衡/高吞吐/手动 + notsent_lowat 永久管理；热更新默认26214）"
        show_network_buffer_params_e56
        _e82_show_tcp_notsent_lowat_state
        show_network_sysctl_conflicts_e58
        _e82_show_notsent_conflicts
...
hot_update_from_legacy() {
    title "V198e82：稳态热更新（保留 e81 能力 + 默认 tcp_notsent_lowat=26214）"
    set +e
    trap - ERR
    ...
    if declare -F e76_config_semantic_ok >/dev/null 2>&1 && e76_config_semantic_ok "$CONFIG"; then
        ...
        warn "未发现有效现网节点配置。V198e82 已安装为 xrv；请执行 xrv → 1 重新搭建。"
    fi
    ...
    set -e
    trap '_err_handler $? $LINENO "$BASH_COMMAND"' ERR
    info "V198e82 热更新完成。现在可输入：xrv；热更新默认 tcp_notsent_lowat=26214；进入 13 可切换 26214/32768/65536/131072/手动。"
    return 0
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e82 - (Reality / VLESSENC 自选 / SS2022 / notsent默认26214)${none}"
        ...
        echo "  9) 全景网络连接审计日志与商用级别网络流量记账本"
        echo "  10) 高级发烧：内核双轨编译(APT/源码)/网络/应用层 60余项极限全控板"
        echo -e "  ${cyan}11) 从旧 ex198e 环境稳态热更新到 V198e82${none}"
        echo -e "  ${cyan}12) 清理空间（安全释放 / 快照收敛 / 显示硬盘与 Swap）${none}"
...
#==============================================================================
# EOF - Apex Vanguard V198e82 tcp_notsent_lowat 26214 Control Ready.
#==============================================================================
