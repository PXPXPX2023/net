#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex188t16.sh (The Apex Vanguard - Pure Genesis V188t16)
# 快捷方式: xrv
#
# V188t16 终极原教旨修复日志:
#   1. 灵魂归位: 100% 还原 ex188.sh 中写死的 1GB 极简 Swap。
#   2. 参数归位: 100% 还原 do_perf_tuning (BBR/Sysctl 暴力写入集)。
...
#   6. 探针补全: 找回并物理植入遗失的 21 个 check_ 状态探针，修复面板闪退。
#   7. JSON净化: 彻底修复 x25519 密钥生成时带有不可见回车符导致 jq 解析暴毙的问题。
#   8. 终极防御: 全面修复 set -euo pipefail 下 read 变量未绑定引发的终端闪退。
# ==============================================================================

# 检查 Bash 运行环境
if test -z "$BASH_VERSION"; then
    echo "Error: Please run this script with bash: bash ex188t16.sh"
    exit 1
fi
...
# ── 全局常量与路径 ──────────────────────────────────────────────
readonly SCRIPT_VERSION="ex188t16-Ultimate"
