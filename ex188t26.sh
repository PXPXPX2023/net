#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex188t26.sh (The Apex Vanguard - Ultimate Genesis 3000)
# 快捷方式: xrv
# 版本号: V188t26.Mega.Unabridged
#
# 【V188t26 终极重装降临 - 拒绝任何代码压缩】
#   1. 完整溯源: 100% 继承 ex99.sh 的重型架构，找回丢失的 1400 行防御逻辑与注释。
#   2. 权限镇压: 强行将 Xray 提权至 User=root，彻底解决 nobody 用户在 Debian 11 下的权限断层。
#   3. 镜像防线: 为 Xray 核心安装加入 JsDelivr/Fastgit 镜像双轨下载，超时立即物理熔断。
#   4. 绝缘写入: 为所有 JQ 逻辑加装 select(. != null) 绝缘层，彻底根除 JSON 语法崩溃。
#   5. 满血面板: 恢复并增强「十五边形战神矩阵」，包含 CPU 绑核、ZRAM 阶梯挂载等全部逻辑。
#   6. 狂暴调优: 全量植入 ex188 系列最引以为傲的 200+ 行 Sysctl 极限高压网络栈。
# ==============================================================================
...
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 本脚本深度依赖 Bash 高级特性，请执行: bash ex188t26.sh"
    exit 1
fi
...
# ------------------------------------------------------------------------------
# [ 0x02: 全域 UI 引擎与十六进制常量 (完美继承 ex99) ]
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# [ 0x06: 权限强锁防线 (解决 Status 23 与 Permission Denied) ]
# ------------------------------------------------------------------------------
...
fix_permissions() {
    ...
    # 强制 Root 接管，防止 low-privilege 用户干扰导致启动失败
...
# ------------------------------------------------------------------------------
# [ 0x08: Systemd 特种兵级提权 (修复 nobody 暴毙与重构资源墙) ]
# ------------------------------------------------------------------------------

fix_xray_systemd_limits() {
    info "正在对 Xray 守护进程实施 Root 级越权管理与资源扩容..."
    ...
    # 强制以 User=root 运行，解除 CapabilityBoundingSet，赋予对底层 Sysctl 调优的绝对控制权
...
# ------------------------------------------------------------------------------
# [ 0x09: 环境预检与依赖大网 (Pre-flight Checks) ]
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# [ 0x0A: Geo 全球规则库无人值守更新引擎 (10分钟黄金错峰) ]
# ------------------------------------------------------------------------------
...
_toggle_affinity_on() {
    ...
    if test -f "$limit_file"; then
        ...
        local TARGET_CPU="0"
        # 预留核心 0 给系统硬中断，Xray 进程绑定到核心 1 (若为多核)
...
# ------------------------------------------------------------------------------
# [ 0x0C: 核心：130+ 实体 SNI 扫描引擎与智能避障 (全量无压缩) ]
# ------------------------------------------------------------------------------
...
verify_sni_strict() {
    ...
    print_magenta ">>> 检测标准: 强制 TLSv1.3 + ALPN h2 + OCSP Stapling 状态装订"
    
    # 投送深度探针，设置 5 秒超时防止线程死锁
    local out
    ...
    if ! echo "$out" | grep -qi "TLSv1.3"; then 
        print_red " ✗ 质检拦截: 目标服务器未开启 TLSv1.3 协议，Reality 握手将会暴露特征！"
        pass=0
    fi
    ...
    if ! echo "$out" | grep -qi "OCSP response:"; then 
        print_red " ✗ 质检拦截: 目标服务器未配置 OCSP Stapling 证书状态装订，易被防火墙阻断！"
        pass=0
    fi
    ...
    while true; do
        if test -f "$SNI_CACHE_FILE"; then
            ...
            if test "$sel" = "m" || test "$sel" = "M"; then
                ...
                # 安全转码构建 JSON 字符串数组，用于 Reality 的 serverNames
                local jq_args=()
                for s in "${arr[@]}"; do 
                    jq_args+=("\"$s\"")
                done
            ...
            if verify_sni_strict "$BEST_SNI"; then
                ...
                local force_use=""
                read -rp "是否无视警告，强制使用该残缺域名？(y/n): " force_use
...
do_install_xanmod_main_official() {
    ...
    # 废弃在 Debian 11 下极易报错的 /etc/apt/keyrings signed-by 新语法
...
# ------------------------------------------------------------------------------
# [ 0x12: 编译安装原生 Linux 主线内核与 BBR3 强制封装 ]
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# [ 0x13: 极限压榨：低延迟系统底层网络栈调优 (The Unabridged Sysctl Beast) ]
# ------------------------------------------------------------------------------

do_perf_tuning() {
    title "极限压榨：全量注入低延迟系统底层网络栈调优"
...
# 防止高并发大流量下 "Neighbor table overflow"
# ==========================================
...
EOSERVICE
    ...
    info "极限低延迟参数注入完成！为了使内核彻底消化重组内存，系统将在 30 秒后自动重启生效..."
    sleep 30
    reboot
}
# ------------------------------------------------------------------------------
# [ 0x14: 独立组件调优 - TX Queue 发送队列极速特化 ]
# ------------------------------------------------------------------------------

do_txqueuelen_opt() {
    title "网卡发送队列 (TX Queue) 极速并发特化调优"
...
_turn_on_app() {
    ...
    if test "$(check_dnsmasq_state)" = "true"; then
        _safe_jq_write '.dns = {"servers":["127.0.0.1"], "queryStrategy":"UseIP"}' "路由至 Dnsmasq"
    else
...
# ------------------------------------------------------------------------------
# [ 0x19: 十五边形战神矩阵选单 (上帝之手) ]
# ------------------------------------------------------------------------------

do_app_level_tuning_menu() {
    while true; do
        clear
        title "全域 15 项极限微操 (The 15-Panel Matrix UI)"
        ...
        if test "$gc_status" = "默认 100" || test "$gc_status" = "100"; then app_off_count=$((app_off_count+1)); fi
        ...
        local s2; if test "$out_keepalive" = "30"; then s2="${cyan}已开启 (30s/15s)${none}"; else s2="${gray}系统默认${none}"; fi
        local s3; if test "$sniff_status" = "true"; then s3="${cyan}已开启${none}"; else s3="${gray}未开启${none}"; fi
        local s4; if test "$dns_status" = "UseIP"; then s4="${cyan}已开启${none}"; else s4="${gray}未开启${none}"; fi
        local s6; if test "$policy_status" = "60"; then s6="${cyan}已开启 (闲置60s/握手3s)${none}"; else s6="${gray}默认 300s 慢回收${none}"; fi
        local s7; if test "$affinity_state" = "true"; then s7="${cyan}已锁死单核 (零切换损耗)${none}"; else s7="${gray}系统负载均衡${none}"; fi
        
        local s8; if test "$dnsmasq_state" = "true"; then s8="${cyan}本地内存极速解析中 (0.1ms)${none}"; else s8="${gray}未开启 (依赖原生 DoH)${none}"; fi
        local s9; if test "$thp_state" = "true"; then s9="${cyan}已关闭 THP${none}"; elif test "$thp_state" = "unsupported"; then s9="${gray}不支持${none}"; else s9="${gray}系统默认开启${none}"; fi
        local s10; if test "$mtu_state" = "true"; then s10="${cyan}智能防堵探测中${none}"; elif test "$mtu_state" = "unsupported"; then s10="${gray}不支持${none}"; else s10="${gray}未开启${none}"; fi
        local s11; if test "$cpu_state" = "true"; then s11="${cyan}全核火力全开模式${none}"; elif test "$cpu_state" = "unsupported"; then s11="${gray}不支持${none}"; else s11="${gray}节能省电待机${none}"; fi
        local s12; if test "$ring_state" = "true"; then s12="${cyan}已极限反向收缩 (降低延迟)${none}"; elif test "$ring_state" = "unsupported"; then s12="${gray}不支持${none}"; else s12="${gray}系统大缓冲 (吞吐优先)${none}"; fi
        local s13; if test "$zram_state" = "true"; then s13="${cyan}已挂载 ZRAM 压缩引擎${none}"; elif test "$zram_state" = "unsupported"; then s13="${gray}不支持${none}"; else s13="${gray}未启用${none}"; fi
        local s14; if test "$journal_state" = "true"; then s14="${cyan}纯内存极速化${none}"; elif test "$journal_state" = "unsupported"; then s14="${gray}不支持${none}"; else s14="${gray}磁盘 IO 写入中${none}"; fi
        local s15; if test "$prio_state" = "true"; then s15="${cyan}OOM 免死保护 / IO 抢占提权${none}"; else s15="${gray}系统默认调度${none}"; fi
        ...
        echo -e "  7) 开启或关闭 Xray 进程物理绑核 & GOMAXPROCS 并发锁              | 当前状态: $s7"
        echo -e ""
        echo -e "  ${magenta}--- Linux 系统层与内核黑科技 (8-15) ---${none}"
        echo -e "  8)  开启或关闭【Dnsmasq 本地极速内存缓存引擎 (21000并发/锁TTL)】 | 当前状态: $s8"
        echo -e "  9)  开启或关闭【透明大页 (THP - Transparent Huge Pages)】        | 当前状态: $s9"
        echo -e "  10) 开启或关闭【TCP PMTU 黑洞智能探测 (Probing=1)】              | 当前状态: $s10"
        echo -e "  11) 开启或关闭【CPU 频率调度器锁定 (Performance)】               | 当前状态: $s11"
        echo -e "  12) 开启或关闭【网卡硬件环形缓冲区 (Ring Buffer) 反向收缩】      | 当前状态: $s12"
        echo -e "  13) 开启或关闭【ZRAM】(淘汰慢速 Swap，阶梯内存自动挂载)          | 当前状态: $s13"
        echo -e "  14) 开启或关闭【日志系统 Journald 纯内存化】(斩断 I/O 羁绊)      | 当前状态: $s14"
        echo -e "  15) 开启或关闭【系统进程级防杀抢占 (OOM/IO 提权)】               | 当前状态: $s15"
        echo -e "  "
        echo -e "  ${cyan}16) 一键开启或关闭 1-7 项 应用层微操 (自动智能侦测并反转)${none}"
        echo -e "  ${yellow}17) 一键开启或关闭 8-15 项 系统级微操 (自动避障侦测并反转)${none}"
        echo -e "  ${red}18) 上帝之手：一键重置翻转 1-15 项 全域微操 (执行后强制重组并重启系统)${none}"
        echo "  0) 缩回面板，返回上一级控制中枢"
        hr
        
        local app_opt=""
        read -rp "请下达指挥指令: " app_opt || true
        ...
        case "${app_opt:-}" in
            ...
            5)
                ...
                if test -f "$limit_file"; then
                    ...
                    if grep -q "Environment=\"GOGC=" "$limit_file" 2>/dev/null; then
                        if test "$gc_status" = "默认 100" || test "$gc_status" = "100"; then
                            sed -i "s/^Environment=\"GOGC=.*/Environment=\"GOGC=$DYNAMIC_GOGC\"/" "$limit_file" 2>/dev/null || true
                            info "GOGC 动态阶梯极速飙车模式已开启！系统将赋予核心更多的内存占用权限。阈值: ${DYNAMIC_GOGC}"
                        else
                            sed -i "s/^Environment=\"GOGC=.*/Environment=\"GOGC=100\"/" "$limit_file" 2>/dev/null || true
                    ...
                    error "缺失 Limit 配置文件，可能核心网络未完全部署！"
                fi
                local _p=""; read -rp "按 Enter 继续..." _p || true
                ;;
            6)
            ...
            16)
                if test "$app_off_count" -gt 0; then
                    print_magenta ">>> 检测到您的应用层防爆参数未满配，正在为您执行全域强力装载并重构..."
                    _turn_on_app
                    systemctl restart xray >/dev/null 2>&1 || true
                    info "1-7 项 Xray 内部应用层防爆参数已全量就绪并上线服役！"
                else
                    print_magenta ">>> 检测到当前应用层处于饱和装载状态，正在为您执行一键拆卸还原..."
                    _turn_off_app
                    systemctl restart xray >/dev/null 2>&1 || true
                    info "1-7 项配置已完全强制拆卸，无损回归出厂原装生态！"
                fi
                local _p=""; read -rp "按 Enter 继续..." _p || true
                ;;
            17)
                if test "$sys_off_count" -gt 0; then
                    ...
                    systemctl restart xray >/dev/null 2>&1 || true
                    info "8-15 项系统级极限物理微操已全部完成一键激活封口 (不支持的硬件已安全避障)！"
                else
                    print_magenta ">>> 正在剥离系统内核级的超频状态，恢复为保底兼容模式..."
            ...
            18)
                if test "$((app_off_count + sys_off_count))" -gt 0; then
                    if test "$app_off_count" -gt 0; then 
                        print_magenta ">>> 正在全域强力装载并锁定应用层引擎防线..."
                        _turn_on_app
                    fi
                    
                    if test "$sys_off_count" -gt 0; then
                        print_magenta ">>> 正在穿透底层唤醒硬件极限超频黑科技，可能触发硬件避障..."
                        if test "$dnsmasq_state" = "false"; then toggle_dnsmasq >/dev/null 2>&1 || true; fi
                        if test "$thp_state" = "false"; then toggle_thp >/dev/null 2>&1 || true; fi
                        if test "$mtu_state" = "false"; then toggle_mtu >/dev/null 2>&1 || true; fi
                        if test "$cpu_state" = "false"; then toggle_cpu >/dev/null 2>&1 || true; fi
                    ...
                    info "神威全开！1-15 项全域极客微操调优已【全部满血满载】！(系统不支持项已智能绕过)"
                else
                    print_magenta ">>> 接收到最高重置许可，正在强行融化并卸载双层架构超频引擎..."
                    _turn_off_app
                    if test "$dnsmasq_state" = "true"; then toggle_dnsmasq >/dev/null 2>&1 || true; fi
                    if test "$thp_state" = "true"; then toggle_thp >/dev/null 2>&1 || true; fi
                    if test "$mtu_state" = "true"; then toggle_mtu >/dev/null 2>&1 || true; fi
                    ...
                    info "神威强行收敛！1-15 项全域超频修改已被【全部剥离重置】，彻底回归底层原生出厂生态！"
                fi
                
                # [继承 ex99 灵魂：6 秒强制倒计时重启]
                echo ""
                print_red "======================================================================"
                print_yellow "  高能警报：系统全域拓扑与内核队列状态已在物理层面发生不可逆变更！"
                print_yellow "  为了确保所有的 CPU 调度重组、ZRAM 内存阶梯机制与网卡环形列阵完美挂载，"
...
# ------------------------------------------------------------------------------
# [ 0x1A: 伪装矩阵动态偏移重组核心 (无损切换引擎) ]
# ------------------------------------------------------------------------------

_update_matrix() {
    if test ! -f "$CONFIG"; then 
        error "无法执行矩阵切换：尚未部署核心配置文件。"
        return 1
    fi
    
    info "启动伪装防线偏移逻辑（启用安全隔离写入防爆机制）..."
    
    # 构造临时 JSON 数组池以供 jq 深度解析
...
# ------------------------------------------------------------------------------
# [ 0x1B: 核心通信协议框架部署 (纯原生 Bash HereDoc + 绝缘 JSON 注入) ]
# ------------------------------------------------------------------------------
...
do_install() {
    ...
    # 严控计费周期：如果已有安装日期，则不再覆盖，保证历代升级记录统一
    if test ! -f "$INSTALL_DATE_FILE"; then
        date +"%Y-%m-%d %H:%M:%S" > "$INSTALL_DATE_FILE"
    fi
    
    echo -e "  ${cyan}请指派您需要部署的代理协议架构：${none}"
    echo "  1) VLESS-Reality (推荐, 拥有 XTLS-Vision 防御墙，极度隐匿)"
    echo "  2) Shadowsocks-2022 (轻量化突发引擎，建议纯境外部署使用)"
    echo "  3) 混合双轨防御阵列 (并行同时部署上述两套协议)"
    
    local proto_choice=""
    read -rp "  请输入阵列编号 (默认 1): " proto_choice
    ...
    if test "$xray_installed" -eq 0; then
        die "核心获取全面溃败：所有官方源与 CDN 镜像站均遭到网络阻断！部署防线已自动物理熔断，系统拒绝生成危险的空壳配置！"
    fi
    ...
    # 1. 纯净构建底层骨架配置 (注入 AsIs 斩断路由层多余的 DNS 解析，封锁特征)
    cat > "$CONFIG" <<EOF
{
  "log": {
    "loglevel": "warning"
...
EOF
    ...
    if test "$proto_choice" = "1" || test "$proto_choice" = "3"; then
        ...
        # 绝对纯净的 HereDoc JSON 模块生成 (附带 tcpFastOpen 底层加速)
        echo "[$SNI_JSON_ARRAY]" > /tmp/sni_array.json
...
do_summary() {
    ...
    title "The Apex Vanguard 节点架构与全景分发中心"
        ...
        if test "${client_count:-0}" -gt 0; then
            ...
            for ((i=0; i<client_count; i++)); do
                ...
                if test -n "$uuid" && test "$uuid" != "null"; then 
                    hr
                    printf "  ${cyan}【VLESS-Reality (Vision) 混合防御阵列 - 独立用户序列: %d】${none}\n" $((i+1))
                    ...
                    echo -e "\n  ${green}[数据链路] 客户端导入长链:${none}\n  $link\n"
                    
                    if command -v qrencode >/dev/null 2>&1; then 
                        echo -e "  ${cyan}[移动端] 手机扫码导入矩阵:${none}"
    ...
    if test -n "$ss_inbound" && test "$ss_inbound" != "null"; then
        ...
        if test -n "$s_port" && test "$s_port" != "null"; then
            hr
            printf "  ${cyan}【Shadowsocks 轻量化低延迟阵列】${none}\n"
            printf "  ${yellow}%-16s${none} %s\n" "节点名称:" "${REMARK_NAME}-SS"
            printf "  ${yellow}%-16s${none} %s\n" "物理入网 IP:" "$SERVER_IP"
            printf "  ${yellow}%-16s${none} %s\n" "系统监听端口:" "$s_port"
            printf "  ${yellow}%-16s${none} %s\n" "鉴权密钥短链:" "$s_pass"
    ...
    hr
    echo -e "  ${gray}系统配置文件绝对物理坐标: $CONFIG${none}"
    echo -e "  ${gray}安全灾备快照中心锚点坐标: $BACKUP_DIR${none}"
}
...
do_user_manager() {
    while true; do
        clear
        title "多用户与认证生命周期管理中心 (绝缘防爆重构版)"
        
        if test ! -f "$CONFIG"; then 
            error "未发现系统底层核心配置文件，请先完成节点安装部署！"
            local _pause=""
        ...
        if test -z "$clients" || test "$clients" = "null"; then 
            error "错误侦测：未在当前内核运行中发现 VLESS 节点协议！该面板仅为 Reality 用户分配矩阵。"
            local _pause=""
            read -rp "按 Enter 返回主菜单..." _pause || true
            return
        fi

        # 构建临时用户池映射矩阵，辅助后续的数字序号操作
        local tmp_users="/tmp/xray_users_pool.txt"
        echo "$clients" | awk -F'|' '{print NR"|"$1"|"$2}' > "$tmp_users"
        
        echo -e "  ${cyan}【当前服务器挂载的活跃用户与分发名单】${none}"
        ...
        while IFS='|' read -r num uid remark; do
            ...
            echo -e "  $num) 节点身份备注: ${cyan}$remark${none} | 物理签发: ${gray}$utime${none} | 核心 UUID: ${yellow}$uid${none}"
        done < "$tmp_users"
        hr
        
        echo "  a) 为本机签发新增独立凭证 (将自动生成全新的 UUID 与 ShortId)"
        echo "  m) 导入外部系统遗留老用户 (可平滑继承原有 UUID/ShortId 以防失联)"
        echo "  s) 重新分配特定用户的独立 SNI (专属伪装域名调度)"
        echo "  d) 依据物理序号执行用户的销毁删除指令"
        echo "  q) 折叠并退出管理大厅"
        
        local uopt=""
        read -rp "  请您下达指挥指令: " uopt || true
        ...
        if test "$uopt" = "a" || test "$uopt" = "A"; then
            ...
            # 采用绝缘层的安全注入
            if jq --arg id "$nu" --arg email "$u_remark" --arg sid "$ns" '
                (.inbounds[]? | select(.protocol=="vless" and .settings != null) | .settings.clients) += [{"id": $id, "flow": "xtls-rprx-vision", "email": $email}] |
                (.inbounds[]? | select(.protocol=="vless" and .streamSettings.realitySettings != null) | .streamSettings.realitySettings.shortIds) += [$sid]
            ' "$CONFIG" > "$CONFIG.tmp" 2>/dev/null; then
                ...
                if verify_xray_config "$CONFIG.tmp"; then
                    ...
                    fix_permissions
                    systemctl restart xray >/dev/null 2>&1 || true
                    info "新用户认证凭证 [ ${u_remark} ] 已成功被内嵌至底层阵列并物理激活！"
                else
            ...
            local _pause=""
            read -rp "指令确认完毕，按 Enter 继续返回上一级..." _pause || true
            
        elif test "$uopt" = "m" || test "$uopt" = "M"; then
            hr
            echo -e " ${cyan}>>> 外部战损级老用户平滑迁移补全计划 <<<${none}"
            echo -e " ${yellow}安全提示: 将外部失联用户的凭证重新挂载到本机，生成由本机物理 IP 和新公钥构建的回溯长链！${none}"
            ...
            if test -z "$m_sid"; then 
                error "会话 ShortId 不能为空，参数断层！"
                sleep 2
                continue
            fi
            
            local ctime
            ctime=$(date +"%Y-%m-%d %H:%M (导入)")
            
            info "系统正在构建隔离沙盒，执行数据流迁移合并..."
            if jq --arg id "$m_uuid" --arg email "$m_remark" --arg sid "$m_sid" '
                (.inbounds[]? | select(.protocol=="vless" and .settings != null) | .settings.clients) += [{"id": $id, "flow": "xtls-rprx-vision", "email": $email}] |
                (.inbounds[]? | select(.protocol=="vless" and .streamSettings.realitySettings != null) | .streamSettings.realitySettings.shortIds) += [$sid]
            ' "$CONFIG" > "$CONFIG.tmp" 2>/dev/null; then
                ...
                if verify_xray_config "$CONFIG.tmp"; then
                    ...
                    if test -n "$m_sni"; then
                        if jq --arg sni "$m_sni" '
                            (.inbounds[]? | select(.protocol=="vless" and .streamSettings.realitySettings != null) | .streamSettings.realitySettings.serverNames) += [$sni] | 
                            (.inbounds[]? | select(.protocol=="vless" and .streamSettings.realitySettings != null) | .streamSettings.realitySettings.serverNames) |= unique
                        ' "$CONFIG" > "$CONFIG.tmp_sni" 2>/dev/null; then
                            mv -f "$CONFIG.tmp_sni" "$CONFIG"
                            sed -i "/^$m_uuid|/d" "$USER_SNI_MAP" 2>/dev/null || true
                            echo "$m_uuid|$m_sni" >> "$USER_SNI_MAP"
                            info "已为导入用户精准绑定物理级专属 SNI: $m_sni"
                        fi
                    fi
                    
                    systemctl restart xray >/dev/null 2>&1 || true
                    info "外部失联用户已成功回拨至当前雷达中枢，物理生态并轨完毕！"
                else
                    error "验证引擎阻断，配置落盘失败。"
                    rm -f "$CONFIG.tmp" 2>/dev/null || true
                fi
            ...
            local _pause=""
            read -rp "按 Enter 继续返回上一级..." _pause || true
            ...
            if test -n "$target_uuid"; then
                ...
                if test -n "$u_sni"; then
                    ...
                    ' "$CONFIG" > "$CONFIG.tmp" 2>/dev/null; then
                        ...
                        fix_permissions
                        systemctl restart xray >/dev/null 2>&1 || true
                        info "动态指令下发完毕，该用户已被强行绑定独立通道: $u_sni"
                    else
                        error "域名挂载遭遇底层断层拦截。"
                    fi
                fi
            ...
            local dnum=""
            read -rp "【高危指令】请输入要销毁的用户物理序号: " dnum
            
            local total
            total=$(wc -l < "$tmp_users" 2>/dev/null || echo 0)
            
            if test "${total:-0}" -le 1; then
                error "权限越界拦截：您必须在底层防线内保留至少一个上帝级管理员账号！"
            else
                local target_uuid
                ...
                if test -n "$target_uuid"; then
                    ...
                    if jq --arg uid "$target_uuid" --argjson i "$idx" '
                        ...
                        (.inbounds[]? | select(.protocol=="vless" and .streamSettings.realitySettings != null) | .streamSettings.realitySettings.shortIds) |= del(.[$i])
                    ' "$CONFIG" > "$CONFIG.tmp" 2>/dev/null; then
                        ...
                        if verify_xray_config "$CONFIG.tmp"; then
                            ...
                            fix_permissions
                            systemctl restart xray >/dev/null 2>&1 || true
                            info "歼灭协议已完成，该用户及其在系统中衍生的对应凭证已被深度物理蒸发。"
                        else
                            error "内核预审阻断了抹杀行为，防止配置失控崩溃。"
                            rm -f "$CONFIG.tmp" 2>/dev/null || true
                        fi
                    else
                        error "底层 JQ 数据流清洗过程遭遇崩溃。"
                    fi
                else
                    error "无效序号映射，销毁指令作废。"
                fi
...
_global_block_rules() {
    while true; do
        ...
        echo -e "  1) 向防火墙黑洞追加【高危域名/拦截规则组】(如: geosite:category-porn 或 baidu.com)"
        echo -e "  2) 向防火墙黑洞追加【高危物理 IP/区域网段】(如: geoip:cn 或 1.1.1.1)"
        echo -e "  3) 切换底层网络栈 BT/PT/P2P 数据流协议全域物理拦截 | 当前雷达状态: ${cyan}${bt_en}${none}"
        echo -e "  4) 切换全球隐秘广告追踪流与非法遥测源黑洞阻断探针   | 当前雷达状态: ${cyan}${ad_en}${none}"
        echo "  0) 折叠大屏幕并返回主控制台"
        hr
        
        local b_opt=""
        read -rp "请您指定行动干预代码: " b_opt || true
        
        case "${b_opt:-}" in
            1)
                local block_domain=""
                read -rp "请输入要强行引向黑洞路由的域名特征规则: " block_domain
                if test -n "$block_domain"; then
                    info "注入配置安全保护墙，正在拼接底层路由指令..."
                    if jq --arg dom "$block_domain" '
                        (.routing.rules[]? | select(.outboundTag=="block" and .domain != null) | .domain) += [$dom] |
                        (.routing.rules[]? | select(.outboundTag=="block" and .domain != null) | .domain) |= unique
                    ' "$CONFIG" > "$CONFIG.tmp" 2>/dev/null; then
                        if verify_xray_config "$CONFIG.tmp"; then
                            mv -f "$CONFIG.tmp" "$CONFIG"
                            systemctl restart xray >/dev/null 2>&1 || true
                            info "物理封锁令已下发，域名规则 [ $block_domain ] 已成功被填装进内核黑洞引力网！"
                        else
                            error "内核预审阻断，您的输入可能触发了循环崩溃点。"
                            rm -f "$CONFIG.tmp" 2>/dev/null || true
                        fi
                    fi
                fi
            ...
            2)
                local block_ip=""
                read -rp "请输入要强行实施核级切断的 IP 或 CDIR 子网网段: " block_ip
                if test -n "$block_ip"; then
                    info "拉取系统层路由缓存..."
                    if jq --arg ip_addr "$block_ip" '
                        (.routing.rules[]? | select(.outboundTag=="block" and .ip != null) | .ip) += [$ip_addr] |
                        (.routing.rules[]? | select(.outboundTag=="block" and .ip != null) | .ip) |= unique
                    ' "$CONFIG" > "$CONFIG.tmp" 2>/dev/null; then
                        if verify_xray_config "$CONFIG.tmp"; then
                            mv -f "$CONFIG.tmp" "$CONFIG"
                            systemctl restart xray >/dev/null 2>&1 || true
                            info "断层切割完成，IP 物理封锁矩阵 [ $block_ip ] 已成功映射至底层黑洞深渊！"
                        else
                            error "内核引擎无法容忍该异常 IP 规格，配置链写入遭拦截。"
                            rm -f "$CONFIG.tmp" 2>/dev/null || true
                        fi
                    fi
                fi
            ...
            3)
                ...
                info "正在向网络层广播指令..."
                if jq --argjson val "$nv" '
                    (.routing.rules[]? | select(.protocol != null) | select(.protocol | index("bittorrent")) | ._enabled) = $val
                ' "$CONFIG" > "$CONFIG.tmp" 2>/dev/null; then
                    mv -f "$CONFIG.tmp" "$CONFIG"
                    fix_permissions
                    systemctl restart xray >/dev/null 2>&1 || true
            ...
            4)
                ...
                info "正在切换广告黑洞阻隔引擎..."
                if jq --argjson val "$nv" '
                    (.routing.rules[]? | select(.domain != null) | select(.domain | index("geosite:category-ads-all")) | ._enabled) = $val
                ' "$CONFIG" > "$CONFIG.tmp" 2>/dev/null; then
                    mv -f "$CONFIG.tmp" "$CONFIG"
                    fix_permissions
                    systemctl restart xray >/dev/null 2>&1 || true
                    info "广域遥测截断探针已被成功切换部署为: $nv"
                fi
                local _p=""; read -rp "按 Enter 继续..." _p || true
                ;;
            0) 
...
# ------------------------------------------------------------------------------
# [ 0x1F: Reality 回落探测与安全状态雷达核算矩阵 ]
# ------------------------------------------------------------------------------

do_fallback_probe() {
    clear
    title "Reality 物理回落防盗扫探针 (Fallback / 阈值态势审计)"
    info "系统防盗扫阈值与边界限制 (limitFallbackUpload/Download) 配置已安全定格闭环。"
    info "为防止误操作引发宕机，此处仅为您唤醒只读的安全底层探针..."
    
    echo -e "${gray}"
    cat "$CONFIG" 2>/dev/null | grep -E -A 10 '"realitySettings"' || true
    echo -e "${none}"
    
    local _pause=""
    read -rp "底层情报数据流移交完毕，按 Enter 键断开并返回中枢..." _pause || true
}

do_status_menu() {
    while true; do
        clear
        title "商用级全景网络连接审计与自然月流量核算监控系统"
        
        echo "  1) 定制核心结算：设置/修改 每月网卡结算账单清零的截断日期"
        echo "  2) 查看 Xray 系统核心底层守护进程健康度及事件循环"
        echo "  3) 审查本机物理网络入口、外部探针池及系统级 DNS 解析树"
        echo "  4) [最高权限] 呼叫 vnstat 调取网卡全域双向物理流量核算总账"
        echo "  5) [高能预警] 唤醒实时监听探针：追踪高并发网络握手与独立外部 IP 源探测"
        echo "  6) 审查全量日志：Xray 应用层事件与系统层面异常断层拦截轨迹"
        echo "  7) 启用时空备份系统：手动签署配置快照或执行灾难级自动回滚"
        echo "  0) 闭合所有监控雷达网，逃离至系统主控界面"
        hr
        
        local s=""
        read -rp "请您亲自下达监控阵列的调度代码: " s || true
        
        case "${s:-}" in
            1) 
                local current_day
                current_day=$(grep -E "^[[:space:]]*MonthRotate" /etc/vnstat.conf 2>/dev/null | awk '{print $2}' | head -n 1 | tr -d '\r\n')
                current_day=${current_day:-"1"}
                
                echo -e "  引擎核对：当前设置的流量账单结算清零日为每月 ${cyan}${current_day} 号${none}"
                
                local d_day=""
                read -rp "请输入您期望的账单强行截断日期 (输入区间 1-31): " d_day
                ...
                if test "${d_day:-0}" -ge 1 2>/dev/null && test "${d_day:-0}" -le 31 2>/dev/null; then
                    ...
                    info "截断权限已改写，所有的统计图谱将在每月的 $d_day 号凌晨被系统自动物理封印清零。"
                else
                    error "输入的日期参数规格异常，执行已被底座拦截。"
                fi
                local _p=""; read -rp "按 Enter 退出结算接口..." _p || true
                ;;
            2) 
                clear
                ...
                echo ""
                local _p=""; read -rp "状态流探查完成，按 Enter 脱离..." _p || true 
                ;;
            3) 
                clear
                title "物理链路、DNS 与端口池态势"
                echo -e "\n  核心广域网物理出口信标 IP: ${green}$SERVER_IP${none}"
                hr
                echo -e "  本地 DNS 硬件层级解析寻址堆栈: "
                grep "^nameserver" /etc/resolv.conf 2>/dev/null | awk '{print "    " $0}' || echo "    获取栈信息发生错误"
                hr
                echo -e "  系统当前 Xray 端口池底层占用清单 (LISTEN): "
                ss -tlnp 2>/dev/null | grep xray | awk '{print "    " $4}' || echo "    警告：未发现任何属于 Xray 的系统级网络探针挂载！"
                echo ""
                local _p=""; read -rp "审计任务完结，按 Enter 返回..." _p || true 
                ;;
            4) 
                if ! command -v vnstat >/dev/null 2>&1; then 
                    error "您还未在物理主机上布署 Vnstat 流量探测引擎矩阵！"
                    local _p=""; read -rp "按 Enter 返回..." _p || true
                    continue
                fi
                
                clear
                title "商用级原生硬件网卡流量计费审计终端"
                
                local m_day
                m_day=$(grep -E "^[[:space:]]*MonthRotate" /etc/vnstat.conf 2>/dev/null | awk '{print $2}' | head -n 1 | tr -d '\r\n')
                m_day=${m_day:-"1 (系统原生保护默认)"}
                echo -e "  当前硬件月度重置流量计费日设定: ${cyan}每月 $m_day 号${none}"
                hr
                
                # 双重保险调用 vnstat，并对输出字段进行完美的中文化正则翻译
                (vnstat -m 3 2>/dev/null || vnstat -m 2>/dev/null || true) | sed -e 's/estimated/模型推演消耗/ig' -e 's/rx/全双工接收/ig' -e 's/tx/全双工发送/ig' -e 's/total/宏观物理总计/ig' -e 's/daily/以天为维度分流/ig' -e 's/monthly/以自然月为维度分流/ig'
                hr
                
                echo "  1) 开启时空回溯：指定年月查询自然维度的详细流量图谱报表 (如查询 $(date +%Y-%m))"
                echo "  0) 折叠流量面板，断开终端连接"
                
                local vn_opt=""
                read -rp "  向审计层下达调用指令: " vn_opt || true
                
                if test "$vn_opt" = "1"; then
                    local d_month=""
                    read -rp "请输入要查询的特定历史年月节点 (直接按回车系统将自动呈现最近 30 天汇总): " d_month
                    
                    if test -z "$d_month"; then
                        vnstat -d 2>/dev/null | sed -e 's/estimated/模型推演消耗/ig' -e 's/rx/全双工接收/ig' -e 's/tx/全双工发送/ig' -e 's/total/宏观物理总计/ig' -e 's/daily/以天为维度分流/ig' -e 's/monthly/以自然月为维度分流/ig' || true
                    else
                        vnstat -d 2>/dev/null | grep -iE "($d_month| day |estimated|--)" | sed -e 's/estimated/模型推演消耗/ig' -e 's/rx/全双工接收/ig' -e 's/tx/全双工发送/ig' -e 's/total/宏观物理总计/ig' -e 's/daily/以天为维度分流/ig' -e 's/monthly/以自然月为维度分流/ig' || true
                    fi
                    local _p=""; read -rp "按 Enter 退离图谱展示..." _p || true
                fi
                ;;
            5)
                while true; do
                    clear
                    title "战区特级防空：实时外网连接追踪与访客 IP 溯源雷达"
                    ...
                    if test -n "$x_pids"; then
                        echo -e "  ${cyan}【底层握手套接字与并发堆栈链路状态】${none}"
                        ss -ntupa 2>/dev/null | grep -E "pid=($x_pids)[,)]" | awk '{print $1"_"$2}' | sort | uniq -c | sort -nr | awk '{printf "    网络握手状态: %-15s : 当前并行活跃吞吐量 %s\n", $2, $1}' || echo "    静默侦测中... 当前没有任何 TCP 层握手建立。"
                        
                        echo -e "\n  ${cyan}【外网原生探针与高频访客独立 IP 追踪排行榜 (TOP 10)】${none}"
                        ...
                        if test -n "$ips"; then
                            echo "$ips" | sort | uniq -c | sort -nr | head -n 10 | awk '{printf "    物理溯源 IP: %-18s (系统已拦截其分发数据包数: %s 频次)\n", $2, $1}'
                            
                            local total_ips
                            total_ips=$(echo "$ips" | sort | uniq | wc -l || echo 0)
                            echo -e "\n  执行严密过滤后，当前实际通过代理发起活跃请求的独立真实访客总量为: ${yellow}${total_ips}${none}"
                        else
                            echo -e "    ${gray}主线雷达运转状态良好，但外网入口未发现任何持续性的非法或合法物理数据探测。${none}"
                        fi
                    else
                        error "目标雷达脱机宕机：未在系统的任何内存页找到存活的 Xray 主进程。"
                    fi
                    
                    echo -e "\n  ${gray}----------------------------------------------------------------------${none}"
                    echo -e "  ${green}广域探针自动刷新引擎已升空 (以 2 秒物理间隔循环雷达探测)...${none}"
                    echo -e "  指挥控制热键:  [ ${yellow}r${none} ] 立即撕裂缓冲池强行刷新   [ ${yellow}q${none} ] 紧急降落收起面板并逃逸"
            ...
            6)
                clear
                title "应用程序常态轨迹与错误断层留档系统"
                
                echo -e "  ${cyan}>>> 应用程序主线运行逻辑流 (xray_script.log) <<<${none}"
                tail -n 25 "$LOG_DIR/xray_script.log" 2>/dev/null || echo "  防线极度纯洁：数据库内空空如也，系统暂未在硬盘记录任何异常执行轨迹。"
                hr
                echo -e "  ${red}>>> 操作系统及内核严重异常报错审计流 (script_error.log) <<<${none}"
                tail -n 25 "$LOG_DIR/script_error.log" 2>/dev/null || echo "  雷达表明主机生态完好，整个架构内未侦测到任何红线级别的安全泄露报错。"
                
                local _p=""; read -rp "查阅结束，请按 Enter 强行截断输出模式..." _p || true
                ;;
            7)
                clear
                title "全自动防灾数据快照与配置时空穿梭模块"
                
                info "深度探测系统文件结构，寻找一切可以利用的时空快照配置坐标..."
                ls -t "$BACKUP_DIR"/config_*.json 2>/dev/null || echo -e "  ${yellow}高能预警：时空防灾库内一片荒芜，没有找到任何有效的底层物理快照节点。${none}"
                
                echo -e "\n  r) 系统崩溃求生指令：启动强制灾难级恢复，无条件逆向穿越回最近保存的成功快照时空"
                echo -e "  c) 签署物理封印：在现在的状态定格留存，手工执行对环境配置文件制作一次不可磨灭的安全快照"
                echo -e "  0) 取消授权并退出"
                
                local bopt=""
                read -rp "请签发您的系统求生或备案指令: " bopt || true
                
                if test "$bopt" = "r" || test "$bopt" = "R"; then
                    restore_latest_backup
                    local _p=""; read -rp "逆向回溯行为彻底完结，请敲击 Enter 继续..." _p || true
                elif test "$bopt" = "c" || test "$bopt" = "C"; then
                    backup_config
                    info "操作核实合规通过，当前运行期架构状态的系统物理快照已被烙印归档至底层防灾目录。"
                    local _p=""; read -rp "请敲击 Enter 继续..." _p || true
                fi
                ;;
            0) 
                return 
...
# ------------------------------------------------------------------------------
# [ 0x20: 自毁程序 (深层系统生态彻底抹杀还原) ]
# ------------------------------------------------------------------------------

do_uninstall() {
    title "超高危最高指令：系统深度自毁与生态物理还原"
    
    warn "【绝地指令】系统将摧毁整个架构："
    warn "这包括你的加密节点配置库、Xray 守护核心进程树、所有 Dnsmasq 解析表以及防爆日志堆栈。"
    warn "但出于性能考虑，向内核映射的所有 200 项系统 Sysctl 网卡极速加速引擎将被【永久完好保留】！"
    
    local confirm=""
    read -rp "一切配置都将重置并灰飞烟灭。请您三思后再次确认，是否执意引爆自毁程序？(输入y确定执行): " confirm
    
    if test "$confirm" != "y" && test "$confirm" != "Y"; then 
        info "接收到求生意志，引爆操作遭强制终止，自毁序列已被安全休眠并中止。"
        return
    fi
    
    info "最终级物理销毁授权已签收，正在点燃导火索引爆基础网络与底层配置数据链..."
    
    # [1] 保护初装日期，保留系统计费凭证
    local temp_date=""
    if test -f "$INSTALL_DATE_FILE"; then
        temp_date=$(cat "$INSTALL_DATE_FILE")
        info "已成功在销毁前安全抽取了首发配置快照，将在重组后再次回写..."
    fi
    
    # [2] 破坏 DNS 解析拦截缓冲层
    info "正在深层粉碎 Dnsmasq 极速缓存架构并让系统的原生 DNS 解析器重新掌权..."
    systemctl stop dnsmasq >/dev/null 2>&1 || true
    systemctl disable dnsmasq >/dev/null 2>&1 || true
    ...
    # [3] 销毁主守护核心
    info "正在绞杀中止 Xray 引擎网络主进程及取消系统级 Systemd 的内核注册映射..."
    systemctl stop xray >/dev/null 2>&1 || true
    ...
    # [4] 粉碎底层硬盘所有驻留目录
    info "执行系统深度格式化肃清动作：抹除遗留数据阵列、缓存以及一切物理操作轨迹..."
    ...
    print_green "========================================================================="
    print_green "  神威消散。所有的自毁指令序列均已被系统彻底消化执行！"
    print_green "  服务器生态已被物理抹平，完全重启并退回为原装初始的安全出厂状态体系。"
    print_green "========================================================================="
    exit 0
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray 高维控制防线台 (The Apex Vanguard Ultimate Genesis 3000)${none}"
        
        local svc
        svc=$(systemctl is-active xray 2>/dev/null || echo "inactive")
        if test "$svc" = "active"; then 
            svc="${green}引擎狂啸满载挂载中 (Active)${none}"
        else 
            svc="${red}链路截断且心跳静默 (Inactive)${none}"
        fi
        
        local current_kernel
        current_kernel=$(uname -r)
        
        echo -e "  战舰核心当前态势: $svc | 全局快捷热键唤醒: ${cyan}xrv${none}"
        echo -e "  引擎承载底层主板: ${yellow}${current_kernel}${none} | 对外物理信标: ${yellow}${SERVER_IP}${none}"
        echo -e "${blue}======================================================================${none}"
        echo "  1) 创建与重构 绝对安全防爆的双轨协议网阵主栈 (VLESS-Reality / SS-2022)"
        echo "  2) 用户凭证生命周期与独立物理防封属性管理 (带数组绝缘排爆与专属 SNI)"
        echo "  3) 检阅全量配置雷达图信息与获取各个节点阵列数据直连长链路二维码"
        echo "  4) 人工呼叫系统强制覆盖更新，极速拉取并热更全球 Geo IP 探针数据库"
        echo "  5) 发起系统最高序列热更新，调用多轨 CDN 镜像链覆盖升级最新的官方框架"
        echo "  6) 撕裂边界！发动伪装矩阵无缝防爆漂移 (动态偏移剔除阻断并更新 SNI 池)"
        echo "  7) 全域拦截与黑洞防线管控中心 (支持追加/剔除特定的广告与网络爬虫)"
        echo "  8) 扫描 Reality 针对扫描者的防灾阈值，拉起边界回落探针防线的审计报告"
        echo "  9) 全景网络握手连接池状态、追踪外部 IP 溯源以及系统自然月流量大盘中心"
        echo "  10) 进入系统初始化前置机房 (安装内核、突破网卡与全量极客系统参数调优)"
        echo "  0) 收起悬浮屏幕系统，缩回终端底层"
        echo -e "  ${red}88) 启动深空级底层格式化程序：彻底自毁销毁体系生态，抹平物理装载痕迹${none}"
        hr
        
        local num=""
        read -rp "最高统帅，主中枢系统正在静候您的总线级别行动命令: " num || true
        ...
        case "${num:-}" in
            ...
            3) 
                ...
                while true; do 
                    local rb=""
                    read -rp "检阅审计流程完结。按 Enter 键逃逸退回，或敲击 b 立即激活矩阵防封漂移: " rb || true
                ...
                ;;
            4) 
                info "接收最高指令：正在全速释放数据信号，从 Github 与镜像阵列强拉最新底层库..."
                bash "$UPDATE_DAT_SCRIPT" >/dev/null 2>&1 || true
                ensure_xray_is_alive
                local _p=""; read -rp "数据库落盘与覆盖重叠完毕，按 Enter 回归总控台..." _p || true 
                ;;
            5) do_update_core ;;
            6) 
                if choose_sni; then 
                    _update_matrix
                    do_summary
                    local _p=""; read -rp "全新的网络流向指令已牢牢印入守护核心流，按 Enter 键脱离..." _p || true
                fi 
                ;;
            ...
            0) exit 0 ;;
            *) 
                echo -e "${gl_hong}❌ 指令校验遭遇不可逾越的鸿沟：全域系统果断抛弃了您的错误越界逻辑！${gl_bai}"
                sleep 1 
                ;;
        esac
    done
...
# ------------------------------------------------------------------------------
# [ 0x22: 底层基建配置大厅 (系统网阵与内核控制总线) ]
# ------------------------------------------------------------------------------

do_sys_init_menu() {
    while true; do
        clear
        title "系统基础运行环境全景配置、驱动级引擎编译与极客重装调优引导中心"
        
        echo "  1) 同步广域时区校准坐标 (Asia/Kuala_Lumpur) 且强行注入底层支持依赖网阵"
        echo "  2) (兼容版) 首选物理部署官方最高性能的预编译驱动 XANMOD (main) 内核"
        echo "  3) [极客模式] 执行主线驱动拉取，抛弃系统验证强行开启本机物理编译与 BBR3 融合"
        echo "  4) 撕破物理硬件限制：突刺修改动态网卡驱动发送堆栈 (TX Queue) 缩减防堵车延迟"
        echo "  5) 终极高压灌注：向内核寄存器注入 200 多项深海级 Sysctl 的极限网络吞吐底层防线"
        echo "  6) 进入十五边形战神矩阵控制台：通过 28 种参数组合彻底玩转并统合一切应用、系统网络特征"
        echo "  0) 斩断此配置空间控制总线并撤退退回主界面"
        hr
        
        local sopt=""
        read -rp "系统权限开放，正在等候您对最底层架构的指令调度: " sopt || true
        
        case "${sopt:-}" in
            1) 
                preflight
                local _p=""; read -rp "底座依赖补全验证闭环结束。按 Enter 键继续退回..." _p || true 
                ;;
