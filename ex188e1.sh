#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex188e1.sh (The Apex Vanguard - Project Genesis V188e1)
# 快捷方式: xrv
#
# 【V188e1 终极真核融合修补版】
#   1. 编译防砖: 强行注入 CONFIG_VIRTIO 系列驱动，杜绝 KVM/Xen 架构 VPS 编译后丢失硬盘与网卡。
#   2. 空间巡航: 编译前强制排查 /boot 扇区余量，摘除 make install 的静默吞错机制。
#   3. 作用域修复: 寻回 17 个 toggle_* 函数，彻底修复微操面板的 command not found 断层。
#   4. 绝缘护盾: JQ 全量采用 |= 与 select(. != null) 语法，_safe_jq_write 引入 "$@" 传参。
#   5. 原生直连: 彻底移除画蛇添足的反代 CDN 矩阵，恢复最纯正的 Github 官方直连升级。
#   6. 探针修复: 注入自带内存缓存的 _get_ip，彻底解决菜单卡顿与严格模式报错。
# ==============================================================================
...
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex188e1.sh"
    exit 1
fi
...
# V188e1 关键绝缘修复: 引入 "$@" 接收全量参数，确保多参数传值不截断
_safe_jq_write() {
    backup_config
    local tmp_raw
...
# ------------------------------------------------------------------------------
# [ 0x05: 系统与应用层微操核心注入模块 (JSON 绝缘修改) ]
# ------------------------------------------------------------------------------

_turn_on_app() {
    # 采用安全护盾语法 (|=) 与 // {} 空集兜底，杜绝数组崩溃
    _safe_jq_write '
      .routing = (.routing // {}) |
...
# ------------------------------------------------------------------------------
# [ 0x12: 编译安装原生 Linux 主线内核 + BBR3 (全量 VIRTIO 防砖版) ]
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# [ 0x20: Xray 核心通讯底层升维系统 (海外原生直连极速版) ]
# ------------------------------------------------------------------------------
...
_update_matrix() {
    ...
    # 安全使用 slurpfile 导入矩阵数组，拒绝就地拼接导致的 JSON 结构崩塌
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray 高维控制台 (Apex Vanguard V188e1 Industrial Base)${none}"
...
main_menu
# ==============================================================================
# EOF - Apex Vanguard V188e1 System Core Ready.
# ==============================================================================
