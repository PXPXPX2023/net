#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex41.sh (The Apex Vanguard - Project Genesis V14)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 彻底重构多用户分发中心 (do_summary)，引入动态循环，完美解析并展示所有用户的专属节点信息。
#   2. 补全用户管理模块 (新增/导入/修改SNI) 的详细信息面板，告别“只出二维码”的盲盒体验。
#   3. 维持【极低延迟/小包秒开】架构：Xray 并发精简至 4096，TX 队列收缩至 2000。
#   4. 维持 NAPI 软中断配额：dev_weight=64，bias=1，杜绝算力独占带来的微观卡顿。
#   5. 维持内核态忙轮询 (Busy Polling): net.core.busy_poll=50，死盯网卡极限提速。
#   6. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex41.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex41 The Apex Vanguard - Project Genesis V14${none}"
