#!/usr/bin/env bash
# ============================================================
# 脚本名称: g7g43.sh (The Omni-Ascendant V2 Edition)
# 快捷方式: xrv
# 巅峰突破: 
#   1. Reality 严苛质检: TLS 1.3 + ALPN h2 + OCSP 三重强制校验。
#   2. 引入商用级网卡流量计费，支持自定义账单周期与初装日追踪。
#   3. GFW 连通性物理探测，精准甄别 DNS 投毒与中国境内 CDN。
#   4. 增删用户同步配套 ShortId，并当场独立分发专属链接。
#   5. 彻底修复所有空值报错与终端颜色溢出，全自动热更 Geo 库。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash g7g43.sh"
    exit 1
fi
...
run_sni_scanner() {
    ...
    if test -s "$tmp_sni"; then
        ...
        if test "${c:-0}" -lt 10; then
            grep "CN_CDN" "$tmp_sni" | sort -n | head -n $((10 - ${c:-0})) | awk '{print $2, $1}' >> "$SNI_CACHE_FILE"
        fi
    else
...
choose_sni() {
    while true; do
        if test -f "$SNI_CACHE_FILE"; then
            echo -e "\n  ${cyan}【战备缓存：极速 Top 10 (已剔除阻断节点)】${none}"
            local idx=1
            while read -r s t; do
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray G7G43 The Omni-Ascendant V2 Edition${none}"
