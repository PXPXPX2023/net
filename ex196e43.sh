#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e43.sh (The Apex Vanguard - Project Genesis V196e43)
# 快捷方式: xrv
#
# 【V196e43 终极修复版：函数链重构、破死锁、动态 7.x 选库】
#   1. 结构重塑: 彻底修复 Bash 函数声明顺序导致的 command not found 问题。
#   2. OOM 免疫: 固化 1280MB Swap，并在低内存(≤2.5G)时强行降维单线程编译。
#   3. API 排序: 引入 sort -V -r，完美抓取并置顶 Linux 7.0.x 及以上最新 Xanmod 分支。
#   4. 粉碎死结: 物理关闭 CONFIG_IA32_EMULATION，解决 vdso32 与指令集的冲突。
# ==============================================================================

if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex196e43.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="196e43"
