#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e31.sh (The Apex Vanguard - Project Genesis V196e31)
# 快捷方式: xrv
#
# 【V196e31 终局之战版：架构精准修复与函数防遗漏】
#   1. 完美函数链: 修复了前一版中因代码分段导致的 do_sys_init_menu 函数丢失问题。
#   2. 根治编译死机: 恢复 e21 的精准定点修正，仅对 arch/x86/Makefile 进行架构替换，保护底层模块指令集。
#   3. 驱动完美继承: 强制继承宿主机 /boot/config，杜绝因 defconfig 导致的网卡失联与黑屏。
#   4. 斩断证书锁链: 精准抹除 CONFIG_SYSTEM_TRUSTED_KEYS，彻底消灭 signing_key.pem Error 2。
#   5. 生命周期保障: 废除编译后强删旧内核的自杀逻辑，确保 GRUB 退路畅通。
# ==============================================================================

if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex196e31.sh"; exit 1; fi
if test "$EUID" -ne 0; then echo -e "\033[31m[致命错误] 触及底层内核必须拥有最高权限，请使用 root 账户执行！\033[0m"; exit 1; fi
if ! command -v systemctl >/dev/null 2>&1; then echo -e "\033[31m[致命错误] 当前系统缺失 systemd！\033[0m"; exit 1; fi
...
readonly SCRIPT_VERSION="196e31"
...
check_and_create_1gb_swap() {
    ...
    # 融合 dx28 容错机制：尝试 fallocate，如果失败 (比如 ZFS 不支持) 则回退使用 dd
    if ! fallocate -l 1024M "$SWAP_FILE" 2>/dev/null; then
        warn "fallocate 失败，触发容灾降级，正在使用 dd 建立 Swap (可能耗时较长)..."
...
do_install_xanmod_main_official() {
    ...
    if test "$arch" != "x86_64"; then error "系统架构不匹配：仅支持 x86_64！"; local _p=""; read -rp "按 Enter 返回..." _p || true; return 1; fi
    if test ! -f /etc/debian_version; then error "系统发行版排斥：目前仅兼容 Debian / Ubuntu 系！"; local _p=""; read -rp "按 Enter 返回..." _p || true; return 1; fi

    info "正在拉取智能探针，检测本地 CPU 硬件微架构支持级别..."
    local cpu_level_script="/tmp/check_x86-64_psabi.sh"
    set +e
    if ! wget -qO "$cpu_level_script" https://dl.xanmod.org/check_x86-64_psabi.sh 2>/dev/null; then warn "探针脚本下载遇到网络波动，将跳过精准检测。"; fi
    local cpu_level=""
    if test -f "$cpu_level_script"; then
        ...
        if [[ "$detected" =~ ^[1-4]$ ]]; then cpu_level="$detected"; fi
        rm -f "$cpu_level_script" 2>/dev/null || true
    fi
    set -e
    if test -z "$cpu_level"; then cpu_level=1; warn "未能精确检测 CPU 级别，默认使用系统最宽容的 v1 兼容版。"; else info "当前 CPU 硬件支持的微架构最高级别为: v${cpu_level}"; fi

    info "净化并配置 Xanmod 官方最高优 APT 仓库..."
    export DEBIAN_FRONTEND=noninteractive
    set +e
    ...
    if ! wget -qO - https://dl.xanmod.org/archive.key | gpg --dearmor --yes -o /usr/share/keyrings/xanmod-archive-keyring.gpg 2>/dev/null; then error "获取 GPG 密钥链发生错误，官方源被阻断！"; set -e; return 1; fi
    ...
    if ! apt-get update -y; then warn "APT 源刷新异常，继续降级探测..."; fi
    ...
    for ((v=$cpu_level; v>=1; v--)); do
        ...
        if apt-get install -y "$try_pkg" >/dev/null 2>&1; then pkg_name="$try_pkg"; installed=true; print_green ">>> 成功安装核心包: $pkg_name"; break; fi
        warn "源内未命中或阻断 $try_pkg，降级探测 v$((v-1)) ..."
    done
    
    if test "$installed" = "false"; then error "源无法解析 Xanmod 预编译包。"; local _p=""; read -rp "按 Enter 继续..." _p || true; set -e; return 1; fi
    set -e
    info "预编译核心注入成功，重载 GRUB..."
    if command -v update-grub >/dev/null 2>&1; then update-grub >/dev/null 2>&1 || true; else apt-get install -y grub2-common >/dev/null 2>&1 || true; update-grub >/dev/null 2>&1 || true; fi
    warn "系统将在 10 秒后重启..."
    sleep 10
    reboot
}
...
do_kernel_compile_menu() {
    while true; do
        clear
        title "极客内核源码锻造中心 (V196e31 终局护航版)"
        echo "  已彻底根除全局 Makefile 轰炸带来的内核死机！"
        echo "  现已引入宿主机驱动完美继承 + 终极去签机制，绝不断网黑屏，永不卡死！"
        echo ""
        echo -e "  ${cyan}1) [稳如老狗] 编译 Linux 官方主线最新内核 (Mainline + BBR) ${none}"
        echo "     - 源码直连 kernel.org，100% 原生纯净，编译零报错"
        echo ""
        echo -e "  ${magenta}2) [激进压榨] 编译 Xanmod 官方极客内核 (任意自定义版本 + BBR3) ${none}"
        echo "     - 源码直连 GitLab，支持人工手填分支 Tag (例如 6.18.25-rt-xanmod1)"
        echo "     - 植入上帝模式防爆盾，强制剥离冲突指令，兼容全部 x86_64 服务器！"
        echo ""
...
_prepare_compile_env() {
    ...
    rm -rf /root/linux* /root/*.tar* /root/*.gz /root/*.xz 2>/dev/null || true
    sync
...
_execute_compilation() {
    ...
    # 【绝对核心修复】：保留所有旧版内核作为 GRUB 备用防爆退路，不再进行任何自杀式卸载！
...
_compile_kernel_mainline() {
    ...
    # 【核心修复：取代 defconfig，完美继承宿主机网络/显卡驱动】
    if test -f "/boot/config-$(uname -r)"; then
        info "正在继承当前宿主机驱动配置以保证完美兼容..."
        cp "/boot/config-$(uname -r)" .config
        
        info ">> 启动终极清道夫模式：物理抹除宿主机自带的 .pem 签名证书依赖..."
    ...
    # 【终极后门防线】: 彻底斩断 Debian 系安全签名与 Debug 膨胀
...
_compile_kernel_xanmod() {
    title "系统飞升：编译 真·Xanmod 极客内核 (全版本无损兼容防爆模式)"
    warn "警告: 此过程将极度消耗 CPU 与内存 (约 30-60 分钟)，低配机极易死机！"
    local confirm=""; read -rp "确定要开始极客源码编译吗？(y/n): " confirm || true
    if [[ ! "$confirm" =~ ^[yY]$ ]]; then return; fi
    
    local default_tag="6.12.25-xanmod1"
    info "您可以手动指定内核的 GitLab Release Tag 以进行精确编译。"
    local user_tag=""; read -rp "请输入所需编译的 Tag 名称 (例如 6.18.25-rt-xanmod1，直接回车默认使用 ${default_tag}): " user_tag || true
    ...
    # 【核心修复：取代 defconfig，完美继承宿主机驱动并物理去签】
    if test -f "/boot/config-$(uname -r)"; then
        info "正在继承当前宿主机驱动配置以保证完美兼容..."
        cp "/boot/config-$(uname -r)" .config
    ...
    # 【极客上帝模式：精准修正架构断层，取代 e22 危险的全局轰炸】
    info ">> 恢复物理防爆盾：锁定 CONFIG_X86_64_VERSION 并定向修正 arch/x86/Makefile..."
    ...
    sed -i 's/-march=x86-64-v[0-9]/-march=x86-64/g' arch/x86/Makefile 2>/dev/null || true
    sed -i 's/-march=x86-64-v\$(CONFIG_X86_64_VERSION)/-march=x86-64/g' arch/x86/Makefile 2>/dev/null || true

    info ">> 正在清洗配置项，移除危险 Debug 羁绊..."
    ./scripts/config --disable CONFIG_MODULE_SIG
    ./scripts/config --disable CONFIG_MODULE_SIG_ALL
    ./scripts/config --disable CONFIG_MODULE_SIG_FORCE
    ./scripts/config --disable CONFIG_DEBUG_INFO
    ...
    # ★ 致命修复：防交互卡死
    info ">> 正在补全 Xanmod 自动依赖关系..."
    make olddefconfig >/dev/null 2>&1 || true
    
    # 强制注入 Make 参数兜底，双重防爆
    _execute_compilation "CONFIG_X86_64_VERSION=1"
}
...
do_sys_init_menu() {
    while true; do
        ...
        echo "  5) [网络底层] TX Queue 网卡出站队列防拥堵极限缩减 (配置为 12000 收缩)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e31 - (The Apex Vanguard)${none}"
...
# ==============================================================================
# EOF - Apex Vanguard V196e31 System Advanced Control Ready.
# ==============================================================================
# --- ✂️ Part 15 结束，所有代码拼接完毕。请运行 bash ex196e31.sh ---
