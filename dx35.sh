#!/usr/bin/env bash
# ============================================================
# 脚本名称: dx35.sh (The Apex Vanguard - Project Genesis V7)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 彻底重构 JSON 注入引擎，采用纯净 HereDoc 与 jq input 机制，杜绝截断报错。
#   2. 修复主菜单版本溯源错误，重铸安装后的交互闭环 (完美恢复按 b 热切)。
#   3. 卸载逻辑明确边界：仅粉碎 Xray 生态，永久保留系统级并发与网络调优。
#   4. Xray 进程并发上限精准设定为 131072，严防低配机 Go Runtime OOM。
#   5. 编译防爆护盾 Swap 维持在 1GB，加入 dd 命令底层降级创建机制。
#   6. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash dx35.sh"
    exit 1
fi
...
preflight() {
    ...
    local need="jq curl wget xxd unzip qrencode vnstat cron openssl coreutils sed e2fsprogs pkg-config iproute2"
    local install_list=""
    for i in $need; do 
...
# -- 安全写入配置更新模块 (纯粹使用 jq 文件流写入) --
# 为了保证极度安全与格式规整，此版本抛弃单引号变量直接注入，采用 jq --arg 与 slurpfile 组合
update_config_sni() {
    local target_sni="$1"
    local sni_array_str="$2"
    
    echo "[$sni_array_str]" > /tmp/sni_array.json
    jq --arg dest "$target_sni:443" --slurpfile snis /tmp/sni_array.json '
    ...
    if [ $? -eq 0 ]; then
        ...
        rm -f "$CONFIG.tmp" /tmp/sni_array.json
        return 1
    fi
}
...
do_xanmod_compile() {
    ...
    warn "警告: 这是一个极其漫长的过程 (30-60分钟)，执行后系统将自动重启！"
    read -rp "确定要开始编译安装吗？(y/n): " confirm
    if [[ "$confirm" != "y" ]]; then 
        return
    fi
...
EOF
    ...
    if ! swapon --show | grep -q swapfile; then
        # 工业级容错：如果 fallocate 失败 (例如在 ZFS/BTRFS 上)，则降级使用 dd 强行开辟 1G 空间
    ...
    # 精准正则抓取 is_latest 为 true 的稳定版本，防 RC 版本炸机
...
EOSERVICE
    ...
    local RX_QUEUES=$(ls /sys/class/net/$IFACE/queues/ | grep rx- | wc -l)
...
# -- 极限系统网络栈调优 (Limits + Sysctl) --
do_perf_tuning() {
    title "极限压榨：系统底层网络栈调优"
    warn "警告: 该操作将注入极限并发参数，执行后系统将重启！"
...
EOF
    ...
    info "极限参数注入完成！系统将在 30 秒后自动重启以生效..."
    sleep 30
    reboot
}
...
do_txqueuelen_opt() {
    ...
    info "正在设置 txqueuelen=12000 以提升大流量吞吐并发..."
    $IP_CMD link set "$IFACE" txqueuelen 12000

    local SERVICE_FILE="/etc/systemd/system/txqueue.service"
    cat > $SERVICE_FILE <<EOF
[Unit]
Description=Set TX Queue Length for High Throughput
...
do_sys_init_menu() {
    while true; do
        ...
        echo "  3) 编译安装 Xanmod 内核 + BBR3 (极客源码流 / 自动重启)"
        echo "  4) 系统内核网络栈极限调优 (百万并发参数 / 自动重启)"
        echo "  5) 网卡发送队列 (TX Queue) 深度调优 (大流量防丢包)"
        echo "  0) 返回主菜单"
        hr
        read -rp "请选择: " sys_opt
        case "$sys_opt" in
            1) 
                ...
                # 注入非交互式环境变量，完全阻断 apt-get upgrade 卡死在系统确认弹窗上
                export DEBIAN_FRONTEND=noninteractive
                ...
                apt-get install -y wget curl sudo socat ntpdate e2fsprogs pkg-config iproute2
...
EOF
            ...
            if test -n "$target_uuid"; then
                ...
                if test -n "$u_sni"; then
                    ...
                    local port=$(jq -r ".inbounds[] | select(.protocol==\"vless\") | .port" "$CONFIG" | head -1)
                    # 匹配此用户的 shortId (这里简化取对应序号的 shortId)
                    local idx=$((${snum:-0}-1))
...
do_summary() {
    ...
    local v_count=$(jq ".inbounds[] | select(.protocol==\"vless\") | length" "$CONFIG" 2>/dev/null || echo 0)
    
    if test "${v_count:-0}" -gt 0; then
        local uuid=$(jq -r ".inbounds[] | select(.protocol==\"vless\") | .settings.clients[0].id" "$CONFIG" 2>/dev/null)
        local remark=$(jq -r ".inbounds[] | select(.protocol==\"vless\") | .settings.clients[0].email // \"$REMARK_NAME\"" "$CONFIG" 2>/dev/null)
        local port=$(jq -r ".inbounds[] | select(.protocol==\"vless\") | .port" "$CONFIG" 2>/dev/null)
        local sid=$(jq -r ".inbounds[] | select(.protocol==\"vless\") | .streamSettings.realitySettings.shortIds[0]" "$CONFIG" 2>/dev/null)
        ...
        target_sni=${target_sni:-$(jq -r ".inbounds[] | select(.protocol==\"vless\") | .streamSettings.realitySettings.serverNames[0]" "$CONFIG" 2>/dev/null)}
...
do_uninstall() {
    ...
    print_magenta ">>> [1/3] 正在解除 DNS 底层文件强锁保护并尝试恢复原生解析..."
    chattr -i /etc/resolv.conf 2>/dev/null || true
    ...
    print_magenta ">>> [2/3] 正在停止并彻底绞杀 Xray 主进程及系统权限映射..."
    ...
    print_magenta ">>> [3/3] 正在粉碎数据目录、系统日志及物理配置文件..."
    ...
    print_green "卸载完成！Xray 及其相关配置已被彻底粉碎 (您的内核网络栈与并发限制调优已为您永久保留)。"
    exit 0
}

# -- 矩阵切换注入核心 (独立模块化，方便复用) --
_update_matrix() {
    # 把纯 Bash 字符串转成严谨的 JSON 数组再写入
...
do_install() {
    title "Apex Vanguard: 核心部署"
    preflight
    
    # 安装前强制停用一次老进程，防止后续新端口被旧程序占用引发冲突
    systemctl stop xray >/dev/null 2>&1 || true
...
EOF
    ...
    if test "$proto_choice" = "1" || test "$proto_choice" = "3"; then
        ...
        # 2. 绝对纯净的 HereDoc JSON 注入 (无任何单引号嵌套风险)
        echo "[$SNI_JSON_ARRAY]" > /tmp/sni_array.json
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray dx32 The Apex Vanguard - Project Genesis V5${none}"
        ...
        echo "  10) 初次安装、更新系统组件"
        echo "  0) 退出"
        echo -e "  ${red}88) 彻底卸载 (复原系统解析并清空软件痕迹)${none}"
        hr
