#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: exex196e42.sh (The Apex Vanguard - Project Genesis V196e42)
# 快捷方式: xrv
#
# 【V196e42 终极不朽版：破 OOM 死结、1280M Swap 固化、7.x 动态选库】
#   1. OOM 免疫: 尊重 1280MB Swap 固化要求，在低内存机器强制降维单线程链接防暴毙。
#   2. API 排序跃迁: 引入 sort -V -r，完美抓取并置顶 Linux 7.0.x 及以上最新 Xanmod 分支。
#   3. 粉碎编译死结: 物理关闭 CONFIG_IA32_EMULATION，彻底解决 vdso32 与指令集的冲突。
#   4. 满血防爆: 100% 覆盖 select(. != null) 绝缘层，全量菜单与 25 项微操无损回归。
# ==============================================================================

if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash exex196e42.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="196e42"
...
check_and_create_swap() {
    ...
    warn "检测到 Swap 缓冲池不足，正在强行切辟 1280MB Swap 固化缓冲分区..."
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e42 - (The Apex Vanguard)${none}"
...
# ==============================================================================
# EOF - Apex Vanguard V196e42 System Advanced Control Ready.
# ==============================================================================
# --- ✂️ Part 16 结束，所有代码拼接完毕。请运行 bash ex196e42.sh ---
