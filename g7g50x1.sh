#!/usr/bin/env bash
# ============================================================
# 脚本名称: g7g50x1.sh (The Omega Protocol V2 Edition)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 彻底修复底层 jq 写入函数残留的权限 Bug，绝杀 status 23。
#   2. 强化配置变量注入引擎，完美免疫节点别名包含空格导致的撕裂。
#   3. 完美保留 GFW/CDN 探测、TLS1.3 质检、Top 20 矩阵扩容等尖端特性。
#   4. 独立节点备注、半高二维码、vnstat 计费与自毁卸载 100% 稳定运行。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash g7g50x1.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray G7G50x1 The Omega Protocol V2 Edition${none}"
