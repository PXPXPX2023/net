#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex98.sh (The Apex Vanguard - Project Genesis V98)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 逻辑归一：修复 vnstat UI 割裂问题，将“计费日修改”原封不动请回流量统计子界面，恢复上下文连贯的极致体验。
#   2. 深度清剿：彻底切除前期版本残留的 Nano DB (极简库) 死代码与异构探针，保持底层脚本的绝对洁净。
#   3. 绝缘排爆：为所有 jq 查询加装 select(. != null) 绝缘层，彻底断绝因 JSON 字段缺失导致的闪退。
#   4. 容灾升维：继承 3:00 下载与 3:10 重启的错峰热更机制，提供 10 分钟黄金冗余下载期。
...
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex98.sh"
    exit 1
fi
...
do_app_level_tuning_menu() {
    while true; do
        ...
        echo -e "  ${red}18) 上帝之手：一键开启或关闭 1-15 项 全域微操 (满血激活 / 无损还原)${none}"
        echo "  0) 返回上一级"
        hr
        read -rp "请选择: " app_opt
        ...
        case "$app_opt" in
            ...
            16)
                if [ "$app_off_count" -gt 0 ]; then
                    print_magenta ">>> 正在全域强力开启 1-7 项，拉取数据库需数秒，请耐心等待..."
                    _turn_on_app
                    systemctl restart xray >/dev/null 2>&1
                    info "1-7 项 Xray 内部应用层微操已全域强力开启并实时生效！"
                else
                    print_magenta ">>> 正在全域恢复 1-7 项，拉取数据库需数秒，请耐心等待..."
                    _turn_off_app
                    systemctl restart xray >/dev/null 2>&1
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex98 The Apex Vanguard - Project Genesis V98${none}"
