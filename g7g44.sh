#!/usr/bin/env bash
# ============================================================
# 脚本名称: g7g44.sh (The Apex Vanguard Edition)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 彻底修复 jq 注入覆盖导致的 nobody 用户 permission denied (status 23) 致命 Bug。
#   2. 战备雷达缓存阵列全面扩容：极速优质节点 Top 10 升级至 Top 20。
#   3. Reality 严苛质检: TLS 1.3 + ALPN h2 + OCSP 三重强制校验。
#   4. 引入商用级网卡流量计费，支持自定义账单周期与初装日追踪。
#   5. GFW 连通性物理探测，精准甄别 DNS 投毒与中国境内 CDN。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash g7g44.sh"
    exit 1
fi
...
# -- 核心：130+ 实体 SNI 扫描引擎 (GFW投毒+CDN物理测绘) --
run_sni_scanner() {
    title "雷达嗅探：130+ 实体矩阵与国内连通性探测"
    print_yellow "正在执行深度握手测试，并分析 GFW 阻断与中国 CDN，约耗时 80 秒...\n"
    ...
    for sni in $sni_list; do
        ...
        if test "${ms:-0}" -gt 0; then
            ...
            # 采用 jq 精准解析 AliDNS DoH 结果，防正则吞噬
    ...
    # 扩容至 Top 20 排序：优先非 CDN 原生海外直连节点
...
# -- 安全写入配置 (彻底修复 permission denied / status 23) --
_safe_jq_write() {
    local filter="$1"
    local tmp=$(mktemp)
    if jq "$filter" "$CONFIG" > "$tmp" 2>/dev/null; then
        mv "$tmp" "$CONFIG" >/dev/null 2>&1
        # 必须给 644 和 nobody 归属，否则 Xray 启动报 permission denied (status 23)
...
do_user_manager() {
    while true; do
        ...
        echo "  a) 新增用户 (配套生成独立 UUID 与专属 ShortId)"
        echo "  d) 序号删除用户"
        echo "  q) 退出"
        read -rp "指令: " uopt
        ...
        if test "$uopt" = "a"; then
            ...
            local link="vless://${nu}@${SERVER_IP}:${port}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${sni}&fp=chrome&pbk=${pub}&sid=${ns}&type=tcp#${REMARK_NAME}-VIP"
            echo -e "\n  ${cyan}专属分发链接:${none} \n  $link\n"
            ...
            if test "${total:-0}" -le 1; then
                ...
                local target_uuid=$(awk -v id="${dnum:-0}" '$1==id {print $2}' "$tmp_users")
                if test -n "$target_uuid"; then
                    local idx=$((${dnum:-0}-1))
                    # 精准同步删除对应索引的 UUID 与 ShortId
                    _safe_jq_write "(.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .settings.clients) |= del(.${L_B}${idx}${R_B}) | (.inbounds${L_B}${R_B} | select(.protocol==\"vless\") | .streamSettings.realitySettings.shortIds) |= del(.${L_B}${idx}${R_B})"
...
do_summary() {
    ...
    title "The Omni-Ascendant 节点详情中心"
...
EOF
    ...
    # 赋予最终配置准确的读取权限，彻底绝杀 permission denied
    chmod 644 "$CONFIG" >/dev/null 2>&1
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray G7G44 The Omni-Ascendant V2 Edition${none}"
