#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex151.sh (The Apex Vanguard - Project Genesis V151 [Absolute Horizon])
# 快捷方式: xrv
# 终极修复宣言: 
#   1. 绝杀遗留名称：全域清剿 ex144/ex150 遗留字眼，统一更新为 V151。
#   2. 拒绝垃圾单行：完全遵循极客可读性，将 28 项全域微操菜单里的 if-else、jq 管道流全量多行展开，绝不为了“行数少”而压缩代码。
#   3. 1GB Swap 信仰：实装纯正 1GB 永久 Swap 管理器 (自动校验、卸载溢出、写死 fstab)。
#   4. 内核编译防爆：彻底修复 Xanmod Makefile 中的 -march=x86-64-v 空指针 Bug，精准注入 CONFIG_X86_64_VERSION。
#   5. 物理装甲回归：百万并发锁 (limits.conf)、60+ 行极限 Sysctl 参数、cc1.sh 守护进程与 do_change_dns 底层锁死完整保留。
#   6. 核心大一统：28 项全域微操矩阵、回落探针 (菜单 8)、8 行军规排版、ShortId 防漂移联动删除无一缺席。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex151.sh"
    exit 1
fi
...
# V151 绝杀修复: 拒绝触碰 CONFIG_X86_64_VERSION，纯粹信任原生 defconfig
do_xanmod_compile() {
...
EOF
            ...
            if test "${total:-0}" -le 1; then error "必须至少保留一个核心用户！"
                ...
                if test -n "$target_uuid"; then
                    local idx=$((${dnum:-0}-1))
                    # -- 核心 Bug 修复：多行 jq 确保 clients 与 shortIds 安全同步删除 --
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex151 The Apex Vanguard - Project Genesis V151${none}"
