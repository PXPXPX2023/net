#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex100.sh (The Apex Vanguard - Project Genesis V100 [Century Milestone])
# 快捷方式: xrv
# 百版封神: 
#   1. 算力降维：实装 Minimal Perfect Hash (MPH) 路由匹配引擎，百万级规则查表强制压缩至 O(1) 绝对常量时间。
#   2. 反导装甲：为 Reality 引擎注入 60000ms maxTimeDiff 偏移锁，物理级粉碎 GFW 流量重放攻击。
#   3. 智能嗅探：修复非 Reality 节点 (如纯 SS) 计算提速分数时的空转 BUG，微操矩阵实现全协议动态自适应。
#   4. 时空割裂：重构“上帝之手”，引入 6 秒高危倒计时与底层硬重启 (Reboot)，确保 THP/队列等 Kernel Space 变革完美落盘。
#   5. 十七阶战神矩阵：全域极客微操登峰造极，定编 17 项 (应用层 9 项 + 系统级 8 项)。
#   6. 绝对绝缘：延续所有 V99 补丁 (vnstat 计费日无损穿透、3:10 错峰热更、jq 链式空指针免疫、大一统全球库)。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，这不仅是脚本，这是单节点路由架构的暴力美学。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex100.sh"
    exit 1
fi
...
# -- [功能 10->7] 全域 17 项极限微操双向设置枢纽 --
do_app_level_tuning_menu() {
    while true; do
        clear
        title "全域 17 项极限微操 (Xray 提速底牌 & 系统内核微操)"
        if ! test -f "$CONFIG"; then 
        ...
        local s10=$([ "$dnsmasq_state" = "true" ] && echo "${cyan}内存极速解析中 (0.1ms)${none}" || echo "${gray}未开启 (依赖原生DoH)${none}")
        local s11=$([ "$thp_state" = "true" ] && echo "${cyan}已关闭 THP${none}" || ([ "$thp_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}系统默认${none}"))
        local s12=$([ "$mtu_state" = "true" ] && echo "${cyan}智能探测中${none}" || ([ "$mtu_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}未开启${none}"))
        local s13=$([ "$cpu_state" = "true" ] && echo "${cyan}全核火力全开${none}" || ([ "$cpu_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}节能待机${none}"))
        local s14=$([ "$ring_state" = "true" ] && echo "${cyan}已反向收缩${none}" || ([ "$ring_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}系统大缓冲${none}"))
        local s15=$([ "$zram_state" = "true" ] && echo "${cyan}已挂载 ZRAM${none}" || ([ "$zram_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}未启用${none}"))
        local s16=$([ "$journal_state" = "true" ] && echo "${cyan}纯内存极速化${none}" || ([ "$journal_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}磁盘 IO 写入中${none}"))
        local s17=$([ "$prio_state" = "true" ] && echo "${cyan}OOM免死 / IO抢占${none}" || echo "${gray}系统默认调度${none}")

        echo -e "  ${magenta}--- Xray Core 应用层内部调优 (1-9) ---${none}"
        ...
        echo -e "  7) 开启或关闭 Xray 进程物理绑核 & GOMAXPROCS 并发锁 (零切换损耗) | 当前状态: $s7"
        echo -e "  8) 开启或关闭 Minimal Perfect Hash (MPH) 路由匹配极速降维引擎    | 当前状态: $s8"
        echo -e "  9) 开启或关闭 Reality 防重放装甲 (maxTimeDiff 时间偏移绝对拦截)  | 当前状态: $s9"
        echo -e ""
        echo -e "  ${magenta}--- Linux 系统层与内核黑科技 (10-17) ---${none}"
        echo -e "  10) 开启或关闭【Dnsmasq 本地极速内存缓存引擎 (21000并发/锁TTL)】 | 当前状态: $s10"
        echo -e "  11) 开启或关闭【透明大页 (THP - Transparent Huge Pages)】        | 当前状态: $s11"
        echo -e "  12) 开启或关闭【TCP PMTU 黑洞智能探测 (Probing=1)】              | 当前状态: $s12"
        echo -e "  13) 开启或关闭【CPU 频率调度器锁定 (Performance)】               | 当前状态: $s13"
        echo -e "  14) 开启或关闭【网卡硬件环形缓冲区 (Ring Buffer) 反向收缩】      | 当前状态: $s14"
        echo -e "  15) 开启或关闭【ZRAM】(淘汰慢速 Swap，阶梯内存自动检测)          | 当前状态: $s15"
        echo -e "  16) 开启或关闭【日志系统 Journald 纯内存化】(斩断 I/O 羁绊)      | 当前状态: $s16"
        echo -e "  17) 开启或关闭【系统进程级防杀抢占 (OOM/IO 提权)】               | 当前状态: $s17"
        echo -e "  "
        echo -e "  ${cyan}18) 一键开启或关闭 1-9 项 应用层微操 (自动侦测并智能反转)${none}"
        echo -e "  ${yellow}19) 一键开启或关闭 10-17 项 系统级微操 (自动避障侦测并反转)${none}"
        echo -e "  ${red}20) 上帝之手：一键开启或关闭 1-17 项 全域微操 (执行后自动重启系统)${none}"
        echo "  0) 返回上一级"
        hr
        read -rp "请选择: " app_opt
        ...
        case "$app_opt" in
            ...
            9)
                ...
                read -rp "按 Enter 继续..." _
                ;;
            10) toggle_dnsmasq; systemctl restart xray >/dev/null 2>&1; read -rp "按 Enter 继续..." _ ;;
            11) toggle_thp; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            12) toggle_mtu; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            13) toggle_cpu; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            14) toggle_ring; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            15) toggle_zram; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            16) toggle_journal; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            17) toggle_process_priority; systemctl restart xray >/dev/null 2>&1; info "操作已下发，进程已重启生效。"; read -rp "按 Enter 继续..." _ ;;
            18)
                if [ "$app_off_count" -gt 0 ]; then
                    print_magenta ">>> 正在全域强力开启 1-9 项，请耐心等待..."
                    _turn_on_app
                    systemctl restart xray >/dev/null 2>&1
                    info "1-9 项 Xray 内部应用层微操已全域强力开启并实时生效！"
                else
                    print_magenta ">>> 正在全域恢复 1-9 项，请耐心等待..."
                    _turn_off_app
                    systemctl restart xray >/dev/null 2>&1
                    info "1-9 项 Xray 内部应用层微操已全部强制关闭，无损还原官方初始状态！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            19)
                if [ "$sys_off_count" -gt 0 ]; then
                    ...
                    systemctl restart xray >/dev/null 2>&1
                    info "10-17 项系统内核底层黑科技已全部一键激活！(不支持的硬件已自动跳过)"
                else
                    ...
                    systemctl restart xray >/dev/null 2>&1
                    info "10-17 项系统内核底层黑科技已全部一键卸载还原！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            20)
                if [ "$((app_off_count + sys_off_count))" -gt 0 ]; then
                    ...
                    info "上帝降临！1-17 项全域极限微操已【全部满血激活】！(不支持的硬件已自动跳过)"
                else
                    ...
                    [ "$journal_state" = "true" ] && toggle_journal >/dev/null 2>&1
                    [ "$prio_state" = "true" ] && toggle_process_priority >/dev/null 2>&1
                    info "神威收敛！1-17 项全域极限微操已【全部强制关闭】，无损还原初始出厂状态！"
                fi
                ...
                print_yellow "为了确保所有 CPU 调度、内存机制与网卡队列完美落盘生效，"
...
do_sys_init_menu() {
    while true; do
        ...
        echo -e "  ${magenta}6) 全域 17 项极限微操 (Dnsmasq / Xray 提速底牌 / 系统级黑科技)${none}"
        echo "  0) 返回主菜单"
        hr
        read -rp "请选择: " sys_opt
        case "$sys_opt" in
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex100 The Apex Vanguard - Project Genesis V100${none}"
