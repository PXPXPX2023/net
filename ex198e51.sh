#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e51.sh (The Apex Vanguard - Project Genesis V198e51)
# 快捷方式: xrv
# 【V198e51 净化稳态版：去重覆盖 + 持久化自检 + 安全极速默认】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e51.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e51"
...
check_and_clean_space() {
    ...
    # V198e51: 不再 rm -rf /tmp/*、不再删除全部 /var/log/*.log，避免误删 socket/锁文件/审计日志导致服务异常。
    rm -rf \
...
_e51_pause() { local _p=""; read -rp "按 Enter 继续..." _p || true; }
...
choose_best_qdisc() {
    # V198e51: 极速代理默认 fq + BBR。CAKE 不再作为默认项，避免弱 CPU VPS 因整形/分类开销反降速。
...
_setup_zram_1024_e51() {
    local ZRAM_SIZE="1024" zdev="/dev/zram0"
    # e51 规则：总 Swap 上限 1024MB。ZRAM 可用时优先保留 ZRAM，删除 /swapfile 等文件型 Swap。
    _disable_all_file_swaps
...
EOF_TIMER_E44
    local tmp_cron
    tmp_cron=$(mktemp /tmp/cron_e51_XXXXXX) || true
...
verify_sni_strict() {
    ...
    if ! echo "$out" | grep -qi "OCSP response:"; then warn "OCSP Stapling 未发现：这不是致命问题，V198e51 仅提示不强制淘汰。"; fi
...
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
IFACE="$IFACE"
# V198e51: 纯代理吞吐优先保留 GRO/GSO/TSO，降低 CPU 压力；仅关闭 LRO，避免破坏转发语义。
...
do_perf_tuning() {
    ...
    info "配置系统高并发进程限制 (Limits，V198e51 稳态安全版)..."
...
toggle_zram() {
    # e51：废弃旧“按内存倍数创建 ZRAM”的 toggle，统一走 1024MB 总 Swap 上限策略。
    enforce_swap_cap_1024_e51
}
...
do_app_level_tuning_menu() {
    while true; do
        clear
        title "全域 25 项系统级及应用层微操管理中心（V198e51 稳态极速）"
        if ! test -f "$CONFIG"; then error "未发现配置，请先执行核心部署！"; _e51_pause; return; fi
        ...
        case "$app_opt" in
            1) apply_xray_nodelay_default; systemctl restart xray >/dev/null 2>&1 || true; info "TCP NoDelay 已幂等开启。"; _e51_pause ;;
            1f|1F) if [ "$out_fastopen" = "true" ]; then _safe_jq_write '(.outbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=false | (.inbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=false'; info "TCP FastOpen 已关闭。"; else _safe_jq_write '(.outbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=true | (.inbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=true'; warn "TCP FastOpen 已开启；如跨境握手不稳请关闭。"; fi; systemctl restart xray >/dev/null 2>&1 || true; _e51_pause ;;
            2|3|4|5|6) apply_xray_keep_2_to_6_defaults; remove_xray_env_buffer; systemctl restart xray >/dev/null 2>&1 || true; info "2-6 项已按稳态默认幂等固化。"; _e51_pause ;;
            7) set_xray_gogc_300; systemctl daemon-reload >/dev/null 2>&1 || true; systemctl restart xray >/dev/null 2>&1 || true; info "GOGC=300 已固化。"; _e51_pause ;;
            8) force_apply_xray_core_defaults_e51; remove_xray_env_buffer; systemctl restart xray >/dev/null 2>&1 || true; info "Policy=260s + Buffer=128KB 已固化。"; _e51_pause ;;
            9) apply_xray_cpu_schedule_default; systemctl daemon-reload >/dev/null 2>&1 || true; systemctl restart xray >/dev/null 2>&1 || true; info "CPU 调度已按单核/多核自动检测固化。"; _e51_pause ;;
            10) _safe_jq_write '.routing |= (. // {}) | .routing.domainMatcher = "mph"'; systemctl restart xray >/dev/null 2>&1 || true; info "MPH 已开启。"; _e51_pause ;;
            11) if [ -z "$has_reality" ] || [ "$has_reality" = "null" ]; then error "无 Reality 支持。"; else _safe_jq_write '(.inbounds[]? | select(. != null) | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.realitySettings.maxTimeDiff) = 60000'; systemctl restart xray >/dev/null 2>&1 || true; info "Reality 时间锁已开启。"; fi; _e51_pause ;;
            12) enable_dnsmasq_cache_safely; systemctl restart xray >/dev/null 2>&1 || true; _e51_pause ;;
            13) enable_thp_never_default; _e51_pause ;;
            14) apply_hotupdate_fast_sysctl; _e51_pause ;;
            15) enable_cpu_performance_default; _e51_pause ;;
            16) toggle_ring; _e51_pause ;;
            17) toggle_cake_qdisc; update_hw_boot_script; _e51_pause ;;
            18) toggle_cake_flag "ack_filter"; update_hw_boot_script; _e51_pause ;;
            19) toggle_cake_flag "ecn"; update_hw_boot_script; _e51_pause ;;
            20) toggle_cake_flag "wash"; update_hw_boot_script; _e51_pause ;;
            21) toggle_gso; _e51_pause ;;
            22) toggle_irq; _e51_pause ;;
            23) enable_zram_default; _e51_pause ;;
            24) enable_journald_volatile_default; _e51_pause ;;
            25) apply_process_priority_default; systemctl restart xray >/dev/null 2>&1 || true; _e51_pause ;;
            26) _turn_on_app; systemctl restart xray >/dev/null 2>&1 || true; info "应用层 1-11 已幂等激活。"; _e51_pause ;;
            27) apply_safe_turbo_defaults; systemctl restart xray >/dev/null 2>&1 || true; info "安全系统级微操已幂等落地。"; _e51_pause ;;
            28) _turn_on_app; apply_safe_turbo_defaults; systemctl restart xray >/dev/null 2>&1 || true; info "极速稳态一键打通完成。"; _e51_pause ;;
            0) return ;;
        esac
    done
}
...
install_self_entrypoint() {
    ...
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e51.sh" 2>/dev/null || true
        chmod +x "$SYMLINK" "$SCRIPT_DIR/ex198e51.sh" 2>/dev/null || true
    fi
...
apply_hotupdate_fast_sysctl() {
    ...
    mkdir -p /etc/sysctl.d 2>/dev/null || true
    local adv app qdisc
    # V198e51: 热更新默认固化你实测稳定的高 RTT 代理窗口组合。
    adv=1
    app=31
...
enable_dnsmasq_cache_safely() {
    # V198e51: 默认启用 dnsmasq，但必须通过监听与解析验证后才接管 resolv.conf。
...
apply_txqueue_3000_default() {
    # V198e51: 默认 3000，不再用 12000，避免队列过深导致排队延迟和 bufferbloat。
    local IP_CMD IFACE
...
enable_zram_default() {
    # e51：ZRAM 开启同时执行总 Swap 上限 1024MB；ZRAM 可用时删除 /swapfile，避免 2GB+ Swap。
    enforce_swap_cap_1024_e51
}
...
get_ssh_listen_ports() {
    local ports=""
    # V198e51: 优先从当前监听态识别 SSH 端口，避免 sshd_config 使用 Include/云镜像改名时误判为 22。
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e51 稳态极速版)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e51 - (The Apex Vanguard)${none}"
        ...
        echo -e "  ${cyan}11) 从 ex198e32/ex198e33/ex198e34/ex198e35/ex198e36/ex198e37/ex198e38/ex198e39/ex198e40/ex198e41/ex198e42/ex198e43 稳态热更新到 V198e51${none}"
...
rescue_after_v35_breakage() {
    title "V198e51 紧急修复：撤销旧版重启后断流风险项"
...
#==============================================================================
# V198e51 最终覆盖层：128KB Buffer 单位修正 + systemd 环境变量清理
...
toggle_buffer() {
    # V198e51：只在 config.json 的 KB 单位中切换 64/128，永不再写 systemd MB 环境变量。
    local v=""
...
#==============================================================================
# V198e51 覆盖修复层：修正 e40 残留状态/持久化问题
...
_safe_jq_write() {
    # V198e51：以退出码为准，不再强制搜索 Configuration OK 字符串。
...
force_apply_xray_core_defaults_e51() {
...
apply_xray_policy_260_default() { force_apply_xray_core_defaults_e51 >/dev/null 2>&1 || return 1; }
apply_xray_buffer_kb() { local kb="${1:-128}"; if test "$kb" != "128"; then warn "V198e51 默认只固化 128KB；如需其它值请手动改 config.json。"; fi; force_apply_xray_core_defaults_e51 >/dev/null 2>&1 || return 1; remove_xray_env_buffer; }
apply_xray_buffer_128_default() { force_apply_xray_core_defaults_e51 >/dev/null 2>&1 || return 1; remove_xray_env_buffer; }
apply_xray_nodelay_default() { force_apply_xray_core_defaults_e51 >/dev/null 2>&1 || return 1; }
apply_xray_fastopen_off_default() { force_apply_xray_core_defaults_e51 >/dev/null 2>&1 || return 1; }
apply_xray_keep_2_to_6_defaults() { force_apply_xray_core_defaults_e51 >/dev/null 2>&1 || return 1; }
hot_patch_xray_config() { force_apply_xray_core_defaults_e51; }
...
apply_safe_turbo_defaults() {
    title "V198e51 净化安全极速包：DNS 前置 + Swap≤1024MB + 强校验 + 重启持久化"
    repair_dns_if_broken
    check_and_create_swap
    fix_xray_systemd_limits
    apply_hotupdate_fast_sysctl
...
hot_update_from_legacy() {
    title "V198e51 净化稳态热更新：Swap≤1024MB + 去重覆盖 + 持久化自检"
    ...
    local ts bdir
    ts=$(date +%Y%m%d_%H%M%S); bdir="$BACKUP_DIR/hotupdate_e51_${ts}"
    ...
    fix_permissions
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    if ! verify_e51_post_state; then error "V198e51 关键状态校验失败，已保留快照：$bdir"; return 1; fi
    ensure_xray_is_alive
    info "V198e51 热更新完成：Swap≤1024MB、Buffer=128KB、Policy=260s、FQ 默认、GRO/GSO/TSO 保留、dat timer 安全化已落地。"
}


#==============================================================================
# V198e51 Ring Buffer 自适应最大值覆盖层
# 目标：不再用统一 512/1024/2048；读取 Pre-set maximums，RX/TX 分别拉到硬件上限。
#==============================================================================
_ring_parse_value_e51() {
...
_ring_max_rx_e51() { local i="$1"; _ring_parse_value_e51 "Pre-set maximums" "RX" "$i"; }
_ring_max_tx_e51() { local i="$1"; _ring_parse_value_e51 "Pre-set maximums" "TX" "$i"; }
_ring_cur_rx_e51() { local i="$1"; _ring_parse_value_e51 "Current hardware settings" "RX" "$i"; }
_ring_cur_tx_e51() { local i="$1"; _ring_parse_value_e51 "Current hardware settings" "TX" "$i"; }

_apply_ring_target_value_e51() {
    ...
    max_rx=$(_ring_max_rx_e51 "$IFACE")
    max_tx=$(_ring_max_tx_e51 "$IFACE")
    [[ "${max_rx:-}" =~ ^[0-9]+$ ]] || return 1
    ...
    # 兼容旧状态检查文件：写 RX 目标，但 e51 真正以 ring_target_rx/tx 为准。
...
apply_ring_max_adaptive_default() {
    ...
    max_rx=$(_ring_max_rx_e51 "$IFACE")
    max_tx=$(_ring_max_tx_e51 "$IFACE")
    curr_rx=$(_ring_cur_rx_e51 "$IFACE")
    curr_tx=$(_ring_cur_tx_e51 "$IFACE")
    if ! [[ "${max_rx:-}" =~ ^[0-9]+$ ]]; then
...
# 保留菜单 16 入口，但 e51 不再“低延迟收缩”，而是 RX/TX 分别使用硬件最大值。
...
check_ring_state() {
    ...
    max_rx=$(_ring_max_rx_e51 "$IFACE"); max_tx=$(_ring_max_tx_e51 "$IFACE")
    curr_rx=$(_ring_cur_rx_e51 "$IFACE"); curr_tx=$(_ring_cur_tx_e51 "$IFACE")
...
apply_safe_turbo_defaults() {
    title "V198e51 稳态极速包：Dnsmasq + ZRAM≤1024MB + Ring RX/TX自适应最大值"
    repair_dns_if_broken
    check_and_create_swap
    fix_xray_systemd_limits
    apply_hotupdate_fast_sysctl
...
#==============================================================================
# EOF - Apex Vanguard V198e51 System Advanced Control Ready.
#==============================================================================
