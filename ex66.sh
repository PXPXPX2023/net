#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex66.sh (The Apex Vanguard - Project Genesis V66)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 极致提权：Xray 初始 CPU 调度抢占拉升至系统绝对物理最高值 (Nice=-20)。
#   2. 拨动开关化：将 OOM 免死与 I/O 实时抢占转化为第 11 项独立微操，收放自如。
#   3. 状态无损继承引擎：核心升级时自动探取并保留当前 Nice、GOGC 与 OOM 的启停状态，拒绝覆盖。
#   4. 创世级重构：子菜单 7 升级为【11项双向微操枢纽】，一键开启与关闭逻辑完美对齐。
#   5. 维持雷达守望：选项9->4 实时连接数探测，2秒心跳防僵尸。
#   6. 维持防断连生态：官方 XANMOD 预编译打底、无损内存计费快照保留。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex66.sh"
    exit 1
fi
...
fix_xray_systemd_limits() {
    ...
    # 默认值初始化 (极客最高抢占 Nice=-20，OOM 防杀默认开启)
    ...
    if [ -f "$limit_file" ]; then
        ...
        # 若文件中没有 OOMScoreAdjust，说明用户在子菜单中主动关闭了该功能
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex66 The Apex Vanguard - Project Genesis V66${none}"
