#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e5.sh (The Apex Vanguard - Project Genesis V198e5)
# 快捷方式: xrv
# 【V198e5 终极版：召回丢失模块、解封多核极速编译、根除 JQ 回滚 BUG、内核驱动强制内建】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e5.sh"; exit 1; fi
...
preflight() {
    ...
    local need="jq curl wget xxd unzip qrencode vnstat cron openssl coreutils sed e2fsprogs pkg-config iproute2 ethtool bc bison flex dwarves rsync python3 cpio apt-transport-https lz4 liblz4-tool gcc-multilib libc6-dev-i386"
    local missing=""
    for p in $need; do
...
check_and_create_swap() {
    ...
    warn "严格执行指令，直接切辟固化 1380MB Swap 缓冲分区..."
...
do_install_xanmod_main_official() {
    ...
    warn "Xanmod 内核安装完成，系统将在 10 秒后强制重启..."
    sleep 10
    reboot
}
...
_execute_compilation() {
    ...
    info ">>> 侦测可用 CPU 线程并评估安全并发数..."
    local CPU_CORES=$(nproc 2>/dev/null || echo 1)
    # [核心修复：解除1线程编译龟速锁死，依靠1380MB Swap全火力输出]
    THREADS=$CPU_CORES
    
    info "=== 开启多核并发极速编译 (并发线程数: $THREADS，1380MB Swap 提供坚实护航) ==="
    ...
    if test -n "$make_flags"; then
        ...
        if ! eval "$cmd KCFLAGS=\"$make_flags\" $extra_make_args"; then
            error "编译崩塌！请确保您的 VPS 拥有至少 1GB 以上真实的可用 RAM + 1380MB Swap。"
            local _p=""; read -rp "按 Enter 返回主菜单..." _p || true
            return 1
        fi
    else
    ...
    info "保留旧版内核以备万一，配置 GRUB 回退保护并安全重载..."
    if test -f /etc/default/grub; then
        sed -i 's/^GRUB_DEFAULT=.*/GRUB_DEFAULT=saved/g' /etc/default/grub 2>/dev/null || true
        sed -i 's/^GRUB_TIMEOUT=.*/GRUB_TIMEOUT=5/g' /etc/default/grub 2>/dev/null || true
        grep -q "^GRUB_SAVEDEFAULT=true" /etc/default/grub 2>/dev/null || echo "GRUB_SAVEDEFAULT=true" >> /etc/default/grub
        sed -i 's/^GRUB_TIMEOUT_STYLE=.*/GRUB_TIMEOUT_STYLE=menu/g' /etc/default/grub 2>/dev/null || true
    fi
    update-grub >/dev/null 2>&1 || true
...
_compile_kernel_mainline() {
    ...
    export DEBIAN_FRONTEND=noninteractive
    apt-get update -y >/dev/null 2>&1 || true
    apt-get install -y build-essential bc flex bison libssl-dev libelf-dev libncurses-dev dwarves git curl wget lz4 liblz4-tool gcc-multilib libc6-dev-i386 >/dev/null 2>&1 || true
...
do_kernel_compile_menu() {
    while true; do
        ...
        echo "  [已生效] 解封多核并发全火力编译，1380MB Swap 提供底层护航！"
        echo ""
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e5 终极版)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e5 - (The Apex Vanguard)${none}"
...
#==============================================================================
# EOF - Apex Vanguard V198e5 System Advanced Control Ready.
#==============================================================================
