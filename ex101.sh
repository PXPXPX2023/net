#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex101.sh (The Apex Vanguard - Project Genesis V101 [God Domain])
# 快捷方式: xrv
# 终极神域: 
#   1. 零拷贝旁路：实装 routeOnly 盲走快车道，Xray 绕过嗅探拆包，实现类 splice 的纳秒级底层直通。
#   2. 智能拥塞治堵：实装 CAKE 队列管治 (取代 fq)，在跨国高延迟链路上智能削峰填谷，彻底粉碎缓冲膨胀 (Bufferbloat)。
#   3. 硬件级绝对隔离：实装 Hard IRQ Pinning，斩杀 irqbalance，网卡硬件中断死锁 Core 0，Xray 独享 Core 1 真空手术室。
#   4. 双十战神矩阵：全域极客微操登峰造极，定编 20 项 (应用层 10 项 + 系统硬件层 10 项)。
#   5. 上帝裁决：重构“上帝之手” (23项)，引入 6 秒高危倒计时与底层硬重启，确保 Kernel Space 变革完美落盘。
#   6. 绝对绝缘：延续所有 V100 补丁 (MPH 哈希引擎、防重放偏移锁、jq 空指针免疫、大一统全球库)。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，这不仅是脚本，这是单节点路由架构的暴力美学。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex101.sh"
    exit 1
fi
...
# -- [功能 10->7] 全域 20 项极限微操双向设置枢纽 --
do_app_level_tuning_menu() {
    while true; do
        clear
        title "全域 20 项极限微操 (Xray 提速底牌 & 系统内核微操)"
        if ! test -f "$CONFIG"; then 
        ...
        local s7=$([ "$affinity_state" = "true" ] && echo "${cyan}已锁死单核 (零切换)${none}" || echo "${gray}默认 (系统调度)${none}")
        local s8=$([ "$mph_state" = "true" ] && echo "${cyan}O(1) 预编译算法就绪${none}" || echo "${gray}默认 (Linear/AC机)${none}")
        local s9=$([ -z "$has_reality" ] && echo "${gray}无 Reality 节点 (已跳过)${none}" || ([ "$maxtime_state" = "true" ] && echo "${cyan}已开启绝对防线 (60s)${none}" || echo "${gray}默认 (不设防)${none}"))
        local s10=$([ "$routeonly_status" = "true" ] && echo "${cyan}盲走快车道已通车${none}" || echo "${gray}默认全量嗅探${none}")
        
        local s11=$([ "$dnsmasq_state" = "true" ] && echo "${cyan}内存极速解析中 (0.1ms)${none}" || echo "${gray}未开启 (依赖原生DoH)${none}")
        local s12=$([ "$thp_state" = "true" ] && echo "${cyan}已关闭 THP${none}" || ([ "$thp_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}系统默认${none}"))
        local s13=$([ "$mtu_state" = "true" ] && echo "${cyan}智能探测中${none}" || ([ "$mtu_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}未开启${none}"))
        local s14=$([ "$cpu_state" = "true" ] && echo "${cyan}全核火力全开${none}" || ([ "$cpu_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}节能待机${none}"))
        local s15=$([ "$ring_state" = "true" ] && echo "${cyan}已反向收缩${none}" || ([ "$ring_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}系统大缓冲${none}"))
        local s16=$([ "$zram_state" = "true" ] && echo "${cyan}已挂载 ZRAM${none}" || ([ "$zram_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}未启用${none}"))
        local s17=$([ "$journal_state" = "true" ] && echo "${cyan}纯内存极速化${none}" || ([ "$journal_state" = "unsupported" ] && echo "${gray}不支持${none}" || echo "${gray}磁盘 IO 写入中${none}"))
        local s18=$([ "$prio_state" = "true" ] && echo "${cyan}OOM免死 / IO抢占${none}" || echo "${gray}系统默认调度${none}")
        local s19=$([ "$cake_state" = "true" ] && echo "${cyan}CAKE 削峰填谷中${none}" || echo "${gray}默认 FQ 队列${none}")
        local s20=$([ "$irq_state" = "true" ] && echo "${cyan}已锁死 Core 0${none}" || ([ "$irq_state" = "unsupported" ] && echo "${gray}不支持(单核)${none}" || echo "${gray}默认平衡调度${none}"))

        echo -e "  ${magenta}--- Xray Core 应用层内部调优 (1-10) ---${none}"
        ...
        echo -e "  7)  开启或关闭 Xray 进程物理绑核 & GOMAXPROCS (手术室锁死 Core1) | 当前状态: $s7"
        echo -e "  8)  开启或关闭 Minimal Perfect Hash (MPH) 路由匹配极速降维引擎   | 当前状态: $s8"
        echo -e "  9)  开启或关闭 Reality 防重放装甲 (maxTimeDiff 时间偏移绝对拦截) | 当前状态: $s9"
        echo -e "  10) 开启或关闭 零拷贝旁路盲转发 (routeOnly 底层直通快车道)       | 当前状态: $s10"
        echo -e ""
        echo -e "  ${magenta}--- Linux 系统层与内核黑科技 (11-20) ---${none}"
        echo -e "  11) 开启或关闭【Dnsmasq 本地极速内存缓存引擎 (21000并发/锁TTL)】 | 当前状态: $s11"
        echo -e "  12) 开启或关闭【透明大页 (THP - Transparent Huge Pages)】        | 当前状态: $s12"
        echo -e "  13) 开启或关闭【TCP PMTU 黑洞智能探测 (Probing=1)】              | 当前状态: $s13"
        echo -e "  14) 开启或关闭【CPU 频率调度器锁定 (Performance)】               | 当前状态: $s14"
        echo -e "  15) 开启或关闭【网卡硬件环形缓冲区 (Ring Buffer) 反向收缩】      | 当前状态: $s15"
        echo -e "  16) 开启或关闭【ZRAM】(淘汰慢速 Swap，阶梯内存自动检测)          | 当前状态: $s16"
        echo -e "  17) 开启或关闭【日志系统 Journald 纯内存化】(斩断 I/O 羁绊)      | 当前状态: $s17"
        echo -e "  18) 开启或关闭【系统进程级防杀抢占 (OOM/IO 提权)】               | 当前状态: $s18"
        echo -e "  19) 开启或关闭【CAKE 智能队列管治】(取代 fq，强压缓冲膨胀)       | 当前状态: $s19"
        echo -e "  20) 开启或关闭【网卡硬中断物理隔离】(Hard IRQ Pinning 锁死Core0) | 当前状态: $s20"
        echo -e "  "
        echo -e "  ${cyan}21) 一键开启或关闭 1-10 项 应用层微操 (自动侦测并智能反转)${none}"
        echo -e "  ${yellow}22) 一键开启或关闭 11-20 项 系统级微操 (自动避障侦测并反转)${none}"
        echo -e "  ${red}23) 上帝之手：一键开启或关闭 1-20 项 全域微操 (执行后自动重启系统)${none}"
        echo "  0) 返回上一级"
        hr
        read -rp "请选择: " app_opt
        ...
        case "$app_opt" in
            ...
            7)
                if [ "$affinity_state" = "true" ]; then
                    _toggle_affinity_off
                    systemctl restart xray >/dev/null 2>&1
                    info "物理绑核与 GOMAXPROCS 限制已解除，恢复多核自由调度。"
                else
                    _toggle_affinity_on
                    systemctl restart xray >/dev/null 2>&1
                    info "Xray 进程已成功使用铁链锁死在目标物理 Core 1，并发锁已实时激活！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            8)
                if [ "$mph_state" = "true" ]; then
                    ...
                    systemctl restart xray >/dev/null 2>&1
                    info "MPH 路由引擎已关闭，恢复线性/AC匹配模式。"
                else
                    _safe_jq_write '(.routing) |= (. // {}) | (.routing.domainMatcher) = "mph"'
                    systemctl restart xray >/dev/null 2>&1
                    info "Minimal Perfect Hash (MPH) 算法已强力注入，路由查表时间降维至 O(1)！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            9)
                if [ -z "$has_reality" ]; then
                    error "当前节点配置中未检测到 Reality 协议支持，防重放装甲无法挂载。"
                else
                    if [ "$maxtime_state" = "true" ]; then
                        _safe_jq_write 'del(.inbounds[]? | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.realitySettings.maxTimeDiff)'
                        systemctl restart xray >/dev/null 2>&1
                        info "Reality 防重放装甲 (maxTimeDiff) 已解除。"
                    else
                        _safe_jq_write '(.inbounds[]? | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.realitySettings.maxTimeDiff) = 60000'
                        systemctl restart xray >/dev/null 2>&1
                        info "Reality 60秒防重放偏移锁已激活！绝杀 GFW 重复探测！"
                    fi
                fi
                read -rp "按 Enter 继续..." _
                ;;
            10)
                if [ "$routeonly_status" = "true" ]; then
                    ...
                    info "零拷贝盲转旁路已关闭，恢复数据包全流程嗅探。"
                else
                    _safe_jq_write '(.inbounds[]? | select(.protocol=="vless") | .sniffing.routeOnly) = true'
                    info "零拷贝 (routeOnly) 盲走快车道已通车！大幅降低 CPU 负担！"
                fi
                systemctl restart xray >/dev/null 2>&1
                read -rp "按 Enter 继续..." _
                ;;
            11) toggle_dnsmasq; systemctl restart xray >/dev/null 2>&1; read -rp "按 Enter 继续..." _ ;;
            12) toggle_thp; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            13) toggle_mtu; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            14) toggle_cpu; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            15) toggle_ring; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            16) toggle_zram; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            17) toggle_journal; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            18) toggle_process_priority; systemctl restart xray >/dev/null 2>&1; info "操作已下发，进程已重启生效。"; read -rp "按 Enter 继续..." _ ;;
            19) toggle_cake; info "CAKE 队列调优指令已下发。"; read -rp "按 Enter 继续..." _ ;;
            20) toggle_irq; info "硬件中断隔离指令已下发。"; read -rp "按 Enter 继续..." _ ;;
            21)
                if [ "$app_off_count" -gt 0 ]; then
                    print_magenta ">>> 正在全域强力开启 1-10 项，请耐心等待..."
                    _turn_on_app
                    systemctl restart xray >/dev/null 2>&1
                    info "1-10 项 Xray 内部应用层微操已全域强力开启并实时生效！"
                else
                    print_magenta ">>> 正在全域恢复 1-10 项，请耐心等待..."
                    _turn_off_app
                    systemctl restart xray >/dev/null 2>&1
                    info "1-10 项 Xray 内部应用层微操已全部强制关闭，无损还原官方初始状态！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            22)
                if [ "$sys_off_count" -gt 0 ]; then
                    [ "$dnsmasq_state" = "false" ] && toggle_dnsmasq >/dev/null 2>&1
                    [ "$thp_state" = "false" ] && toggle_thp >/dev/null 2>&1
                    [ "$mtu_state" = "false" ] && toggle_mtu >/dev/null 2>&1
                    [ "$cpu_state" = "false" ] && toggle_cpu >/dev/null 2>&1
                    [ "$ring_state" = "false" ] && toggle_ring >/dev/null 2>&1
                    [ "$zram_state" = "false" ] && toggle_zram >/dev/null 2>&1
                    [ "$journal_state" = "false" ] && toggle_journal >/dev/null 2>&1
                    [ "$prio_state" = "false" ] && toggle_process_priority >/dev/null 2>&1
                    [ "$cake_state" = "false" ] && toggle_cake >/dev/null 2>&1
                    [ "$irq_state" = "false" ] && toggle_irq >/dev/null 2>&1
                    systemctl restart xray >/dev/null 2>&1
                    info "11-20 项系统内核底层黑科技已全部一键激活！(不支持的硬件已自动跳过)"
                else
                    [ "$dnsmasq_state" = "true" ] && toggle_dnsmasq >/dev/null 2>&1
                    [ "$thp_state" = "true" ] && toggle_thp >/dev/null 2>&1
                    [ "$mtu_state" = "true" ] && toggle_mtu >/dev/null 2>&1
                    [ "$cpu_state" = "true" ] && toggle_cpu >/dev/null 2>&1
                    [ "$ring_state" = "true" ] && toggle_ring >/dev/null 2>&1
                    [ "$zram_state" = "true" ] && toggle_zram >/dev/null 2>&1
                    [ "$journal_state" = "true" ] && toggle_journal >/dev/null 2>&1
                    [ "$prio_state" = "true" ] && toggle_process_priority >/dev/null 2>&1
                    [ "$cake_state" = "true" ] && toggle_cake >/dev/null 2>&1
                    [ "$irq_state" = "true" ] && toggle_irq >/dev/null 2>&1
                    systemctl restart xray >/dev/null 2>&1
                    info "11-20 项系统内核底层黑科技已全部一键卸载还原！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            23)
                if [ "$((app_off_count + sys_off_count))" -gt 0 ]; then
                    if [ "$app_off_count" -gt 0 ]; then 
                        print_magenta ">>> 正在全域强力激活应用层引擎，请耐心等待..."
                        _turn_on_app
                    fi
                    if [ "$sys_off_count" -gt 0 ]; then
                        print_magenta ">>> 正在全域强力激活底层硬件引擎，请耐心等待..."
                        [ "$dnsmasq_state" = "false" ] && toggle_dnsmasq >/dev/null 2>&1
                        [ "$thp_state" = "false" ] && toggle_thp >/dev/null 2>&1
                        [ "$mtu_state" = "false" ] && toggle_mtu >/dev/null 2>&1
                        [ "$cpu_state" = "false" ] && toggle_cpu >/dev/null 2>&1
                    ...
                    info "上帝降临！1-20 项全域极限微操已【全部满血激活】！(不支持的硬件已自动跳过)"
                else
                    print_magenta ">>> 正在全域卸载并还原应用层与系统层引擎，请耐心等待..."
                    _turn_off_app
                    [ "$dnsmasq_state" = "true" ] && toggle_dnsmasq >/dev/null 2>&1
                    [ "$thp_state" = "true" ] && toggle_thp >/dev/null 2>&1
                    [ "$mtu_state" = "true" ] && toggle_mtu >/dev/null 2>&1
                    ...
                    [ "$cake_state" = "true" ] && toggle_cake >/dev/null 2>&1
                    [ "$irq_state" = "true" ] && toggle_irq >/dev/null 2>&1
                    info "神威收敛！1-20 项全域极限微操已【全部强制关闭】，无损还原初始出厂状态！"
                fi
                ...
                print_yellow "为了确保 CAKE 队列、IRQ 物理绑核与应用层大满贯完美落盘，"
                print_yellow "系统将在 6 秒后自动【强制重启】！请勿中断！"
                print_red "=========================================================="
                echo ""
                for i in {6..1}; do
...
do_sys_init_menu() {
    while true; do
        ...
        echo -e "  ${magenta}6) 全域 20 项极限微操 (CAKE/硬中断隔离/零拷贝)${none}"
        echo "  0) 返回主菜单"
        hr
        read -rp "请选择: " sys_opt
        case "$sys_opt" in
    ...
    while true; do
        ...
        case "$bc" in
            1) 
                ...
                jq --argjson nv "$nv" '(.routing.rules[]? | select(.protocol != null) | select(.protocol | index("bittorrent")) | ._enabled) = $nv' "$CONFIG" > "$CONFIG.tmp"
                mv -f "$CONFIG.tmp" "$CONFIG"
                fix_permissions
                systemctl restart xray
                info "BT 屏蔽已成功切换为: $nv"
            ...
            2) 
                ...
                jq --argjson nv "$nv" '(.routing.rules[]? | select(.domain != null) | select(.domain | index("geosite:category-ads-all")) | ._enabled) = $nv' "$CONFIG" > "$CONFIG.tmp"
                mv -f "$CONFIG.tmp" "$CONFIG"
                fix_permissions
                systemctl restart xray
                info "广告屏蔽已成功切换为: $nv"
    ...
    while true; do
        ...
        case "$s" in
            ...
            3) 
                ...
                clear
                title "商用级网卡流量计费中心"
                local m_day=$(grep -E "^MonthRotate" /etc/vnstat.conf 2>/dev/null | awk '{print $2}' | head -n 1 | tr -d '\r\n')
                m_day=${m_day:-"1 (默认)"}
                ...
                case "$vn_opt" in
                    1) 
                        local current_day=$(grep -E "^MonthRotate" /etc/vnstat.conf 2>/dev/null | awk '{print $2}' | head -n 1 | tr -d '\r\n')
                        ...
                        if test "${d_day:-0}" -ge 1 2>/dev/null && test "${d_day:-0}" -le 31 2>/dev/null; then
                            sed -i '/^[# \t]*MonthRotate/d' /etc/vnstat.conf 2>/dev/null
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex101 The Apex Vanguard - Project Genesis V101${none}"
