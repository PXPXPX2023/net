#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e37.sh (The Apex Vanguard - Project Genesis V196e37)
# 快捷方式: xrv
#
# 【V196e37 真理最终回响版：零缩水、破死锁、全量融合】
#   1. 粉碎死锁: 彻底剔除 make scripts 引发的交互式黑洞，强制采用 make olddefconfig 自动补齐依赖。
#   2. 融合探针: 吸收官方 psABI AWK 探测逻辑，精准定位 CPU 微架构 x86-64-v1~v4。
#   3. 双轨内核: 提供 Xanmod 官方 APT 预编译直装（极稳）与 GitLab 源码编译（极客）双轨。
#   4. 绝对防爆: 编译路线全面继承宿主机 /boot/config，定点切除 PEM 签名与架构断层。
#   5. JQ 绝缘: 100% 覆盖 select(. != null)，深渊级防爆，所有微操全控板全量回归。
# ==============================================================================

if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex196e37.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="196e37"
...
do_kernel_compile_menu() {
    while true; do
        ...
        echo "     - 源码直连 GitLab，智能锁定架构防断层，完美继承现有驱动"
        echo ""
...
_compile_kernel_mainline() {
    ...
    # 【致死 Bug 终结】：必须先复制 .config，然后立刻执行 make olddefconfig 打通非交互通道！
...
_compile_kernel_xanmod() {
    ...
    # 【神级防爆】：精准限定 CONFIG_X86_64_VERSION 防乱拼，彻底废除危险正则！
    ...
    info ">> 正在植入物理隔离盾，防止 Makefile 越级调用残废指令..."
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e37 - (The Apex Vanguard)${none}"
...
# ==============================================================================
# EOF - Apex Vanguard V196e37 System Advanced Control Ready.
# ==============================================================================
# --- ✂️ Part 20 结束，所有代码拼接完毕。请运行 bash ex196e37.sh ---
