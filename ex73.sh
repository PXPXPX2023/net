#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex73.sh (The Apex Vanguard - Project Genesis V73)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 极客容错：完美拦截底层 CPU 调度器路径缺失导致的标准报错，对虚拟化阉割环境实施前置静默包容。
#   2. 极客勘误：修复 XanMod 探针被 bash 误读的 Bug，重构为 awk -f 内存级解析，绕过 noexec。
#   3. 终极收网：融汇全量聊天溯源，达成 Linux 内核、网卡硬件、系统调度与应用层 100% 极速贯通。
#   4. 战神归位：OOM 免死与 I/O 抢占恢复【出厂默认强制注入】，依然受子菜单 11 持久化开关控制。
#   5. 极致提权：Xray 初始 CPU 调度抢占拉升至系统绝对物理最高值 (Nice=-20)。
#   6. 创世级重构：子菜单 7 【11项双向微操枢纽】，一键开启与关闭逻辑完美对齐。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex73.sh"
    exit 1
fi
...
do_app_level_tuning_menu() {
    while true; do
        ...
        echo -e "  "
        echo -e "  ${magenta}12) 一键强力开启以上 1-4 项终极应用层微操${none}"
        echo -e "  ${red}13) 一键强制关闭以上 1-3 项应用层微操 (无损还原初始状态)${none}"
        echo -e "  ${magenta}14) 一键开启或关闭 5-11 项系统级微操 (批量自动化处理)${none}"
        echo "  0) 返回上一级"
        hr
        read -rp "请选择: " app_opt
        ...
        case "$app_opt" in
            ...
            13)
                ...
                ;;
            14)
                echo -e "  ${cyan}1) 一键开启 5-11 项系统级底层微操${none}"
                echo -e "  ${red}2) 一键还原 5-11 项系统默认值${none}"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex71 The Apex Vanguard - Project Genesis V71${none}"
