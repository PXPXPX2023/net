#!/usr/bin/env bash
# ============================================================
# 脚本名称: g7g50x5.sh (The Matrix Nexus Edition)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 引入 [矩阵模式]，支持多选/全选 Top 20 节点构建多维 SNI 容灾阵列。
#   2. 用户管理新增 [专属 SNI 挂载]，独立分配伪装域名，自动组装私有订阅。
#   3. GFW 连通性探测、TLS1.3/ALPN h2 强制质检、防吞噬注入引擎全面就绪。
#   4. vnstat 商业计费、BT/广告双轨拦截、半高无感二维码全量融合。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash g7g50x5.sh"
    exit 1
fi
        ...
        if test -f "$SNI_CACHE_FILE"; then
            ...
            if test "$sel" = "m"; then
                ...
                if test "$m_sel" = "all"; then
                    local arr=($(awk '{print $2}' "$SNI_CACHE_FILE"))
                else
                    local arr=()
                    for i in $m_sel; do
                        local picked=$(awk "NR==$i {print \$2}" "$SNI_CACHE_FILE" 2>/dev/null)
                ...
                local picked=$(awk "NR==$sel {print \$2}" "$SNI_CACHE_FILE" 2>/dev/null)
                if test -n "$picked"; then
                    BEST_SNI="$picked"
                else
                    BEST_SNI=$(awk "NR==1 {print \$2}" "$SNI_CACHE_FILE" 2>/dev/null)
                fi
                SNI_JSON_ARRAY="\"$BEST_SNI\""
            fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray G7G50x5 The Matrix Nexus Edition${none}"
        ...
        echo "  2) 用户管理 (增删用户 / 挂载专属 SNI / 导入外部)"
