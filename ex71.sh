#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex71.sh (The Apex Vanguard - Project Genesis V71)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 终极收网：融汇全量聊天溯源，达成 Linux 内核、网卡硬件、系统调度与应用层 100% 极速贯通。
#   2. 战神归位：OOM 免死与 I/O 抢占恢复【出厂默认强制注入】，依然受子菜单 11 持久化开关控制。
#   3. 智能继承引擎：反向探针精准判别“初次安装的默认注入”与“老用户主动关闭后的状态继承”。
#   4. 极致提权：Xray 初始 CPU 调度抢占拉升至系统绝对物理最高值 (Nice=-20)。
#   5. 创世级重构：子菜单 7 【11项双向微操枢纽】，一键开启与关闭逻辑完美对齐。
...
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex71.sh"
    exit 1
fi
