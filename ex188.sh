#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex188.sh (Xray Advanced Management Script V188)
# 快捷方式: xrv
#
# V188 终极修复与完整版日志:
#   1. 作用域修复: 找回遗失的 17 个 toggle_* 函数，彻底修复一键执行 28 项时的 command not found。
#   2. 排版重铸: 100% 还原节点链接信息分发中心 (do_summary) 的垂直排版格式。
#   3. 菜单补全: 恢复预编译 XANMOD (main) 的前端面板入口与 Vnstat 流量审计的高级交互项。
#   4. 容错加固: 所有微操指令严格遵循 set -euo pipefail，防止非 0 退出码引发的脚本自尽。
# ==============================================================================

# 检查 Bash 运行环境
if test -z "$BASH_VERSION"; then
    echo "Error: Please run this script with bash: bash ex188.sh"
    exit 1
fi
...
config_cake_advanced() {
    ...
    # V188 致命修复: 确保 CAKE 模块在 tc qdisc 前挂载
...
config_cake_advanced() {
    ...
    # V188 核心修复: 确保 CAKE 模块在 tc qdisc 前挂载
    modprobe sch_cake 2>/dev/null || true
...
# ==============================================================================
# V188 核心修复：所有 toggle 开关全部提前声明，免疫 Bash "command not found" 崩溃。
...
do_install() {
    ...
    if ! bash -c "$(curl -fsSL --connect-timeout 10 https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install >/dev/null 2>&1; then
        warn "通过官方脚本拉取失败，您可以稍后通过主菜单 5 尝试重试。"
    fi
...
EOF
    ...
    # === V188 修复: JQ 采用 --slurpfile 隔离合并免疫解析错误 ===
    if [[ "$proto_choice" == "1" || "$proto_choice" == "3" ]]; then
    ...
    while true; do
        ...
        case "$uopt" in
            ...
            m|M)
                ...
                read -rp "请输入要导入的 UUID: " m_uuid
                if [[ -z "$m_uuid" ]]; then continue; fi
                read -rp "请输入对应的 ShortId: " m_sid
                ...
                local m_sni
                read -rp "绑定专属 SNI (直接回车使用系统默认): " m_sni
                if [[ -n "$m_sni" ]]; then
    ...
    while true; do
        ...
        case "$s" in
            ...
            3) 
                ...
                echo "  0) 退出流量中心"
                read -rp "  执行系统任务: " vn_opt
                case "$vn_opt" in
                    1) 
                        read -rp "输入物理结算日标 (1-31): " d_day
                        ...
                        ;;
                    2)
                        read -rp "请输入要穿梭的历史锚点 (格式如 $(date +%Y-%m)，不填默认近 30 天): " d_month
                        if [[ -z "$d_month" ]]; then 
            ...
            5)
                while true; do
                    ...
                    hr
                    read -rp "  键入新的提权参数 (q 撤退): " new_nice
            ...
            8)
                ...
                if [[ "$bopt" == "c" ]]; then backup_config; info "快照已入库封存。"; read -rp "Enter..." _; fi
                ;;
            0) return ;;
        esac
    done
...
do_fallback_probe() {
    ...
    echo ""
    read -rp "情报核验结束，按 Enter 键返回..." _
}

do_uninstall() {
    title "物理级毁灭清理与系统生态还原"
    read -rp "危险指令: 执行后将彻底剥离所有网络拦截、守护进程及私钥配置，无可撤销。确信？(y/n): " confirm
    if [[ "$confirm" != "y" ]]; then 
        return
    fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray 高维控制台 (Apex Vanguard V188 Industrial Base)${none}"
        ...
        echo -e "  引擎态势: $svc | 热键调用: ${cyan}xrv${none} | 物理信标: ${yellow}${SERVER_IP}${none}"
        echo -e "${blue}======================================================================${none}"
        ...
        case "$num" in
            ...
            3) 
                do_summary
                while true; do 
                    read -rp "按 Enter 退回，或敲击 b 开启伪装矩阵漂移: " rb
                    if [[ "$rb" == "b" || "$rb" == "B" ]]; then 
                        if choose_sni; then 
                            _update_matrix
                            do_summary
            ...
            6) 
                if choose_sni; then 
                    _update_matrix
                    do_summary
                    read -rp "指令已写入守护核心，按 Enter 继续..." _
                fi 
                ;;
