#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e44.sh (The Apex Vanguard - Project Genesis V198e44)
# 快捷方式: xrv
# 【V198e44 净化稳态版：去重覆盖 + 持久化自检 + 安全极速默认】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e44.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e44"
...
check_and_clean_space() {
    ...
    # V198e44: 不再 rm -rf /tmp/*、不再删除全部 /var/log/*.log，避免误删 socket/锁文件/审计日志导致服务异常。
    rm -rf \
...
_e44_pause() { local _p=""; read -rp "按 Enter 继续..." _p || true; }
...
choose_best_qdisc() {
    # V198e44: 极速代理默认 fq + BBR。CAKE 不再作为默认项，避免弱 CPU VPS 因整形/分类开销反降速。
...
EOF_TIMER_E44
    local tmp_cron
    tmp_cron=$(mktemp /tmp/cron_e44_XXXXXX) || true
...
verify_sni_strict() {
    ...
    if ! echo "$out" | grep -qi "OCSP response:"; then warn "OCSP Stapling 未发现：这不是致命问题，V198e44 仅提示不强制淘汰。"; fi
...
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
IFACE="$IFACE"
# V198e44: 纯代理吞吐优先保留 GRO/GSO/TSO，降低 CPU 压力；仅关闭 LRO，避免破坏转发语义。
...
do_perf_tuning() {
    ...
    info "配置系统高并发进程限制 (Limits，V198e44 稳态安全版)..."
...
do_app_level_tuning_menu() {
    while true; do
        clear
        title "全域 25 项系统级及应用层微操管理中心（V198e44 稳态极速）"
        if ! test -f "$CONFIG"; then error "未发现配置，请先执行核心部署！"; _e44_pause; return; fi
        ...
        case "$app_opt" in
            1) apply_xray_nodelay_default; systemctl restart xray >/dev/null 2>&1 || true; info "TCP NoDelay 已幂等开启。"; _e44_pause ;;
            1f|1F) if [ "$out_fastopen" = "true" ]; then _safe_jq_write '(.outbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=false | (.inbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=false'; info "TCP FastOpen 已关闭。"; else _safe_jq_write '(.outbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=true | (.inbounds[]? | select(.streamSettings?.sockopt? != null) | .streamSettings.sockopt.tcpFastOpen)=true'; warn "TCP FastOpen 已开启；如跨境握手不稳请关闭。"; fi; systemctl restart xray >/dev/null 2>&1 || true; _e44_pause ;;
            2|3|4|5|6) apply_xray_keep_2_to_6_defaults; remove_xray_env_buffer; systemctl restart xray >/dev/null 2>&1 || true; info "2-6 项已按稳态默认幂等固化。"; _e44_pause ;;
            7) set_xray_gogc_300; systemctl daemon-reload >/dev/null 2>&1 || true; systemctl restart xray >/dev/null 2>&1 || true; info "GOGC=300 已固化。"; _e44_pause ;;
            8) force_apply_xray_core_defaults_e44; remove_xray_env_buffer; systemctl restart xray >/dev/null 2>&1 || true; info "Policy=260s + Buffer=128KB 已固化。"; _e44_pause ;;
            9) apply_xray_cpu_schedule_default; systemctl daemon-reload >/dev/null 2>&1 || true; systemctl restart xray >/dev/null 2>&1 || true; info "CPU 调度已按单核/多核自动检测固化。"; _e44_pause ;;
            10) _safe_jq_write '.routing |= (. // {}) | .routing.domainMatcher = "mph"'; systemctl restart xray >/dev/null 2>&1 || true; info "MPH 已开启。"; _e44_pause ;;
            11) if [ -z "$has_reality" ] || [ "$has_reality" = "null" ]; then error "无 Reality 支持。"; else _safe_jq_write '(.inbounds[]? | select(. != null) | select(.protocol=="vless" and .streamSettings?.security=="reality") | .streamSettings.realitySettings.maxTimeDiff) = 60000'; systemctl restart xray >/dev/null 2>&1 || true; info "Reality 时间锁已开启。"; fi; _e44_pause ;;
            12) enable_dnsmasq_cache_safely; systemctl restart xray >/dev/null 2>&1 || true; _e44_pause ;;
            13) enable_thp_never_default; _e44_pause ;;
            14) apply_hotupdate_fast_sysctl; _e44_pause ;;
            15) enable_cpu_performance_default; _e44_pause ;;
            16) toggle_ring; _e44_pause ;;
            17) toggle_cake_qdisc; update_hw_boot_script; _e44_pause ;;
            18) toggle_cake_flag "ack_filter"; update_hw_boot_script; _e44_pause ;;
            19) toggle_cake_flag "ecn"; update_hw_boot_script; _e44_pause ;;
            20) toggle_cake_flag "wash"; update_hw_boot_script; _e44_pause ;;
            21) toggle_gso; _e44_pause ;;
            22) toggle_irq; _e44_pause ;;
            23) enable_zram_default; _e44_pause ;;
            24) enable_journald_volatile_default; _e44_pause ;;
            25) apply_process_priority_default; systemctl restart xray >/dev/null 2>&1 || true; _e44_pause ;;
            26) _turn_on_app; systemctl restart xray >/dev/null 2>&1 || true; info "应用层 1-11 已幂等激活。"; _e44_pause ;;
            27) apply_safe_turbo_defaults; systemctl restart xray >/dev/null 2>&1 || true; info "安全系统级微操已幂等落地。"; _e44_pause ;;
            28) _turn_on_app; apply_safe_turbo_defaults; systemctl restart xray >/dev/null 2>&1 || true; info "极速稳态一键打通完成。"; _e44_pause ;;
            0) return ;;
        esac
    done
}
...
install_self_entrypoint() {
    ...
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e44.sh" 2>/dev/null || true
        chmod +x "$SYMLINK" "$SCRIPT_DIR/ex198e44.sh" 2>/dev/null || true
    fi
...
apply_hotupdate_fast_sysctl() {
    ...
    mkdir -p /etc/sysctl.d 2>/dev/null || true
    local adv app qdisc
    # V198e44: 热更新默认固化你实测稳定的高 RTT 代理窗口组合。
    adv=1
    app=31
...
enable_dnsmasq_cache_safely() {
    # V198e44: 默认启用 dnsmasq，但必须通过监听与解析验证后才接管 resolv.conf。
...
apply_txqueue_3000_default() {
    # V198e44: 默认 3000，不再用 12000，避免队列过深导致排队延迟和 bufferbloat。
    local IP_CMD IFACE
...
get_ssh_listen_ports() {
    local ports=""
    # V198e44: 优先从当前监听态识别 SSH 端口，避免 sshd_config 使用 Include/云镜像改名时误判为 22。
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e44 稳态极速版)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e44 - (The Apex Vanguard)${none}"
        ...
        echo -e "  ${cyan}11) 从 ex198e32/ex198e33/ex198e34/ex198e35/ex198e36/ex198e37/ex198e38/ex198e39/ex198e40/ex198e41/ex198e42/ex198e43 稳态热更新到 V198e44${none}"
...
rescue_after_v35_breakage() {
    title "V198e44 紧急修复：撤销旧版重启后断流风险项"
...
#==============================================================================
# V198e44 最终覆盖层：128KB Buffer 单位修正 + systemd 环境变量清理
...
toggle_buffer() {
    # V198e44：只在 config.json 的 KB 单位中切换 64/128，永不再写 systemd MB 环境变量。
    local v=""
...
#==============================================================================
# V198e44 覆盖修复层：修正 e40 残留状态/持久化问题
...
_safe_jq_write() {
    # V198e44：以退出码为准，不再强制搜索 Configuration OK 字符串。
...
force_apply_xray_core_defaults_e44() {
...
apply_xray_policy_260_default() { force_apply_xray_core_defaults_e44 >/dev/null 2>&1 || return 1; }
apply_xray_buffer_kb() { local kb="${1:-128}"; if test "$kb" != "128"; then warn "V198e44 默认只固化 128KB；如需其它值请手动改 config.json。"; fi; force_apply_xray_core_defaults_e44 >/dev/null 2>&1 || return 1; remove_xray_env_buffer; }
apply_xray_buffer_128_default() { force_apply_xray_core_defaults_e44 >/dev/null 2>&1 || return 1; remove_xray_env_buffer; }
apply_xray_nodelay_default() { force_apply_xray_core_defaults_e44 >/dev/null 2>&1 || return 1; }
apply_xray_fastopen_off_default() { force_apply_xray_core_defaults_e44 >/dev/null 2>&1 || return 1; }
apply_xray_keep_2_to_6_defaults() { force_apply_xray_core_defaults_e44 >/dev/null 2>&1 || return 1; }
hot_patch_xray_config() { force_apply_xray_core_defaults_e44; }
...
apply_safe_turbo_defaults() {
    title "V198e44 净化安全极速包：DNS 前置 + 强校验 + 重启持久化"
    repair_dns_if_broken
    check_and_create_swap
    fix_xray_systemd_limits
    apply_hotupdate_fast_sysctl
...
hot_update_from_legacy() {
    title "V198e44 净化稳态热更新：去重覆盖 + 持久化自检"
    ...
    local ts bdir
    ts=$(date +%Y%m%d_%H%M%S); bdir="$BACKUP_DIR/hotupdate_e44_${ts}"
    ...
    fix_permissions
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    if ! verify_e44_post_state; then error "V198e44 关键状态校验失败，已保留快照：$bdir"; return 1; fi
    ensure_xray_is_alive
    info "V198e44 热更新完成：Buffer=128KB、Policy=260s、FQ 默认、GRO/GSO/TSO 保留、dat timer 安全化已落地。"
}
...
#==============================================================================
# EOF - Apex Vanguard V198e44 System Advanced Control Ready.
#==============================================================================
