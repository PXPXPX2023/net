#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex76.sh (The Apex Vanguard - Project Genesis V76)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 终极勘误：全域清理历史版本残影，实现表里如一的 V76 视觉与内核统一。
#   2. 智能反转引擎：批量控制 (选项12与13) 升级为全自动状态侦测。缺省即补齐开启，全开即一键无损还原，取消所有多余弹窗。
#   3. 极客容错：完美拦截底层 CPU 调度器路径缺失导致的标准报错，对虚拟化阉割环境实施前置静默包容。
#   4. 战神归位：OOM 免死与 I/O 抢占【出厂默认强制注入】，升级核心时自动继承探取状态。
#   5. 极致提权：Xray 初始 CPU 调度抢占拉升至系统绝对物理最高值 (Nice=-20)。
#   6. 创世级重构：子菜单 7 【11项双向微操枢纽】，全盘掌控内核硬件与应用层。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex76.sh"
    exit 1
fi
...
toggle_cpu() {
    if [ ! -d "/sys/devices/system/cpu/cpu0/cpufreq" ]; then
        warn "当前 VPS 虚拟化架构 (如 LXC/OpenVZ) 被母鸡阉割，不支持修改 CPU 调度器，操作静默跳过。"
        return
    fi
...
do_app_level_tuning_menu() {
    while true; do
        ...
        case "$app_opt" in
            ...
            11) toggle_process_priority; read -rp "按 Enter 继续..." _ ;;
            12)
                local all_app_on="true"
                [ "$out_status" != "true" ] && all_app_on="false"
                [ "$sniff_status" != "true" ] && all_app_on="false"
                [ "$dns_status" != "UseIP" ] && all_app_on="false"
                if [ "$gc_status" = "默认 100" ] || [ "$gc_status" = "100" ]; then all_app_on="false"; fi

                if [ "$all_app_on" = "false" ]; then
                    _safe_jq_write '(.outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt) = {"tcpNoDelay":true, "tcpFastOpen":true, "tcpKeepAliveInterval":15}'
                    _safe_jq_write '(.inbounds[] | select(.protocol=="vless") | .sniffing.metadataOnly) = true | (.inbounds[] | select(.protocol=="vless") | .sniffing.routeOnly) = true'
                    _safe_jq_write '.dns = {"servers":["https://1.1.1.1/dns-query","https://8.8.8.8/dns-query"], "queryStrategy":"UseIP"}'
                ...
                ;;
            13)
                local all_sys_on="true"
                [ "$(check_thp_state)" != "true" ] && all_sys_on="false"
                [ "$(check_mtu_state)" != "true" ] && all_sys_on="false"
                [ "$(check_cpu_state)" != "true" ] && [ -d "/sys/devices/system/cpu/cpu0/cpufreq" ] && all_sys_on="false"
                [ "$(check_ring_state)" != "true" ] && all_sys_on="false"
                [ "$(check_zram_state)" != "true" ] && all_sys_on="false"
                [ "$(check_journal_state)" != "true" ] && all_sys_on="false"
                [ "$(check_process_priority_state)" != "true" ] && all_sys_on="false"

                if [ "$all_sys_on" = "false" ]; then
                    ...
                    [ "$(check_journal_state)" = "true" ] && toggle_journal >/dev/null 2>&1
                    [ "$(check_process_priority_state)" = "true" ] && toggle_process_priority >/dev/null 2>&1
                    info "5-11 项系统级底层提权黑科技已全部一键卸载还原！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            0)
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex76 The Apex Vanguard - Project Genesis V76${none}"
