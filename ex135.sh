#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex135.sh (The Apex Vanguard - Project Genesis V135 [Absolute Horizon])
# 快捷方式: xrv
# 王权回归: 
#   1. 母本守护：以纯正 ex118 为绝对基座，100% 还原所有多行 if/else 逻辑与 jq 结构，杜绝一切偷工减料的单行压缩。
#   2. 参数神圣不可侵犯：完整保留 ex118 的 17 项 BBR3 与内核优化参数，仅在末尾无缝追加 tcp_adv_win_scale 与 tcp_app_win 增量控制。
#   3. 军规排版：严格执行 8 行冒号对齐展示 (协议框架/外网IP/端口/伪装SNI/公钥/ShortId/uTLS引擎/UUID)。
#   4. 绝杀漂移：整合 ax131 的数组同步截断算法，彻底解决删除用户导致的 ShortId 错位断流。
#   5. 物理级调度：完整嵌入 RPS/XPS 万兆智能散列探针与 BQL 底层硬件队列强压限制。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex135.sh"
    exit 1
fi
...
if [ "$QUEUE_COUNT" -gt 1 ]; then
    # 多队列散列
    for q in /sys/class/net/$IFACE/queues/rx-*; do echo "$MASK" > "$q/rps_cpus" 2>/dev/null; done
    for q in /sys/class/net/$IFACE/queues/tx-*; do echo "$MASK" > "$q/xps_cpus" 2>/dev/null; done
else
    # 单队列绑死
...
_turn_off_app() {
    # [致命修复] 即使关闭，也要加入 .sniffing = (.sniffing // {}) 容错，免疫因用户手动删减 JSON 节点导致的一键关闭爆破
    _safe_jq_write '
...
do_sys_init_menu() {
    while true; do
        ...
        echo "  5) 系统内核网络栈极限调优 (含 tcp_adv_win_scale 与 tcp_app_win)"
        echo "  6) 全域 28 项极限微操 (CAKE/RPS 散列/零拷贝/聚合反转)"
...
print_node_block() {
    ...
    echo -e "  ${yellow}%-15s${none} : %s" "协议框架" "$protocol"
    echo -e "  ${yellow}%-15s${none} : %s" "外网IP" "$ip"
    echo -e "  ${yellow}%-15s${none} : %s" "端口" "$port"
    echo -e "  ${yellow}%-15s${none} : %s" "伪装SNI" "${sni:-缺失}"
    echo -e "  ${yellow}%-15s${none} : %s" "公钥(pbk)" "${pbk:-缺失}"
    echo -e "  ${yellow}%-15s${none} : %s" "ShortId" "${shortid:-缺失}"
    echo -e "  ${yellow}%-15s${none} : %s" "uTLS引擎" "$utls"
    echo -e "  ${yellow}%-15s${none} : %s" "用户 UUID" "$uuid"
}
...
do_summary() {
    ...
    if [ -n "$ss_inbound" ] && [ "$ss_inbound" != "null" ]; then
        ...
        echo -e "  ${yellow}%-15s${none} : %s" "协议框架" "Shadowsocks"
        echo -e "  ${yellow}%-15s${none} : %s" "外网IP" "$ip"
        echo -e "  ${yellow}%-15s${none} : %s" "端口" "$s_port"
        echo -e "  ${yellow}%-15s${none} : %s" "伪装SNI" "-"
        echo -e "  ${yellow}%-15s${none} : %s" "公钥(pbk)" "-"
        echo -e "  ${yellow}%-15s${none} : %s" "ShortId" "-"
        echo -e "  ${yellow}%-15s${none} : %s" "加密方式" "$s_method"
        echo -e "  ${yellow}%-15s${none} : %s" "密码/Key" "$s_pass"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex135 The Apex Vanguard - Project Genesis V135${none}"
