#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex188t22.sh (The Apex Vanguard - Enterprise Ultimate Genesis)
# 快捷方式: xrv
#
# 【V188t22 终极重构与增量日志】
#   1. 修复内核拉取: 彻底修复 XanMod 官方 APT 源 `Unable to locate package` 的致命漏洞
#      (适配最新 Debian/Ubuntu 安全策略，改用 archive.key 与 signed-by 语法)。
#   2. 拒绝缩水: 代码全量展开，引入企业级日志记录与 Trap 异常捕获追溯系统。
#   3. 绝对自检: Xray 配置落盘前强制触发 `xray -test` 引擎预检，拒绝暴毙。
#   4. 持续在线: 将 ethtool/rps/cake 硬件层调优全部封装为 Systemd 守护进程，重启持久化。
#   5. 变量净化: 修复 x25519 密钥生成中的隐藏 \r\n，杜绝 JQ 语法解析崩溃。
#   6. 探针与调度: 21项 check_ 探针与 28项调度面板全量物理植入，变量绝对安全。
...
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 本系统底层依赖 Bash 特性，请执行: bash ex188t22.sh"
    exit 1
fi
...
readonly SCRIPT_VERSION="ex188t22-Enterprise-Genesis"
...
EOF
    ...
    systemctl daemon-reload >/dev/null 2>&1 || true
    info "守护进程资源限额已更新并下发至 Systemd 总线。"
}
