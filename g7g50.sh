#!/usr/bin/env bash
# ============================================================
# 脚本名称: g7g50.sh (The Omega Protocol Edition)
# 快捷方式: xrv
# 终极修复: 
#   1. 彻底修复底层 jq 写入函数残留的 600 权限覆盖 Bug，绝杀 status 23 报错。
#   2. 引入权限强锁模块，确保 nobody 用户永远拥有 644 读取权限。
#   3. 完美保留 GFW/CDN 探测、TLS1.3 质检、Top 20 矩阵扩容等全部尖端特性。
#   4. 独立节点备注、半高二维码、vnstat 计费与自毁卸载 100% 稳定运行。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash g7g50.sh"
    exit 1
fi
...
EOF
    ...
    # 强锁写入最终读取权限，绝对防御 status 23 权限报错
    fix_permissions
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray G7G50 The Omega Protocol Edition${none}"
