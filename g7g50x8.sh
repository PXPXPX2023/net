#!/usr/bin/env bash
# ============================================================
# 脚本名称: g7g50x8.sh (The Apex Vanguard V6 Edition)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 商用流量计费中心汉化，动态将 estimated 替换为"估计流量"。
#   2. 流量中心新增按月回溯引擎，支持精准调取指定月份的 30 天流量明细。
...
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash g7g50x8.sh"
    exit 1
fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray G7G50x8 The Apex Vanguard V6 Edition${none}"
