#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex91.sh (The Apex Vanguard - Project Genesis V91)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 三剑合璧之 Nano DB：新增切换至极简路由规则库 (V2Fly 标准轻量版)，大幅降低 CPU Cache Miss。
#   2. 三剑合璧之 物理绑核：新增 CPU Affinity 智能探取与注入，将 Xray 死死锁在固定物理核心，实现零上下文切换。
#   3. 三剑合璧之 并发干预：新增 GOMAXPROCS 环境变量硬覆盖，防止 Go 调度器在劣质虚拟化环境中瞎开线程抢锁。
#   4. 十六边形战神矩阵：全域扩容为 16 项极限微操 (应用层 8 项 + 系统级 8 项)。
#   5. 智能反转引擎：17/18/19 项上帝开关全量适配新架构，依然遵循缺省即补、满血即卸、硬件避障的铁律。
#   6. 状态无损继承：无论升级核心还是重启，CPUAffinity 与 GOMAXPROCS 将被完美提取并永久固化。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex91.sh"
    exit 1
fi
...
# --- 新增的 Dnsmasq 本地极速内存缓存引擎 (第 9 项微操) ---
toggle_dnsmasq() {
    ...
    while true; do
        ...
        case "$app_opt" in
            ...
            8)
                ...
                read -rp "按 Enter 继续..." _
                ;;
            9) toggle_dnsmasq; read -rp "按 Enter 继续..." _ ;;
            ...
            16) toggle_process_priority; info "操作已下发。"; read -rp "按 Enter 继续..." _ ;;
            17)
...
EOF
            ...
            rm -f /tmp/new_new_client.json "$CONFIG.tmp1"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex91 The Apex Vanguard - Project Genesis V91${none}"
