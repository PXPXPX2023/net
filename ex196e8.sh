#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e8.sh (The Apex Vanguard - Project Genesis V196e8)
# 快捷方式: xrv
#
# 【V196e8 终极勘误与真理重塑】
#   1. 根绝编译血案: 彻底废弃对 CONFIG_X86_64_VERSION 及 GENERIC_CPU 的人工冲突干预，根除 -march= 暴毙！
#   2. 斩杀野蛮管道: 剥离 `yes ""` 的暴力传参，使用原生标准的 make olddefconfig。
...
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex196e8.sh"
    exit 1
fi
...
# ------------------------------------------------------------------------------
# 【V196e8 终极勘误 1】：真·预编译官方 XANMOD 部署模块 (修复多重源与 404 死锁)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 【V196e8 终极勘误 2】：真·Xanmod 源码极客编译引擎 (抛弃 Mainline 假货，直取 GitLab)
# ------------------------------------------------------------------------------
...
do_xanmod_compile() {
    ...
    # 【核心防御闭环】：在此处直接使用官方 defconfig。
    # 绝对不再使用 scripts/config 篡改任何与 CPU 微架构 (CONFIG_X86_64_VERSION/GENERIC_CPU) 有关的宏！
    # 将推导权全部交还给 Kconfig，让其自行补全 L1_CACHE_SHIFT 和 -march 架构值。
    
    make defconfig >/dev/null 2>&1 || true
    make scripts >/dev/null 2>&1 || true
    
    info "正在向官方 .config 中注入原生 BBR3 拥塞控制组件..."
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e8 - (The Apex Vanguard)${none}"
...
main_menu
# ==============================================================================
# EOF - Apex Vanguard V196e8 System Advanced Control Ready.
# ==============================================================================
