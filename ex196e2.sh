#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e2.sh (The Apex Vanguard - Project Genesis V196e2)
# 快捷方式: xrv
#
# 【V196e2 终极勘误与真理重塑】
#   1. 真·源码溯源: 彻底废弃停更的 GitHub 镜像，重构探测引擎直连 GitLab 官方仓库，拉取正宗 Xanmod 源码。
#   2. APT 源净化: 修复预编译安装时 GPG 密钥 404 及 sources.list 重复冲突导致的静默瘫痪。
#   3. 剔除朽木: 移除现代系统不兼容的 gnupg1，解除安装依赖链条断裂死锁。
#   4. 完美融合: 全量继承 V193 的 JQ 绝缘护盾 (select(. != null))、严格模式防爆以及 25 项微操。
#   5. 架构级防截断修复: 彻底修复系统菜单函数丢失及循环套娃 Bug，100% 全量工业级铺开。
# ==============================================================================

# 必须用 bash 运行
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex196e2.sh"
    exit 1
fi
...
# ------------------------------------------------------------------------------
# 【V196e2 终极勘误 1】：真·预编译官方 XANMOD 部署模块 (修复多重源与 404 死锁)
# ------------------------------------------------------------------------------
...
do_install_xanmod_main_official() {
    ...
    if test -f "$cpu_level_script"; then
        ...
        # 安全调用探针提取微架构级别
        cpu_level=$("$cpu_level_script" | grep -oE 'x86-64-v[1-4]' | grep -oE '[1-4]' | tail -n 1 || echo "")
    ...
    info "触发 APT 数据同步..."
    # 必须去掉 >/dev/null 2>&1 才能暴露源错误，但我们容忍警告
...
# ------------------------------------------------------------------------------
# 【V196e2 终极勘误 2】：真·Xanmod 源码极客编译引擎 (抛弃 Mainline 假货，直取 GitLab)
# ------------------------------------------------------------------------------
...
do_xanmod_compile() {
    ...
    set +e
    # 解析 GitLab 获取 Xanmod Linux 的最新 Release Tag
    ...
    info "正在执行极限解压..."
    tar -xzf "$KERNEL_FILE"
    # 获取解压后的目录名 (一般形如 linux-6.12.1-xanmod1)
    ...
    # Xanmod 已经内嵌 BBR3，但为了防漏依然注入强制激活指令
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e2 - (The Apex Vanguard)${none}"
...
main_menu
# ==============================================================================
# EOF - Apex Vanguard V196e2 System Advanced Control Ready.
# ==============================================================================
