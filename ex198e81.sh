#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e81.sh (The Apex Vanguard - Project Genesis V198e81)
# 快捷方式: xrv
# 【V198e81 三协议稳态版：Reality Vision / VLESSENC Vision / SS2022 + tcp_notsent_lowat=65536 热更新默认】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e81.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e81"
...
hot_update_from_legacy() {
    ...
    if ! verify_e66_post_state; then
        ...
        warn "可以再次执行：bash ex198e81.sh --hot-update，或进入 10→5 手动应用。"
        return 1
    fi
    ensure_xray_is_alive || true
...
hot_update_from_legacy() {
    ...
    if ! verify_e68_post_state; then
        ...
        warn "可再次执行：bash ex198e81.sh --hot-update，或进入 10→6→28 / 10→5 手动应用。"
        return 1
    fi
...
hot_update_from_legacy() {
    ...
    if ! e69_verify_post_state; then
        ...
        warn "可再次执行：bash ex198e81.sh --hot-update；如 Xray 未运行，执行 journalctl -u xray --no-pager -n 80。"
        return 1
    fi
...
#==============================================================================
# V198e81 Final Override Layer
# 目标：保持 e80 三协议与 13) 菜单能力不变，仅将热更新默认 tcp_notsent_lowat 调整为 65536。
#==============================================================================

readonly E81_NOTSENT_LOWAT_FILE="$CONFIG_DIR/tcp_notsent_lowat.txt"

_e81_install_self_entrypoint() {
    mkdir -p "$SCRIPT_DIR" "$CONFIG_DIR" 2>/dev/null || true
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        chmod +x "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e81.sh" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
        chmod +x "$SCRIPT_DIR/ex198e81.sh" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
    fi
    echo "198e81" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    hash -r 2>/dev/null || true
    info "V198e81 已安装/刷新快捷入口：$SYMLINK"
}
# 兼容旧流程里仍调用 e77_install_self_entrypoint 的位置。
e77_install_self_entrypoint() { _e81_install_self_entrypoint "$@"; }

_e81_current_notsent_lowat() {
    sysctl -n net.ipv4.tcp_notsent_lowat 2>/dev/null | awk '{$1=$1; print}' || echo "unknown"
}

_e81_persistent_notsent_lowat() {
    if test -f "$NETWORK_OPT_CONF"; then
...
_e81_backup_and_strip_notsent_conflicts() {
    mkdir -p "$BACKUP_DIR/sysctl" 2>/dev/null || true
    local f
    for f in /etc/sysctl.conf /etc/sysctl.d/*.conf; do
        test -f "$f" || continue
        ...
        if grep -Eq '^[[:space:]]*net\.ipv4\.tcp_notsent_lowat[[:space:]]*=' "$f" 2>/dev/null; then
            cp -af "$f" "$BACKUP_DIR/sysctl/$(basename "$f").notsent.e81.$(date +%Y%m%d_%H%M%S).bak" 2>/dev/null || true
...
_e81_set_tcp_notsent_lowat() {
    local val="${1:-}" label="${2:-手动}"
    if ! _e81_validate_notsent_lowat "$val"; then
    ...
    if test -f "$NETWORK_OPT_CONF"; then
        cp -af "$NETWORK_OPT_CONF" "$BACKUP_DIR/sysctl/99-network-optimized.conf.notsent.e81.$(date +%Y%m%d_%H%M%S).bak" 2>/dev/null || true
    else
        touch "$NETWORK_OPT_CONF" 2>/dev/null || true
    fi

    sed -i -E '/^[[:space:]]*net\.ipv4\.tcp_notsent_lowat[[:space:]]*=/d' "$NETWORK_OPT_CONF" 2>/dev/null || true
    cat >> "$NETWORK_OPT_CONF" <<EOF_E81_NOTSENT

# V198e81 ${label}: TCP unsent write queue low watermark. 常用：65536=低延迟响应；131072=大网稳态/高RTT推荐。
net.ipv4.tcp_notsent_lowat = ${val}
EOF_E81_NOTSENT

    echo "$val" > "$E81_NOTSENT_LOWAT_FILE" 2>/dev/null || true

    if sysctl -w "net.ipv4.tcp_notsent_lowat=${val}" >/tmp/xray-notsent-e81.log 2>&1; then
        info "运行态已应用：net.ipv4.tcp_notsent_lowat=${val}"
    else
        warn "当前内核可能不支持该值或被云厂商限制；永久配置已写入，详情：/tmp/xray-notsent-e81.log"
        sed 's/^/    /' /tmp/xray-notsent-e81.log 2>/dev/null || true
    fi
    ...
    _e81_show_tcp_notsent_lowat_state
}

_e81_show_tcp_notsent_lowat_state() {
    local run perm saved
    run=$(_e81_current_notsent_lowat)
    perm=$(_e81_persistent_notsent_lowat)
    saved=$(cat "$E81_NOTSENT_LOWAT_FILE" 2>/dev/null || echo "未记录")
    echo ""
...
_e81_manual_tcp_notsent_lowat() {
    local v=""
    echo ""
    warn "手动值建议在 65536 / 131072 / 262144 中测试；过大可能增加排队延迟，过小可能限制高 RTT 吞吐。"
    read -rp "请输入 tcp_notsent_lowat 数值（0 或 4096~4194304，默认 65536）: " v || true
    v=${v:-65536}
    _e81_set_tcp_notsent_lowat "$v" "手动输入"
}
...
do_network_params_menu_e56() {
    while true; do
        clear
        title "网络参数应用中心（V198e81：均衡/高吞吐/手动 + notsent_lowat 永久管理；热更新默认65536）"
        show_network_buffer_params_e56
        _e81_show_tcp_notsent_lowat_state
        show_network_sysctl_conflicts_e58
        _e81_show_notsent_conflicts
        ...
        local opt=""; read -rp "请选择操作: " opt || true
        case "${opt:-}" in
            1) apply_default_full_sysctl_profile_e60; _e81_set_tcp_notsent_lowat 131072 "默认高吞吐档"; _e52_pause ;;
            2) apply_balanced_network_buffer_params_e63; _e52_pause ;;
            3) manual_network_buffer_params_e56; _e52_pause ;;
            ...
            5) if test -f "$NETWORK_OPT_CONF"; then sed 's/^/    /' "$NETWORK_OPT_CONF"; else warn "文件不存在：$NETWORK_OPT_CONF"; fi; _e52_pause ;;
            6) cleanup_network_sysctl_conflicts_e58; apply_default_full_sysctl_profile_e60; _e81_set_tcp_notsent_lowat 131072 "冲突清理后默认高吞吐档"; _e52_pause ;;
            7) _e81_set_tcp_notsent_lowat 65536 "低延迟响应档"; _e52_pause ;;
            8) _e81_set_tcp_notsent_lowat 131072 "稳态高吞吐档"; _e52_pause ;;
            9) _e81_manual_tcp_notsent_lowat; _e52_pause ;;
            10) _e81_show_tcp_notsent_lowat_state; _e81_show_notsent_conflicts; _e52_pause ;;
            0) return ;;
            *) warn "无效操作。"; sleep 1 ;;
        esac
    done
...
hot_update_from_legacy() {
    title "V198e81：稳态热更新（保留 e80 能力 + 默认 tcp_notsent_lowat=65536）"
    set +e
    trap - ERR

    mkdir -p "$CONFIG_DIR" "$SCRIPT_DIR" "$BACKUP_DIR" 2>/dev/null || true
    _e81_install_self_entrypoint || true
    local bdir
    if declare -F e76_state_snapshot >/dev/null 2>&1; then
        bdir=$(e76_state_snapshot "hotupdate_e81")
    else
        bdir="$BACKUP_DIR/hotupdate_e81_$(date +%Y%m%d_%H%M%S)"; mkdir -p "$bdir" 2>/dev/null || true
        cp -a "$CONFIG" "$bdir/config.json.bak" 2>/dev/null || true
        cp -a "$NETWORK_OPT_CONF" "$bdir/99-network-optimized.conf.bak" 2>/dev/null || true
    fi
    info "已创建热更新快照：$bdir"
    ...
    # e81 热更新默认低延迟/响应档：65536。用户仍可在 13 里切换为 131072 或手动值。
    _e81_set_tcp_notsent_lowat 65536 "热更新默认低延迟响应值" || true
    ...
    if declare -F e76_config_semantic_ok >/dev/null 2>&1 && e76_config_semantic_ok "$CONFIG"; then
        ...
        warn "未发现有效现网节点配置。V198e81 已安装为 xrv；请执行 xrv → 1 重新搭建。"
    fi

    echo "198e81" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    _e81_show_tcp_notsent_lowat_state || true
    if declare -F e77_audit_runtime_state >/dev/null 2>&1; then e77_audit_runtime_state || true; fi
    set -e
    trap '_err_handler $? $LINENO "$BASH_COMMAND"' ERR
    info "V198e81 热更新完成。现在可输入：xrv；热更新默认 tcp_notsent_lowat=65536；进入 13 可切换 65536/131072/手动。"
    return 0
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e81 - (Reality / VLESSENC 自选 / SS2022 / notsent默认65536)${none}"
        ...
        dns_mode=$(cat "${E67_DNS_MODE_FILE:-/usr/local/etc/xray/dns_mode.txt}" 2>/dev/null || echo auto)
        tz_line=$(date '+%Z %z' 2>/dev/null || echo unknown)
        notsent_now=$(_e81_current_notsent_lowat)
        notsent_perm=$(_e81_persistent_notsent_lowat)
        echo -e "  引擎态势: $svc | 全局快捷指令: ${cyan}xrv${none} | 外部识别 IP: ${yellow}$(_get_ip 2>/dev/null || echo "探测异常")${none}"
        echo -e "  系统装配内核: ${cyan}${sys_ver}${none} | 微架构侦测: x64-v${yellow}${arch_level}${none}"
        ...
        echo "  9) 全景网络连接审计日志与商用级别网络流量记账本"
        echo "  10) 高级发烧：内核双轨编译(APT/源码)/网络/应用层 60余项极限全控板"
        echo -e "  ${cyan}11) 从旧 ex198e 环境稳态热更新到 V198e81${none}"
        echo -e "  ${cyan}12) 清理空间（安全释放 / 快照收敛 / 显示硬盘与 Swap）${none}"
...
#==============================================================================
# EOF - Apex Vanguard V198e81 tcp_notsent_lowat Control Ready.
#==============================================================================
