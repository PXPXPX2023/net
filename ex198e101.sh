#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e101.sh (The Apex Vanguard - Project Genesis V198e101)
# 快捷方式: xrv
# 【V198e101 三协议稳态版：e97基线 + 内置BBR低资源硬熔断 + XanMod源码路线提示】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e101.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e101"
...
#==============================================================================
# V198e101 FINAL OVERRIDE LAYER
# 目标：
#   1) 保留 ex198e101 完整主菜单 1-20/88，不裁剪任何目录入口。
...
if declare -F _e90_build_builtin_bbr_kernel >/dev/null 2>&1; then
    eval "$(declare -f _e90_build_builtin_bbr_kernel | sed '1s/_e90_build_builtin_bbr_kernel/_e90_build_builtin_bbr_kernel_e101_base/')"
fi
if declare -F hot_update_from_legacy >/dev/null 2>&1; then
    eval "$(declare -f hot_update_from_legacy | sed '1s/hot_update_from_legacy/hot_update_from_legacy_e101_base/')"
fi
if declare -F _e96_bbr_module_source_report >/dev/null 2>&1; then
    eval "$(declare -f _e96_bbr_module_source_report | sed '1s/_e96_bbr_module_source_report/_e96_bbr_module_source_report_e101_base/')" 2>/dev/null || true
fi

readonly E101_AUDIT_DIR="$CONFIG_DIR/audit_e101" 2>/dev/null || E101_AUDIT_DIR="$CONFIG_DIR/audit_e101"

_e101_pause() { local _p=""; read -rp "按 Enter 继续..." _p || true; }
_e101_ts() { date +%Y%m%d_%H%M%S; }
_e101_total_mem_swap_mb() { free -m 2>/dev/null | awk '/^Mem:/{m=$2}/^Swap:/{s=$2} END{print int(m+s)}'; }
_e101_root_free_mb() { df -Pm / 2>/dev/null | awk 'NR==2{print int($4)}'; }
_e101_boot_free_mb() { if findmnt -rn /boot >/dev/null 2>&1; then df -Pm /boot 2>/dev/null | awk 'NR==2{print int($4)}'; else df -Pm / 2>/dev/null | awk 'NR==2{print int($4)}'; fi; }

_e101_prepare_dirs() {
    mkdir -p "$E101_AUDIT_DIR" "$E89_BBRX_BUILD_ROOT" "$E89_BBRX_BACKUP_ROOT" "$E89_BBRX_CURRENT" 2>/dev/null || true
}
...
_e101_builtin_full_compile_precheck() {
    local kind="${1:-}" conf="${2:-}" total root_free boot_free cfgv ok=1 log
    cfgv="$(_e89_bbr_kconfig_value 2>/dev/null || echo unknown)"
    total="$(_e101_total_mem_swap_mb 2>/dev/null || echo 0)"
    root_free="$(_e101_root_free_mb 2>/dev/null || echo 0)"
    boot_free="$(_e101_boot_free_mb 2>/dev/null || echo 0)"
    log="$E101_AUDIT_DIR/builtin-compile-precheck.$(_e101_ts).log"
    mkdir -p "$E101_AUDIT_DIR" 2>/dev/null || true

    title "V198e101：内置型 BBR 完整内核编译硬门禁"
    ...
    if test "$ok" -ne 1; then
        ...
        echo "    export XRAY_E101_ALLOW_BUILTIN_FULL_COMPILE=1"
        echo "    # 兼容旧变量：export XRAY_E100_ALLOW_BUILTIN_FULL_COMPILE=1"
        echo "    # 兼容旧变量：export XRAY_E98_ALLOW_BUILTIN_FULL_COMPILE=1"
        if test "${XRAY_E101_ALLOW_BUILTIN_FULL_COMPILE:-0}" != "1" && test "${XRAY_E100_ALLOW_BUILTIN_FULL_COMPILE:-0}" != "1" && test "${XRAY_E98_ALLOW_BUILTIN_FULL_COMPILE:-0}" != "1"; then return 1; fi
    fi

    if test "${XRAY_E101_ALLOW_BUILTIN_FULL_COMPILE:-0}" != "1" && test "${XRAY_E100_ALLOW_BUILTIN_FULL_COMPILE:-0}" != "1" && test "${XRAY_E98_ALLOW_BUILTIN_FULL_COMPILE:-0}" != "1"; then
        warn "为避免误触，V198e101 默认禁止内置型 BBR 完整编译。"
        warn "确认资源充足并已快照后，可手动 export XRAY_E101_ALLOW_BUILTIN_FULL_COMPILE=1 再尝试。"
...
_e90_build_builtin_bbr_kernel() {
    local kind="$1" conf="$2"
    _e101_prepare_dirs
    _e101_builtin_full_compile_precheck "$kind" "$conf" || { _e101_pause; return 1; }
    if declare -F _e90_build_builtin_bbr_kernel_e101_base >/dev/null 2>&1; then
        _e90_build_builtin_bbr_kernel_e101_base "$kind" "$conf"
    else
...
_e101_builtin_explain() {
    title "V198e101：内置型 BBR / XanMod 说明"
    cat <<'EOF_E101_EXPLAIN'
...
因此：
  ...
  - e101 已在 17 子菜单显式增加 9) 内置型 BBR：完整内核源码补丁编译新内核；
  - e101 的完整编译 Mem+Swap 门槛为 1000MB，但低资源 VPS 仍有 OOM/IO 卡死/GRUB 风险。
...
_e96_bbr_module_source_report() {
    ...
    cfgv="$(_e89_bbr_kconfig_value 2>/dev/null || echo unknown)"
    if test "$cfgv" = "y"; then
        title "V198e101：BBR 源码检查（内置型 y 专用提示）"
        echo "  当前内核：$(uname -r 2>/dev/null || echo unknown)"
    ...
    if declare -F _e96_bbr_module_source_report_e101_base >/dev/null 2>&1; then
        _e96_bbr_module_source_report_e101_base
    else
        title "V198e101：BBR 模块源码 / headers 检查"
        _e96_bbr_find_or_prepare_source >/dev/null 2>&1 && info "已找到 tcp_bbr.c。" || warn "未找到 tcp_bbr.c。"
    fi
}

_e101_builtin_bbr_full_compile_menu() {
    local m conf go cfgv
    while true; do
        cfgv="$(_e89_bbr_kconfig_value 2>/dev/null || echo unknown)"
        title "V198e101：内置型 BBR 完整源码补丁编译入口"
        echo "  当前替换路线：$(_e101_bbr_mode_text)"
        echo "  CONFIG_TCP_CONG_BBR：$cfgv"
        ...
        read -rp "请选择: " m || true
        case "${m:-}" in
            1) _e101_builtin_full_compile_precheck "bbr3" "$E89_BBRX_CURRENT/bbr3_replace.conf" || true; _e101_pause ;;
            2) _e89_collect_bbr3_params recommended || return; conf="$(_e89_write_bbr3_conf)"; info "已写入：$conf"; read -rp "确认走完整内核源码补丁编译？[y/N]: " go || true; [[ "$go" =~ ^[yY]$ ]] && _e90_build_builtin_bbr_kernel bbr3 "$conf"; _e101_pause ;;
            3) _e89_collect_bbr3_params custom || return; conf="$(_e89_write_bbr3_conf)"; info "已写入：$conf"; read -rp "确认走完整内核源码补丁编译？[y/N]: " go || true; [[ "$go" =~ ^[yY]$ ]] && _e90_build_builtin_bbr_kernel bbr3 "$conf"; _e101_pause ;;
            4) conf="$E89_BBRX_CURRENT/bbr3_replace.conf"; if test -f "$conf"; then read -rp "确认使用 $conf 完整编译新内核？[y/N]: " go || true; [[ "$go" =~ ^[yY]$ ]] && _e90_build_builtin_bbr_kernel bbr3 "$conf"; else warn "配置不存在，请先执行 2 或 3。"; fi; _e101_pause ;;
            5) _e89_collect_bbrv1_params recommended || return; conf="$(_e89_write_bbrv1_conf)"; info "已写入：$conf"; warn "当前若为 XanMod/BBR3，不建议用 BBR v1 模板。"; read -rp "仍确认完整编译新内核？[y/N]: " go || true; [[ "$go" =~ ^[yY]$ ]] && _e90_build_builtin_bbr_kernel bbrv1 "$conf"; _e101_pause ;;
            7) _e101_builtin_explain; _e101_pause ;;
            8) if declare -F _e97_why_intel_compile_stuck >/dev/null 2>&1; then _e97_why_intel_compile_stuck; else _e101_builtin_explain; fi; _e101_pause ;;
            0) return ;;
            *) warn "无效操作。"; sleep 1 ;;
        esac
    done
...
_e101_bbr3_flow() {
    ...
    while true; do
        cfgv="$(_e89_bbr_kconfig_value 2>/dev/null || echo unknown)"
        title "V198e101：BBR3 / XanMod → bbrx3_v1 参数中心"
        echo "  当前替换路线：$(_e101_bbr_mode_text)"
        if test "$cfgv" = "y"; then
        ...
        read -rp "请选择: " m || true
        case "${m:-}" in
            1) _e89_show_bbr3_table; _e101_pause ;;
            2) _e89_collect_bbr3_params recommended || return; conf="$(_e89_write_bbr3_conf)"; info "已写入：$conf"; read -rp "是否立即替换 bbr / 内置型完整编译？[y/N]: " go || true; [[ "$go" =~ ^[yY]$ ]] && _e89_build_replace_bbr bbr3 "$conf"; _e101_pause ;;
            3) _e89_collect_bbr3_params custom || return; conf="$(_e89_write_bbr3_conf)"; info "已写入：$conf"; read -rp "是否立即替换 bbr / 内置型完整编译？[y/N]: " go || true; [[ "$go" =~ ^[yY]$ ]] && _e89_build_replace_bbr bbr3 "$conf"; _e101_pause ;;
            4) conf="$E89_BBRX_CURRENT/bbr3_replace.conf"; if test -f "$conf"; then _e89_build_replace_bbr bbr3 "$conf"; else warn "配置不存在，请先执行 2 或 3。"; fi; _e101_pause ;;
            7) _e101_builtin_explain; _e101_pause ;;
            9) _e101_builtin_bbr_full_compile_menu ;;
            0) return ;;
            *) warn "无效操作。"; sleep 1 ;;
        esac
    done
...
_e101_bbrv1_flow() {
    ...
    while true; do
        cfgv="$(_e89_bbr_kconfig_value 2>/dev/null || echo unknown)"
        title "V198e101：普通 Linux 主线 BBR v1 → bbrx_v1 参数中心"
        echo "  当前替换路线：$(_e101_bbr_mode_text)"
        if test "$cfgv" = "y"; then
        ...
        read -rp "请选择: " m || true
        case "${m:-}" in
            1) _e89_show_bbrv1_table; _e101_pause ;;
            2) _e89_collect_bbrv1_params recommended || return; conf="$(_e89_write_bbrv1_conf)"; info "已写入：$conf"; read -rp "是否立即替换 bbr / 内置型完整编译？[y/N]: " go || true; [[ "$go" =~ ^[yY]$ ]] && _e89_build_replace_bbr bbrv1 "$conf"; _e101_pause ;;
            3) _e89_collect_bbrv1_params custom || return; conf="$(_e89_write_bbrv1_conf)"; info "已写入：$conf"; read -rp "是否立即替换 bbr / 内置型完整编译？[y/N]: " go || true; [[ "$go" =~ ^[yY]$ ]] && _e89_build_replace_bbr bbrv1 "$conf"; _e101_pause ;;
            4) conf="$E89_BBRX_CURRENT/bbrv1_replace.conf"; if test -f "$conf"; then _e89_build_replace_bbr bbrv1 "$conf"; else warn "配置不存在，请先执行 2 或 3。"; fi; _e101_pause ;;
            7) _e101_builtin_explain; _e101_pause ;;
            9) _e101_builtin_bbr_full_compile_menu ;;
            0) return ;;
            *) warn "无效操作。"; sleep 1 ;;
...
# 兼容旧菜单名称：让旧调用进入 e101 flow。
_e98_bbr3_flow() { _e101_bbr3_flow; }
_e98_bbrv1_flow() { _e101_bbrv1_flow; }
_e96_bbr3_flow() { _e101_bbr3_flow; }
_e96_bbrv1_flow() { _e101_bbrv1_flow; }
...
do_bbr_body_replace_menu_e89() {
    while true; do
        clear
        title "V198e101：BBR 本体替换 / 自定义编译中心"
        echo "  自动建议：$(_e89_bbr_detect 2>/dev/null || true)"
        echo "  当前替换路线：$(_e101_bbr_mode_text)"
        echo ""
        echo "  1) 自动检测当前 BBR 类型，并推荐编译方案"
        ...
        read -rp "请选择: " opt || true
        case "${opt:-}" in
            1) _e89_bbr_detect; _e101_pause ;;
            2) _e101_bbrv1_flow ;;
            3) _e101_bbr3_flow ;;
            4) _e89_bbr_show_backups; _e101_pause ;;
            5) _e89_bbr_rollback_latest || warn "如果本次是内置型整内核编译，请通过云厂商 VNC/GRUB 选择旧内核回滚。"; _e101_pause ;;
            6) _e89_bbr_status; _e101_pause ;;
            7) _e101_builtin_explain; _e101_pause ;;
            8) if declare -F _e97_why_intel_compile_stuck >/dev/null 2>&1; then _e97_why_intel_compile_stuck; else _e101_builtin_explain; fi; _e101_pause ;;
            9) _e101_builtin_bbr_full_compile_menu ;;
            0) return ;;
...
hot_update_from_legacy() {
    title "V198e101：稳态热更新（完整主菜单保留 + BBR目录修复 + 真BBRv3源码/补丁1000MB门槛）"
    if declare -F hot_update_from_legacy_e101_base >/dev/null 2>&1; then
        hot_update_from_legacy_e101_base "$@" || true
    fi
    _e101_prepare_dirs
    echo "198e101" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    if test -f "${SCRIPT_PATH:-$0}"; then
        cp -f "${SCRIPT_PATH:-$0}" "$SYMLINK" 2>/dev/null || true
        cp -f "${SCRIPT_PATH:-$0}" "$SCRIPT_DIR/ex198e101.sh" 2>/dev/null || true
        cp -f "${SCRIPT_PATH:-$0}" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
        chmod +x "$SYMLINK" "$SCRIPT_DIR/ex198e101.sh" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
    fi
    hash -r 2>/dev/null || true
    info "V198e101 热更新完成。"
...
main_menu() {
    while true; do
        ...
        actual_qdisc="$(tc qdisc show dev "$iface" 2>/dev/null | head -n 1 || echo unknown)"
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e101 - (真BBRv3源码/补丁1000MB门槛 + 目录修复 + 防误编译)${none}"
        echo -e "  引擎态势: ${green}${running}${none} | 全局快捷指令: ${cyan}xrv${none} | 外部识别 IP: ${yellow}${ip}${none}"
        echo -e "  系统装配内核: ${cyan}$(uname -r 2>/dev/null || echo unknown)${none} | 微架构侦测: x64-v${yellow}$(detect_x86_64_level 2>/dev/null || echo '?')${none} | sysctl qdisc/cc: ${yellow}$(_e88_sysctl_get net.core.default_qdisc 2>/dev/null || sysctl -n net.core.default_qdisc 2>/dev/null || echo '?')/$(_e88_sysctl_get net.ipv4.tcp_congestion_control 2>/dev/null || sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null || echo '?')${none} | BBR配置:${yellow}${bbr_cfg}${none}"
        echo -e "  实际root队列: ${cyan}${actual_qdisc}${none}"
        echo -e "  DNS模式: ${cyan}$(cat "${E67_DNS_MODE_FILE:-/usr/local/etc/xray/dns_mode.txt}" 2>/dev/null || echo auto)${none} | 时间轴: ${cyan}${nowtz}${none} | notsent: ${yellow}$(_e88_current_notsent_lowat 2>/dev/null || echo '?')${none}/${cyan}$(_e88_persistent_notsent_lowat 2>/dev/null || echo '?')${none} | adv/app: ${yellow}$(_e88_sysctl_get net.ipv4.tcp_adv_win_scale 2>/dev/null || sysctl -n net.ipv4.tcp_adv_win_scale 2>/dev/null || echo '?')/$(_e88_sysctl_get net.ipv4.tcp_app_win 2>/dev/null || sysctl -n net.ipv4.tcp_app_win 2>/dev/null || echo '?')${none}"
        ...
        echo "  9) 全景网络连接审计日志与商用级别网络流量记账本"
        echo "  10) 高级发烧：内核双轨编译(APT/源码)/网络/应用层 60余项极限全控板"
        echo -e "  ${green}11) 从旧 ex198e 环境稳态热更新到 V198e101${none}"
        echo -e "  ${cyan}12) 清理空间（安全释放 / 快照收敛 / 显示硬盘与 Swap）${none}"
        echo -e "  ${cyan}13) 应用网络参数（缓冲/notsent/adv_win/app_win/txqueue/OOOX/MSS）${none}"
        echo -e "  ${cyan}14) TCP 重传 / 单连接诊断中心（只读排障）${none}"
        echo -e "  ${cyan}15) 检查现有配置并重新输出节点${none}"
        echo -e "  ${cyan}16) 状态隔离审计 + VLESSENC 能力检查${none}"
        echo -e "  ${magenta}17) BBR 本体替换 / 自定义编译中心（e101真BBRv3源码/补丁1000MB门槛 + 内置型入口）${none}"
        echo -e "  ${cyan}18) V198e92 安全审计 / Top20复测 / 启动防砖中心${none}"
        echo -e "  ${cyan}19) Xray 跨境链路速度诊断中心（V198e95补强版）${none}"
        echo -e "  ${cyan}20) TC 带宽控制模块（统一接管/精细上限/默认不启用）${none}"
        echo "  0) 折叠命令控制台，返回底层终端"
        ...
        case "${num:-}" in
            ...
            2) do_user_manager ;;
            3) do_summary; while true; do local rb=""; read -rp "输出完毕，按 Enter 退出或者按 b 发起特征替换: " rb || true; if test "$rb" = "b" || test "$rb" = "B"; then if choose_sni; then _update_matrix; do_summary; else break; fi; else break; fi; done ;;
            4) print_magenta ">>> 初始化云网络联通."; bash "$UPDATE_DAT_SCRIPT" >/dev/null 2>&1 || true; ensure_xray_is_alive; info "配置已穿透替换系统！当前路由库完成热加载！"; _e101_pause ;;
            5) do_update_core ;;
            6) if choose_sni; then _update_matrix; do_summary; fi ;;
            7) _global_block_rules ;;
            8) do_fallback_probe ;;
            ...
            14) do_tcp_diag_menu_e63 ;;
            15) do_install_repair_e77 ;;
            16) e77_audit_runtime_state; _e101_pause ;;
            17) do_bbr_body_replace_menu_e89 ;;
            18) do_e92_audit_menu ;;
            19) if declare -F do_xray_speed_diag_menu_e95 >/dev/null 2>&1; then do_xray_speed_diag_menu_e95; else do_xray_speed_diag_menu_e94; fi ;;
            20) if declare -F do_tc_bandwidth_menu_e95 >/dev/null 2>&1; then do_tc_bandwidth_menu_e95; else do_tc_bandwidth_menu_e94; fi ;;
...
#==============================================================================
# EOF - Apex Vanguard V198e101 Safety Hot Update Layer.
#==============================================================================
