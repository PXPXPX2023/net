#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex188t27.sh (The Apex Vanguard - Ultimate Genesis 3000+)
# 快捷方式: xrv
# 版本号: V188t27.Mega.Unabridged.Final
#
# 【V188t27 终极融合补全编年史】
#   1. 找回丢失的灵魂: 彻底修复 ensure_xray_is_alive 与配置灾备回滚模块被吞噬的严重 Bug。
#   2. 绝对全量展开: 100% 继承 ex99.sh 的战神矩阵与 ex188 的高压网络栈，不压缩任何一行代码。
#   3. 权限镇压: 强行将 Xray 提权至 User=root，彻底解决 nobody 用户在 Debian 11 下的权限断层。
#   4. 镜像防线: 为 Xray 核心安装加入 JsDelivr/Fastgit 镜像双轨下载，超时立即物理熔断。
#   5. 绝缘写入: 为所有 JQ 逻辑加装 select(. != null) 绝缘层，彻底根除 JSON 语法崩溃。
# ==============================================================================
...
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 本脚本深度依赖 Bash 高级特性，请执行: bash ex188t27.sh"
    exit 1
fi
...
# ------------------------------------------------------------------------------
# [ 0x09: Systemd 特种兵级提权 (修复 nobody 暴毙与重构资源防线) ]
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# [ 0x13: 极限压榨：低延迟系统底层网络栈调优 (全量展现，绝不压行) ]
# ------------------------------------------------------------------------------
...
# 防止高并发大流量下出现 "Neighbor table overflow"
...
# ------------------------------------------------------------------------------
# [ 0x1A: 核心通信协议框架部署 (原生 Bash HereDoc + 绝缘 JSON 注入) ]
# ------------------------------------------------------------------------------
...
do_install() {
    ...
    echo -e "  ${cyan}请指派您需要部署的代理协议安全架构：${none}"
...
# ------------------------------------------------------------------------------
# [ 0x1C: 多用户管理与外部导入中心 (绝缘 JQ 数组防崩保护版) ]
# ------------------------------------------------------------------------------
        ...
        case "${b_opt:-}" in
            1)
                ...
                if test -n "$block_domain"; then
                    ...
                    if _safe_jq_write "
                        (.routing.rules[]? | select(.outboundTag==\"block\" and .domain != null) | .domain) += [\"$block_domain\"] |
            ...
            2)
                ...
                if test -n "$block_ip"; then
                    info "拉取系统层路由缓存..."
                    if _safe_jq_write "
                        (.routing.rules[]? | select(.outboundTag==\"block\" and .ip != null) | .ip) += [\"$block_ip\"] |
...
# ------------------------------------------------------------------------------
# [ 0x20: 最高防线控制中枢主屏幕展示区 (The Genesis Hub) ]
# ------------------------------------------------------------------------------
