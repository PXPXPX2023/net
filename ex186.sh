#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex186.sh (Xray Advanced Management Script V186)
# 快捷方式: xrv
#
# V186 核心级灾难修复日志:
#   1. 内核验证修复: 修复 mktemp 缺失 .json 后缀导致 Xray 核心抛出 Failed to get format 的致命验证错误。
#   2. 布尔逻辑倒转: 修正 verify_sni_strict 函数中的 Bash 退出码反转漏洞，恢复 SNI 失败时的交互提示屏障。
#   3. 规范延续: 全篇统一采用工业级 [INFO]/[WARN]/[ERROR] 日志标准，杜绝冗余修辞。
#   4. JSON 重构: 彻底淘汰危险的 jq input 合并流，强制采用 --slurpfile 隔离写入。
# ==============================================================================

# 检查 Bash 运行环境
if test -z "$BASH_VERSION"; then
    echo "Error: Please run this script with bash: bash ex186.sh"
    exit 1
fi
...
# 安全 JSON 写入接口 (V186 修复: 强制追加 .json 后缀以适配核心检测)
_safe_jq_write() {
    backup_config
    local tmp_raw
...
verify_sni_strict() {
    ...
    # V186 核心修复: 在 Bash 中 0 表示成功，非 0 表示失败！
    local pass=0
...
do_app_level_tuning_menu() {
    while true; do
        ...
        echo -e "  ${red}28) 一键执行: 应用全量网络调优并重启系统${none}"
        echo "  0) 返回上一级菜单"
        hr
...
EOF
    ...
    # === V186 核心重构: JQ 使用 --slurpfile 安全隔离并合并配置 ===
    if [[ "$proto_choice" == "1" || "$proto_choice" == "3" ]]; then
...
_global_block_rules() {
    while true; do
        ...
        echo -e "  2) 全局广告域名黑洞过滤 | 状态: ${yellow}${ad_en:-未知}${none}"
        echo "  0) 返回上级菜单"
        read -rp "请选择: " bc
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray 高级管理系统 (Industrial Base V186)${none}"
