#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e27.sh (The Apex Vanguard - Project Genesis V196e27)
# 快捷方式: xrv
#
# 【V196e27 真理闭环版：完美破除编译证书报错与失联卡死】
#   1. 解决签名暴毙: 精准清空 CONFIG_SYSTEM_TRUSTED_KEYS 等证书路径，解决 PEM 找不到的 Error 2。
#   2. 驱动基因继承: 拷贝宿主机 /boot/config，保留所有显卡(DRM)与网卡驱动，彻底告别无头黑屏。
#   3. 解决交互死锁: 预处理后执行 make olddefconfig 自动补全默认值，避免编译隐形卡死。
#   4. 维持防爆打击: sed 抹除 Makefile march 断层机制；JQ 全域覆盖 select(. != null)。
#   5. 保留退路: 废除编译后强删旧内核逻辑，保障 GRUB 回滚安全。
# ==============================================================================

if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex196e27.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="196e27"
...
do_kernel_compile_menu() {
    while true; do
        clear
        title "极客内核源码锻造中心 (V196e27 真理终结版)"
        echo "  修复了签名密钥缺失导致的 Error 2 问题及显卡驱动剔除带来的黑屏隐患！"
        echo "  现已引入终极清道夫机制与驱动基因完美继承，彻防卡死！"
        echo ""
...
_compile_kernel_mainline() {
    ...
    # ★ 致命修复：必须立刻执行 olddefconfig！否则后面的 make 会陷入隐形交互死锁！
    info ">> 正在补全自动依赖关系..."
...
_compile_kernel_xanmod() {
    title "系统飞升：编译 真·Xanmod 极客内核 (全版本上帝防爆模式)"
    warn "警告: 此过程将极度消耗 CPU 与内存 (约 30-60 分钟)，低配机极易死机！"
    ...
    # 【极客上帝模式】: 彻底斩断 Makefile 里引发 error 1 的隐患参数！
    info ">> 启动架构修正：物理抹除 arch/x86 目录下的动态架构拼写断层..."
    find arch/x86/ -type f -name "Makefile" -exec sed -i 's/-march=x86-64-v[^ \t]*/-march=x86-64/g' {} + 2>/dev/null || true

    info ">> 正在清洗配置项，移除危险的微架构开关与 Debug 羁绊..."
    ./scripts/config --disable CONFIG_MODULE_SIG
    ./scripts/config --disable CONFIG_MODULE_SIG_ALL
    ./scripts/config --disable CONFIG_MODULE_SIG_FORCE
    ./scripts/config --disable CONFIG_DEBUG_INFO
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e27 - (The Apex Vanguard)${none}"
...
# ==============================================================================
# EOF - Apex Vanguard V196e27 System Advanced Control Ready.
# ==============================================================================
# --- ✂️ Part 14 结束，所有代码拼接完毕。请运行 bash ex196e27.sh ---
