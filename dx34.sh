#!/usr/bin/env bash
# ============================================================
# 脚本名称: dx34.sh (The Apex Vanguard - Project Genesis V6)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 彻底修复 Bash 字符串截断导致 JSON 写入失败、不弹节点信息的致命 Bug。
#   2. 重构 JSON 注入引擎，全面采用 HereDoc 机制，保障底层格式绝对稳固。
#   3. 补全核心部署后的交互闭环，恢复无感热切 SNI 矩阵 (按 b 重选) 功能。
#   4. Xray 并发精确下调至 131072 严防 OOM，卸载模块实现 DNS 与生态全量复原。
#   5. 拒绝任何代码折叠，100% 结构化铺开，无损继承百万并发压榨与内核编译。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash dx34.sh"
    exit 1
fi
