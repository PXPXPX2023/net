#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex82.sh (The Apex Vanguard - Project Genesis V82)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 极限切割：将 tcpFastOpen 提速与 Socket KeepAlive 心跳保活完全拆分为独立微操。
#   2. 毒辣策略：实装 Xray Policy 策略组，精确控制 connIdle=60s 与 handshake=3s，极速斩杀死连接。
#   3. 六边形矩阵：扩容为 13 项极客双向微操枢纽 (应用层 6 项 + 系统底层 7 项)。
#   4. 智能反转引擎：14/15/16 项全量自适应硬件探测，缺省即补齐，全满即一键卸载复原。
#   5. 极客容错：完美拦截虚拟化阉割环境的 Bash 底层报错，实施前置静默包容。
#   6. 战神归位：OOM 免死与 I/O 抢占【出厂强制注入】，Nice=-20 最高主权，无损继承。
#   7. 拒绝任何代码折叠，100% 结构化全量铺开，工业级防呆与容错全面上线。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex82.sh"
    exit 1
fi
...
check_cpu_state() {
    if [ ! -d "/sys/devices/system/cpu/cpu0/cpufreq" ]; then echo "false"; return; fi
...
toggle_cpu() {
    if [ ! -d "/sys/devices/system/cpu/cpu0/cpufreq" ]; then return; fi
...
_turn_on_app() {
    _safe_jq_write '(.outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt.tcpNoDelay) = true | (.outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt.tcpFastOpen) = true | (.inbounds[] | select(.protocol=="vless") | .streamSettings.sockopt.tcpNoDelay) = true | (.inbounds[] | select(.protocol=="vless") | .streamSettings.sockopt.tcpFastOpen) = true'
    _safe_jq_write '(.outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt.tcpKeepAliveIdle) = 30 | (.outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt.tcpKeepAliveInterval) = 15 | (.inbounds[] | select(.protocol=="vless") | .streamSettings.sockopt.tcpKeepAliveIdle) = 30 | (.inbounds[] | select(.protocol=="vless") | .streamSettings.sockopt.tcpKeepAliveInterval) = 15'
...
_turn_off_app() {
    _safe_jq_write 'del(.outbounds[] | select(.protocol=="freedom") | .streamSettings.sockopt) | del(.inbounds[] | select(.protocol=="vless") | .streamSettings.sockopt)'
...
# -- [功能 10->7] 应用层与系统层 13 项双向设置枢纽 --
do_app_level_tuning_menu() {
    while true; do
        clear
        title "全域 13 项极限微操 (Xray 提速底牌 & 系统内核微操)"
        if ! test -f "$CONFIG"; then 
        ...
        # 缺省探测引擎 (系统层 - 7~13项 硬件自适应剔除)
        local sys_off_count=0
        if [ -f "/sys/kernel/mm/transparent_hugepage/enabled" ]; then
            [ "$(check_thp_state)" = "false" ] && sys_off_count=$((sys_off_count+1))
        fi
        [ "$(check_mtu_state)" = "false" ] && sys_off_count=$((sys_off_count+1))
        if [ -d "/sys/devices/system/cpu/cpu0/cpufreq" ]; then
            [ "$(check_cpu_state)" = "false" ] && sys_off_count=$((sys_off_count+1))
        fi
        local IFACE=$(ip route get 1.1.1.1 | awk '{print $5; exit}')
        if ethtool -g "$IFACE" >/dev/null 2>&1; then
            [ "$(check_ring_state)" = "false" ] && sys_off_count=$((sys_off_count+1))
        fi
        if modprobe -n zram >/dev/null 2>&1 || lsmod | grep -q zram; then
            [ "$(check_zram_state)" = "false" ] && sys_off_count=$((sys_off_count+1))
        fi
        [ "$(check_journal_state)" = "false" ] && sys_off_count=$((sys_off_count+1))
        [ "$(check_process_priority_state)" = "false" ] && sys_off_count=$((sys_off_count+1))
        ...
        local s7=$([ "$(check_thp_state)" = "true" ] && echo "${cyan}已关闭 THP${none}" || echo "${gray}系统默认${none}")
        local s8=$([ "$(check_mtu_state)" = "true" ] && echo "${cyan}智能探测中${none}" || echo "${gray}未开启${none}")
        local s9=$([ "$(check_cpu_state)" = "true" ] && echo "${cyan}全核火力全开${none}" || echo "${gray}节能待机 / 不支持${none}")
        local s10=$([ "$(check_ring_state)" = "true" ] && echo "${cyan}已反向收缩${none}" || echo "${gray}系统大缓冲 / 不支持${none}")
        local s11=$([ "$(check_zram_state)" = "true" ] && echo "${cyan}已挂载 ZRAM${none}" || echo "${gray}未启用 / 不支持${none}")
        local s12=$([ "$(check_journal_state)" = "true" ] && echo "${cyan}纯内存极速化${none}" || echo "${gray}磁盘 IO 写入中${none}")
        local s13=$([ "$(check_process_priority_state)" = "true" ] && echo "${cyan}OOM免死 / IO抢占${none}" || echo "${gray}系统默认调度${none}")
        ...
        echo -e "  4) 开启或关闭 内置并发 DoH 引擎 (Xray Native DNS)               | 当前状态: $s4"
        echo -e "  5) 执行或关闭 GOGC 内存阶梯飙车调优 (自动侦测物理内存)          | 当前设定: ${cyan}${gc_status}${none}"
        echo -e "  6) 开启或关闭 Xray Policy 策略组优化 (连接生命周期极速回收)     | 当前状态: $s6"
        echo -e ""
        echo -e "  ${magenta}--- Linux 系统层与内核黑科技 (7-13) ---${none}"
        echo -e "  7)  开启或关闭【透明大页 (THP - Transparent Huge Pages)】       | 当前状态: $s7"
        echo -e "  8)  开启或关闭【TCP PMTU 黑洞智能探测 (Probing=1)】             | 当前状态: $s8"
        echo -e "  9)  开启或关闭【CPU 频率调度器锁定 (Performance)】              | 当前状态: $s9"
        echo -e "  10) 开启或关闭【网卡硬件环形缓冲区 (Ring Buffer) 反向收缩】     | 当前状态: $s10"
        echo -e "  11) 开启或关闭【ZRAM】(淘汰慢速 Swap，阶梯内存自动检测)         | 当前状态: $s11"
        echo -e "  12) 开启或关闭【日志系统 Journald 纯内存化】(斩断 I/O 羁绊)     | 当前状态: $s12"
        echo -e "  13) 开启或关闭【系统进程级防杀抢占 (OOM/IO 提权)】              | 当前状态: $s13"
        echo -e "  "
        echo -e "  ${cyan}14) 一键开启或关闭 1-6 项 应用层微操 (自动侦测并智能反转)${none}"
        echo -e "  ${yellow}15) 一键开启或关闭 7-13 项 系统级微操 (自动避障侦测并反转)${none}"
        echo -e "  ${red}16) 上帝之手：一键开启或关闭 1-13 项 全域微操 (满血激活 / 无损还原)${none}"
        echo "  0) 返回上一级"
        hr
        read -rp "请选择: " app_opt
        ...
        case "$app_opt" in
            ...
            6)
                ...
                read -rp "按 Enter 继续..." _
                ;;
            7) toggle_thp; info "操作已执行。"; read -rp "按 Enter 继续..." _ ;;
            8) toggle_mtu; info "操作已执行。"; read -rp "按 Enter 继续..." _ ;;
            9) toggle_cpu; info "操作已执行。"; read -rp "按 Enter 继续..." _ ;;
            10) toggle_ring; info "操作已执行。"; read -rp "按 Enter 继续..." _ ;;
            11) toggle_zram; info "操作已执行。"; read -rp "按 Enter 继续..." _ ;;
            12) toggle_journal; info "操作已执行。"; read -rp "按 Enter 继续..." _ ;;
            13) toggle_process_priority; info "操作已执行。"; read -rp "按 Enter 继续..." _ ;;
            14)
            ...
            15)
                if [ "$sys_off_count" -gt 0 ]; then
                    if [ -f "/sys/kernel/mm/transparent_hugepage/enabled" ]; then [ "$(check_thp_state)" = "false" ] && toggle_thp >/dev/null 2>&1; fi
                    [ "$(check_mtu_state)" = "false" ] && toggle_mtu >/dev/null 2>&1
                    if [ -d "/sys/devices/system/cpu/cpu0/cpufreq" ]; then [ "$(check_cpu_state)" = "false" ] && toggle_cpu >/dev/null 2>&1; fi
                    if ethtool -g "$IFACE" >/dev/null 2>&1; then [ "$(check_ring_state)" = "false" ] && toggle_ring >/dev/null 2>&1; fi
                    if modprobe -n zram >/dev/null 2>&1 || lsmod | grep -q zram; then [ "$(check_zram_state)" = "false" ] && toggle_zram >/dev/null 2>&1; fi
                    [ "$(check_journal_state)" = "false" ] && toggle_journal >/dev/null 2>&1
                    [ "$(check_process_priority_state)" = "false" ] && toggle_process_priority >/dev/null 2>&1
                    info "7-13 项系统内核底层黑科技已全部一键激活！(不支持的硬件已自动跳过)"
                else
                    if [ -f "/sys/kernel/mm/transparent_hugepage/enabled" ]; then [ "$(check_thp_state)" = "true" ] && toggle_thp >/dev/null 2>&1; fi
                    [ "$(check_mtu_state)" = "true" ] && toggle_mtu >/dev/null 2>&1
                    if [ -d "/sys/devices/system/cpu/cpu0/cpufreq" ]; then [ "$(check_cpu_state)" = "true" ] && toggle_cpu >/dev/null 2>&1; fi
                    if ethtool -g "$IFACE" >/dev/null 2>&1; then [ "$(check_ring_state)" = "true" ] && toggle_ring >/dev/null 2>&1; fi
                    if modprobe -n zram >/dev/null 2>&1 || lsmod | grep -q zram; then [ "$(check_zram_state)" = "true" ] && toggle_zram >/dev/null 2>&1; fi
                    [ "$(check_journal_state)" = "true" ] && toggle_journal >/dev/null 2>&1
                    [ "$(check_process_priority_state)" = "true" ] && toggle_process_priority >/dev/null 2>&1
                    info "7-13 项系统内核底层黑科技已全部一键卸载还原！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            16)
                if [ "$((app_off_count + sys_off_count))" -gt 0 ]; then
                    ...
                    systemctl restart xray >/dev/null 2>&1
                    info "上帝降临！1-13 项全域极限微操已【全部满血激活】！(不支持的硬件已自动跳过)"
                else
                    _turn_off_app
                    if [ -f "/sys/kernel/mm/transparent_hugepage/enabled" ]; then [ "$(check_thp_state)" = "true" ] && toggle_thp >/dev/null 2>&1; fi
                    [ "$(check_mtu_state)" = "true" ] && toggle_mtu >/dev/null 2>&1
                    ...
                    systemctl restart xray >/dev/null 2>&1
                    info "神威收敛！1-13 项全域极限微操已【全部强制关闭】，无损还原初始出厂状态！"
                fi
                read -rp "按 Enter 继续..." _
                ;;
            0)
...
do_sys_init_menu() {
    while true; do
        ...
        echo -e "  ${magenta}7) 全域 13 项极限微操 (Xray 提速底牌 & 系统级黑科技)${none}"
        echo "  0) 返回主菜单"
        hr
        read -rp "请选择: " sys_opt
        case "$sys_opt" in
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex82 The Apex Vanguard - Project Genesis V82${none}"
