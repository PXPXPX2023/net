#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e13.sh (The Apex Vanguard - Project Genesis V196e13)
# 快捷方式: xrv
#
# 【V196e13 终极勘误与真理重塑】
#   1. 根绝编译血案: 斩断 cp /boot/config 毒瘤！完全信任 Xanmod 原生 defconfig 推导架构，根除 -march 暴毙！
#   2. 双通道内核锻造: 主菜单并列新增 Kernel.org 官方最新主线 / Xanmod GitLab 极客版双轨源码编译。
#   3. JQ 绝对绝缘: 全域覆盖 select(. != null) 护盾，杜绝任何数组/对象为空时的语法崩溃。
#   4. APT 源净化: 修复预编译安装时 GPG 密钥 404 及 sources.list 重复冲突导致的静默瘫痪。
#   5. 完美融合: 全量继承 25 项系统微操及全域网络栈压榨引擎。
# ==============================================================================

# 必须用 bash 运行
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex196e13.sh"
    exit 1
fi
...
readonly SCRIPT_VERSION="196e13"
...
# ------------------------------------------------------------------------------
# 【V196e13 核心修复 1】：预编译官方 XANMOD 部署模块 (修复多重源与 404)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 【V196e13 核心修复 2】：真·双通道内核源码编译引擎 (Mainline 主线 vs Xanmod)
# ------------------------------------------------------------------------------
...
do_kernel_compile_mainline() {
    title "系统飞升：编译 Linux 官方主线最新内核 (Mainline稳如老狗版)"
...
do_kernel_compile_xanmod() {
    ...
    # 【完全信任内核体系】：绝不复制旧宿主机 config 污染新树
    # Xanmod 源码自带完善的 defconfig，我们让它自己推导全套架构！
...
# --- ✂️ 请将此行及其下方的 Part 11 紧接在 Part 10 结尾粘贴 ✂️ ---
...
# --- ✂️ 请将此行及其下方的 Part 12 紧接在 Part 11 结尾粘贴 ✂️ ---
    ...
    while true; do
        ...
        case "${bc:-}" in
            1) 
                ...
                ensure_xray_is_alive; info "BT 协议阻断墙修改为: $nv" 
                ;;
            2) 
...
do_sys_init_menu() {
    while true; do
        ...
        echo "  4) [极客锻造] 编译 Linux 官方主线最新内核 (Mainline + BBR / 零报错)"
        echo -e "  ${magenta}5) [激进压榨] 编译 Xanmod 官方极客内核 (GitLab 源码 + BBR3)${none}"
        echo "  6) [网络底层] TX Queue 网卡出站队列防拥堵极限缩减 (配置为 2000 收缩)"
        echo "  7) [极限压榨] 全域系统底层网络栈结构重塑 (Limits + Sysctl)"
        echo "  8) [上帝微操] 应用层及系统内核层双轨 25 项神级优化全控板 (Dnsmasq/CAKE)"
        echo -e "  ${cyan}9) [极度发烧] 深入 CAKE 高级模型配置 (设定 Diffserv 调度、物理带宽上限)${none}"
        echo "  0) 折返中央主轴系统"
        hr
...
EOF
            ...
            4) _compile_kernel_mainline ;;
            5) _compile_kernel_xanmod ;;
            6) do_txqueuelen_opt ;;
            7) do_perf_tuning ;;
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e13 - (The Apex Vanguard)${none}"
...
main_menu
# ==============================================================================
# EOF - Apex Vanguard V196e13 System Advanced Control Ready.
# ==============================================================================

# --- ✂️ Part 12 结束，所有代码拼接完毕。请运行 bash ex196e13.sh ---
