#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e99.sh (The Apex Vanguard - Project Genesis V198e99)
# 快捷方式: xrv
# 【V198e99 三协议稳态版：Reality Vision / VLESSENC Vision / SS2022 + tcp_notsent_lowat=26214 + BBRv3源码编译1000MB门槛 热更新默认】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e99.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e99"
...
_compile_kernel_mainline_legacy_e53() {
    ...
    if [ "$bbr_type" = "bbr3" ]; then
        _e99_apply_google_bbr3_source_or_patch
    else
...
hot_update_from_legacy() {
    ...
    if ! verify_e66_post_state; then
        ...
        warn "可以再次执行：bash ex198e99.sh --hot-update，或进入 10→5 手动应用。"
        return 1
    fi
    ensure_xray_is_alive || true
...
hot_update_from_legacy() {
    ...
    if ! verify_e68_post_state; then
        ...
        warn "可再次执行：bash ex198e99.sh --hot-update，或进入 10→6→28 / 10→5 手动应用。"
        return 1
    fi
...
hot_update_from_legacy() {
    ...
    if ! e69_verify_post_state; then
        ...
        warn "可再次执行：bash ex198e99.sh --hot-update；如 Xray 未运行，执行 journalctl -u xray --no-pager -n 80。"
        return 1
    fi
...
_e82_install_self_entrypoint() {
    ...
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        chmod +x "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e99.sh" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
        chmod +x "$SCRIPT_DIR/ex198e99.sh" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
    fi
    echo "198e99" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    hash -r 2>/dev/null || true
    info "V198e99 已安装/刷新快捷入口：$SYMLINK"
}
...
# V198e99 ${label}: TCP unsent write queue low watermark. 常用：26214=极低排队；32768=低延迟；65536=均衡；131072=高吞吐。
...
do_network_params_menu_e56() {
    while true; do
        clear
        title "网络参数应用中心（V198e99：均衡/高吞吐/手动 + notsent_lowat 永久管理；热更新默认26214）"
        show_network_buffer_params_e56
        _e82_show_tcp_notsent_lowat_state
        show_network_sysctl_conflicts_e58
        _e82_show_notsent_conflicts
...
hot_update_from_legacy() {
    title "V198e99：稳态热更新（保留 e81 能力 + 默认 tcp_notsent_lowat=26214）"
    set +e
    trap - ERR
    ...
    if declare -F e76_config_semantic_ok >/dev/null 2>&1 && e76_config_semantic_ok "$CONFIG"; then
        ...
        warn "未发现有效现网节点配置。V198e99 已安装为 xrv；请执行 xrv → 1 重新搭建。"
    fi
    ...
    set -e
    trap '_err_handler $? $LINENO "$BASH_COMMAND"' ERR
    info "V198e99 热更新完成。现在可输入：xrv；热更新默认 tcp_notsent_lowat=26214；进入 13 可切换 26214/32768/65536/131072/手动。"
    return 0
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e99 - (Reality / VLESSENC 自选 / SS2022 / notsent默认26214)${none}"
        ...
        echo "  9) 全景网络连接审计日志与商用级别网络流量记账本"
        echo "  10) 高级发烧：内核双轨编译(APT/源码)/网络/应用层 60余项极限全控板"
        echo -e "  ${cyan}11) 从旧 ex198e 环境稳态热更新到 V198e99${none}"
        echo -e "  ${cyan}12) 清理空间（安全释放 / 快照收敛 / 显示硬盘与 Swap）${none}"
...
_e88_install_self_entrypoint() {
    ...
    if test -f "$SCRIPT_PATH"; then
        ...
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e99.sh" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
        chmod +x "$SYMLINK" "$SCRIPT_DIR/ex198e99.sh" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
        hash -r 2>/dev/null || true
    fi
    echo "198e99" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
}
...
_e88_show_network_param_discussion() {
    title "V198e99 可放入 13 菜单管理的网络参数说明"
    cat <<'EOF_E88_DISCUSS'
...
do_network_params_menu_e56() {
    while true; do
        clear
        title "网络参数应用中心（V198e99：缓冲 / notsent / fq+BBR / adv_win / txqueue / offload / MSS）"
...
hot_update_from_legacy() {
    title "V198e99：稳态热更新（保留 e82 三协议 + 扩展 13 网络参数中心）"
    set +e
    trap - ERR
    ...
    local bdir
    if declare -F e76_state_snapshot >/dev/null 2>&1; then
        bdir=$(e76_state_snapshot "hotupdate_e99")
    else
        bdir="$BACKUP_DIR/hotupdate_e99_$(date +%Y%m%d_%H%M%S)"; mkdir -p "$bdir" 2>/dev/null || true
        cp -a "$CONFIG" "$bdir/config.json.bak" 2>/dev/null || true
        cp -a "$NETWORK_OPT_CONF" "$bdir/99-network-optimized.conf.bak" 2>/dev/null || true
    fi
    info "已创建热更新快照：$bdir"
    ...
    set -e
    trap '_err_handler $? $LINENO "$BASH_COMMAND"' ERR
    info "V198e99 热更新完成。现在可输入：xrv → 13；新增 fq+BBR、adv_win、txqueuelen、offload、MSS 管理。"
    return 0
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e99 - (Reality / VLESSENC 自选 / SS2022 / Menu13网络参数中心)${none}"
...
_e88_show_menu13_manual_notes() {
    title "V198e99 菜单13：五项网络参数注释与默认值"
...
_e88_set_tcp_adv_win_scale_manual() {
    ...
    _e88_set_sysctl_key net.ipv4.tcp_adv_win_scale "$v" "V198e99 手动：tcp_adv_win_scale，默认1，吞吐2-3"
...
_e88_set_tcp_app_win_manual() {
    ...
    _e88_set_sysctl_key net.ipv4.tcp_app_win "$v" "V198e99 手动：tcp_app_win，默认31，范围0~31"
...
_e88_apply_menu13_defaults() {
    title "应用 V198e99 菜单13默认五项：adv=1 / app=31 / txqueue=3000 / OOOX / MSS=ON"
    _e88_set_sysctl_key net.ipv4.tcp_adv_win_scale 1 "V198e99 默认：tcp_adv_win_scale=1，吞吐实验2-3"
    _e88_set_sysctl_key net.ipv4.tcp_app_win 31 "V198e99 默认：tcp_app_win=31，范围0~31"
...
do_network_params_menu_e56() {
    while true; do
        clear
        title "网络参数应用中心（V198e99：缓冲 / notsent / fq+BBR / 五项手动参数注释）"
        if declare -F show_network_buffer_params_e56 >/dev/null 2>&1; then show_network_buffer_params_e56; fi
        _e88_show_tcp_notsent_lowat_state
        _e88_show_core_network_state
        _e88_show_menu13_manual_notes
...
hot_update_from_legacy() {
    title "V198e99：稳态热更新（保留 e83 三协议 + 菜单13五项网络参数 + RPS/RFS + initcwnd/IP检测）"
    set +e
    trap - ERR
    ...
    set -e
    trap '_err_handler $? $LINENO "$BASH_COMMAND"' ERR
    info "V198e99 热更新完成。现在可输入：xrv → 13；新增 26-32：五项参数、RPS/RFS、initcwnd/initrwnd、IPv4/IPv6/gai.conf 查看。"
    return 0
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e99 - (Reality / VLESSENC 自选 / SS2022 / Menu13五项+RPS/initcwnd/IP检测)${none}"
...
_e88_apply_rps_rfs() {
    title "V198e99：应用 RPS/RFS 多核收包优化 + 读回验证"
...
_e88_show_init_window_state() {
    title "V198e99：TCP 起步窗口 initcwnd / initrwnd 当前状态"
    local route cw rw
    route=$(ip route show default 2>/dev/null | head -n1 || true)
...
_e88_reset_init_window_default() {
    local clean
    title "V198e99：恢复 initcwnd/initrwnd 为系统默认"
    clean=$(_e88_default_route_clean 2>/dev/null || true)
...
_e88_show_ip_gai_state() {
    title "V198e99：IPv4/IPv6 出口检测 + gai.conf 优先级查看（只读，不禁用 IPv6）"
    echo "  出口检测："
    printf "    IPv4: "
...
do_network_params_menu_e56() {
    while true; do
        clear
        title "网络参数应用中心（V198e99：e85稳态 + RPS/RFS + initcwnd/initrwnd + IPv4/IPv6检测）"
        if declare -F show_network_buffer_params_e56 >/dev/null 2>&1; then show_network_buffer_params_e56; fi
        _e88_show_tcp_notsent_lowat_state
        _e88_show_core_network_state
        _e88_show_menu13_manual_notes
...
hot_update_from_legacy() {
    title "V198e99：稳态热更新（保留 e85 主体 + RPS/RFS + initcwnd/IP查看菜单）"
    set +e
    trap - ERR
    ...
    _e88_set_sysctl_key net.ipv4.tcp_fastopen 0 "V198e99 热更新：保持 tcp_fastopen=0，不默认启用 TFO=3" || true
    ...
    set -e
    trap '_err_handler $? $LINENO "$BASH_COMMAND"' ERR
    info "V198e99 热更新完成。现在可输入：xrv → 13；新增 33-41：RPS/RFS、initcwnd/initrwnd、IPv4/IPv6/gai.conf 查看。"
    return 0
}
...
_e88_reset_init_window_default20() {
    local clean
    title "V198e99：恢复 initcwnd/initrwnd 默认20（移除默认路由显式值）"
    clean=$(_e88_default_route_clean 2>/dev/null || true)
    if test -z "$clean"; then warn "未检测到默认路由，无法恢复。"; return 1; fi
    if ip route change $clean 2>/dev/null; then
...
_e88_auto_init_window_16_32() {
    title "V198e99：initcwnd/initrwnd 16~32 自动检测、自动跳跃"
...
do_network_params_menu_e56() {
    while true; do
        clear
        title "网络参数应用中心（V198e99：e86审计修正 + initcwnd自动跳跃 + IPv4/IPv6只读检测）"
        if declare -F show_network_buffer_params_e56 >/dev/null 2>&1; then show_network_buffer_params_e56; fi
        _e88_show_tcp_notsent_lowat_state
        _e88_show_core_network_state
        _e88_show_menu13_manual_notes
...
hot_update_from_legacy() {
    title "V198e99：稳态热更新（审计 e86 + 菜单13 initcwnd 档位重排）"
    set +e
    trap - ERR
    ...
    set -e
    trap '_err_handler $? $LINENO "$BASH_COMMAND"' ERR
    info "V198e99 热更新完成。现在可输入：xrv → 13；已更新 36-42：默认20恢复、18/22/32、16-32自动跳跃、手动、IPv4/IPv6只读检测。"
    return 0
}
...
_e88_reset_init_window_default21() {
    title "V198e99：恢复 initcwnd/initrwnd 默认21（移除默认路由显式值后重写）"
    local clean
    clean=$(_e88_default_route_clean 2>/dev/null || true)
...
_e88_manual_init_window() {
    title "V198e99：手动输入 initcwnd/initrwnd"
    _e88_show_init_window_state
    local cw rw
...
do_network_params_menu_e56() {
    while true; do
        title "网络参数应用中心（V198e99：e87审计修正 + initcwnd默认21档位）"
        _e88_show_tcp_notsent_lowat_state
        _e88_show_core_network_state
        _e88_show_menu13_manual_notes
        echo ""
...
hot_update_from_legacy() {
    title "V198e99：稳态热更新（审计 e87 + initcwnd/initrwnd 默认21）"
    set +e
    trap - ERR
    ...
    set -e
    trap '_err_handler $? $LINENO "$BASH_COMMAND"' ERR
    info "V198e99 热更新完成。现在可输入：xrv → 13；35-40 已更新：查看、默认21、18、26、32、手动。"
    return 0
}

#==============================================================================
# V198e99 FINAL OVERRIDE LAYER
# 目标：保留 e88 三协议/菜单13/initcwnd=21；重新设计真正 BBRv3 源码/补丁内核实验区门禁。
# 变更：源码编译内存+Swap 门槛下调为 >=1000MB；安全并发按每 1000MB 约 1 线程估算。
# 说明：热更新不会自动安装/编译内核；仅更新 xrv 脚本与实验区门禁/并发逻辑。
#==============================================================================

readonly E99_SOURCE_COMPILE_GATE_MB="1000"
readonly E99_SOURCE_COMPILE_THREAD_MB="1000"

_e99_mem_swap_total_mb() {
    local mem_mb swp_mb
    mem_mb=$(free -m 2>/dev/null | awk '/^Mem:/ {print int($2)}' | head -n 1 || echo 0)
    swp_mb=$(free -m 2>/dev/null | awk '/^Swap:/ {print int($2)}' | head -n 1 || echo 0)
    if ! [[ "${mem_mb:-}" =~ ^[0-9]+$ ]]; then mem_mb=0; fi
    if ! [[ "${swp_mb:-}" =~ ^[0-9]+$ ]]; then swp_mb=0; fi
    echo $((mem_mb + swp_mb))
}

_e99_install_self_entrypoint() {
    mkdir -p "$SCRIPT_DIR" "$CONFIG_DIR" "$BACKUP_DIR" 2>/dev/null || true
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e99.sh" 2>/dev/null || true
...
_e99_apply_google_bbr3_source_or_patch() {
    info ">>> V198e99：强开真正 BBRv3 源码/补丁路线。"
    local patch_file="bbr3.patch" src_url src_tmp applied=0

    # 先尝试 patch 路线；如果仓库没有 patch 或目标内核代码结构不匹配，再使用官方 v3 tcp_bbr.c 源码覆盖路线。
    for src_url in \
        "https://raw.githubusercontent.com/google/bbr/v3/patches/bbr3.patch" \
        "https://raw.githubusercontent.com/google/bbr/main/patches/bbr3.patch"
    do
        rm -f "$patch_file" 2>/dev/null || true
        if curl -fsSL --connect-timeout 10 --max-time 60 "$src_url" -o "$patch_file" 2>/dev/null && test -s "$patch_file"; then
            if patch --dry-run -p1 < "$patch_file" >/dev/null 2>&1; then
                patch -p1 < "$patch_file" >/dev/null 2>&1 && applied=1
                if test "$applied" -eq 1; then
                    info "BBRv3 patch 已合入：$src_url"
                    break
                fi
            else
                warn "BBRv3 patch 与当前源码树不匹配，尝试下一方案：$src_url"
            fi
        fi
    done

    if test "$applied" -ne 1; then
        src_url="https://raw.githubusercontent.com/google/bbr/v3/net/ipv4/tcp_bbr.c"
        src_tmp="/tmp/tcp_bbr.bbrv3.$$"
        if curl -fsSL --connect-timeout 10 --max-time 60 "$src_url" -o "$src_tmp" 2>/dev/null && test -s "$src_tmp"; then
            if test -f "net/ipv4/tcp_bbr.c"; then
                cp -af "net/ipv4/tcp_bbr.c" "net/ipv4/tcp_bbr.c.before-e99-bbrv3.$(date +%Y%m%d_%H%M%S).bak" 2>/dev/null || true
                cp -f "$src_tmp" "net/ipv4/tcp_bbr.c"
                applied=1
                info "已使用 Google BBR v3 分支 tcp_bbr.c 覆盖当前源码树：$src_url"
            else
                warn "当前源码树缺少 net/ipv4/tcp_bbr.c，无法覆盖 BBRv3 源码。"
            fi
        else
            warn "无法下载 Google BBR v3 tcp_bbr.c：$src_url"
        fi
        rm -f "$src_tmp" 2>/dev/null || true
    fi

    if test "$applied" -ne 1; then
        warn "BBRv3 源码/补丁未能成功注入；将继续启用 CONFIG_TCP_CONG_BBR，但这可能只是目标内核自带 BBR。"
    fi
...
# 覆盖 e52/e53 源码编译预检：内存+Swap 1000MB 即放行，磁盘与 /boot 仍保持安全门槛。
_kernel_source_precheck_e52() {
    local root_free boot_free mem_mb swap_mb total_mb
    root_free=$(df -Pm / 2>/dev/null | awk 'NR==2{print int($4)}' || echo 0)
    boot_free=$(df -Pm /boot 2>/dev/null | awk 'NR==2{print int($4)}' || echo 0)
    if ! [[ "${boot_free:-}" =~ ^[0-9]+$ ]]; then boot_free=0; fi
    if test "${boot_free:-0}" -eq 0 2>/dev/null; then boot_free="$root_free"; fi
    mem_mb=$(free -m 2>/dev/null | awk '/^Mem:/ {print int($2)}' || echo 0)
    swap_mb=$(free -m 2>/dev/null | awk '/^Swap:/ {print int($2)}' || echo 0)
    if ! [[ "${mem_mb:-}" =~ ^[0-9]+$ ]]; then mem_mb=0; fi
    if ! [[ "${swap_mb:-}" =~ ^[0-9]+$ ]]; then swap_mb=0; fi
    total_mb=$((mem_mb + swap_mb))
    ...
    if test "${total_mb:-0}" -lt "$E99_SOURCE_COMPILE_GATE_MB" 2>/dev/null; then
        warn "源码编译要求内存+Swap >= ${E99_SOURCE_COMPILE_GATE_MB}MB。当前不足，已拒绝。"
        return 1
    fi
    if test ! -d /boot || test ! -w /boot; then
        warn "/boot 不可写，已拒绝源码内核安装。"
...
_kernel_source_gate_e53() {
    title "V198e99：真正 BBRv3 源码/补丁内核实验区门禁（内存+Swap >=1000MB）"
    warn "你选择的是源码/补丁版 BBRv3 路线，不是只看 sysctl=bbr，也不是普通发行版自带 BBR。"
    warn "源码编译内核可能导致 GRUB 卡死、busybox、断网、模块缺失；必须有 VPS 快照 / VNC / 救援模式。"
    warn "V198e99 已按你的要求将源码编译门槛下调为：内存+Swap >= ${E99_SOURCE_COMPILE_GATE_MB}MB。"
    _kernel_source_precheck_e52 || { _e52_pause; return 1; }
    echo ""
    echo "  仍需手工确认环境变量，防止误触发真实编译："
    echo "  export XRAY_ALLOW_SOURCE_KERNEL_COMPILE=1"
    echo "  然后重新进入该菜单。"
    if test "${XRAY_ALLOW_SOURCE_KERNEL_COMPILE:-0}" != "1"; then
        _e52_pause
    ...
    local confirm=""
    warn "检测到 XRAY_ALLOW_SOURCE_KERNEL_COMPILE=1。接下来将进入真实源码编译流程，风险自担。"
    read -rp "确认已有快照/VNC/救援模式，并继续源码编译？(type YES): " confirm || true
    if test "$confirm" != "YES"; then
        warn "未输入 YES，已取消源码编译。"
        _e52_pause
        return 1
...
# 覆盖源码编译执行器：按每 1000MB 内存+Swap 约 1 个 make 线程估算，低内存 VPS 仍至少 -j1。
_execute_compilation() {
    local extra_make_args="${1:-}"
    local make_flags="${2:-}"

    info "执行源码内核编译（V198e99：内存+Swap >=1000MB 门槛，关键安装步骤 fail-fast）..."
    local mem_mb swp_mb total_mb CPU_CORES THREADS cmd COMPILED_VER
    mem_mb=$(free -m 2>/dev/null | awk '/^Mem:/ {print int($2)}' | head -n 1 || echo 1024)
    swp_mb=$(free -m 2>/dev/null | awk '/^Swap:/ {print int($2)}' | head -n 1 || echo 0)
    if ! [[ "${mem_mb:-}" =~ ^[0-9]+$ ]]; then mem_mb=1024; fi
    if ! [[ "${swp_mb:-}" =~ ^[0-9]+$ ]]; then swp_mb=0; fi
    total_mb=$((mem_mb + swp_mb))
    CPU_CORES=$(nproc 2>/dev/null || echo 1)
    if ! [[ "${CPU_CORES:-}" =~ ^[0-9]+$ ]] || test "$CPU_CORES" -lt 1 2>/dev/null; then CPU_CORES=1; fi

    if test "${total_mb:-0}" -lt "$E99_SOURCE_COMPILE_GATE_MB" 2>/dev/null; then
        error "内存+Swap ${total_mb}MB，小于 ${E99_SOURCE_COMPILE_GATE_MB}MB 门槛，拒绝编译。"
        return 1
    fi

    THREADS=$((total_mb / E99_SOURCE_COMPILE_THREAD_MB))
    test "$THREADS" -lt 1 2>/dev/null && THREADS=1
    test "$THREADS" -gt "$CPU_CORES" 2>/dev/null && THREADS=$CPU_CORES
    cmd="make -j$THREADS"
    info "安全编译线程：$THREADS（内存+Swap ${total_mb}MB；计算规则：每 ${E99_SOURCE_COMPILE_THREAD_MB}MB 约 1 线程，上限 CPU=${CPU_CORES}）"
...
do_kernel_compile_menu() {
    while true; do
        clear
        title "高风险内核实验区（V198e99：真 BBRv3 源码/补丁路线，1000MB 门槛版）"
        echo "  说明：热更新不会自动安装/编译内核。此菜单只供手动实验。"
        echo "  [路线] 你偏好的是真正 BBRv3 源码/补丁版 bbr，不把 Ubuntu 自带 bbr 直接当 BBRv3。"
        echo "  [门槛] 源码编译门槛：12GB 根分区 / /boot 300MB / 内存+Swap >= ${E99_SOURCE_COMPILE_GATE_MB}MB。"
        echo "  [并发] make 并发：每 ${E99_SOURCE_COMPILE_THREAD_MB}MB 内存+Swap 约 1 线程，最低 -j1，上限 CPU 核数。"
        echo "  [安全] modules_install / make install / initramfs / update-grub 仍 fail-fast，不静默忽略失败。"
        echo ""
        echo -e "  ${cyan}1) [相对推荐] APT 安装 XanMod 预编译内核（仍需快照/VNC）${none}"
        echo -e "  ${magenta}2) [高风险锁定] 源码编译 XanMod（内存+Swap >=1000MB 才放行门禁）${none}"
        echo -e "  ${yellow}3) [高风险锁定] 源码编译 Linux Mainline / BBRv3 补丁路线（内存+Swap >=1000MB 才放行门禁）${none}"
        echo "  0) 返回上级菜单"
        hr
        local k_opt=""
        read -rp "请下达实验区指令 (0-3): " k_opt || true
...
hot_update_from_legacy() {
    title "V198e99：稳态热更新（保留 e88 三协议/菜单13/initcwnd=21 + BBRv3源码编译1000MB门槛）"
    set +e
    trap - ERR

    mkdir -p "$CONFIG_DIR" "$SCRIPT_DIR" "$BACKUP_DIR" 2>/dev/null || true
    _e99_install_self_entrypoint || true
    local bdir
    if declare -F e76_state_snapshot >/dev/null 2>&1; then
        bdir=$(e76_state_snapshot "hotupdate_e99")
    else
    ...
    _e88_set_sysctl_key net.ipv4.tcp_fastopen 0 "V198e99 热更新：保持 tcp_fastopen=0，不默认启用 TFO=3" || true
    _e88_set_tcp_notsent_lowat 26214 "V198e99 热更新默认极低排队响应值" || true
    _e88_apply_menu13_defaults || true
    _e88_apply_rps_rfs || true
    _e88_set_init_window 21 21 "e99-hotupdate-default21" || true
    ...
    _e88_show_init_window_state || true
    echo ""
    info "V198e99 编译门槛审计：源码编译内存+Swap >= ${E99_SOURCE_COMPILE_GATE_MB}MB；并发按每 ${E99_SOURCE_COMPILE_THREAD_MB}MB 约 1 线程。"
    if declare -F e77_audit_runtime_state >/dev/null 2>&1; then e77_audit_runtime_state || true; fi
    set -e
    trap '_err_handler $? $LINENO "$BASH_COMMAND"' ERR
    info "V198e99 热更新完成。现在可输入：xrv → 10 → 内核实验区；源码编译门槛已改为 >=1000MB。"
    return 0
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e99 - (Reality / VLESSENC 自选 / SS2022 / BBRv3源码门槛1000MB)${none}"
        local svc sys_ver arch_level dns_mode tz_line notsent_now notsent_perm qdisc cc adv app total_mb
        svc=$(systemctl is-active xray 2>/dev/null || echo "inactive")
        if test "$svc" = "active"; then svc="${green}平稳在线 (Running)${none}"; else svc="${red}离线静默 (Stopped)${none}"; fi
        sys_ver=$(uname -r 2>/dev/null || echo "未探测到数据")
        if declare -F detect_x86_64_level >/dev/null 2>&1; then arch_level="$(detect_x86_64_level 2>/dev/null || echo '?')"; else arch_level="?"; fi
        ...
        adv=$(_e88_sysctl_get net.ipv4.tcp_adv_win_scale)
        app=$(_e88_sysctl_get net.ipv4.tcp_app_win)
        total_mb=$(_e99_mem_swap_total_mb 2>/dev/null || echo 0)
        echo -e "  引擎态势: $svc | 全局快捷指令: ${cyan}xrv${none} | 外部识别 IP: ${yellow}$(_get_ip 2>/dev/null || echo "探测异常")${none}"
        echo -e "  系统装配内核: ${cyan}${sys_ver}${none} | 微架构侦测: x64-v${yellow}${arch_level}${none} | qdisc/cc: ${yellow}${qdisc}/${cc}${none}"
        echo -e "  DNS模式: ${cyan}${dns_mode}${none} | 时间轴: ${cyan}${tz_line}${none} | notsent: ${yellow}${notsent_now}${none}/${cyan}${notsent_perm}${none} | adv/app: ${yellow}${adv}/${app}${none}"
        echo -e "  BBRv3源码编译门槛: 内存+Swap ${yellow}${total_mb}MB${none} / ${cyan}${E99_SOURCE_COMPILE_GATE_MB}MB${none} | 热更新不会自动编译内核"
        echo -e "${blue}======================================================================${none}"
        echo "  1) 创建/重构协议节点：Reality Vision / VLESSENC 自选 / SS2022"
        echo "  2) 用户凭证生命周期与独立参数化管理控制"
        echo "  3) 节点链接与多维信息输出聚合中心"
        ...
        echo -e "  ${cyan}13) 应用网络参数（缓冲/notsent/adv_win/app_win/txqueue/OOOX/MSS）${none}"
        echo -e "  ${cyan}14) TCP 重传 / 单连接诊断中心（只读排障）${none}"
        echo -e "  ${cyan}15) GitHub raw.githubusercontent.com 解析修复中心${none}"
        echo "  0) 退出控制台"
        echo -e "${blue}======================================================================${none}"
        local choice=""; read -rp "请选择作战模块: " choice || true
        case "${choice:-}" in
            1) do_install ;;
            2) do_user_menu ;;
            3) do_show_links ;;
            4) install_update_dat; _e52_pause ;;
            5) do_update_core ;;
            6) do_sni_menu ;;
            7) do_firewall_menu ;;
            8) do_fallback_menu ;;
            9) do_audit_menu ;;
            10) advanced_menu ;;
            11) hot_update_from_legacy auto; _e52_pause ;;
            12) if declare -F safe_space_cleanup_menu_e56 >/dev/null 2>&1; then safe_space_cleanup_menu_e56; else check_and_clean_space; _e52_pause; fi ;;
            13) do_network_params_menu_e56 ;;
            14) if declare -F e77_tcp_diagnostic_menu >/dev/null 2>&1; then e77_tcp_diagnostic_menu; else warn "TCP 诊断中心函数未加载。"; _e52_pause; fi ;;
            15) if declare -F e79_github_raw_dns_fix_menu >/dev/null 2>&1; then e79_github_raw_dns_fix_menu; else warn "GitHub DNS 修复中心函数未加载。"; _e52_pause; fi ;;
            0) exit 0 ;;
            *) warn "指令无效。"; sleep 1 ;;
        esac
    done
...
# 当脚本被直接执行到此处时，覆盖前面 e88 入口后进入 e99 主菜单。
preflight
main_menu


#==============================================================================
# EOF - Apex Vanguard V198e99 BBRv3 Source Compile Gate 1000MB Ready.
#==============================================================================
