#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e41.sh (The Apex Vanguard - Project Genesis V198e41)
# 快捷方式: xrv
# 【V198e41 稳态极速热更新版：安全加速包默认启用 + DNS 防翻车 + Reality 指纹收敛】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e41.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e41"
...
check_and_clean_space() {
    ...
    # V198e41: 不再 rm -rf /tmp/*、不再删除全部 /var/log/*.log，避免误删 socket/锁文件/审计日志导致服务异常。
    rm -rf \
...
choose_best_qdisc() {
    # V198e41: 极速代理默认 fq + BBR。CAKE 不再作为默认项，避免弱 CPU VPS 因整形/分类开销反降速。
...
write_safe_global_limits() {
    # V198e41: 修复 V198e35/V198e36 残留的 100000KB stack 问题。
...
verify_sni_strict() {
    ...
    if ! echo "$out" | grep -qi "OCSP response:"; then warn "OCSP Stapling 未发现：这不是致命问题，V198e41 仅提示不强制淘汰。"; fi
...
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
IFACE="$IFACE"
# V198e41: 纯代理吞吐优先保留 GRO/GSO/TSO，降低 CPU 压力；仅关闭 LRO，避免破坏转发语义。
...
do_perf_tuning() {
    ...
    info "配置系统高并发进程限制 (Limits，V198e41 稳态安全版)..."
    write_safe_global_limits
...
do_app_level_tuning_menu() {
    while true; do
        clear
        title "全域 25 项系统级及应用层微操管理中心（V198e41 稳态极速）"
        ...
        case "$app_opt" in
            ...
            27) apply_safe_turbo_defaults; systemctl restart xray >/dev/null 2>&1 || true; info "V198e41 安全系统级微操已幂等落地。"; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            28) _turn_on_app; apply_safe_turbo_defaults; systemctl restart xray >/dev/null 2>&1 || true; info "V198e41 极速稳态一键打通完成：应用层 + 安全系统级微操已幂等落地。"; local _p=""; read -rp "按 Enter 继续..." _p || true ;;
            0) return ;;
        esac
    done
}
...
install_self_entrypoint() {
    ...
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e41.sh" 2>/dev/null || true
        chmod +x "$SYMLINK" "$SCRIPT_DIR/ex198e41.sh" 2>/dev/null || true
    fi
...
apply_hotupdate_fast_sysctl() {
    ...
    mkdir -p /etc/sysctl.d 2>/dev/null || true
    local adv app qdisc
    # V198e41: 热更新默认固化你实测稳定的高 RTT 代理窗口组合。
    adv=1
    app=31
...
enable_dnsmasq_cache_safely() {
    # V198e41: 默认启用 dnsmasq，但必须通过监听与解析验证后才接管 resolv.conf。
...
apply_txqueue_3000_default() {
    # V198e41: 默认 3000，不再用 12000，避免队列过深导致排队延迟和 bufferbloat。
    local IP_CMD IFACE
...
apply_safe_turbo_defaults() {
    # V198e41: 热更新默认启用“安全极速包”，但不启用高风险项：Origin Guard / CAKE / ECN / TFO / 禁 IPv6 / 源码内核编译。
    title "V198e41 安全极速包：随热更新自动启用"
    repair_dns_if_broken
    check_and_create_swap
    fix_xray_systemd_limits
    apply_hotupdate_fast_sysctl
...
hot_update_from_legacy() {
    local mode="${1:-manual}"
    title "V198e41 稳态极速热更新迁移器（兼容 ex198e32-ex198e37）"
    ...
    repair_dns_if_broken
    install_update_dat
    # V198e41: 先移除旧版遗留的自动入口守卫，避免重启后链路被 iptables 误伤。
    ...
    if ensure_xray_is_alive; then
        # V198e41: 热更新不再自动启用 Origin Guard。V198e35/V198e36 的自动守卫会在重启后误伤部分真实链路。
        remove_origin_guard auto >/dev/null 2>&1 || true
        echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
        info "热更新完成：旧配置、旧用户和旧端口已保留，V198e41 安全极速包已生效。Origin Guard 仍默认关闭，可在防火墙菜单中手动开启。"
        if test "$mode" != "auto"; then
            do_summary
            local _p=""; read -rp "按 Enter 返回主菜单..." _p || true
        fi
...
get_ssh_listen_ports() {
    local ports=""
    # V198e41: 优先从当前监听态识别 SSH 端口，避免 sshd_config 使用 Include/云镜像改名时误判为 22。
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e41 稳态极速版)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e41 - (The Apex Vanguard)${none}"
        ...
        echo -e "  ${cyan}11) 从 ex198e32/ex198e33/ex198e34/ex198e35/ex198e36/ex198e37 稳态热更新到 V198e41${none}"
...
rescue_after_v35_breakage() {
    title "V198e41 紧急修复：撤销旧版重启后断流风险项"
...
apply_safe_turbo_defaults() {
    title "V198e41 安全极速包：e37/e38 一键热更新默认落地"
    repair_dns_if_broken
    check_and_create_swap
    fix_xray_systemd_limits
    apply_hotupdate_fast_sysctl
...
hot_update_from_legacy() {
    title "V198e41 稳态极速热更新：保留节点参数，一键落地默认加速项"
    if test ! -f "$CONFIG"; then
    ...
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    ensure_xray_is_alive
    info "V198e41 热更新完成：NoDelay 已开启，FastOpen 默认关闭，GOGC=300，Policy=260s，Dnsmasq/THP/ZRAM/OOM 默认按安全条件落地。"
}



#==============================================================================
# V198e41 最终覆盖层：128KB Buffer 单位修正 + systemd 环境变量清理
...
toggle_buffer() {
    # V198e41：只在 config.json 的 KB 单位中切换 64/128，永不再写 systemd MB 环境变量。
    local v=""
    v=$(jq -r '.policy.levels["0"].bufferSize // empty' "$CONFIG" 2>/dev/null || echo "")
    if test "$v" = "128"; then
...
apply_safe_turbo_defaults() {
    title "V198e41 安全极速包：默认启用 128KB Buffer，并清理 systemd MB 单位变量"
    repair_dns_if_broken
    check_and_create_swap
    fix_xray_systemd_limits
    apply_hotupdate_fast_sysctl
...
hot_update_from_legacy() {
    title "V198e41 稳态极速热更新：保留节点参数，默认修正 Buffer=128KB"
    if test ! -f "$CONFIG"; then
        die "未找到 $CONFIG，不能热更新。请先确认 Xray 已安装。"
    fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then
    ...
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    ensure_xray_is_alive
    info "V198e41 热更新完成：config.json bufferSize=128KB 已落地，systemd XRAY_RAY_BUFFER_SIZE 已清理，NoDelay 开启，FastOpen 关闭，GOGC=300，Policy=260s。"
}



#==============================================================================
# V198e41 覆盖修复层：修正 e40 残留状态/持久化问题
...
apply_safe_turbo_defaults() {
    title "V198e41 安全极速包：修正 e40 状态残留，默认 FQ + 128KB Buffer + GSO/GRO 保留"
    repair_dns_if_broken
    check_and_create_swap
    fix_xray_systemd_limits
    apply_hotupdate_fast_sysctl
...
hot_update_from_legacy() {
    title "V198e41 稳态极速热更新：修复 e40 状态残留并保留节点参数"
    if test ! -f "$CONFIG"; then
        die "未找到 $CONFIG，不能热更新。请先确认 Xray 已安装。"
    fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then
    ...
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    ensure_xray_is_alive
    info "V198e41 热更新完成：Policy=260、Buffer=128KB、systemd Buffer 环境变量清理、CAKE 子项残留清理、GSO/GRO/TSO 保留。"
}
...
#==============================================================================
# EOF - Apex Vanguard V198e41 System Advanced Control Ready.
#==============================================================================
