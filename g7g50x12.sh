#!/usr/bin/env bash
# ============================================================
# 脚本名称: g7g50x12.sh (The Apex Vanguard V10 Edition)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 彻底移除了 BBR 底层网络模块，回归极致纯粹的 Xray 代理控制中心。
#   2. 保留 Xray 核心无损热更机制，重组菜单排序，将彻底卸载隔离至 88) 绝对防误触。
#   3. 彻底解决复制粘贴截断导致的 unexpected EOF 语法错误。
#   4. 商用流量计费中心完美汉化，支持按月精准回溯 30 天流量明细。
#   5. 彻底根治 status 23 权限拦截 Bug，采用 755/644 绝对防线穿透。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash g7g50x12.sh"
    exit 1
fi
...
do_update_core() {
    ...
    print_magenta ">>> 正在连接 GitHub 拉取 Xray 最新正式版本..."
    ...
    info "更新成功！节点服务已无缝重启。"
    echo -e "  当前最新版本: ${cyan}$cur_ver${none}"
