#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e85.sh (The Apex Vanguard - Project Genesis V198e85)
# 快捷方式: xrv
# 【V198e85 三协议稳态版：Reality Vision / VLESSENC Vision / SS2022 + tcp_notsent_lowat=26214 热更新默认】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e85.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e85"
...
hot_update_from_legacy() {
    ...
    if ! verify_e66_post_state; then
        ...
        warn "可以再次执行：bash ex198e85.sh --hot-update，或进入 10→5 手动应用。"
        return 1
    fi
    ensure_xray_is_alive || true
...
hot_update_from_legacy() {
    ...
    if ! verify_e68_post_state; then
        ...
        warn "可再次执行：bash ex198e85.sh --hot-update，或进入 10→6→28 / 10→5 手动应用。"
        return 1
    fi
...
hot_update_from_legacy() {
    ...
    if ! e69_verify_post_state; then
        ...
        warn "可再次执行：bash ex198e85.sh --hot-update；如 Xray 未运行，执行 journalctl -u xray --no-pager -n 80。"
        return 1
    fi
...
_e82_install_self_entrypoint() {
    ...
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        chmod +x "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e85.sh" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
        chmod +x "$SCRIPT_DIR/ex198e85.sh" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
    fi
    echo "198e85" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    hash -r 2>/dev/null || true
    info "V198e85 已安装/刷新快捷入口：$SYMLINK"
}
...
# V198e85 ${label}: TCP unsent write queue low watermark. 常用：26214=极低排队；32768=低延迟；65536=均衡；131072=高吞吐。
...
do_network_params_menu_e56() {
    while true; do
        clear
        title "网络参数应用中心（V198e85：均衡/高吞吐/手动 + notsent_lowat 永久管理；热更新默认26214）"
        show_network_buffer_params_e56
        _e82_show_tcp_notsent_lowat_state
        show_network_sysctl_conflicts_e58
        _e82_show_notsent_conflicts
...
hot_update_from_legacy() {
    title "V198e85：稳态热更新（保留 e81 能力 + 默认 tcp_notsent_lowat=26214）"
    set +e
    trap - ERR
    ...
    if declare -F e76_config_semantic_ok >/dev/null 2>&1 && e76_config_semantic_ok "$CONFIG"; then
        ...
        warn "未发现有效现网节点配置。V198e85 已安装为 xrv；请执行 xrv → 1 重新搭建。"
    fi
    ...
    set -e
    trap '_err_handler $? $LINENO "$BASH_COMMAND"' ERR
    info "V198e85 热更新完成。现在可输入：xrv；热更新默认 tcp_notsent_lowat=26214；进入 13 可切换 26214/32768/65536/131072/手动。"
    return 0
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e85 - (Reality / VLESSENC 自选 / SS2022 / notsent默认26214)${none}"
        ...
        echo "  9) 全景网络连接审计日志与商用级别网络流量记账本"
        echo "  10) 高级发烧：内核双轨编译(APT/源码)/网络/应用层 60余项极限全控板"
        echo -e "  ${cyan}11) 从旧 ex198e 环境稳态热更新到 V198e85${none}"
        echo -e "  ${cyan}12) 清理空间（安全释放 / 快照收敛 / 显示硬盘与 Swap）${none}"
...
E85_NOTSENT_LOWAT_FILE="$CONFIG_DIR/tcp_notsent_lowat.txt"
E85_NETWORK_MENU_STATE_FILE="$CONFIG_DIR/network_menu_e85.txt"

_e85_install_self_entrypoint() {
    mkdir -p "$SCRIPT_DIR" 2>/dev/null || true
    if test -f "$SCRIPT_PATH"; then
        cp -f "$SCRIPT_PATH" "$SYMLINK" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e85.sh" 2>/dev/null || true
        cp -f "$SCRIPT_PATH" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
        chmod +x "$SYMLINK" "$SCRIPT_DIR/ex198e85.sh" "$SCRIPT_DIR/ex198e_latest.sh" 2>/dev/null || true
        hash -r 2>/dev/null || true
    fi
    echo "198e85" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
}
...
_e85_escape_key_re() { printf '%s' "$1" | sed 's/[.[\*^$()+?{}|\\]/\\&/g'; }
...
_e85_sysctl_persistent() {
    local key="$1" file="${2:-$NETWORK_OPT_CONF}" re val
    re=$(_e85_escape_key_re "$key")
    if test -f "$file"; then
...
_e85_strip_key_conflicts() {
    ...
    for f in /etc/sysctl.conf /etc/sysctl.d/*.conf; do
        ...
        if grep -Eq "^[[:space:]]*${re}[[:space:]]*=" "$f" 2>/dev/null; then
            cp -af "$f" "$BACKUP_DIR/sysctl/$(basename "$f").e85.$(date +%Y%m%d_%H%M%S).bak" 2>/dev/null || true
...
_e85_set_sysctl_key() {
    local key="$1" value="$2" label="${3:-e85}" re
    mkdir -p /etc/sysctl.d "$CONFIG_DIR" "$BACKUP_DIR/sysctl" 2>/dev/null || true
    _e85_strip_key_conflicts "$key"
    if test -f "$NETWORK_OPT_CONF"; then
        cp -af "$NETWORK_OPT_CONF" "$BACKUP_DIR/sysctl/99-network-optimized.conf.e85.$(date +%Y%m%d_%H%M%S).bak" 2>/dev/null || true
    else
        touch "$NETWORK_OPT_CONF" 2>/dev/null || true
    fi
    re=$(_e85_escape_key_re "$key")
    sed -i -E "/^[[:space:]]*${re}[[:space:]]*=/d" "$NETWORK_OPT_CONF" 2>/dev/null || true
    cat >> "$NETWORK_OPT_CONF" <<EOF_E85_SYSCTL_KEY

# V198e85 ${label}
${key} = ${value}
EOF_E85_SYSCTL_KEY
    if sysctl -w "${key}=${value}" >/tmp/xray-e85-sysctl.log 2>&1; then
        info "运行态已应用：${key}=${value}"
    else
        warn "当前内核可能不支持：${key}=${value}；永久配置已写入，详情：/tmp/xray-e85-sysctl.log"
        sed 's/^/    /' /tmp/xray-e85-sysctl.log 2>/dev/null || true
    fi
    echo "${key}=${value}" >> "$E85_NETWORK_MENU_STATE_FILE" 2>/dev/null || true
}

_e85_validate_notsent_lowat() {
    local v="${1:-}"
    [[ "$v" =~ ^[0-9]+$ ]] || return 1
    if test "$v" -eq 0 2>/dev/null; then return 0; fi
    test "$v" -ge 4096 2>/dev/null && test "$v" -le 4194304 2>/dev/null
...
_e85_current_notsent_lowat() { _e85_sysctl_get net.ipv4.tcp_notsent_lowat; }
_e85_persistent_notsent_lowat() { _e85_sysctl_persistent net.ipv4.tcp_notsent_lowat; }

_e85_set_tcp_notsent_lowat() {
    local val="${1:-}" label="${2:-手动}"
    if ! _e85_validate_notsent_lowat "$val"; then
        error "tcp_notsent_lowat 数值非法：$val；允许 0 或 4096~4194304。"
        return 1
    fi
    _e85_set_sysctl_key net.ipv4.tcp_notsent_lowat "$val" "${label}: TCP unsent write queue low watermark"
    echo "$val" > "$E85_NOTSENT_LOWAT_FILE" 2>/dev/null || true
    _e85_show_tcp_notsent_lowat_state
}

_e85_show_tcp_notsent_lowat_state() {
    local run perm saved
    run=$(_e85_current_notsent_lowat)
    perm=$(_e85_persistent_notsent_lowat)
    saved=$(cat "$E85_NOTSENT_LOWAT_FILE" 2>/dev/null || echo "未记录")
    echo ""
    echo -e "  ${cyan}tcp_notsent_lowat 状态：${none}"
    printf "    %-30s %s\n" "运行态(sysctl):" "$run"
    printf "    %-30s %s\n" "永久配置($NETWORK_OPT_CONF):" "$perm"
...
_e85_manual_tcp_notsent_lowat() {
    local v=""
    echo ""
    warn "手动值建议先测 26214 / 32768 / 65536 / 131072；过大可能增加排队延迟，过小可能限制高 RTT 吞吐。"
    read -rp "请输入 tcp_notsent_lowat 数值（0 或 4096~4194304，默认 26214）: " v || true
    v=${v:-26214}
    _e85_set_tcp_notsent_lowat "$v" "手动输入"
}

_e85_show_core_network_state() {
    local iface qdisc txq
    iface=$(_e85_default_iface)
    echo ""
    ...
    for key in \
        ...
        printf "    %-34s %-18s %-18s\n" "$key" "$(_e85_sysctl_get "$key")" "$(_e85_sysctl_persistent "$key")"
    done
    echo ""
...
_e85_show_network_param_discussion() {
    title "V198e85 可放入 13 菜单管理的网络参数说明"
    cat <<'EOF_E85_DISCUSS'
...
_e85_apply_fq_bbr_stable_core() {
    title "应用 e85 稳态核心：fq + bbr + ECN=0 + TFO=0 + MTU probing=1"
    if declare -F apply_fq_qdisc_default >/dev/null 2>&1; then apply_fq_qdisc_default || true; fi
    _e85_set_sysctl_key net.core.default_qdisc fq "fq pacing 队列"
    _e85_set_sysctl_key net.ipv4.tcp_congestion_control bbr "BBR 拥塞控制"
    _e85_set_sysctl_key net.ipv4.tcp_ecn 0 "关闭 ECN，减少兼容性问题"
    _e85_set_sysctl_key net.ipv4.tcp_ecn_fallback 1 "ECN fallback"
    _e85_set_sysctl_key net.ipv4.tcp_fastopen 0 "关闭 TFO，避免跨境兼容性毛刺"
    _e85_set_sysctl_key net.ipv4.tcp_mtu_probing 1 "启用 MTU probing"
    _e85_set_sysctl_key net.ipv4.tcp_slow_start_after_idle 0 "关闭空闲后慢启动"
    _e85_set_sysctl_key net.ipv4.tcp_autocorking 0 "关闭 autocorking，降低代理小包等待"
    _e85_set_sysctl_key net.ipv4.tcp_tso_win_divisor 6 "TSO 窗口分割"
    _e85_show_core_network_state
}

_e85_set_adv_app_profile() {
    ...
    _e85_set_sysctl_key net.ipv4.tcp_adv_win_scale "$scale" "$label"
    _e85_set_sysctl_key net.ipv4.tcp_app_win "$app" "$label"
    _e85_show_core_network_state
}
...
_e85_manual_adv_app() {
    ...
    app=${app:-31}
    _e85_set_adv_app_profile "$scale" "$app" "手动接收窗口参数"
}
...
_e85_set_txqueuelen() {
    ...
    echo "$val" > "$CONFIG_DIR/txqueuelen_e85.txt" 2>/dev/null || true
    service_script="/usr/local/bin/xray-txqueue-e85.sh"
    cat > "$service_script" <<EOF_E85_TXQ
#!/usr/bin/env bash
set -e
ip link set dev "$iface" txqueuelen "$val" 2>/dev/null || true
EOF_E85_TXQ
    chmod +x "$service_script" 2>/dev/null || true
    cat > /etc/systemd/system/xray-txqueue-e85.service <<EOF_E85_TXQ_SERVICE
[Unit]
Description=Xray e85 txqueuelen runtime restore
After=network-online.target
Wants=network-online.target
...
EOF_E85_TXQ_SERVICE
    systemctl daemon-reload >/dev/null 2>&1 || true
    systemctl enable --now xray-txqueue-e85.service >/dev/null 2>&1 || true
...
_e85_manual_txqueue() {
    ...
    v=${v:-3000}
    _e85_set_txqueuelen "$v"
}

_e85_apply_offload_default() {
    local iface script
    iface=$(_e85_default_iface)
    ...
    script="/usr/local/bin/xray-offload-e85.sh"
    cat > "$script" <<EOF_E85_OFFLOAD
#!/usr/bin/env bash
set -e
if command -v ethtool >/dev/null 2>&1; then
    ethtool -K "$iface" gro on gso on tso on lro off rx-gro-hw on tx-udp-segmentation on 2>/dev/null || true
...
EOF_E85_OFFLOAD
    chmod +x "$script" 2>/dev/null || true
    cat > /etc/systemd/system/xray-offload-e85.service <<EOF_E85_OFFLOAD_SERVICE
[Unit]
Description=Xray e85 NIC offload restore
After=network-online.target
Wants=network-online.target
...
[Install]
WantedBy=multi-user.target
EOF_E85_OFFLOAD_SERVICE
    systemctl daemon-reload >/dev/null 2>&1 || true
    systemctl enable --now xray-offload-e85.service >/dev/null 2>&1 || true
    info "GRO/GSO/TSO 已保持 ON，LRO 已保持 OFF，并创建开机恢复服务。"
    _e85_show_core_network_state
}

_e85_apply_mss_clamp() {
    local script
    if declare -F _apply_mss_chain_e52 >/dev/null 2>&1; then _apply_mss_chain_e52 || true; fi
    script="/usr/local/bin/xray-mss-clamp-e85.sh"
    cat > "$script" <<'EOF_E85_MSS_SCRIPT'
#!/usr/bin/env bash
set -e
modprobe iptable_mangle >/dev/null 2>&1 || true
modprobe xt_TCPMSS >/dev/null 2>&1 || true
...
EOF_E85_MSS_SCRIPT
    chmod +x "$script" 2>/dev/null || true
    cat > /etc/systemd/system/xray-mss-clamp-e85.service <<EOF_E85_MSS_SERVICE
[Unit]
Description=Xray e85 MSS clamp restore
After=network-online.target
Wants=network-online.target
...
EOF_E85_MSS_SERVICE
    systemctl daemon-reload >/dev/null 2>&1 || true
    systemctl enable --now xray-mss-clamp-e85.service >/dev/null 2>&1 || true
    info "MSS clamp 已应用并写入开机恢复服务。"
    iptables -t mangle -S XRAY_MSS_CLAMP 2>/dev/null | sed 's/^/    /' || true
}

_e85_apply_response_profile() {
    title "应用 e85 响应优先基线（不重构节点）"
    _e85_apply_fq_bbr_stable_core
    _e85_set_tcp_notsent_lowat 26214 "e85 响应优先基线"
    _e85_set_adv_app_profile 1 31 "e85 响应优先基线"
    _e85_set_txqueuelen 3000
    _e85_apply_offload_default
    _e85_apply_mss_clamp
    echo "response-26214" > "$CONFIG_DIR/network_profile_e85.txt" 2>/dev/null || true
}

_e85_apply_throughput_profile() {
    title "应用 e85 高吞吐基线（不重构节点）"
    ...
    _e85_apply_fq_bbr_stable_core
    _e85_set_tcp_notsent_lowat 131072 "e85 高吞吐基线"
    _e85_set_adv_app_profile 1 31 "e85 高吞吐基线"
    _e85_set_txqueuelen 3000
    _e85_apply_offload_default
    _e85_apply_mss_clamp
    echo "throughput-131072" > "$CONFIG_DIR/network_profile_e85.txt" 2>/dev/null || true
}
...
do_network_params_menu_e56() {
    while true; do
        clear
        title "网络参数应用中心（V198e85：缓冲 / notsent / fq+BBR / adv_win / txqueue / offload / MSS）"
        ...
        echo "  14) 应用 e85 响应优先基线：fq+BBR / notsent=26214 / txqueue=3000 / offload / MSS"
        echo "  15) 应用 e85 高吞吐基线：高 buffer / fq+BBR / notsent=131072 / txqueue=3000 / offload / MSS"
        ...
        local opt=""; read -rp "请选择操作: " opt || true
        case "${opt:-}" in
            1) apply_default_full_sysctl_profile_e60; _e85_set_tcp_notsent_lowat 131072 "默认高吞吐档"; _e52_pause ;;
            2) apply_balanced_network_buffer_params_e63; _e52_pause ;;
            3) manual_network_buffer_params_e56; _e52_pause ;;
            4) remove_legacy_hotupdate_sysctl_e61; if test -f "$NETWORK_OPT_CONF"; then if declare -F _e65_apply_sysctl_file_line_by_line >/dev/null 2>&1; then _e65_apply_sysctl_file_line_by_line "$NETWORK_OPT_CONF" || true; else sysctl -p "$NETWORK_OPT_CONF" || true; fi; _e63_force_runtime_key_values 1 31 2>/dev/null || true; else warn "文件不存在：$NETWORK_OPT_CONF"; fi; _e52_pause ;;
            5) if test -f "$NETWORK_OPT_CONF"; then sed 's/^/    /' "$NETWORK_OPT_CONF"; else warn "文件不存在：$NETWORK_OPT_CONF"; fi; _e52_pause ;;
            6) cleanup_network_sysctl_conflicts_e58; apply_default_full_sysctl_profile_e60; _e85_set_tcp_notsent_lowat 131072 "冲突清理后默认高吞吐档"; _e52_pause ;;
            7) _e85_set_tcp_notsent_lowat 26214 "极低排队响应档"; _e52_pause ;;
            8) _e85_set_tcp_notsent_lowat 32768 "低延迟响应档"; _e52_pause ;;
            9) _e85_set_tcp_notsent_lowat 65536 "均衡推荐档"; _e52_pause ;;
            10) _e85_set_tcp_notsent_lowat 131072 "稳态高吞吐档"; _e52_pause ;;
            11) _e85_manual_tcp_notsent_lowat; _e52_pause ;;
            12) _e85_show_tcp_notsent_lowat_state; _e85_show_notsent_conflicts; _e52_pause ;;
            13) _e85_show_network_param_discussion; _e52_pause ;;
            14) _e85_apply_response_profile; _e52_pause ;;
            15) _e85_apply_throughput_profile; _e52_pause ;;
            16) _e85_apply_fq_bbr_stable_core; _e52_pause ;;
            17) _e85_set_adv_app_profile 1 31 "推荐稳态接收窗口"; _e52_pause ;;
            18) _e85_set_adv_app_profile 2 31 "高 RTT 实验接收窗口"; _e52_pause ;;
            19) _e85_manual_adv_app; _e52_pause ;;
            20) _e85_set_txqueuelen 1000; _e52_pause ;;
            21) _e85_set_txqueuelen 3000; _e52_pause ;;
            22) _e85_manual_txqueue; _e52_pause ;;
            23) _e85_apply_offload_default; _e52_pause ;;
            24) _e85_apply_mss_clamp; _e52_pause ;;
            25) _e85_show_core_network_state; _e52_pause ;;
            0) return ;;
            *) warn "无效操作。"; sleep 1 ;;
        esac
    done
...
hot_update_from_legacy() {
    title "V198e85：稳态热更新（保留 e82 三协议 + 扩展 13 网络参数中心）"
    set +e
    trap - ERR

    mkdir -p "$CONFIG_DIR" "$SCRIPT_DIR" "$BACKUP_DIR" 2>/dev/null || true
    _e85_install_self_entrypoint || true
    local bdir
    if declare -F e76_state_snapshot >/dev/null 2>&1; then
        bdir=$(e76_state_snapshot "hotupdate_e85")
    else
        bdir="$BACKUP_DIR/hotupdate_e85_$(date +%Y%m%d_%H%M%S)"; mkdir -p "$bdir" 2>/dev/null || true
        cp -a "$CONFIG" "$bdir/config.json.bak" 2>/dev/null || true
        cp -a "$NETWORK_OPT_CONF" "$bdir/99-network-optimized.conf.bak" 2>/dev/null || true
    fi
    info "已创建热更新快照：$bdir"
    ...
    # e85 继续沿用 e82 的热更新默认：极低排队/响应档 26214。其余参数仅进入 13 菜单，不在热更新时强行改动。
    _e85_set_tcp_notsent_lowat 26214 "热更新默认极低排队响应值" || true
    ...
    echo "198e85" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    _e85_show_tcp_notsent_lowat_state || true
    _e85_show_core_network_state || true
    if declare -F e77_audit_runtime_state >/dev/null 2>&1; then e77_audit_runtime_state || true; fi
    set -e
    trap '_err_handler $? $LINENO "$BASH_COMMAND"' ERR
    info "V198e85 热更新完成。现在可输入：xrv → 13；新增 fq+BBR、adv_win、txqueuelen、offload、MSS 管理。"
    return 0
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e85 - (Reality / VLESSENC 自选 / SS2022 / Menu13网络参数中心)${none}"
        ...
        dns_mode=$(cat "${E67_DNS_MODE_FILE:-/usr/local/etc/xray/dns_mode.txt}" 2>/dev/null || echo auto)
        tz_line=$(date '+%Z %z' 2>/dev/null || echo unknown)
        notsent_now=$(_e85_current_notsent_lowat)
        notsent_perm=$(_e85_persistent_notsent_lowat)
        qdisc=$(_e85_sysctl_get net.core.default_qdisc)
        cc=$(_e85_sysctl_get net.ipv4.tcp_congestion_control)
        echo -e "  引擎态势: $svc | 全局快捷指令: ${cyan}xrv${none} | 外部识别 IP: ${yellow}$(_get_ip 2>/dev/null || echo "探测异常")${none}"
...
E85_NET_MANUAL_STATE_FILE="$CONFIG_DIR/network_manual_e85.txt"
E85_MSS_STATE_FILE="$CONFIG_DIR/mss_clamp_e85.txt"
E85_OFFLOAD_STATE_FILE="$CONFIG_DIR/offload_ooox_e85.txt"
...
# 兼容旧流程仍调用 e85/e77 安装入口的情况。
e85_install_self_entrypoint() { _e85_install_self_entrypoint "$@"; }
e77_install_self_entrypoint() { _e85_install_self_entrypoint "$@"; }

_e85_note_line() { printf "    %-28s %s\n" "$1" "$2"; }

_e85_show_menu13_manual_notes() {
    title "V198e85 菜单13：五项网络参数注释与默认值"
    local iface txq mss_state off_state
    iface=$(_e85_default_iface 2>/dev/null || true)
    if test -n "$iface"; then
        txq=$(ip -o link show "$iface" 2>/dev/null | sed -n 's/.*qlen \([0-9][0-9]*\).*/\1/p' | head -n1)
    fi
    mss_state=$(cat "$E85_MSS_STATE_FILE" 2>/dev/null || echo "unknown")
    off_state=$(cat "$E85_OFFLOAD_STATE_FILE" 2>/dev/null || echo "unknown")

    echo "  【默认策略】"
    _e85_note_line "tcp_adv_win_scale" "默认 1；吞吐实验 2-3；过大可能放大排队/内存压力。当前：$(_e85_sysctl_get net.ipv4.tcp_adv_win_scale)"
    _e85_note_line "tcp_app_win" "默认 31；允许 0~31；普通代理建议 31。当前：$(_e85_sysctl_get net.ipv4.tcp_app_win)"
    _e85_note_line "txqueuelen" "默认 3000；1000 偏低排队，3000 偏稳态吞吐。当前：${txq:-unknown}"
    _e85_note_line "GRO/GSO/TSO/LRO" "默认 OOOX，即 GRO=ON/GSO=ON/TSO=ON/LRO=OFF。记录：$off_state"
    _e85_note_line "MSS clamp" "默认 ON；用于降低 PMTU 黑洞/分片异常导致的卡顿。记录：$mss_state"
    echo ""
    echo "  【符号说明】"
...
_e85_set_tcp_adv_win_scale_manual() {
    local v cur
    cur=$(_e85_sysctl_get net.ipv4.tcp_adv_win_scale)
    echo ""
    ...
    _e85_set_sysctl_key net.ipv4.tcp_adv_win_scale "$v" "V198e85 手动：tcp_adv_win_scale，默认1，吞吐2-3"
    echo "tcp_adv_win_scale=$v" >> "$E85_NET_MANUAL_STATE_FILE" 2>/dev/null || true
}

_e85_set_tcp_app_win_manual() {
    local v cur
    cur=$(_e85_sysctl_get net.ipv4.tcp_app_win)
    echo ""
    ...
    _e85_set_sysctl_key net.ipv4.tcp_app_win "$v" "V198e85 手动：tcp_app_win，默认31，范围0~31"
    echo "tcp_app_win=$v" >> "$E85_NET_MANUAL_STATE_FILE" 2>/dev/null || true
}

_e85_set_txqueuelen_manual() {
    local v iface
    iface=$(_e85_default_iface 2>/dev/null || true)
    echo ""
    ...
    _e85_set_txqueuelen "$v"
    echo "txqueuelen=$v" >> "$E85_NET_MANUAL_STATE_FILE" 2>/dev/null || true
}

_e85_apply_offload_profile() {
    ...
    script="/usr/local/bin/xray-offload-e85.sh"
    cat > "$script" <<EOF_E85_OFFLOAD_SCRIPT
#!/usr/bin/env bash
set +e
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
IFACE="$iface"
...
exit 0
EOF_E85_OFFLOAD_SCRIPT
    chmod +x "$script" 2>/dev/null || true
    cat > /etc/systemd/system/xray-offload-e85.service <<EOF_E85_OFFLOAD_SERVICE
[Unit]
Description=Xray e85 GRO/GSO/TSO/LRO restore
After=network-online.target
Wants=network-online.target
...
EOF_E85_OFFLOAD_SERVICE
    systemctl daemon-reload >/dev/null 2>&1 || true
    systemctl enable --now xray-offload-e85.service >/dev/null 2>&1 || true
    echo "$pattern" > "$E85_OFFLOAD_STATE_FILE" 2>/dev/null || true
    info "已应用并持久化：GRO/GSO/TSO/LRO=$pattern（默认推荐 OOOX）。"
    _e85_show_core_network_state
}

_e85_manual_offload_profile() {
    local p
    echo ""
    ...
    p=${p:-OOOX}
    _e85_apply_offload_profile "$p"
}

_e85_mss_clamp_off() {
    title "关闭 MSS clamp（移除 XRAY_MSS_CLAMP 链与 e85 恢复服务）"
    systemctl disable --now xray-mss-clamp-e85.service >/dev/null 2>&1 || true
    rm -f /etc/systemd/system/xray-mss-clamp-e85.service /usr/local/bin/xray-mss-clamp-e85.sh 2>/dev/null || true
    systemctl daemon-reload >/dev/null 2>&1 || true
    local IPT
    ...
    echo "OFF" > "$E85_MSS_STATE_FILE" 2>/dev/null || true
    info "MSS clamp 已关闭。默认推荐是 ON；仅在明确不需要 PMTU 防卡顿时关闭。"
}

_e85_mss_clamp_on() {
    # 复用 e85/e83 已有宽容实现，失败只警告。
    if declare -F _e85_apply_mss_clamp >/dev/null 2>&1; then
        _e85_apply_mss_clamp || true
    elif declare -F _apply_mss_chain_e52 >/dev/null 2>&1; then
        _apply_mss_chain_e52 || true
    fi
    echo "ON" > "$E85_MSS_STATE_FILE" 2>/dev/null || true
    info "MSS clamp 默认 ON 已记录。"
}

_e85_manual_mss_clamp() {
    local v
    echo ""
    ...
    v=${v:-1}
    case "$v" in
        1|on|ON|On|o|O) _e85_mss_clamp_on ;;
        0|off|OFF|Off|x|X) _e85_mss_clamp_off ;;
        *) error "输入无效。"; return 1 ;;
    esac
}

_e85_apply_menu13_defaults() {
    title "应用 V198e85 菜单13默认五项：adv=1 / app=31 / txqueue=3000 / OOOX / MSS=ON"
    _e85_set_sysctl_key net.ipv4.tcp_adv_win_scale 1 "V198e85 默认：tcp_adv_win_scale=1，吞吐实验2-3"
    _e85_set_sysctl_key net.ipv4.tcp_app_win 31 "V198e85 默认：tcp_app_win=31，范围0~31"
    _e85_set_txqueuelen 3000 || true
    _e85_apply_offload_profile OOOX || true
    _e85_mss_clamp_on || true
    echo "e85-defaults" > "$CONFIG_DIR/network_profile_e85.txt" 2>/dev/null || true
    _e85_show_menu13_manual_notes
}
...
do_network_params_menu_e56() {
    while true; do
        clear
        title "网络参数应用中心（V198e85：缓冲 / notsent / fq+BBR / 五项手动参数注释）"
        if declare -F show_network_buffer_params_e56 >/dev/null 2>&1; then show_network_buffer_params_e56; fi
        _e85_show_tcp_notsent_lowat_state
        _e85_show_core_network_state
        _e85_show_menu13_manual_notes
        ...
        echo "  12) 仅查看 tcp_notsent_lowat 当前值与定义来源"
        echo "  13) 查看影响翻墙网络速度的参数说明"
        echo "  14) 应用 e85 响应优先基线：fq+BBR / notsent=26214 / txqueue=3000 / OOOX / MSS=ON"
        echo "  15) 应用 e85 高吞吐基线：高 buffer / notsent=131072 / txqueue=3000 / OOOX / MSS=ON"
        echo "  16) 仅应用 fq + bbr + ECN=0 + TFO=0 + MTU probing=1 稳态核心"
        echo "  17) 切换 tcp_adv_win_scale=1 + tcp_app_win=31（推荐稳态）"
        echo "  18) 切换 tcp_adv_win_scale=2 + tcp_app_win=31（高 RTT 实验）"
        ...
        echo "  25) 仅查看关键网络状态 / qdisc / offload"
        echo "  26) 一键应用 e85 默认五项：adv=1 / app=31 / txqueue=3000 / OOOX / MSS=ON"
        ...
        case "${opt:-}" in
            ...
            12) _e85_show_tcp_notsent_lowat_state; _e85_show_notsent_conflicts; _e52_pause ;;
            13) _e85_show_network_param_discussion; _e52_pause ;;
            14) _e85_apply_response_profile; _e85_apply_menu13_defaults; _e52_pause ;;
            15) _e85_apply_throughput_profile; _e85_apply_menu13_defaults; _e85_set_tcp_notsent_lowat 131072 "e85 高吞吐基线"; _e52_pause ;;
            16) _e85_apply_fq_bbr_stable_core; _e52_pause ;;
            17) _e85_set_adv_app_profile 1 31 "推荐稳态接收窗口"; _e52_pause ;;
            18) _e85_set_adv_app_profile 2 31 "高 RTT 实验接收窗口"; _e52_pause ;;
            19) _e85_manual_adv_app; _e52_pause ;;
            20) _e85_set_txqueuelen 1000; _e52_pause ;;
            21) _e85_set_txqueuelen 3000; _e52_pause ;;
            22) _e85_set_txqueuelen_manual; _e52_pause ;;
            23) _e85_apply_offload_profile OOOX; _e52_pause ;;
            24) _e85_mss_clamp_on; _e52_pause ;;
            25) _e85_show_core_network_state; _e52_pause ;;
            26) _e85_apply_menu13_defaults; _e52_pause ;;
            27) _e85_set_tcp_adv_win_scale_manual; _e52_pause ;;
            28) _e85_set_tcp_app_win_manual; _e52_pause ;;
            29) _e85_set_txqueuelen_manual; _e52_pause ;;
            30) _e85_manual_offload_profile; _e52_pause ;;
            31) _e85_manual_mss_clamp; _e52_pause ;;
            32) _e85_show_menu13_manual_notes; _e85_show_core_network_state; _e52_pause ;;
            0) return ;;
            *) warn "无效操作。"; sleep 1 ;;
        esac
    done
...
hot_update_from_legacy() {
    title "V198e85：稳态热更新（保留 e83 三协议 + 菜单13五项网络参数手动管理）"
    set +e
    trap - ERR
    ...
    _e85_set_tcp_notsent_lowat 26214 "热更新默认极低排队响应值" || true
    _e85_apply_menu13_defaults || true
    ...
    echo "198e85" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    _e85_show_tcp_notsent_lowat_state || true
    _e85_show_menu13_manual_notes || true
    _e85_show_core_network_state || true
    if declare -F e77_audit_runtime_state >/dev/null 2>&1; then e77_audit_runtime_state || true; fi
    set -e
    trap '_err_handler $? $LINENO "$BASH_COMMAND"' ERR
    info "V198e85 热更新完成。现在可输入：xrv → 13；新增 26-32：五项参数注释、一键默认与手动修改。"
    return 0
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e85 - (Reality / VLESSENC 自选 / SS2022 / Menu13五项网络参数)${none}"
        ...
        qdisc=$(_e85_sysctl_get net.core.default_qdisc)
        cc=$(_e85_sysctl_get net.ipv4.tcp_congestion_control)
        adv=$(_e85_sysctl_get net.ipv4.tcp_adv_win_scale)
        app=$(_e85_sysctl_get net.ipv4.tcp_app_win)
        echo -e "  引擎态势: $svc | 全局快捷指令: ${cyan}xrv${none} | 外部识别 IP: ${yellow}$(_get_ip 2>/dev/null || echo "探测异常")${none}"
        echo -e "  系统装配内核: ${cyan}${sys_ver}${none} | 微架构侦测: x64-v${yellow}${arch_level}${none} | qdisc/cc: ${yellow}${qdisc}/${cc}${none}"
...
#==============================================================================
# EOF - Apex Vanguard V198e85 Menu13 Manual Network Control Ready.
#==============================================================================
