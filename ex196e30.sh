#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e30.sh (The Apex Vanguard - Project Genesis V196e30)
# 快捷方式: xrv
#
# 【V196e30 终极涅槃天网版：真理的完全体】
#   1. 根治编译死机: 废除 e22 的全局 Find Makefile 轰炸，恢复 e21 的精准定点修正，保护底层模块指令集。
#   2. 驱动完美继承: 强制继承宿主机 /boot/config，杜绝因 defconfig 导致的网卡失联与黑屏。
#   3. 斩断证书锁链: 精准抹除 CONFIG_SYSTEM_TRUSTED_KEYS，彻底消灭 signing_key.pem Error 2。
#   4. 生命周期保障: 废除编译后强删旧内核的自杀逻辑，确保 GRUB 退路畅通。
#   5. JQ 绝缘层护盾: 全域覆盖 select(. != null)，抵御空数组导致的内核熔断。
# ==============================================================================

if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex196e30.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="196e30"
...
do_kernel_compile_menu() {
    while true; do
        clear
        title "极客内核源码锻造中心 (V196e30 真理天网版)"
        echo "  已根除 e22 中的全局 Find 暴走机制，保护所有底层指令集！"
        echo "  现已引入终极清道夫机制与驱动基因完美继承，彻防卡死，并停止旧内核强制卸载！"
        echo ""
...
_execute_compilation() {
    ...
    # 【核心修复】：保留所有旧版内核作为 GRUB 备用防爆退路，不再进行任何 dpkg purge 的自杀式删除！
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e30 - (The Apex Vanguard)${none}"
...
# ==============================================================================
# EOF - Apex Vanguard V196e30 System Advanced Control Ready.
# ==============================================================================
# --- ✂️ Part 15 结束，所有代码拼接完毕。请运行 bash ex196e30.sh ---
