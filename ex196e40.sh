#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e40.sh (The Apex Vanguard - Project Genesis V196e40)
# 快捷方式: xrv
#
# 【V196e40 终极不朽版：动态选库、斩断32位死结、全量菜单回归】
#   1. 全量回归: 彻底修复指令 10 报 command not found 的低级漏切错误。
#   2. API 动态分支: 实时连接 GitLab 拉取 Xanmod 核心库，默认置顶 6.18.25-rt-xanmod1。
#   3. 粉碎编译暴毙: 物理关闭 CONFIG_IA32_EMULATION，彻底解决 vdso32 与指令集的冲突。
#   4. JQ 绝缘: 100% 覆盖 select(. != null)，深渊级防爆。
# ==============================================================================

if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex196e40.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="196e40"
...
_fetch_xanmod_tags() {
    info "正在连接 GitLab API 检索最新 Xanmod 内核分支库..."
    ...
    local fallback_tags=("$default_tag" "6.12.25-xanmod1" "6.6.78-xanmod1")
    
    if test -z "$tags_json" || echo "$tags_json" | grep -q "message"; then
        warn "API 检索超时或受阻，启用本地备用版本清单..."
        TAG_LIST=("${fallback_tags[@]}")
    else
        # 提取最新 Tag，过滤 rc 和 beta，保留前 15 个最新稳定版/RT版
        mapfile -t TAG_LIST < <(echo "$tags_json" | jq -r '.[].name' | grep -vE "rc|beta" | head -n 15)
        # 确保默认版本永远置顶
    ...
    for tag in "${TAG_LIST[@]}"; do
        if [ "$tag" = "$default_tag" ]; then
            echo -e "  ${magenta}$i) $tag (默认推荐 - 破死锁验证版)${none}"
        else
            echo -e "  $i) $tag"
        fi
        i=$((i + 1))
    ...
    if test "$sel" = "0"; then
        read -rp "请精准输入 Tag 名称 (例如 6.18.25-rt-xanmod1): " LATEST_TAG
    else
        local idx=$((sel - 1))
...
_compile_kernel_xanmod() {
    ...
    if ! tar -tzf "$KERNEL_FILE" >/dev/null 2>&1; then
        ...
        rm -f "$KERNEL_FILE" 2>/dev/null || true
        set +e
        LATEST_TAG=$(curl -sL --connect-timeout 10 https://gitlab.com/api/v4/projects/xanmod%2Flinux/repository/tags | jq -r '.[].name' 2>/dev/null | grep -E "^6\.1[0-9]\.[0-9]+(-rt)?-xanmod[0-9]+$" | head -n 1 || echo "6.18.25-rt-xanmod1")
        set -e
    ...
    # 【神级防爆 2】：VDSO32 底层补丁防御 (防止某些旧版架构强行开启)
...
# ==============================================================================
# 【神级归位】do_sys_init_menu 核心菜单与双轨内核通道
# ==============================================================================
do_install_xanmod_official_fusion() {
    title "系统飞升：安装官方预编译 XANMOD 内核 (APT 双轨融合版)"
    
    local cpu_arch=$(uname -m 2>/dev/null || echo "")
    if [ "$cpu_arch" != "x86_64" ]; then error "官方源安装当前仅支持 x86_64 架构！"; return 1; fi
    if [ ! -r /etc/os-release ]; then error "无法确定操作系统类型！"; return 1; fi
    ...
    if ! wget -qO - "$key_url" | gpg --dearmor -o "$keyring" --yes 2>/dev/null; then error "官方密钥下载失败，XanMod 源配置异常！"; return 1; fi
    ...
    if [ "$installed" = "false" ]; then error "软件源中未找到适配此 CPU 的 XanMod 内核包！"; return 1; fi
...
do_kernel_compile_menu() {
    while true; do
        clear
        title "极客内核源码锻造中心 (双通道死锁粉碎版)"
        echo "  已彻底修复 e22 全局暴走与架构断层！"
        echo "  已通过 make olddefconfig 粉碎一切静默编译死锁！"
        echo "  现已引入：宿主机驱动基因完美继承 + 终极 PEM 签名切除 + 32位兼容层斩断！"
        echo ""
        ...
        echo "     - 源码直连 GitLab API，智能抓取动态列表自选，斩断兼容层防爆"
        echo ""
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e40 - (The Apex Vanguard)${none}"
...
# ==============================================================================
# EOF - Apex Vanguard V196e40 System Advanced Control Ready.
# ==============================================================================
# --- ✂️ Part 20 结束，所有代码拼接完毕。请运行 bash ex196e40.sh ---
