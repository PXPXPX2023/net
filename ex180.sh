...
#    ╚═╝   ╚═╝  ╚═╝╚══════╝    ╚═╝  ╚═╝╚═╝     ╚══════╝╚═╝  ╚═╝
# ==============================================================================
# 脚本名称: ex180.sh (The Apex Vanguard - Project Genesis V180 [The Absolute Genesis])
# 快捷方式: xrv
#
# V180 终极大一统修补宣言:
#   1. 编译引擎复活: 补齐遗失的 do_xanmod_compile，恢复内核源码防砖提取与 BBR3 编译。
#   2. 分发中心重构: 彻底修复 do_summary 的 JQ 溢出 Bug，实现多用户无损链接分发。
#   3. 雷达重构: 废弃易被阻断的 AliDNS，回归纯粹的 TCP 时延连通性嗅探验证。
#   4. 免疫误杀: 彻底修复 set -e 下的短路与 ((var++)) 算术陷阱，实现 0 崩溃。
...
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行: bash ex180.sh"
    exit 1
fi
...
run_sni_scanner() {
    ...
    print_cyan ">>> V180 战地重构：已彻底剥离 AliDNS，完全基于 TCP 物理握手验证纯净度！\n"
...
toggle_process_priority() {
    ...
    if grep -q "^OOMScoreAdjust=-500" "$lf" 2>/dev/null; then
...
EOF
                ...
                if ! crontab -l 2>/dev/null | grep -q "cc1.sh"; then
                    (crontab -l 2>/dev/null || true ; echo "0 4 */10 * * /usr/local/bin/cc1.sh") | crontab - 2>/dev/null || true
                fi
...
do_update_core() {
    ...
    systemctl restart xray >/dev/null 2>&1 || true
    local cur_ver
    cur_ver=$($XRAY_BIN version 2>/dev/null | head -n1 | awk '{print $2}' || echo "未知")
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray ex180 The Apex Vanguard - Project Genesis V180 (大一统创世版)${none}"
...
# ==============================================================================
# EOF: 极客工业级底层标志，本行如果存在即代表 V180 核心引擎全系防爆版输出圆满完成！
# ==============================================================================
