#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e9.sh (The Apex Vanguard - Project Genesis V196e9)
# 快捷方式: xrv
#
# 【V196e9 终极勘误与真理重塑】
#   1. 根绝编译血案: 窃取宿主机 config，物理篡改 arch/x86/Makefile，根除 -march= 暴毙！
#   2. 缓存对齐修复: 强制硬写入 CONFIG_X86_L1_CACHE_SHIFT=6 解决 7.x 汇编宏丢失问题。
#   3. 真·源码溯源: 直连 GitLab 官方仓库拉取正宗 7.x Xanmod 源码，剔除臃肿的 DEBUG 信息。
#   4. APT 源净化: 修复预编译安装时 GPG 密钥 404 及 sources.list 重复冲突导致的静默瘫痪。
#   5. 完美融合: 全量继承 V193/V196 的 JQ 绝缘护盾 (select(. != null)) 与 25 项系统微操。
# ==============================================================================

# 必须用 bash 运行
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex196e9.sh"
    exit 1
fi
...
# ------------------------------------------------------------------------------
# 【V196e9 终极勘误 1】：真·预编译官方 XANMOD 部署模块 (修复多重源与 404 死锁)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 【V196e9 终极勘误 2】：真·Xanmod 源码极客编译引擎 (抛弃 Mainline 假货，直取 GitLab)
# ------------------------------------------------------------------------------
...
do_xanmod_compile() {
    ...
    # [核心级防御修复]: 绝杀 x86-64-v 畸形换行符 Bug，提取最纯净的 CPU 等级
    local arch_level="1"
    ...
    set -e
    info "探针锁定宿主机 CPU 微架构级别为: x86-64-v${arch_level}"
    ...
    # 【核心级防熔断】：针对 Kconfig 互斥导致 CONFIG_X86_64_VERSION 丢失的终极解法
    # 1. 主动关闭 GENERIC_CPU 防止单选项冲突
    ./scripts/config --disable CONFIG_GENERIC_CPU 2>/dev/null || true
    # 2. 强力注入微架构等级
    ./scripts/config --enable "CONFIG_X86_64_V${arch_level}" 2>/dev/null || true
    ./scripts/config --set-val CONFIG_X86_64_VERSION "${arch_level}" 2>/dev/null || true
    # 3. 硬核补齐 L1 Cache 缓存对齐宏 (否则内核 asm-offsets.s 必崩)
    ./scripts/config --set-val CONFIG_X86_L1_CACHE_SHIFT 6 2>/dev/null || true
    ./scripts/config --set-val CONFIG_X86_INTERNODE_CACHE_SHIFT 6 2>/dev/null || true
    
    # 物理暴力防掉档：无论 Kconfig 怎么重算，强行把配置塞在末尾
    echo "CONFIG_X86_64_VERSION=${arch_level}" >> .config
    echo "CONFIG_X86_L1_CACHE_SHIFT=6" >> .config
    echo "CONFIG_X86_INTERNODE_CACHE_SHIFT=6" >> .config

    # 让系统安全验证配置（绝不使用 yes "" |）
    make olddefconfig >/dev/null 2>&1 || true

    # 【上帝之手】：物理篡改 Makefile！
    # 既然 Kconfig 可能会吃掉 CONFIG_X86_64_VERSION，我们直接在 Makefile 层把它按死！
    # 彻底根绝 cc1: error: bad value (‘x86-64-v’) for ‘-march=’ switch
    info "正在对 arch/x86/Makefile 进行物理降维锁定..."
    find . -name Makefile -type f -exec sed -i "s/-march=x86-64-v\$(CONFIG_X86_64_VERSION)/-march=x86-64-v${arch_level}/g" {} + 2>/dev/null || true
    find . -name Makefile -type f -exec sed -i "s/-march=x86-64-v\${CONFIG_X86_64_VERSION}/-march=x86-64-v${arch_level}/g" {} + 2>/dev/null || true
...
do_summary() {
    ...
    local client_count=$(jq -r '.inbounds[]? | select(.protocol=="vless") | .settings.clients | length' "$CONFIG" 2>/dev/null || echo 0)
    
    if test "${client_count:-0}" -gt 0; then
        local port=$(jq -r '.inbounds[]? | select(.protocol=="vless") | .port' "$CONFIG" 2>/dev/null || echo "443")
        local pub=$(jq -r '.inbounds[]? | select(.protocol=="vless") | .streamSettings.realitySettings.publicKey // "缺失"' "$CONFIG" 2>/dev/null)
        local all_snis=$(jq -r '.inbounds[]? | select(.protocol=="vless") | .streamSettings.realitySettings.serverNames | join(", ") // "缺失"' "$CONFIG" 2>/dev/null)
        local main_sni=$(jq -r '.inbounds[]? | select(.protocol=="vless") | .streamSettings.realitySettings.serverNames[0] // "缺失"' "$CONFIG" 2>/dev/null)
        
        for ((i=0; i<client_count; i++)); do
            local uuid=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .settings.clients[$i].id" "$CONFIG" 2>/dev/null || echo "")
            local remark=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .settings.clients[$i].email // \"$REMARK_NAME\"" "$CONFIG" 2>/dev/null)
            local sid=$(jq -r ".inbounds[]? | select(.protocol==\"vless\") | .streamSettings.realitySettings.shortIds[$i] // \"缺失\"" "$CONFIG" 2>/dev/null)
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e9 - (The Apex Vanguard)${none}"
...
main_menu
# ==============================================================================
# EOF - Apex Vanguard V196e9 System Advanced Control Ready.
# ==============================================================================

# --- ✂️ Part 5 结束，脚本完全拼接完毕 ✂️ ---
