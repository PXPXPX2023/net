#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex149.sh (The Apex Vanguard - Project Genesis V149 [Absolute Horizon])
# 快捷方式: xrv
# 终极修复宣言 (绝对防截断全量版): 
#   1. 绝杀编译死锁：彻底移除对 Xanmod 架构变量 (CONFIG_X86_64_V*) 的鲁莽干预，完全信任原生 defconfig，彻底解决 -march 空指针导致的 Error 1/2 崩溃。保留防 OOM 1GB 永久 Swap 逻辑。
#   2. 硬件绝对保护：探测到网卡底层限制，永久锁死 GSO/GRO 强制修改逻辑，绝不执行危险指令，防范内核断流。
#   3. 物理装甲全量回归：完美复刻 V62 的 100万并发锁 (limits.conf)、60+ 行极限 Sysctl 参数、cc1.sh 守护进程与 resolvconf 底层锁死。
#   4. 核心大一统：保留 28 项全域微操矩阵、回落探针 (菜单 8)、8 行军规排版、ShortId 防漂移联动删除。
#   5. 拒绝代码阉割：所有 jq 管道流与复杂的 if-else 逻辑全面恢复多行可读结构。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex149.sh"
    exit 1
fi
...
# V149 绝杀修复: 拒绝触碰 CONFIG_X86_64_VERSION，纯粹信任原生 defconfig
do_xanmod_compile() {
    ...
    # 核心绝杀：只生成基础配置，绝不干预 CPU 架构选择，防止依赖断层！
    make defconfig
...
# V149 绝杀防护：强行跳过固定无法更改设置的网卡
toggle_gso_off() { 
    local IFACE=$(ip route get 1.1.1.1 | awk '{print $5; exit}')
    if [ "$(check_gso_off_state)" = "unsupported" ]; then 
        print_red "检测到当前机器网卡底层限制 (fixed)，为防止内核断网失联，已永久物理锁死 GSO/GRO 硬件卸载状态，禁止强制干预！"
        sleep 3
        return
    fi
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex149 The Apex Vanguard - Project Genesis V149${none}"
