#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex116.sh (The Apex Vanguard - Project Genesis V116 [The Absolute Absolute])
# 快捷方式: xrv
# 创世终章: 
#   1. 状态机绝杀：废除脆弱的 tc qdisc 内存态探针，实装 /etc/xray/flags 物理锚点池。根除字符串重叠 BUG，确保 ack-filter/ecn/wash 跨越重启与重载，实现真正的【永久有效】。
#   2. ECN/Wash 引擎并网：新增 24/25 项微操，在 CAKE 队列中注入显式拥塞通知与报文清洗，实现跨洋高延迟链路下的 0 丢包平滑降速。
#   3. 港口管治完全体：继承 Oceanic 跨洋调度、Besteffort 盲走降负与 Bandwidth 硬件拆解补偿。
#   4. 逻辑断链修复：确保所有 CAKE 高阶参数在 update_hw_boot_script 中完美拼接，免疫“阅后即焚”。
#   5. 大一统矩阵扩容：全域扩张至 25 项极限微操 + 3 项上帝开关，构建史无前例的 28 项单节点路由物理形态。
#   6. 不追求代码行数的精简，只追求工业级的绝对健壮，敬这架构的巅峰。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex116.sh"
    exit 1
fi
    ...
    while true; do
        if test -f "$SNI_CACHE_FILE"; then
            ...
            if test "$sel" = "m"; then
                ...
                if test ${#arr[@]} -eq 0; then error "选择无效"; continue; fi
                BEST_SNI="${arr[0]}"
                local jq_args=()
...
validate_port() {
    ...
    if test "${p:-0}" -ge 1 2>/dev/null && test "${p:-0}" -le 65535 2>/dev/null; then
        if ss -tuln | grep -q ":${p} "; then print_red "端口 $p 已被占用！"; return 1; fi
        return 0
    fi
    return 1
}
...
do_txqueuelen_opt() {
    ...
    local IFACE=$(ip route get 1.1.1.1 | awk '{print $5; exit}')
    if [ -z "$IFACE" ]; then error "无法识别默认出口网卡。"; read -rp "Enter..." _; return 1; fi
...
config_cake_advanced() {
    ...
    if [ -n "$c_rtt" ]; then final_opts="$final_opts $c_rtt"; fi
    if [ -n "$c_diff" ]; then final_opts="$final_opts $c_diff"; fi
...
# [神级修复] 使用物理锚点文件实现绝对永久记忆，免疫 qdisc 状态重叠
...
toggle_dnsmasq() {
    if [ "$(check_dnsmasq_state)" = "true" ]; then
        ...
        chattr -i /etc/resolv.conf 2>/dev/null || true; rm -f /etc/resolv.conf 2>/dev/null || true
        if [ -f /etc/resolv.conf.bak ]; then mv /etc/resolv.conf.bak /etc/resolv.conf; else echo "nameserver 8.8.8.8" > /etc/resolv.conf; fi
        ...
        export DEBIAN_FRONTEND=noninteractive
        apt-get update || yum makecache; apt-get install -y dnsmasq || yum install -y dnsmasq
...
EOF
        ...
        rm -f /etc/resolv.conf 2>/dev/null || true; echo "nameserver 127.0.0.1" > /etc/resolv.conf; chattr +i /etc/resolv.conf 2>/dev/null || true
...
toggle_mtu() {
    ...
    if [ "$(check_mtu_state)" = "true" ]; then sed -i 's/^net.ipv4.tcp_mtu_probing.*/net.ipv4.tcp_mtu_probing = 0/' "$conf" 2>/dev/null || true
    else
        if grep -q "net.ipv4.tcp_mtu_probing" "$conf" 2>/dev/null; then sed -i 's/^net.ipv4.tcp_mtu_probing.*/net.ipv4.tcp_mtu_probing = 1/' "$conf"
        else echo "net.ipv4.tcp_mtu_probing = 1" >> "$conf"; fi
    fi
...
toggle_cpu() {
    ...
    if [ "$(check_cpu_state)" = "true" ]; then for cpu in /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor; do if [ -f "$cpu" ]; then echo schedutil > "$cpu" 2>/dev/null || true; fi; done
    else for cpu in /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor; do if [ -f "$cpu" ]; then echo performance > "$cpu" 2>/dev/null || true; fi; done; fi
    update_hw_boot_script
}
...
toggle_ring() {
    ...
    else ethtool -G "$IFACE" rx 512 tx 512 2>/dev/null || true; fi
    update_hw_boot_script
}
...
toggle_gso_off() {
    ...
    if [ "$(check_gso_off_state)" = "true" ]; then ethtool -K "$IFACE" gro on gso on tso on 2>/dev/null || true
    else ethtool -K "$IFACE" gro off gso off tso off 2>/dev/null || true; fi
    update_hw_boot_script
}
...
toggle_zram() {
    ...
    if [ "$(check_zram_state)" = "true" ]; then
        swapoff /dev/zram0 2>/dev/null || true; rmmod zram 2>/dev/null || true; systemctl disable xray-zram.service --now 2>/dev/null || true
        rm -f /etc/systemd/system/xray-zram.service /usr/local/bin/xray-zram.sh
    else
        local TOTAL_MEM=$(free -m | awk '/Mem/{print $2}'); local ZRAM_SIZE
        if [ "$TOTAL_MEM" -lt 500 ]; then ZRAM_SIZE=$((TOTAL_MEM * 2)); elif [ "$TOTAL_MEM" -lt 1024 ]; then ZRAM_SIZE=$((TOTAL_MEM * 3 / 2)); else ZRAM_SIZE=$TOTAL_MEM; fi
...
WantedBy=multi-user.target
EOFZ
        systemctl daemon-reload; systemctl enable xray-zram.service; systemctl start xray-zram.service
    fi
}
...
toggle_journal() {
    ...
    if [ "$(check_journal_state)" = "true" ]; then sed -i 's/^Storage=volatile/#Storage=auto/' "$conf" 2>/dev/null || true
    else
...
toggle_process_priority() {
    ...
    if grep -q "^OOMScoreAdjust=-500" "$limit_file"; then
        sed -i '/^OOMScoreAdjust=/d' "$limit_file"; sed -i '/^IOSchedulingClass=/d' "$limit_file"; sed -i '/^IOSchedulingPriority=/d' "$limit_file"
    else
        echo "OOMScoreAdjust=-500" >> "$limit_file"; echo "IOSchedulingClass=realtime" >> "$limit_file"; echo "IOSchedulingPriority=2" >> "$limit_file"
    fi
    systemctl daemon-reload
}
...
toggle_irq() {
    ...
    local IFACE=$(ip route get 1.1.1.1 | awk '{print $5; exit}')
    local CORES=$(nproc); local DEFAULT_MASK=$(printf "%x" $(( (1<<CORES)-1 )))
    if [ "$(check_irq_state)" = "true" ]; then
        for irq in $(grep "$IFACE" /proc/interrupts 2>/dev/null | awk '{print $1}' | tr -d ':'); do echo "$DEFAULT_MASK" > /proc/irq/$irq/smp_affinity 2>/dev/null || true; done
        ...
        for irq in $(grep "$IFACE" /proc/interrupts 2>/dev/null | awk '{print $1}' | tr -d ':'); do echo 1 > /proc/irq/$irq/smp_affinity 2>/dev/null || true; done
    fi
    update_hw_boot_script
}
...
_turn_on_app() {
    ...
    local limit_file="/etc/systemd/system/xray.service.d/limits.conf"
    if [ -f "$limit_file" ]; then
        sed -i '/^Environment="XRAY_RAY_BUFFER_SIZE=/d' "$limit_file"; echo "Environment=\"XRAY_RAY_BUFFER_SIZE=64\"" >> "$limit_file"
        local TOTAL_MEM=$(free -m | awk '/Mem/{print $2}'); local DYNAMIC_GOGC=100
        if [ "$TOTAL_MEM" -ge 1800 ]; then DYNAMIC_GOGC=1000; elif [ "$TOTAL_MEM" -ge 900 ]; then DYNAMIC_GOGC=500; else DYNAMIC_GOGC=300; fi
...
do_app_level_tuning_menu() {
    while true; do
        ...
        if ! test -f "$CONFIG"; then error "未发现配置，请先执行核心安装！"; read -rp "Enter..." _; return; fi

        # App 1-11
        local out_fastopen=$(jq -r '.outbounds[]? | select(.protocol=="freedom") | .streamSettings.sockopt.tcpFastOpen // "false"' "$CONFIG" 2>/dev/null | head -1)
        local out_keepalive=$(jq -r '.outbounds[]? | select(.protocol=="freedom") | .streamSettings.sockopt.tcpKeepAliveIdle // "false"' "$CONFIG" 2>/dev/null | head -1)
        local sniff_status=$(jq -r '.inbounds[]? | select(.protocol=="vless" or .protocol=="shadowsocks") | .sniffing.metadataOnly // "false"' "$CONFIG" 2>/dev/null | head -1)
        local dns_status=$(jq -r '.dns.queryStrategy // "false"' "$CONFIG" 2>/dev/null | head -1)
        local policy_status=$(jq -r '.policy.levels["0"].connIdle // "false"' "$CONFIG" 2>/dev/null | head -1)
...
do_summary() {
    ...
    local has_ss=$(jq -r '.inbounds[]? | select(.protocol=="shadowsocks") | .protocol' "$CONFIG" 2>/dev/null | head -n 1)
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray ex116 The Apex Vanguard - Project Genesis V116${none}"
