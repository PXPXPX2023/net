#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e18.sh (The Apex Vanguard - Project Genesis V196e18)
# 快捷方式: xrv
#
# 【V196e18 终极勘误与真理重塑】
#   1. 彻底修复 Xanmod 7.x 编译暴毙: 强制注入 CONFIG_X86_64_VERSION=1 兜底，解决 -march 拼写断层。
#   2. 锁定稳定版源码: 避开 7.x 过于前沿的水土不服，倾向选择极度稳定的 6.x / 6.12 系列。
#   3. 补全遗失函数: 彻底找回缺失的 do_sys_init_menu 核心系统初始化菜单。
#   4. 三通道内核锻造: 新增 Mainline+BBR, Xanmod+BBR3, Mainline+BBR3 极客编译三重路线。
#   5. JQ 绝对绝缘: 全域覆盖 select(. != null) 护盾，杜绝任何数组为空时的语法崩溃。
# ==============================================================================

# 必须用 bash 运行
if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex196e18.sh"
    exit 1
fi
...
readonly SCRIPT_VERSION="196e18"
...
# ------------------------------------------------------------------------------
# 【V196e18 核心修复 2】：真·三通道内核源码编译引擎 (Mainline 主线 vs Xanmod)
# ------------------------------------------------------------------------------
...
do_kernel_compile_menu() {
    while true; do
        ...
        echo ""
        echo -e "  ${magenta}2) [激进压榨] 编译 Xanmod 官方极客内核 (Xanmod 6.x + BBR3) ${none}"
        echo "     - 源码直连 GitLab，包含高阶网络补丁与 BBR3 拥塞控制"
        echo "     - 彻底斩除 config 拷贝操作，100% 信任内核 Kconfig 推导"
        echo "     - 已强制降级锚定至稳定的 6.12/6.x 分支，根绝 7.x 架构探测暴毙 Bug"
        echo ""
...
_compile_kernel_xanmod() {
    ...
    set +e
    # 强制拉取 6.12 稳定分支的最新 Tag，避开 7.x 激进探测逻辑
    ...
    # 【神级防爆盾】：强行注入 x86_64_v1 架构参数，解决 GCC 报错 bad value (‘x86-64-v’)
    info "正在注入 x86_64 微架构防爆盾，解决编译断层..."
    ./scripts/config --enable CONFIG_X86_64_VERSION 2>/dev/null || true
    ./scripts/config --set-val CONFIG_X86_64_VERSION 1 2>/dev/null || true
...
# ==============================================================================
# --- ✂️ Part 14 结束，所有代码拼接完毕。请运行 bash ex196e17.sh ---
