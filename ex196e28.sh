#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e28.sh (The Apex Vanguard - Project Genesis V196e28)
# 快捷方式: xrv
#
# 【V196e28 无死角护航版：驱动全满，破除证书签名拦截】
#   1. 解决签名暴毙: sed 精准清空 CONFIG_SYSTEM_TRUSTED_KEYS，切断 PEM 检查，解决 Error 2。
#   2. 驱动完美继承: 强制继承宿主机 /boot/config，保留全量网卡/显卡驱动，杜绝失联黑屏。
#   3. 阻断隐形死锁: 预处理后执行 make olddefconfig 自动补全默认值，避免编译交互卡死。
#   4. 内存溢出防护: 彻底禁用所有 DWARF / DEBUG_INFO 膨胀选项。
#   5. 保底退路: 废除编译后强删旧内核逻辑，保障 GRUB 随时可回滚。
# ==============================================================================

if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex196e28.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="196e28"
...
do_kernel_compile_menu() {
    while true; do
        clear
        title "极客内核源码锻造中心 (V196e28 真理防爆盾版)"
        echo "  修复了签名密钥缺失导致的 Error 2 问题及显卡驱动剔除带来的黑屏隐患！"
        echo "  现已引入终极清道夫机制与驱动基因完美继承，并停止旧内核强制卸载，彻防失联！"
        echo ""
...
_execute_compilation() {
    ...
    info "保留所有旧版内核作为 GRUB 备用防爆退路，不再强制卸载..."
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e28 - (The Apex Vanguard)${none}"
...
# ==============================================================================
# EOF - Apex Vanguard V196e28 System Advanced Control Ready.
# ==============================================================================
# --- ✂️ Part 14 结束，所有代码拼接完毕。请运行 bash ex196e28.sh ---
