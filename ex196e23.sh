#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex196e23.sh (The Apex Vanguard - Project Genesis V196e23)
# 快捷方式: xrv
#
# 【V196e23 温和真理版：告别黑屏与失联】
#   1. 驱动基因继承: 拷贝宿主机 /boot/config，保留所有显卡(DRM)与网卡驱动，告别无头黑屏。
#   2. 撤销暴力卸载: 废除编译后强删旧内核的逻辑，保留 GRUB 回滚退路，确保 100% 安全。
#   3. 维持防爆降维打击: 依然保留 sed 抹除 Makefile march 断层的绝杀机制。
#   4. 开放极客自选版本: 支持任意手工指定 Git Tag (例如 6.18.25-rt-xanmod1)。
# ==============================================================================

if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex196e23.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="196e23"
...
do_kernel_compile_menu() {
    while true; do
        clear
        title "极客内核源码锻造中心 (V196e23 真理防失联版)"
        echo "  针对之前由于过度精简导致重启黑屏 (无显卡驱动) 和清除旧内核的问题，"
        echo "  现已引入宿主机驱动基因继承模式 (olddefconfig)，保留退路，100% 防失联！"
        echo ""
...
_compile_kernel_xanmod() {
    ...
    info ">> 启动上帝模式：物理抹除 Makefile 中的动态架构拼写断层..."
    find . -type f -name "Makefile" -exec sed -i 's/-march=x86-64-v[^ \t]*/-march=x86-64/g' {} + 2>/dev/null || true
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V196e23 - (The Apex Vanguard)${none}"
...
# ==============================================================================
# EOF - Apex Vanguard V196e23 System Advanced Control Ready.
# ==============================================================================
# --- ✂️ Part 12 结束，所有代码拼接完毕。请运行 bash ex196e23.sh ---
