#!/usr/bin/env bash
#==============================================================================
# 脚本名称: ex198e63.sh (The Apex Vanguard - Project Genesis V198e63)
# 快捷方式: xrv
# 【V198e63 全量Sysctl覆盖版：热更新自动清理空间 + 应用完整网络协议栈参数】
#==============================================================================
if test -z "${BASH_VERSION:-}"; then echo "Error: 请使用 bash 执行本脚本: bash ex198e63.sh"; exit 1; fi
...
readonly SCRIPT_VERSION="198e63"
...
# V198e63: 主菜单 12 / 内核实验区前置安全空间清理 + 快照收敛。
...
# V198e63: 主菜单 13 /etc/sysctl.d/99-network-optimized.conf 网络缓冲参数管理。
...
# V198e63: 清理旧热更新遗留的 rmem/wmem 冲突定义。
...
cleanup_network_sysctl_conflicts_e58() {
    title "清理旧版 Sysctl 冲突源（V198e63）"
    local found=0 f

    # V198e63 强规则：旧热更新文件 99-xray-hotupdate-fast.conf 不再保留。
...
# V198e63 manual network buffer profile
...
manual_network_buffer_params_e56() {
    title "手动输入网络缓冲参数"
    warn "输入为空则使用当前永久文件值；若当前未写入，则使用 V198e63 默认值。"
...
do_network_params_menu_e56() {
    while true; do
        ...
        show_network_sysctl_conflicts_e58
        echo ""
        echo "  1) 应用 V198e63 默认 rmem/wmem 参数"
...
do_sys_init_menu() {
    while true; do
        clear
        title "环境底层组件拉齐与结构重建区 (V198e63 空间清理强化版)"
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray System Advanced Management V198e63 - (The Apex Vanguard)${none}"
        ...
        echo -e "  ${cyan}11) 从 ex198e32/ex198e33/ex198e34/ex198e35/ex198e36/ex198e37/ex198e38/ex198e39/ex198e40/ex198e41/ex198e42/ex198e43/ex198e54/ex198e55/ex198e56/ex198e57 稳态热更新到 V198e63${none}"
...
hot_update_from_legacy() {
    title "V198e63 稳态热更新：删除旧 99-xray-hotupdate-fast.conf + 自动执行 12/13"
    ...
    # V198e63：热更新时自动执行主菜单 12 的安全空间清理。
    ...
    # V198e63：热更新时自动执行主菜单 13 的默认网络参数写入与 sysctl 应用。
    cleanup_network_sysctl_conflicts_e58 || true
    ...
    fix_permissions
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    if ! verify_e58_post_state; then error "V198e63 关键状态校验失败，已保留快照：$bdir"; return 1; fi
    ensure_xray_is_alive
    info "V198e63 热更新完成：已自动执行 12 清理空间、13 默认网络参数，并保留第 9 项 IP/单连接审计中心。"
}
...
do_status_menu() {
    while true; do
        clear
        title "主控台: 监控、账单及 IP 审计核心（V198e63）"
        echo "  1) [系统守护] 追踪主线程服务健康率"
        echo "  2) [全网穿透] 检测基础外网及监听位态"
        echo "  3) [总计账单] 调取网卡出入核算账册 (vnstat)"
...
# V198e63: 热更新后额外写入版本号；不默认启用任何 IP 限制规则。
...
write_full_network_optimized_conf_e60() {
    ...
    target_qdisc="fq"
    # V198e63: 默认稳态必须是 fq；CAKE 只能由高级菜单显式开启，13/热更新不得把永久文件写成 cake。
    new_scale="${1:-1}"
    new_app="${2:-31}"
    ...
    info "写入 V198e63 完整内核 Sysctl 协议栈参数：$NETWORK_OPT_CONF"
    cat > "$NETWORK_OPT_CONF" << EOF_E60_SYSCTL
#==============================================================================
# Xray / VPS Network Optimized Sysctl Profile - V198e63
# 默认采用高吞吐缓冲档：7990577 / 97108868，适合高 RTT/高带宽跨境代理链路。
# 由 ex198e63.sh 管理。热更新会覆盖本文件，并删除旧 99-xray-hotupdate-fast.conf。
#==============================================================================
net.core.default_qdisc = ${target_qdisc}
net.ipv4.tcp_congestion_control = bbr
net.ipv4.neigh.default.base_reachable_time_ms = 60000
...
EOF_E60_SYSCTL
    ...
    if ! sysctl -p "$NETWORK_OPT_CONF" >"$sysctl_log" 2>&1; then
        ...
        info "V198e63 完整底层 Sysctl 参数已成功注入。"
        rm -f "$sysctl_log" 2>/dev/null || true
    fi
...
apply_default_network_buffer_params_e56() {
    title "应用 V198e63 默认高吞吐完整网络协议栈参数"
...
do_network_params_menu_e56() {
    while true; do
        clear
        title "网络参数应用中心（V198e63 完整协议栈：$NETWORK_OPT_CONF）"
        show_network_buffer_params_e56
        show_network_sysctl_conflicts_e58
        echo ""
        echo "  1) 应用 V198e63 默认高吞吐完整网络协议栈参数"
        echo "  2) 仅手动输入 rmem/wmem/tcp_rmem/tcp_wmem 参数（保留其它协议栈参数）"
        echo "  3) 重新加载 $NETWORK_OPT_CONF"
        echo "  4) 查看该永久配置文件完整内容"
        echo "  5) 删除旧 99-xray-hotupdate-fast.conf 并应用 V198e63 默认完整协议栈"
        echo "  0) 返回主菜单"
        hr
        local opt=""; read -rp "请选择操作: " opt || true
        case "${opt:-}" in
...
hot_update_from_legacy() {
    title "V198e63 稳态热更新：删除旧 99-xray-hotupdate-fast.conf + 自动执行 12 + 完整应用 13"
    if test ! -f "$CONFIG"; then die "未找到 $CONFIG，不能热更新。请先确认 Xray 已安装。"; fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then die "现有 config.json 不是合法 JSON，拒绝热更新，避免节点损坏。"; fi
    local ts bdir
    ts=$(date +%Y%m%d_%H%M%S); bdir="$BACKUP_DIR/hotupdate_e60_${ts}"
    ...
    # V198e63：热更新必须完整覆盖 /etc/sysctl.d/99-network-optimized.conf，
    # 不再只追加 rmem/wmem 六个键，避免核心协议栈参数缺失或旧文件覆盖。
    apply_default_full_sysctl_profile_e60 || warn "V198e63 完整网络协议栈参数应用未完全成功，请进入 13 手动查看。"
    _backup_and_remove_legacy_hotupdate_sysctl_e60 || true

    fix_permissions
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    if ! verify_e60_post_state; then error "V198e63 关键状态校验失败，已保留快照：$bdir"; return 1; fi
    ensure_xray_is_alive
    info "V198e63 热更新完成：已自动执行 12 清理空间、13 默认完整网络协议栈参数，并保留第 9 项 IP/单连接审计中心。"
}
...
#==============================================================================
# V198e63 final sysctl conflict hardening layer
...
cleanup_network_sysctl_conflicts_e58() {
    title "清理旧版 Sysctl 冲突源（V198e63 强制删除旧热更新文件）"
    local found=0 f
...
apply_hotupdate_fast_sysctl() {
    # V198e63：旧函数名保留兼容，但不再创建 /etc/sysctl.d/99-xray-hotupdate-fast.conf。
...
do_network_params_menu_e56() {
    while true; do
        ...
        show_network_sysctl_conflicts_e58
        echo ""
        echo "  1) 应用 V198e63 默认高吞吐完整协议栈参数（fq / bbr / ecn=0 / fastopen=0 / rmem=97108868）"
        echo "  2) 仅手动输入 rmem/wmem/tcp_rmem/tcp_wmem 参数（保留其它协议栈参数）"
        echo "  3) 重新加载 $NETWORK_OPT_CONF"
        echo "  4) 查看该永久配置文件完整内容"
        echo "  5) 删除旧 99-xray-hotupdate-fast.conf 并应用 V198e63 默认完整协议栈"
...
verify_e60_post_state() {
    ...
    for kv in \
        ...
        if test "$rv" != "$2"; then warn "当前运行 $1=$rv，与 V198e63 默认 $2 不一致。"; fi
    done
...
hot_update_from_legacy() {
    title "V198e63 稳态热更新：强制删除旧 99-xray-hotupdate-fast.conf + 自动执行 12 + 完整应用 13"
    if test ! -f "$CONFIG"; then die "未找到 $CONFIG，不能热更新。请先确认 Xray 已安装。"; fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then die "现有 config.json 不是合法 JSON，拒绝热更新，避免节点损坏。"; fi
    local ts bdir
    ...
    # V198e63：最后阶段再次强制完整覆盖 99-network-optimized.conf，并删除旧冲突文件。
    apply_default_full_sysctl_profile_e60 || warn "V198e63 完整网络协议栈参数应用未完全成功，请进入 13 手动查看。"
    remove_legacy_hotupdate_sysctl_e61
    ...
    if ! verify_e60_post_state; then error "V198e63 关键状态校验失败，已保留快照：$bdir"; return 1; fi
    ensure_xray_is_alive
    info "V198e63 热更新完成：旧 99-xray-hotupdate-fast.conf 已删除，12 清理空间与 13 默认完整协议栈已自动应用。"
}



#==============================================================================
# V198e63 final hardening layer
# 重点：
...
cleanup_network_sysctl_conflicts_e58() {
    title "清理关键网络 Sysctl 冲突源（V198e63 强化版）"
    local found=0 f
    if test -e "/etc/sysctl.d/99-xray-hotupdate-fast.conf"; then
        found=1
        warn "发现旧热更新 Sysctl 文件 /etc/sysctl.d/99-xray-hotupdate-fast.conf，已备份并删除。"
    ...
    # V198e63：不仅清理 99-sysctl.conf，也清理 /etc/sysctl.conf 与其它 sysctl.d 中的关键网络重复定义。
...
# V198e63：手动缓冲参数前，若完整协议栈文件不存在或被破坏，先写入完整默认档，再只替换六个缓冲键。
_write_network_buffer_params_e56() {
...
apply_balanced_network_buffer_params_e63() {
    title "应用 V198e63 均衡稳态缓冲档（3990577 / 57108868）"
    write_full_network_optimized_conf_e60 1 31 >/dev/null 2>&1 || true
    _write_network_buffer_params_e56 \
...
apply_default_network_buffer_params_e56() {
    title "应用 V198e63 默认高吞吐完整协议栈参数（7990577 / 97108868）"
    write_full_network_optimized_conf_e60 1 31
    show_network_buffer_params_e56
    show_network_sysctl_conflicts_e58
}
...
do_tcp_diag_menu_e63() {
    while true; do
        clear
        title "TCP 重传 / 单连接诊断中心（V198e63，只读排障）"
...
hot_update_from_legacy() {
    title "V198e63 稳态热更新：强制单一 Sysctl 源 + 自动执行 12 + 默认高吞吐 13"
    if test ! -f "$CONFIG"; then die "未找到 $CONFIG，不能热更新。请先确认 Xray 已安装。"; fi
    if ! jq empty "$CONFIG" >/dev/null 2>&1; then die "现有 config.json 不是合法 JSON，拒绝热更新，避免节点损坏。"; fi
    local ts bdir
    ...
    force_apply_xray_core_defaults_e52
    remove_xray_env_buffer
    apply_default_full_sysctl_profile_e60 || warn "V198e63 默认高吞吐完整协议栈应用未完全成功，请进入 13 手动查看。"
    cleanup_network_sysctl_conflicts_e58 || true
    sysctl -p "$NETWORK_OPT_CONF" >/dev/null 2>&1 || true
    _e63_force_runtime_key_values 1 31
    ...
    fix_permissions
    echo "$SCRIPT_VERSION" > "$SCRIPT_VERSION_FILE" 2>/dev/null || true
    if ! verify_e63_post_state; then error "V198e63 关键状态校验失败，已保留快照：$bdir"; return 1; fi
    ensure_xray_is_alive
    info "V198e63 热更新完成：已自动执行 12 清理空间、13 默认高吞吐完整协议栈，并新增 14 TCP 诊断中心。"
}
...
main_menu() {
    while true; do
        ...
        echo "  9) 全景网络连接审计日志与商用级别网络流量记账本"
        echo "  10) 高级发烧：内核双轨编译(APT/源码)/网络/应用层 60余项极限全控板"
        echo -e "  ${cyan}11) 从旧 ex198e 环境稳态热更新到 V198e63${none}"
        echo -e "  ${cyan}12) 清理空间（安全释放 / 快照收敛 / 显示硬盘与 Swap）${none}"
...
#==============================================================================
# EOF - Apex Vanguard V198e63 System Advanced Control Ready.
#==============================================================================
