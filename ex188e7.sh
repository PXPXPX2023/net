#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: ex188e7.sh (The Apex Vanguard - Project Genesis V188e7)
# 快捷方式: xrv
#
# 【V188e7 破壁觉醒版 - 终极修复日志】
#   1. 毁灭级修复: 选项 3 预编译内核彻底重构，加入原生 v4->v3->v2->v1 降级轮询引擎，无视 apt-cache 报错。
#   2. 基因级修复: 选项 4 源码编译重构 Makefile 物理破坏逻辑与 .config 注入顺序，彻底斩除 x86-64-v 编译错误。
#   3. 源盾升级: 采用 gpg.key 最新标准，强制开启 set +e 隔离防护，彻底解决 Debian 11/12 源阻断。
#   4. 容错护盾: 全量采用 |= 与 select(. != null) 绝缘防线，拒绝任何隐式短路退出。
# ==============================================================================

if test -z "${BASH_VERSION:-}"; then
    echo "Error: 请使用 bash 执行本脚本: bash ex188e7.sh"
    exit 1
fi
...
_err_handler() {
    ...
    echo -e "${yellow} >> 战舰核心遇到异常断层，系统防线已触发保护机制！${none}" >&2
...
ensure_xray_is_alive() {
    ...
    if systemctl is-active --quiet xray; then
        ...
        hr
        restore_latest_backup
        local _p=""; read -rp "请按 Enter 键返回..." _p || true
        return 1
    fi
}
...
# ------------------------------------------------------------------------------
# 【终极核心 1：官方预编译 XANMOD (破壁降级引擎)】
# ------------------------------------------------------------------------------

do_install_xanmod_main_official() {
    title "系统飞升：安装预编译 XANMOD 内核 (纯净官方源双轨护盾)"
    ...
    if test -z "$cpu_level"; then
        cpu_level=1
        warn "探测受阻，将使用保底 v1 架构..."
    else
        info "当前 CPU 完美支持: v${cpu_level}"
    fi

    print_magenta ">>> 正在拉取底层安全证书，重置官方源 (实时同步防封)..."
    # 【核心防御】：采用 set +e 阻止 apt update 因为源不全导致的直接崩溃退出
    set +e
    export DEBIAN_FRONTEND=noninteractive
    apt-get update -y >/dev/null 2>&1
    # 强制安装 apt-transport-https 与证书，这是 Debian 11 识别第三方源的死穴！
    ...
    # 彻底使用 gpg.key (取代 archive.key) 避免精简版系统 dearmor 失败
    if ! wget -qO - https://dl.xanmod.org/gpg.key | gpg --dearmor --yes -o /usr/share/keyrings/xanmod-archive-keyring.gpg 2>/dev/null; then
        error "GPG 密钥链植入失败！源已阻断。"
        set -e; return 1
    fi
    ...
    info "预编译部署完毕！10 秒后自动重启..."
    sleep 10
    reboot
}

# ------------------------------------------------------------------------------
# 【终极核心 2】：源码暴力编译 Xanmod (物理抹杀 Makefile Bug)
# ------------------------------------------------------------------------------

do_xanmod_compile() {
    title "系统飞升：源码编译 XANMOD 官方内核 + BBR3 (极客锻造模式)"
    
    warn "警告: 这是一个极其漫长的过程 (30-60分钟)，低配机极易引发死机断连！强烈建议优先使用菜单 3。"
    local confirm=""
    ...
    # 【神级护盾：物理歼灭 Makefile 报错的终极指令】
    # 这就是你之前报错 error: bad value (‘x86-64-v’) 的死穴！
    # 必须对 $ 符号进行严格转义（\$），否则 sed 会失效，导致无法替换！
    info "物理破坏 Makefile 架构验证逻辑，彻底斩断 GCC 报错链条..."
    sed -i 's/-march=x86-64-v\$(CONFIG_X86_64_VERSION)/-march=x86-64-v2/g' arch/x86/Makefile 2>/dev/null || true
    
    # 在执行 make 之前，强行在 .config 里钉死版本号
    sed -i '/CONFIG_X86_64_VERSION/d' .config 2>/dev/null || true
    sed -i '/CONFIG_X86_64_V/d' .config 2>/dev/null || true
    echo "CONFIG_X86_64_V2=y" >> .config
    echo "CONFIG_X86_64_VERSION=2" >> .config
    ...
    ./scripts/config --disable CONFIG_SYSTEM_REVOCATION_LIST 2>/dev/null || true
    ...
    info "奇迹再现！纯血版 Xanmod 源码内核编译挂载全部顺利结束。"
    warn "系统将在 15 秒后自动重启..."
    sleep 15
    reboot
}
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}======================================================================${none}"
        echo -e "  ${magenta}Xray 高维控制台 (Apex Vanguard V188e7 终极形态)${none}"
...
main_menu
# ==============================================================================
# EOF - Apex Vanguard V188e7 System Core Ready.
# ==============================================================================
