#!/usr/bin/env bash
# ============================================================
# 脚本名称: ex144.sh (The Apex Vanguard - Project Genesis V144 [Absolute Horizon])
# 快捷方式: xrv
# 巅峰重铸宣言 (绝对完整防截断版): 
#   1. 物理装甲全量回归：完美复刻 V62 的 100万并发锁 (limits.conf)、60+ 行极限 Sysctl 参数、cc1.sh 守护进程与 do_change_dns 底层锁死。
#   2. Xanmod 安全编译：集成 4G 临时 Swap 防 OOM，官方 scripts/config 静默注入 BBR3，剔除证书依赖陷阱。
#   3. 核心大一统：保留 28 项全域微操矩阵、回落探针 (菜单 8)、8 行军规排版、ShortId 防漂移联动删除。
#   4. 拒绝代码阉割：所有 jq 管道流与复杂的 if-else 逻辑全面恢复多行可读结构，彻底废弃恶心的单行压缩。
#   5. 硬件级记忆熔断：针对 GSO/GRO 硬件级锁死的机器 (fixed)，实现底层探测与双重物理熔断，绝不执行危险指令。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash ex144.sh"
    exit 1
fi
...
do_xanmod_compile() {
    ...
    if [ "$TOTAL_MEM" -lt 4000 ]; then
        info "创建 4GB 临时 Swap 防 OOM..."
        fallocate -l 4G /tmp/compile_swap 2>/dev/null || dd if=/dev/zero of=/tmp/compile_swap bs=1M count=4096 2>/dev/null
        chmod 600 /tmp/compile_swap
        mkswap /tmp/compile_swap >/dev/null 2>&1
        swapon /tmp/compile_swap >/dev/null 2>&1
    fi
    ...
    ./scripts/config --disable CONFIG_X86_64_V1
    ./scripts/config --disable CONFIG_X86_64_V2
    ./scripts/config --disable CONFIG_X86_64_V3
    ./scripts/config --disable CONFIG_X86_64_V4
    ./scripts/config --enable CONFIG_X86_64_V${cpu_level}
    ...
    if ! make -j${THREADS} bindeb-pkg; then
        error "编译失败，已熔断！"
        swapoff /tmp/compile_swap 2>/dev/null
        rm -f /tmp/compile_swap 2>/dev/null
    ...
    swapoff /tmp/compile_swap 2>/dev/null || true
    rm -f /tmp/compile_swap 2>/dev/null || true
    
    info "编译完成！请重启以应用 BBR3！"
    read -rp "按 Enter 返回..." _
}

# V62 满血回归: 60+行极限内核网络栈参数
do_perf_tuning() {
    title "系统底层网络栈特化调优 (V62 完整版)"
    warn "将注入 100 万并发极限参数并重启！"
    read -rp "继续吗？(y/n): " confirm
    if [[ "$confirm" != "y" ]]; then return; fi
...
EOF
    ...
    info "全量 V62/V142 底层矩阵参数注入完成！系统将在 30 秒后自动重启以生效..."
    sleep 30
    reboot
}
    ...
    while true; do
        ...
        case "$bc" in
            1) 
                ...
                '
                systemctl restart xray; info "BT 屏蔽切换为: $nv" 
                ;;
            2) 
                ...
                '
                systemctl restart xray; info "广告屏蔽切换为: $nv" 
                ;;
            0) return ;;
        esac
    done
...
_update_matrix() {
    ...
    ' "$CONFIG" > "$CONFIG.tmp"
    if [ $? -eq 0 ]; then mv -f "$CONFIG.tmp" "$CONFIG"; fix_permissions; systemctl restart xray >/dev/null 2>&1; fi
    rm -f /tmp/sni_array.json
}

do_install() {
    title "Apex Vanguard Ultimate Final: 核心部署"
    preflight; systemctl stop xray >/dev/null 2>&1 || true
    if [ ! -f "$INSTALL_DATE_FILE" ]; then date +"%Y-%m-%d %H:%M:%S" > "$INSTALL_DATE_FILE"; fi
    
    echo -e "  ${cyan}请选择要安装的代理协议：${none}"
    echo "  1) VLESS-Reality (推荐, 强力防封)"
    echo "  2) Shadowsocks (建议落地机使用)"
    echo "  3) 两个都安装 (双管齐下)"
    read -rp "  请输入编号: " proto_choice; proto_choice=${proto_choice:-1}

    if test "$proto_choice" = "1" || test "$proto_choice" = "3"; then
        while true; do 
            read -rp "请输入 VLESS 监听端口 (回车默认443): " input_p; input_p=${input_p:-443}
            if validate_port "$input_p"; then LISTEN_PORT="$input_p"; break; fi
        done
        read -rp "请输入节点别名 (默认 xp-reality): " input_remark; REMARK_NAME=${input_remark:-xp-reality}
    ...
    if test "$proto_choice" = "2" || test "$proto_choice" = "3"; then
        while true; do 
            read -rp "请输入 SS 监听端口 (回车默认8388): " input_s; input_s=${input_s:-8388}
...
EOF
    ...
    if test "$proto_choice" = "1" || test "$proto_choice" = "3"; then
        ...
        echo "$pub" > "$PUBKEY_FILE"; echo "$uuid|$ctime" > "$USER_TIME_MAP"
        echo "[$SNI_JSON_ARRAY]" > /tmp/sni_array.json
...
EOF
    ...
    fix_permissions; systemctl enable xray >/dev/null 2>&1; systemctl restart xray >/dev/null 2>&1
    info "网络架构部署完成！"; do_summary
    
    while true; do
        read -rp "按 Enter 返回主菜单，或输入 b 重配矩阵: " opt
        if [[ "$opt" == "b" || "$opt" == "B" ]]; then
            choose_sni; if test $? -eq 0; then _update_matrix; do_summary; else break; fi
        else break; fi
    done
}
...
do_status_menu() {
    while true; do
        ...
        read -rp "选择: " s
        case "$s" in
            1) clear; title "Xray 进程守护状态"; systemctl status xray --no-pager || true; echo ""; read -rp "按 Enter 继续..." _ ;;
            2) echo -e "\n  公网 IP: ${green}$SERVER_IP${none}"; hr; echo -e "  DNS 解析流向: "; grep "^nameserver" /etc/resolv.conf | awk '{print "    " $0}'; hr; echo -e "  Xray 本地监听: "; ss -tlnp | grep xray | awk '{print "    " $4}'; read -rp "按 Enter 继续..." _ ;;
            3) 
                if ! command -v vnstat >/dev/null 2>&1; then warn "未安装 vnstat"; read -rp "Enter..." _; continue; fi
                clear; title "商用级网卡流量计费中心"
                local m_day=$(grep -E "^[[:space:]]*MonthRotate" /etc/vnstat.conf 2>/dev/null | awk '{print $2}' | head -n 1 | tr -d '\r\n'); m_day=${m_day:-"1 (默认)"}
                echo -e "  每月重置日: ${cyan}每月 $m_day 号${none}"; hr
                ...
                hr
                echo "  1) 设置/修改 每月账单清零日"; echo "  2) 查看看某个月按天详情"; echo "  q) 返回"
                read -rp "  指令: " vn_opt
                case "$vn_opt" in
                    1) read -rp "请输入新清零日 (1-31): " d_day; if test "${d_day:-0}" -ge 1 2>/dev/null && test "${d_day:-0}" -le 31 2>/dev/null; then sed -i '/^[#[:space:]]*MonthRotate/d' /etc/vnstat.conf 2>/dev/null; echo "MonthRotate $d_day" >> /etc/vnstat.conf; systemctl restart vnstat 2>/dev/null; info "已重置为每月 $d_day 号。"; else error "无效。"; fi; read -rp "Enter..." _ ;;
                    2) read -rp "年月 (如 $(date +%Y-%m)，回车近30天): " d_month; if test -z "$d_month"; then vnstat -d 2>/dev/null | sed -e 's/estimated/估计流量/ig' -e 's/rx/接收/ig' -e 's/tx/发送/ig' -e 's/total/总计/ig' -e 's/daily/按天/ig' -e 's/monthly/按月/ig'; else vnstat -d 2>/dev/null | grep -iE "($d_month| day |estimated|--)" | sed -e 's/estimated/估计流量/ig' -e 's/rx/接收/ig' -e 's/tx/发送/ig' -e 's/total/总计/ig' -e 's/daily/按天/ig' -e 's/monthly/按月/ig'; fi; read -rp "Enter..." _ ;;
                    q) ;;
                esac
                ;;
            4)
                while true; do
                    clear; title "商用级实时连接与独立 IP 统计"
                    ...
                    if [ -n "$x_pids" ]; then
                        ...
                        if [ -n "$ips" ]; then echo "$ips" | sort | uniq -c | sort -nr | head -n 10 | awk '{printf "    IP: %-18s (并发: %s)\n", $2, $1}'; local total_ips=$(echo "$ips" | sort | uniq | wc -l); echo -e "\n  独立外部 IP 总数: ${yellow}${total_ips}${none}"; else echo -e "    ${gray}暂无外部真实连接。${none}"; fi
                    else echo -e "  ${red}Xray 进程未运行。${none}"; fi
                    echo -e "\n  ${gray}---------------------------------------------------${none}"
                    echo -e "  ${green}雷达运行中 (每 2 秒自动刷新)... 快捷键: [ ${yellow}q${none} ] 返回${none}"
                    read -t 2 -n 1 -s cmd; if [[ "$cmd" == "q" || "$cmd" == "Q" || "$cmd" == $'\e' ]]; then break; fi
                done
                ;;
            5)
                while true; do
                    clear; title "实时修改 Xray CPU 优先级 (Nice 调度)"
                    local limit_file="/etc/systemd/system/xray.service.d/limits.conf"; local current_nice="-20"
                    if [ -f "$limit_file" ]; then if grep -q "^Nice=" "$limit_file"; then current_nice=$(awk -F'=' '/^Nice=/ {print $2}' "$limit_file" | head -1); fi; fi
                    echo -e "  当前 Xray Nice: ${cyan}${current_nice}${none} (-20至-10)"; hr
                    read -rp "  新 Nice 值 (q 返回): " new_nice; if [[ "$new_nice" == "q" || "$new_nice" == "Q" ]]; then break; fi
                    if [[ "$new_nice" =~ ^-[1-2][0-9]$ ]] && [ "$new_nice" -ge -20 ] && [ "$new_nice" -le -10 ]; then sed -i "s/^Nice=.*/Nice=$new_nice/" "$limit_file"; systemctl daemon-reload; info "更新为 $new_nice，5 秒后重启..."; sleep 5; systemctl restart xray; info "已生效。"; read -rp "Enter..." _; break; else error "无效数字！"; sleep 2; fi
                done
                ;;
            0) return ;;
        esac
...
do_uninstall() {
    ...
    read -rp "永久保留底层调优，粉碎 Xray？(输入 y 确定): " confirm; if test "$confirm" != "y"; then return; fi
    
    local temp_date=""; if test -f "$INSTALL_DATE_FILE"; then temp_date=$(cat "$INSTALL_DATE_FILE"); print_magenta ">>> 提取初装日期快照..."; fi
    
    print_magenta ">>> 粉碎 Dnsmasq 并恢复 DNS..."
    systemctl stop dnsmasq >/dev/null 2>&1 || true; systemctl disable dnsmasq >/dev/null 2>&1 || true
    export DEBIAN_FRONTEND=noninteractive; apt-get purge -y dnsmasq >/dev/null 2>&1 || true; rm -f /etc/dnsmasq.conf >/dev/null 2>&1
    chattr -i /etc/resolv.conf 2>/dev/null || true; if [ -f /etc/resolv.conf.bak ]; then mv -f /etc/resolv.conf.bak /etc/resolv.conf 2>/dev/null; fi
    systemctl stop resolvconf.service >/dev/null 2>&1 || true; systemctl disable resolvconf.service >/dev/null 2>&1 || true
    if systemctl list-unit-files | grep -q systemd-resolved; then systemctl enable systemd-resolved >/dev/null 2>&1 || true; systemctl start systemd-resolved >/dev/null 2>&1 || true; fi
    ...
    if test -n "$temp_date"; then mkdir -p "$CONFIG_DIR" 2>/dev/null; echo "$temp_date" > "$INSTALL_DATE_FILE"; fi
    print_green "卸载完成！"; exit 0
}
...
main_menu() {
    while true; do
        ...
        case "$num" in
            1) do_install ;;
            2) do_user_manager ;;
            3) do_summary; while true; do read -rp "按 Enter 返回，或 b 重选 SNI: " rb; if [[ "$rb" == "b" || "$rb" == "B" ]]; then choose_sni; if test $? -eq 0; then _update_matrix; do_summary; else break; fi; else break; fi; done ;;
            4) print_magenta ">>> 同步最新规则库..."; bash "$UPDATE_DAT_SCRIPT" >/dev/null 2>&1; systemctl restart xray >/dev/null 2>&1; info "更新成功"; read -rp "Enter..." _ ;;
            5) do_update_core ;;
            6) choose_sni; if test $? -eq 0; then _update_matrix; do_summary; while true; do read -rp "按 Enter 返回，或 b 继续分配: " rb; if [[ "$rb" == "b" || "$rb" == "B" ]]; then choose_sni; if test $? -eq 0; then _update_matrix; do_summary; else break; fi; else break; fi; done; fi ;;
