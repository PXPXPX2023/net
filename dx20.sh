#!/usr/bin/env bash
# ============================================================
# 脚本名称: dx20.sh (The Apex Vanguard - Ultimate Final)
# 快捷方式: xrv
# 巅峰突破: 
#   1. 拒绝无脑折叠，全量展开结构化代码，彻底恢复工业级可读性与完整度。
#   2. 新增【初次安装、更新系统组件】模块，一键拉齐新机环境与时区。
#   3. 深度融合 ChangeDNS 系统级域名解析引擎，正则强锁 resolvconf 防止飘移。
#   4. 嵌入 Xanmod 内核 + BBR3 源码级自动编译，与极限网络并发调优 (Sysctl)。
#   5. 纯净寡头节点库定稿，矩阵全量重组、十六进制防吞噬引擎、商业计费全量融贯。
# ============================================================

# 必须用 bash 运行
if test -z "$BASH_VERSION"; then
    echo "错误: 请用 bash 运行此脚本: bash dx20.sh"
    exit 1
fi
...
preflight() {
    ...
    local need="jq curl wget xxd unzip qrencode vnstat cron openssl coreutils sed"
    local install_list=""
    for i in $need; do 
...
do_change_dns() {
    ...
    info "已成功锁定 DNS，当前 Nameserver 流向："
    cat /etc/resolv.conf | grep nameserver
...
do_xanmod_compile() {
    ...
    rm -rf /var/log/*.log /var/log/*/*.log /tmp/*
    rm -rf /var/lib/docker/* 2>/dev/null || true
    rm -rf /usr/src/linux* /usr/src/bbr* /usr/src/xanmod*
    rm -rf /compile/* 2>/dev/null || true
    rm -rf /root/linux* /root/*.tar* /root/*.xz
    sync
...
EOF
    ...
    apt update
    apt install -y build-essential bc bison flex libssl-dev libelf-dev git wget curl xz-utils ethtool numactl
    ...
    echo "=== 开始拉取 Kernel 源码 ==="
    cd $BUILD_DIR
    local KERNEL_URL=$(curl -s https://www.kernel.org/releases.json | jq -r '.releases'${L_B}'0'${R_B}'.source')
...
do_perf_tuning() {
    ...
    rm -f /etc/sysctl.d/99-bbr*.conf /etc/sysctl.d/99-ipv6-disable.conf
    rm -f /etc/sysctl.d/99-network-optimized.conf /etc/sysctl.d/99-pro*.conf
    rm -f /etc/sysctl.d/99-xanmod-bbr3.conf /usr/lib/sysctl.d/50-pid-max.conf
    rm -f /usr/lib/sysctl.d/99-protect-links.conf /etc/security/limits.conf

    echo "正在写入 Limits 极限并发配置..."
...
do_sys_init_menu() {
    while true; do
        ...
        case "$sys_opt" in
            1) 
                print_magenta ">>> 正在拉取系统更新与底层环境组件，这可能需要一点时间..."
                apt update && apt upgrade -y && apt dist-upgrade -y && apt full-upgrade -y && apt autoremove -y && apt update && apt install wget -y && apt install curl -y && apt-get install sudo -y && apt-get install wget -y && apt-get update && apt install -y socat && apt-get -y install sudo && apt-get -y install curl && apt install -y ntpdate && timedatectl set-timezone Asia/Kuala_Lumpur && ntpdate us.pool.ntp.org && hwclock --systohc
...
main_menu() {
    while true; do
        clear
        echo -e "${blue}===================================================${none}"
        echo -e "  ${magenta}Xray dx20 The Apex Vanguard Ultimate Final${none}"
